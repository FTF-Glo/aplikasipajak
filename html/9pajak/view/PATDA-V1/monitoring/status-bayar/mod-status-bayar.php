...
function displayMenu()
{
    global $DIR, $a, $m, $data;
        echo "\t<ul>\n";
        echo "\t\t<li><a class='tab' href=\"view/{$DIR}/monitoring/status-bayar/svc-list-status-bayar.php?q=" . base64_encode("{'a':'$a', 'm':'$m','u':'$data->uname','i':'1','s':'1','f':'fPatdaLapor2','mod':'pel'}") . "\">Sudah Bayar</a></li>\n";
        echo "\t\t<li><a class='tab' href=\"view/{$DIR}/monitoring/status-bayar/svc-list-status-bayar.php?q=" . base64_encode("{'a':'$a', 'm':'$m','u':'$data->uname','i':'2','s':'2','f':'fPatdaLapor2','mod':'pel'}") . "\">Belum Bayar</a></li>\n";
        echo "\t\t<li><a class='tab' href=\"view/{$DIR}/monitoring/status-bayar/svc-list-status-bayar.php?q=" . base64_encode("{'a':'$a', 'm':'$m','u':'$data->uname','i':'4','s':'4','f':'fPatdaLapor2','mod':'pel'}") . "\">Semua Data</a></li>\n";
        echo "\t\t<li><a class='tab' href=\"view/{$DIR}/monitoring/status-bayar/svc-list-belum-lapor.php?q=" . base64_encode("{'a':'$a', 'm':'$m','u':'$data->uname','i':'3','s':'3','f':'fPatdaLapor2','mod':'pel'}") . "\">Belum Lapor</a></li>\n";
        // echo "\t\t<li><a class='tab' href=\"view/{$DIR}/monitoring/status-bayar/svc-grafik-filter.php?q=" . base64_encode("{'a':'$a', 'm':'$m','u':'$data->uname','i':'4','s':'4','f':'fPatdaLapor2','mod':'pel'}") . "\">Grafik Laporan</a></li>\n";
        echo "\t</ul>\n";
}
?>
...
<script type="text/javascript">
    ...
    function toPdf(sts) {
        ...
        var arr_field = ["sptpd", "wp_alamat", "wp_nama", "simpatda_dibayar", "jenis", "expired_date1", "expired_date2", "simpatda_tahun_pajak", "simpatda_bulan_pajak", "bank", "jenis_lapor", "CPM_KECAMATAN", "CPM_KELURAHAN", "operator", "payment_code", "kurang_bayar"]
        var str_data = "";
    ...
    function toExcelV2(sts) {
        ...
        $.ajax({
            ...
            success: function(msg) {
                ...
                if (msg < 20000) {
                    str_data += "&nmfile=" + nmfileAll;
                    strOfLink += '<a href="view/<?php echo "{$DIR}/{$modul}/{$submodul}" ?>/svc-toexcel-v2.php?q=' + q + str_data + '&p=all&total=' + msg + '">' + nmfileAll + '</a><br/>';
                } else {
                    for (var page = 1; page <= sumOfPage; page++) {
                        var data = str_data + "&nmfile=" + nmfile + page;
                        strOfLink += '<a href="view/<?php echo "{$DIR}/{$modul}/{$submodul}" ?>/svc-toexcel-v2.php?q=' + q + data + '&p=' + page + '">' + nmfile + page + '</a><br/>';
                    }
                }
                $("#contentLink").html(strOfLink);
                $("#cBox").css("display", "block");
    ...
    function checkK(id) {
        check = $('#kurang_bayar-' + id).is(':checked');

        if (check) {
            $('#kurang_bayar-' + id).val('1')
        } else {
            $('#kurang_bayar-' + id).val('0')
        }
    }
</script>
<div id="tabsContent">
...
<div id="cBox" class="animate" style="width: 205px; height: 300px; position: absolute; right: 50px; top: 200px; border: 1px solid gray; background-color: #eaeaea; display: none; overflow: auto;">
    <div style="overflow: auto; background-color: #c0c0c0; width: 198px; padding: 3px;">
        ...
        <div id="closeCBox" class="btn btn-sm btn-secondary" style=" padding: 3px; background-color: #eaeaea; border: 1px solid gray; float: right; cursor: pointer; margin-right: 5px;">Close</div>
    </div>
    <div id="contentLink" style="padding: 3px; width: 196px; height: 260px; overflow: auto;"></div>
</div>
<div id="cAkumulasi" style="position: absolute; top: 155px; right: 60px; display: block; overflow: auto;margin-top: 0px;">
    ...
    <input style="float: right;" type="button" name="updateCount" id="updateCount" class="btn btn-sm btn-secondary" value="Update" onClick="updateCount()" />
</div>