<?php
session_start();
$modul = "monitoringdevice";
$sRootPath = str_replace('\\', '/', str_replace(DIRECTORY_SEPARATOR . "monitoringdevice", '', dirname(__FILE__))) . '/';
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
		$('#table-alarm').jtable({
			...
			selecting: false,
			actions: {
				listAction: "/<?php echo $modul?>/svc-deviceid.php?function=getDeviceAlarm&a=<?php echo $a?>",
			},
			fields: {
				NO : {title: 'No',width: '3%'},
	...
	var intPage, intStatus;
	
	function refreshPage(){
		clearInterval(intPage);
		
		var current = $('.jtable-goto-page select').val();
		var numPage = $('.jtable-goto-page select option:last').val();
		
		current = (current == numPage)? 1 : (eval(current) + 1);
		$('#table-alarm').jtable('changeP',current);
		
		
		var refreshPageInterval = eval($('#refreshPageInterval').val()) * 1000;
		intPage = setInterval(refreshPage, refreshPageInterval);
		console.log('go page',current);
		console.log('refreshPageInterval',refreshPageInterval)
	}
	
	function refreshStatus(){
		clearInterval(intStatus);
		totalAlarm();
		var refreshStatusInterval = eval($('#refreshStatusInterval').val()) * 1000;
		intStatus = setInterval(refreshStatus, refreshStatusInterval);
    ...
    function totalAlarm(){	
        ...
        $.ajax({
            type: "post",
            data: "a=<?php echo $a ?>&function=getTotalAlarm",
    ...
    function filePreview(file){			
        ...
        $.ajax({
            type: "post",
            data: "a=<?php echo $a ?>&function=filePreview&file="+file,
            url: "view/<?php echo $DIR ?>/monitoring/dashboard-tapbox/svc-deviceid.php",
            dataType: "json",
            async: true,
            success: function (res) {
    ...
    function listDevice(){
        ...
        $('#table-device').jtable({
			...
			selecting: true,
			actions: {
				listAction: "/<?php echo $modul?>/svc-deviceid.php?function=getDevice&a=<?php echo $a?>&strdeviceid="+$("#strdeviceid").val(),
			},
			fields: {
				NO : {title: 'No',width: '3%'},
...
<div class="row">
	...
	<div class="col-lg-3">
		...
		<div class="panel panel-default">
			...
			<div class="panel-heading">
				<i class="fa fa-gear fa-fw"></i> Konfigurasi
			</div>						
			<div class="panel-body">
				<div class="list-group">
				    <a class="list-group-item">
					   Interval Pindah Halaman (detik): <input type="number" id="refreshPageInterval" value="5" class="form-control">
				    </a>
				    <a class="list-group-item">
					   Interval Resfresh Status (detik): <input type="number" id="refreshStatusInterval" value="10" class="form-control">
				    </a>
				</div>
			</div>
		</div>
