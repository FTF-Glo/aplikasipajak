...
class LaporPajak extends Pajak
{
    ...
    {
        parent::__construct();
        $PAJAK = isset($_POST['nama_kegiatan']) ? $_POST['nama_kegiatan'] : array();
        // var_dump($PAJAK);
        // die;
    ...
    {
        $Op = new ObjekPajak();
        $arr_rekening = $this->getRekening("4.1.01.06");
        $list_nop = array();

        $query = "
			SELECT DOC.*, DATE_FORMAT(DOC.CPM_TGL_JATUH_TEMPO,'%d-%m-%Y') as CPM_TGL_JATUH_TEMPO
			FROM PATDA_HOTEL_DOC AS DOC WHERE DOC.CPM_ID = '{$this->_id}' LIMIT 0,1";
        ...
        $pajak['ARR_TIPE_PAJAK'] = $this->arr_tipe_pajak_hotel;
        ...
        $html = "<div class=\"filtering\">
                        ...
                        <table>
                            <tr valign=\"bottom\">
                                ...
                                <td><br><input type=\"hidden\" style=\"width:50px; height:30px;\" class=\"btn btn-sm btn-secondary\" id=\"cari-{$id}\"></td>           
                            </tr>
                        </table>
    ...
    public function grid_data()
    {
        try {
            ...
            #count utk pagging
            $query = "SELECT COUNT(*) AS RecordCount FROM {$this->PATDA_HOTEL_DOC} pj
                        INNER JOIN {$this->PATDA_HOTEL_PROFIL} pr ON pj.CPM_ID_PROFIL = pr.CPM_ID
                        INNER JOIN {$this->PATDA_HOTEL_DOC_TRANMAIN} tr ON pj.CPM_ID = tr.CPM_TRAN_HOTEL_ID
                        WHERE {$where}";
            $result = mysqli_query($this->Conn, $query);
            ...
            $query = "SELECT pj.CPM_ID, pj.CPM_NO, pj.CPM_TAHUN_PAJAK,
                        ...
                        FROM {$this->PATDA_HOTEL_DOC} pj INNER JOIN {$this->PATDA_HOTEL_PROFIL} pr ON pj.CPM_ID_PROFIL = pr.CPM_ID
                        INNER JOIN {$this->PATDA_HOTEL_DOC_TRANMAIN} tr ON pj.CPM_ID = tr.CPM_TRAN_HOTEL_ID
                        WHERE {$where}
    ...
    public function grid_data_pembanding()
    {
        try {
            ...
            #print_r($arr_config);
            $Conn_gw = mysqli_connect($dbHost, $dbUser, $dbPwd, $dbName); # or die('mysql not connected');
            //mysql_select_db($dbName, $Conn_gw);# or die("db {$dbName} not available");
            ...
            $q = $query = "select
                        ...
                        sum(REPLACE(TransactionAmount,'.','')) as total
                        from {$dbTable}
                        WHERE {$where}
            ...
            #count utk pagging
            $query = "SELECT CPM_DEVICE_ID, CPM_NPWPD from PATDA_HOTEL_PROFIL WHERE CPM_AKTIF='1' {$where}";
            $result = mysqli_query($this->Conn, $query);
            $dataAll = array();
            $x = 0;
            while ($data = mysqli_fetch_assoc($result)) {
            ...
            foreach ($dataAll as $row) {
                ...
                $row = array_merge($row, array("NO" => ++$no));
                $row['CPM_NPWPD'] = "<a href='javascript:void(0)' onclick=\"javascript:getDetTranTapbox('{$json}')\">" . $row['CPM_NPWPD'] . "</a>";
                ...
                <script type=\"text/javascript\">
                    ...
                    $(document).ready(function() {
                        ...
                        $('#laporanPajak-{$this->_i}').jtable({
                            ...
                            pageSize: {$this->pageSize},
                            sorting: true,
                            defaultSorting: 'kode_kegiatan DESC',
                            selecting: true,
                            actions: {
                            ...
                            fields: {
                                NO : {title: 'No',width: '3%'},
                                kode_kegiatan: {key: true,list: false},
                                nama_kegiatan: {title:'Nama Kegiatan',width: '10%'}
                        ...
                        $('#cari-{$this->_i}').click(function (e) {
                            e.preventDefault();
                            $('#laporanPajak-{$this->_i}').jtable('load', {
                                kode_kegiatan : $('#kode_kegiatan-{$this->_i}').val(),
                                nama_kegiatan : $('#nama_kegiatan-{$this->_i}').val()
    ...
    {
        #count utk pagging
        $query = "SELECT COUNT(*) AS RecordCount FROM pelaksana_kegiatan";
        $result = mysqli_query($this->Conn, $query);
        $row = mysqli_fetch_assoc($result);
        $recordCount = $row['RecordCount'];
        ...
        #query select list data
        $query = "SELECT * FROM pelaksana_kegiatan 
                            ORDER BY {$_GET["jtSorting"]} LIMIT {$_GET["jtStartIndex"]},{$_GET["jtPageSize"]}";
        $result = mysqli_query($this->Conn, $query);
        // var_dump($this->_m);
        // die;
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $row['READ'] = strpos($row['kode_kegiatan'], ";{$_SESSION['uname']};") === false ? 0 : 1;

            $base64 = "a={$this->_a}&m={$this->_m}&mod={$this->_mod}&f={$this->_f}&id={$row['kode_kegiatan']}&s={$row['nama_kegiatan']}&i={$this->_i}&f={$this->_f}&read={$row['READ']}";
            $url = "main.php?param=" . base64_encode($base64);
            // var_dump($base64);
            // die;

            $row['nama_kegiatan'] = "<a href=\"{$url}\" title=\"Klik untuk detail\">{$row['nama_kegiatan']}</a>";
            $rows[] = $row;
        }
        $jTableResult = array();
        $jTableResult['Result'] = "OK";
        ...
        $jTableResult['TotalRecordCount'] = $recordCount;
        $jTableResult['Records'] = $rows;
        // var_dump($jTableResult['Records']);
        // die;
        print $this->Json->encode($jTableResult);

        mysqli_close($this->Conn);
    }
    public function grid_table_pelayanan_s()
    {
        $DIR = "PATDA-V1";
        $modul = "hotel";
        $html = "<link href=\"inc/{$DIR}/jtable/themes/jtable.min.css\" rel=\"stylesheet\" type=\"text/css\" />
    ...
    public function grid_data_pelayanan_s()
    {
        try {
    ...
    {
        $query = "SELECT * FROM {$this->PATDA_HOTEL_DOC_TRANMAIN} WHERE CPM_TRAN_HOTEL_ID='{$this->CPM_ID}' AND CPM_TRAN_FLAG='0'";
        $res = mysqli_query($this->Conn, $query);
        $data = mysqli_fetch_assoc($res);

        return $data['CPM_TRAN_HOTEL_VERSION'];
    }
    ...
    private function save_pajak($cpm_no = '')
    {
        ...
        if ($validasi['result'] == true || ($validasi['data']['CPM_TRAN_STATUS'] == '4')) {
            ...
            #update profil baru
            $query = "UPDATE {$this->PATDA_HOTEL_PROFIL} SET CPM_APPROVE ='1' WHERE CPM_NPWPD = '{$this->CPM_NPWPD}' AND CPM_AKTIF='1'";
            ...
            $this->CPM_BAYAR_LAINNYA = ($this->CPM_BAYAR_LAINNYA) ? str_replace(",", "", $this->CPM_BAYAR_LAINNYA) : 0;
            ...
            $query = sprintf(
                "INSERT INTO {$this->PATDA_HOTEL_DOC}
                    (CPM_ID, CPM_ID_PROFIL, CPM_NO,
                    CPM_MASA_PAJAK, CPM_TAHUN_PAJAK,CPM_TOTAL_OMZET,
                    CPM_TOTAL_PAJAK, CPM_TARIF_PAJAK,
                    CPM_METODE_HITUNG, CPM_KETERANGAN, CPM_VERSION, CPM_AUTHOR,
                    CPM_ID_TARIF, CPM_BAYAR_LAINNYA, CPM_DPP,
    ...
    {
        $nama_kegiatan = isset($_POST['nama_kegiatan']) ? mysqli_real_escape_string($this->Conn, trim($_POST['nama_kegiatan'])) : '';

        if (!empty($nama_kegiatan)) {
            $nama_kegiatan = strtoupper($nama_kegiatan);
            $sql = "INSERT INTO pelaksana_kegiatan (nama_kegiatan) VALUES ('$nama_kegiatan')";

            if (mysqli_query($this->Conn, $sql)) {
                $_SESSION['_success'] = 'Kegiatan ' . $nama_kegiatan . ' berhasil disimpan';
                return true;
            } else {
                $_SESSION['_error'] = 'Maaf Nama Kegiatan ' . $nama_kegiatan . ' Sudah Dimasukan';
                return false;
            }
        } else {
            $_SESSION['_error'] = 'Nama kegiatan tidak boleh kosong';
            return false;
        }
    }
    private function save_tranmain($param)
    {
        #insert tranmain
        $CPM_TRAN_ID = c_uuid();
        $CPM_TRAN_HOTEL_ID = $this->CPM_ID;
        $param['CPM_TRAN_INFO'] = isset($param['CPM_TRAN_INFO']) ? $param['CPM_TRAN_INFO'] : '';

        $query = "UPDATE {$this->PATDA_HOTEL_DOC_TRANMAIN} SET CPM_TRAN_FLAG = '1' WHERE CPM_TRAN_HOTEL_ID = '{$CPM_TRAN_HOTEL_ID}'";
        $res = mysqli_query($this->Conn, $query);

        $query = sprintf(
            "INSERT INTO {$this->PATDA_HOTEL_DOC_TRANMAIN}
                    (CPM_TRAN_ID, CPM_TRAN_HOTEL_ID, CPM_TRAN_HOTEL_VERSION, CPM_TRAN_STATUS, CPM_TRAN_FLAG, CPM_TRAN_DATE,
                    CPM_TRAN_OPR, CPM_TRAN_OPR_DISPENDA, CPM_TRAN_READ, CPM_TRAN_INFO)
                    VALUES ( '%s','%s','%s','%s','%s',
                             '%s','%s','%s','%s','%s')",
            $CPM_TRAN_ID,
            $CPM_TRAN_HOTEL_ID,
            $param['CPM_TRAN_HOTEL_VERSION'],
    ...
    private function update_tgl_input()
    {
        ...
        $query = "UPDATE {$this->PATDA_HOTEL_DOC} SET CPM_TGL_INPUT = '{$tgl_input}'
    ...
    private function update_tgl_lapor()
    {
        ...
        $query = "UPDATE {$this->PATDA_HOTEL_DOC} SET CPM_TGL_LAPOR = '{$tgl_input}'
                  WHERE CPM_ID ='{$this->CPM_ID}'";
    ...
    private function update_tgl_lapor_ditolak($cpm_no, $tgl_lapor, $tgl_input)
    {
        ...
        if ($tgl_input == 'NULL') {
            $query = "UPDATE {$this->PATDA_HOTEL_DOC} SET CPM_TGL_LAPOR = '{$tgl_lapor}'
                  WHERE CPM_NO ='{$cpm_no}'";
        } else {
            $query = "UPDATE {$this->PATDA_HOTEL_DOC} SET CPM_TGL_LAPOR = '{$tgl_lapor}', CPM_TGL_INPUT = '{$tgl_input}'
                  WHERE CPM_NO ='{$cpm_no}'";
        }
    ...
    public function save_a()
    {
        ...
        if ($this->save_pajak($this->CPM_NO)) {
            $param = array();
            $param['CPM_TRAN_HOTEL_VERSION'] = "1";
    ...
    public function new_version()
    {
        ...
        if ($this->save_pajak($this->CPM_NO)) {
            ...
            $query = "UPDATE {$this->PATDA_HOTEL_DOC_TRANMAIN} SET CPM_TRAN_FLAG ='1' WHERE CPM_TRAN_HOTEL_ID='{$id}'";
            mysqli_query($this->Conn, $query);

            $param['CPM_TRAN_HOTEL_VERSION'] = $new_version;
            $param['CPM_TRAN_STATUS'] = "1";
            $param['CPM_TRAN_FLAG'] = "0";
            $param['CPM_TRAN_DATE'] = date("d-m-Y");
            $param['CPM_TRAN_OPR'] = $this->CPM_AUTHOR;
    ...
    public function update()
    {
        ...
        if ($validasi['result'] == true) {
            ...
            $query = sprintf(
                "UPDATE {$this->PATDA_HOTEL_DOC} SET
                    CPM_TOTAL_OMZET = '%s',
                    CPM_TOTAL_PAJAK = '%s',
                    CPM_TARIF_PAJAK = '%s',
                    CPM_BAYAR_LAINNYA = '%s',
    ...
    {
        $query = "DELETE FROM {$this->PATDA_HOTEL_DOC} WHERE CPM_ID ='{$this->CPM_ID}'";
        $res = mysqli_query($this->Conn, $query);
        if ($res) {
            $query = "DELETE FROM {$this->PATDA_HOTEL_DOC_TRANMAIN} WHERE CPM_TRAN_HOTEL_ID ='{$this->CPM_ID}'";
            mysqli_query($this->Conn, $query);
        }
    }
    ...
    {
        $query = "UPDATE {$this->PATDA_HOTEL_DOC} SET CPM_TGL_JATUH_TEMPO = {$expired_date}
                  WHERE CPM_ID ='{$this->CPM_ID}'";
        return mysqli_query($this->Conn, $query);
    }
    ...
    public function print_sptpd_base()
    {
        ...
        $html = "<table width=\"710\" class=\"main\" cellpadding=\"5\" border=\"1\" cellspacing=\"0\">
                    ...
                    <tr>
                        <td width=\"710\" colspan=\"2\" align=\"left\"><strong>II. DIISI OLEH PENGUSAHA HOTEL</strong></td>
                    </tr>
                    <tr>
                        <td width=\"710\" colspan=\"2\"><table width=\"700\" align=\"center\" cellpadding=\"1\" border=\"1\" cellspacing=\"0\">
                                <tr>
                                    <td align=\"left\" colspan=\"2\">&nbsp;&nbsp;&nbsp;a. Klasifikasi Hotel : {$DATA['profil']['CPM_REKENING']} - {$DATA['pajak']['ARR_REKENING'][$DATA['profil']['CPM_REKENING']]['nmrek']}
                                    </td>
                                </tr>
                                <tr>
                                    <td align=\"left\" width=\"270\">&nbsp;&nbsp;&nbsp;b. Pembayaran Pemakaian kamar hotel</td>
                        ...
                        <td colspan=\"2\"><table width=\"710\" border=\"1\" cellpadding=\"3\">
                                <tr>
                                    ...
                                    <td width=\"310\" valign=\"top\" align=\"center\">
                                        ...
                                        <b style=\"font-size:55px\">PAJAK HOTEL</b><br/>
					...
					<tr style=\"font-size:32px\">
                        <td width=\"710\" colspan=\"2\" align=\"center\"><table width=\"700\" align=\"center\" cellpadding=\"1\" border=\"0\" cellspacing=\"0\">
                                <tr>
                                    ...
                                    <td align=\"left\" width=\"270\">Golongan Hotel</td>
                        ...
                        <td><table width=\"900\" border=\"0\" class=\"child\" cellpadding=\"0\" cellspacing=\"0\">
                                <tr>
                                    <td><table width=\"900\" border=\"1\" cellpadding=\"3\">
                                            ...
                                            <tr>
                                                <td>1.</td>
                                                <td align=\"left\">Pembayaran pajak Hotel {$DATA['profil']['CPM_NAMA_OP']}
    ...
    public function read_dokumen()
    {
        if (isset($_REQUEST['read']) && $_REQUEST['read'] == 0) {
            ...
            $select = "SELECT CPM_TRAN_READ FROM {$this->PATDA_HOTEL_DOC_TRANMAIN} WHERE CPM_TRAN_ID='{$idtran}'";
            $result = mysqli_query($this->Conn, $select);
            $data = mysqli_fetch_assoc($result);

            $read = $data['CPM_TRAN_READ'];
            $read = (trim($read) == "") ? ";{$_SESSION['uname']};" : "{$read};{$_SESSION['uname']};";
            $query = "UPDATE {$this->PATDA_HOTEL_DOC_TRANMAIN} SET CPM_TRAN_READ = '{$read}' WHERE CPM_TRAN_ID='{$idtran}'";
            mysqli_query($this->Conn, $query);
        }
    }
    ...
    public function get_previous_pajak($npwpd, $nop)
    {
        ...
        $query = "
			SELECT DOC.*, DATE_FORMAT(DOC.CPM_TGL_JATUH_TEMPO,'%d-%m-%Y') as CPM_TGL_JATUH_TEMPO
			FROM PATDA_HOTEL_DOC AS DOC
			INNER JOIN PATDA_HOTEL_PROFIL PR ON DOC.CPM_ID_PROFIL = PR.CPM_ID
        ...
        $pajak['CPM_TERBILANG'] = $this->SayInIndonesian($pajak['CPM_TOTAL_PAJAK']);
        $pajak['CPM_JENIS_PAJAK'] = $this->id_pajak;
        $pajak['ARR_TIPE_PAJAK_HOTEL'] = $this->arr_tipe_pajak_hotel;
