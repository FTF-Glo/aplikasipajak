...
class WajibPajak extends Pajak
{
    ...
    {
        $kode_kegiatan = isset($_POST['kode_kegiatan']) ? mysqli_real_escape_string($this->Conn, trim($_POST['kode_kegiatan'])) : '';
        $query = "DELETE FROM pelaksana_kegiatan WHERE kode_kegiatan = '$kode_kegiatan'";
        $res = mysqli_query($this->Conn, $query);
        if ($res == true) {
            //$res = $this->delete_core_user();

            if ($res) {
                $_SESSION['_success'] = 'Kegiatan berhasil dihapus';
            } else {
                $_SESSION['_error'] = 'Kegiatan gagal dihapus';
            }
        } else {
            $_SESSION['_error'] = 'Kegiatan gagal dihapus';
        }
    ...
    public function update()
    {
        ...
        if (!empty($nama_kegiatan)) {
            ...
            $sql = "UPDATE pelaksana_kegiatan SET nama_kegiatan='$nama_kegiatan' WHERE kode_kegiatan='$kode_kegiatan'";
            // var_dump($sql);
            // die;
            if (mysqli_query($this->Conn, $sql)) {
                $_SESSION['_success'] = 'Kegiatan ' . $nama_kegiatan . ' berhasil Update';
                return true;
            } else {
    ...
    public function save_s()
    {
        ...
        if (mysqli_num_rows($res) == 0) {
            ...
            //tambahan
            $id_kecamatan_real = $this->get_id_kecamatan_real($this->CPM_KECAMATAN_WP);
            $id_kelurahan_real = $this->get_id_kelurahan_real($this->CPM_KELURAHAN_WP);
            //end
            //var_dump($id_kecamatan_real, $id_kelurahan_real);die;
            ...
            $kec = $id_kecamatan_real;
            $kel = substr($id_kelurahan_real, -2, 2);
            ...
            if ($res == true) {
                ...
                if ($res) {
                    $_SESSION['_success'] = 'WP berhasil disimpan';
                    $_SESSION['passtampil'] = $this->NPASSWORD;
                    $_SESSION['usertampil'] = $this->CPM_USER;
                } else {
                    $_SESSION['_error'] = 'WP gagal disimpan';
                }
            } else {
    ...
    private function save_core_user()
    {
        ...
        if ($res == true) {
            ...
            ('%s','%s','%s')", $this->CPM_USER, "aPatda", $this->CTR_RM_ID);
            $res = mysqli_query($this->Conn, $query) or die(mysqli_error($this->Conn));
        }
    ...
    public function getDataWP()
    {
        ...
        $query = "SELECT * FROM pelaksana_kegiatan WHERE kode_kegiatan = '{$this->_id}'";
    ...
    public function filtering($id)
    {
        ...
        $opt_asalwp = '<option value="">All</option><option value="1">Dalam Daerah</option><option value="2">Luar Daerah</option>';
                ...
                <script type=\"text/javascript\">
                    ...
                    $(document).ready(function() {
                        $('#{$idTable}-{$this->_i}').jtable({
                            ...
                            fields: {
                                NO : {title: 'No',width: '3%'},
                                kode_kegiatan: {list: false}, 
                                nama_kegiatan: {title: 'Nama Pelaksana Kegiatan'}, 
    ...
    public function grid_data()
    {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $base64 = "a={$this->_a}&m={$this->_m}&mod={$this->_mod}&f={$this->_f}&id={$row['kode_kegiatan']}&s={$row['CPM_STATUS']}&i={$this->_i}";
