...
class ObjekPajak extends Pajak {
    ...
    public function update() {
        ...
        $query = sprintf("UPDATE PATDA_HOTEL_PROFIL SET
                    ...
                    $this->CPM_TELEPON_WP, $this->CPM_TELEPON_OP, $this->CPM_JENIS_HOTEL);
	...
	public function update_device() {
		...
		if(count($acc_device) != 0){
			$query = sprintf("UPDATE PATDA_HOTEL_PROFIL 
				SET CPM_DEVICE_ID = '%s' 
    ...
    public function delete(){
		...
		if(isset($_POST['CPM_ID'])){
			...
			$query = "select count(*) as total FROM PATDA_HOTEL_PROFIL A INNER JOIN 
			PATDA_HOTEL_DOC B ON A.CPM_ID = B.CPM_ID_PROFIL
			WHERE A.CPM_ID = '{$_POST['CPM_ID']}'";
			
			$res = mysqli_query($this->Conn, $query);
			$res_data = mysqli_fetch_assoc($res);
			if($res_data['total'] == 0){
				$query = "DELETE FROM PATDA_HOTEL_PROFIL WHERE CPM_ID = '{$_POST['CPM_ID']}'";
				$res = mysqli_query($this->Conn, $query);
				if($res){ 
    ...
    public function save() {
        $query = "SELECT * FROM PATDA_HOTEL_PROFIL 
        ...
        $query = "UPDATE PATDA_HOTEL_PROFIL SET CPM_AKTIF ='0' WHERE CPM_NPWPD = '{$this->CPM_NPWPD}' AND CPM_NOP='{$this->CPM_NOP}'";
        $res = mysqli_query($this->Conn, $query);

        if($this->CPM_NOP==''){
            // get last nop
            $res_nop = mysqli_query($this->Conn, "SELECT CPM_NOP from PATDA_HOTEL_PROFIL WHERE LENGTH(CPM_NOP)=12 order by CPM_NOP desc limit 1");
        ...
        $query = sprintf("INSERT INTO PATDA_HOTEL_PROFIL 
                    ...
                    CPM_APPROVE, CPM_AUTHOR, CPM_NOP, CPM_TELEPON_WP, CPM_TELEPON_OP, CPM_JENIS_HOTEL, longitude, latitude)
                    VALUES ( '%s','%s','%s',
                             '%s','%s','%s',
                             '%s','%s',
                             '%s','%s',
                             ...
                             '%s','%s','%s','%s','%s','%s','%s','%s')",
                              ...
                              $this->CPM_AUTHOR, $this->CPM_NOP, $this->CPM_TELEPON_WP, $this->CPM_TELEPON_OP, $this->CPM_JENIS_HOTEL, $this->longitude, $this->latitude
        );
    ...
    public function get_last_profil($npwpd, $nop) {
        ...
        $query = "SELECT 
			...
			FROM PATDA_WP WP 
				LEFT JOIN PATDA_HOTEL_PROFIL P ON P.CPM_NPWPD = WP.CPM_NPWPD AND P.CPM_AKTIF='1' AND P.CPM_NOP='{$nop}'
    ...
    public function getRekening($kdrek='') {
        return parent::getRekening("4.1.01.06");
    }
    ...
    public function grid_data() {
        try {
            ...
            #count utk pagging
            $query = "SELECT COUNT(*) AS RecordCount FROM {$this->PATDA_HOTEL_PROFIL}
					WHERE {$where}";
            $result = mysqli_query($this->Conn, $query);
			...
			$query = "SELECT * FROM {$this->PATDA_HOTEL_PROFIL} WHERE {$where}
	...
	public function check_nop(){
		...
		$query = sprintf("SELECT COUNT(*) AS TOTAL FROM PATDA_HOTEL_PROFIL 
	...
	public function get_list_nop($npwpd){
		...
		$query = "SELECT PR.*,KEC.CPM_KECAMATAN,KEL.CPM_KELURAHAN FROM {$this->PATDA_HOTEL_PROFIL} PR
    ...
    // public function get_list_kegiatan($id){
	// 	$where = 
