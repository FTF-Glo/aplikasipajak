...
class LaporPajak extends Pajak
{
    ...
    {
        $Op = new ObjekPajak();
        $arr_rekening = $this->getRekening("4.1.01.08");
        $list_nop = array();

        $query = "
			SELECT DOC.*, DATE_FORMAT(DOC.CPM_TGL_JATUH_TEMPO,'%d-%m-%Y') as CPM_TGL_JATUH_TEMPO
			FROM PATDA_HIBURAN_DOC AS DOC WHERE DOC.CPM_ID = '{$this->_id}' LIMIT 0,1";
    ...
    public function filtering($id)
    {
        ...
        $reks = $this->getRekening("4.1.01.08");
        ...
        <div  class=\"p-2\">
            <div class=\"row\"> 
                <div class=\"col-12 pb-1\" style=\"display:flex; align-items:center;justify-content:end;\"> 
                    <button class=\"btn btn-success\" style=\"border-radius:8px\" type=\"button\" data-toggle=\"collapse\" data-target=\"#collapsFilter-{$id}\" aria-expanded=\"false\" aria-controls=\"collapsFilter-{$id}\">
        ...
        $html .= "   <div class=\" form-group col-md-3\"> 
                                        ...
                                        <div class=\" form-group col-md-12\">    
                                            ...
                                            <button type=\"button\" class=\"btn btn-success\" onclick=\"javascript:download_excel('{$id}','function/PATDA-V1/cetakpanjang/svc-download-bentang-panjang-min.xls.php')\"><i class=\"fa fa-print\"></i> Cetak Bentang Panjang</button>            
                    ...
                    $(document).ready(function() {
                        ...
                        $('#laporanPajak-{$this->_i}').jtable({
                            ...
                            fields: {
                                ...
                                CPM_MASA_PAJAK: {title: 'Masa Pajak',width: '15%'},
    ...
    public function grid_data()
    {
        try {
            ...
            #count utk pagging
            $query = "SELECT COUNT(*) AS RecordCount FROM {$this->PATDA_HIBURAN_DOC} pj
                            INNER JOIN {$this->PATDA_HIBURAN_PROFIL} pr ON pj.CPM_ID_PROFIL = pr.CPM_ID
                            INNER JOIN {$this->PATDA_HIBURAN_DOC_TRANMAIN} tr ON pj.CPM_ID = tr.CPM_TRAN_HIBURAN_ID
                            WHERE {$where}";
            ...
            $query = "SELECT pj.CPM_ID, pj.CPM_NO, pj.CPM_TAHUN_PAJAK,
                            ...
                            FROM {$this->PATDA_HIBURAN_DOC} pj INNER JOIN {$this->PATDA_HIBURAN_PROFIL} pr ON pj.CPM_ID_PROFIL = pr.CPM_ID
                            INNER JOIN {$this->PATDA_HIBURAN_DOC_TRANMAIN} tr ON pj.CPM_ID = tr.CPM_TRAN_HIBURAN_ID
    ...
    public function grid_data_pembanding()
    {
        try {
            ...
            #count utk pagging
            $query = "SELECT CPM_DEVICE_ID, CPM_NPWPD from PATDA_HIBURAN_PROFIL WHERE CPM_AKTIF='1' {$where}";
            $result = mysqli_query($this->Conn, $query);
            $dataAll = array();
            $x = 0;
            while ($data = mysqli_fetch_assoc($result)) {
                    ...
                    $(document).ready(function() {
                        ...
                        $('#laporanPajak-{$this->_i}').jtable({
                            ...
                            fields: {
                                NO : {title: 'No',width: '3%'},
                                CPM_ID: {key: true,list: false},
                                TIMESTAMP: {title:'Tanggal Input',width: '10%', type: 'date', displayFormat: 'dd-mm-yy'}, 
                                ...
                                CPM_TOTAL_PAJAK: {title: 'Total Pajak',width: '10%'},
                                CPM_VERSION: {title: 'Versi Dok',width: '8%'},
                                " . ($this->_s == 0 ? "CPM_TRAN_STATUS: {title: 'Status',width: '10%'}," : "") . "
                                " . ($this->_s == 4 ? "CPM_TRAN_INFO: {title: 'Keterangan',width: '10%'}," : "") . "
                                CPM_AUTHOR: {title: 'User Input',width: '10%'}
                            },
    ...
    public function grid_data_pelayanan()
    {
        try {
            ...
            $query = "SELECT pj.CPM_ID, pj.CPM_NO, pj.CPM_TAHUN_PAJAK,
                            ...
                            pj.CPM_TOTAL_PAJAK, pr.CPM_NPWPD, pr.CPM_NAMA_WP,pr.CPM_NAMA_OP, tr.CPM_TRAN_STATUS, tr.CPM_TRAN_INFO, tr.CPM_TRAN_FLAG,
                            tr.CPM_TRAN_READ, tr.CPM_TRAN_ID, pj.CPM_TGL_INPUT, tr.CPM_TRAN_DATE, pj.CPM_PIUTANG,pj.TIMESTAMP
                            FROM {$this->PATDA_HIBURAN_DOC} pj INNER JOIN {$this->PATDA_HIBURAN_PROFIL} pr ON pj.CPM_ID_PROFIL = pr.CPM_ID
                            INNER JOIN {$this->PATDA_HIBURAN_DOC_TRANMAIN} tr ON pj.CPM_ID = tr.CPM_TRAN_HIBURAN_ID
                            WHERE {$where}
                            ORDER BY {$_GET["jtSorting"]} LIMIT {$_GET["jtStartIndex"]},{$_GET["jtPageSize"]}";
    ...
    {
        $query = "SELECT * FROM PATDA_HIBURAN_DOC_TRANMAIN WHERE CPM_TRAN_HIBURAN_ID='{$this->CPM_ID}' AND CPM_TRAN_FLAG='0'";
        $res = mysqli_query($this->Conn, $query);
        $data = mysqli_fetch_assoc($res);

        return $data['CPM_TRAN_HIBURAN_VERSION'];
    }
    ...
    private function validasi_pajak($input = 1)
    {
        ...
        $query = "SELECT pj.CPM_NO, pj.CPM_TAHUN_PAJAK, pj.CPM_MASA_PAJAK, tr.CPM_TRAN_STATUS, tr.CPM_TRAN_FLAG
                FROM {$this->PATDA_HIBURAN_DOC} pj
                INNER JOIN {$this->PATDA_HIBURAN_DOC_TRANMAIN} tr ON pj.CPM_ID = tr.CPM_TRAN_HIBURAN_ID
                INNER JOIN {$this->PATDA_HIBURAN_PROFIL} pr ON pj.CPM_ID_PROFIL = pr.CPM_ID
                WHERE
                (
    ...
    private function save_pajak($cpm_no = '')
    {
        ...
        if ($validasi['result'] == true || ($validasi['data']['CPM_TRAN_STATUS'] == '4')) {
            ...
            #update profil baru
            $query = "UPDATE {$this->PATDA_HIBURAN_PROFIL} SET CPM_APPROVE ='1' WHERE CPM_NPWPD = '{$this->CPM_NPWPD}' AND CPM_AKTIF='1'";
            ...
            $query = sprintf(
                "INSERT INTO {$this->PATDA_HIBURAN_DOC}
                    (CPM_ID,CPM_ID_PROFIL,CPM_NO,
                    CPM_MASA_PAJAK,CPM_TAHUN_PAJAK,CPM_TOTAL_OMZET,
                    CPM_TOTAL_PAJAK,CPM_TARIF_PAJAK,CPM_KETERANGAN,
                    CPM_METODE_HITUNG,CPM_TGL_JATUH_TEMPO,CPM_VERSION,
    ...
    private function save_tranmain($param)
    {
        #insert tranmain
        $CPM_TRAN_ID = c_uuid();
        $CPM_TRAN_HIBURAN_ID = $this->CPM_ID;

        $query = "UPDATE {$this->PATDA_HIBURAN_DOC_TRANMAIN} SET CPM_TRAN_FLAG = '1' WHERE CPM_TRAN_HIBURAN_ID = '{$CPM_TRAN_HIBURAN_ID}'";
        $res = mysqli_query($this->Conn, $query);

        $query = sprintf(
            "INSERT INTO {$this->PATDA_HIBURAN_DOC_TRANMAIN}
                    (CPM_TRAN_ID, CPM_TRAN_HIBURAN_ID, CPM_TRAN_HIBURAN_VERSION, CPM_TRAN_STATUS, CPM_TRAN_FLAG, CPM_TRAN_DATE,
                    CPM_TRAN_OPR, CPM_TRAN_OPR_DISPENDA, CPM_TRAN_READ,CPM_TRAN_INFO)
                    VALUES ( '%s','%s','%s','%s','%s',
                             '%s','%s','%s','%s','%s')",
            $CPM_TRAN_ID,
            $CPM_TRAN_HIBURAN_ID,
            $param['CPM_TRAN_HIBURAN_VERSION'],
    ...
    private function update_tgl_input()
    {
        ...
        $query = "UPDATE {$this->PATDA_HIBURAN_DOC} SET CPM_TGL_INPUT = '{$tgl_input}'
    ...
    private function update_tgl_lapor()
    {
        ...
        $query = "UPDATE {$this->PATDA_HIBURAN_DOC} SET CPM_TGL_LAPOR = '{$tgl_input}'
                  WHERE CPM_ID ='{$this->CPM_ID}'";
    ...
    private function update_tgl_lapor_ditolak($cpm_no, $tgl_lapor, $tgl_input)
    {
        ...
        if ($tgl_input == 'NULL') {
            $query = "UPDATE {$this->PATDA_HIBURAN_DOC} SET CPM_TGL_LAPOR = '{$tgl_lapor}'
                  WHERE CPM_NO ='{$cpm_no}'";
        } else {
            $query = "UPDATE {$this->PATDA_HIBURAN_DOC} SET CPM_TGL_LAPOR = '{$tgl_lapor}', CPM_TGL_INPUT = '{$tgl_input}'
                  WHERE CPM_NO ='{$cpm_no}'";
        }
    ...
    public function save()
    {
        ...
        if ($this->save_pajak($this->CPM_NO)) {
            $param = array();
            $param['CPM_TRAN_HIBURAN_VERSION'] = "1";
    ...
    public function new_version()
    {
        ...
        if ($this->save_pajak($this->CPM_NO)) {
            ...
            $query = "UPDATE {$this->PATDA_HIBURAN_DOC_TRANMAIN} SET CPM_TRAN_FLAG ='1' WHERE CPM_TRAN_HIBURAN_ID='{$id}'";
            mysqli_query($this->Conn, $query);

            $param['CPM_TRAN_HIBURAN_VERSION'] = $new_version;
            $param['CPM_TRAN_STATUS'] = "1";
            $param['CPM_TRAN_FLAG'] = "0";
            $param['CPM_TRAN_DATE'] = date("d-m-Y");
            $param['CPM_TRAN_OPR'] = $this->CPM_AUTHOR;
    ...
    public function update_final()
    {
        ...
        if ($this->update()) {
            $param['CPM_TRAN_HIBURAN_VERSION'] = $this->CPM_VERSION;
            $param['CPM_TRAN_STATUS'] = "2";
            $param['CPM_TRAN_FLAG'] = "0";
            $param['CPM_TRAN_DATE'] = date("d-m-Y");
            $param['CPM_TRAN_OPR'] = $this->CPM_AUTHOR;
    ...
    public function update()
    {
        ...
        if ($validasi['result'] == true) {
            ...
            $query = sprintf(
                "UPDATE {$this->PATDA_HIBURAN_DOC} SET
                    CPM_TOTAL_OMZET = '%s',
                    CPM_TOTAL_PAJAK = '%s',
                    CPM_TARIF_PAJAK = '%s',
                    CPM_BAYAR_LAINNYA = '%s',
    ...
    {
        $query = "DELETE FROM {$this->PATDA_HIBURAN_DOC} WHERE CPM_ID ='{$this->CPM_ID}'";
        $res = mysqli_query($this->Conn, $query);
        if ($res) {
            $query = "DELETE FROM {$this->PATDA_HIBURAN_DOC_TRANMAIN} WHERE CPM_TRAN_HIBURAN_ID ='{$this->CPM_ID}'";
            mysqli_query($this->Conn, $query);
        }
    }
    ...
    // {
    //     $query = "UPDATE {$this->PATDA_HIBURAN_DOC} SET CPM_TGL_JATUH_TEMPO = {$expired_date}
    //               WHERE CPM_ID ='{$this->CPM_ID}'";
    //     return mysqli_query($this->Conn, $query);
    // }
    ...
    public function print_sptpd_base()
    {
        ...
        $html = "<table width=\"710\" class=\"main\" cellpadding=\"5\" border=\"1\" cellspacing=\"0\">
                    ...
                    <tr>
                        <td width=\"710\" colspan=\"2\" align=\"left\"><strong>II. DIISI OLEH PENGUSAHA HIBURAN</strong></td>
                    </tr>
                    <tr>
                        <td width=\"710\" colspan=\"2\"><table width=\"700\" align=\"center\" cellpadding=\"1\" border=\"1\" cellspacing=\"0\">
                                <tr>
                                    <td align=\"left\" colspan=\"2\">&nbsp;&nbsp;&nbsp;a. Klasifikasi Hiburan : {$DATA['profil']['CPM_REKENING']} - {$DATA['pajak']['ARR_REKENING'][$DATA['profil']['CPM_REKENING']]['nmrek']}
                                    </td>
                                </tr>
                                <tr>
                                    <td align=\"left\" width=\"270\">&nbsp;&nbsp;&nbsp;b. Pembayaran Hiburan</td>
                        ...
                        <td colspan=\"2\"><table width=\"710\" border=\"1\" cellpadding=\"3\">
                                <tr>
                                    ...
                                    <td width=\"310\" valign=\"top\" align=\"center\">
                                        ...
                                        <b style=\"font-size:55px\">PAJAK HIBURAN</b><br/>
					...
					<tr style=\"font-size:32px\">
                        <td width=\"710\" colspan=\"2\" align=\"center\"><table width=\"700\" align=\"center\" cellpadding=\"1\" border=\"0\" cellspacing=\"0\">
                                <tr>
                                    ...
                                    <td align=\"left\" width=\"270\">Golongan Hiburan</td>
            ...
            <tr>
                <td><table width=\"960\" border=\"1\" cellpadding=\"5\">
                        <tr>
                            ...
                            <td rowspan=\"2\">Untuk Pembayaran Pajak Hiburan</td>
                        </tr>
                        <tr>
...
<tr>
    ...
    public function read_dokumen()
    {
        if (isset($_REQUEST['read']) && $_REQUEST['read'] == 0) {
            ...
            $select = "SELECT CPM_TRAN_READ FROM {$this->PATDA_HIBURAN_DOC_TRANMAIN} WHERE CPM_TRAN_ID='{$idtran}'";
            $result = mysqli_query($this->Conn, $select);
            $data = mysqli_fetch_assoc($result);

            $read = $data['CPM_TRAN_READ'];
            $read = (trim($read) == "") ? ";{$_SESSION['uname']};" : "{$read};{$_SESSION['uname']};";
            $query = "UPDATE {$this->PATDA_HIBURAN_DOC_TRANMAIN} SET CPM_TRAN_READ = '{$read}' WHERE CPM_TRAN_ID='{$idtran}'";
            mysqli_query($this->Conn, $query);
        }
    }
    ...
    function download_excel()
    {
        ...
        while ($rowData = mysqli_fetch_assoc($result)) {
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('F' . $row, $this->arr_bulan[(int) $rowData['CPM_MASA_PAJAK']]);
            $objPHPExcel->getActiveSheet()->setCellValue('G' . $row, $rowData['CPM_TOTAL_PAJAK']);
            $row++;
        }

        // Rename sheet
        $objPHPExcel->getActiveSheet()->setTitle('Daftar Pembayaran Pajak');
    ...
    public function get_previous_pajak($npwpd, $nop)
    {
        ...
        $query = "
			SELECT DOC.*, DATE_FORMAT(DOC.CPM_TGL_JATUH_TEMPO,'%d-%m-%Y') as CPM_TGL_JATUH_TEMPO
			FROM PATDA_HIBURAN_DOC AS DOC
			INNER JOIN PATDA_HIBURAN_PROFIL PR ON DOC.CPM_ID_PROFIL = PR.CPM_ID
