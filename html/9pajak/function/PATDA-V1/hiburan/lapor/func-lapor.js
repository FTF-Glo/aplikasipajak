$(document).ready(function () {
    ...
    form.validate({
        rules: {
            ...
            "PAJAK[CPM_GOL_HIBURAN]": "required",
        ...
        messages: {
            ...
            "PAJAK[CPM_GOL_HIBURAN]": "harus diisi",
    ...
    var function_getval_tapbox = function () {
        ...
        $.ajax({
            ...
            url: "function/PATDA-V1/hiburan/lapor/svc-lapor.php",
            dataType: "json",
    ...
    $("#CPM_NPWPD").keyup(function () {
        ...
        $("#CPM_NAMA_OP").val('');
        $("#CPM_ALAMAT_OP").val('');
        $("#CPM_GOL_HIBURAN option").prop('selected', false).attr('disabled', 'disabled');
    ...
    $("#btn-search-npwpd").click(function () {
        ...
        $.ajax({
            type: "POST",
            data: "PAJAK[CPM_NPWPD]=" + npwpd + "&function=search_npwpd&PAJAK[CPM_JENIS_PAJAK]=2",
            url: "function/PATDA-V1/hiburan/lapor/svc-lapor.php",
            dataType: "json",
            success: function (res) {
                ...
                if (res.result == 1) {
                    ...
                    $("#CPM_GOL_HIBURAN option[value='" + res.CPM_GOL_HIBURAN + "']").prop('selected', true).removeAttr('disabled');
                    $('#CPM_TARIF_PAJAK').val($("#CPM_GOL_HIBURAN option[value='" + res.CPM_GOL_HIBURAN + "']").attr('tarif'));
                    function_getval_tapbox();
                } else {
...
function getDetTranTapbox(q, json) {
    ...
    $.ajax({
        data: "i=6&p=" + json + "&q=" + q,
        type: "post",
        url: "view/PATDA-V1/hiburan/svc-list-pembanding-detail.php",
...
// }
function selisihBulan(awal_bulan) {
    // Memecah string tanggal awal menjadi hari, bulan, dan tahun
    var hari = parseInt(awal_bulan.substring(0, 2), 10);
    var bulan = parseInt(awal_bulan.substring(3, 5), 10) - 1; // Bulan dimulai dari 0 di JavaScript
    var tahun = parseInt(awal_bulan.substring(6, 10), 10);

    // Mendapatkan tanggal saat ini
    var date = new Date();
    var hari_ini = date.getDate();
    var bulan_sekarang = date.getMonth(); // Bulan saat ini (dimulai dari 0)
    var tahun_sekarang = date.getFullYear();

    // Menghitung selisih bulan
    var selisih_bulan = (bulan_sekarang + (12 * (tahun_sekarang - tahun))) - bulan;

    // Memeriksa apakah sudah melewati tanggal 15 di bulan berikutnya
    if (hari_ini >= 16 && bulan_sekarang > bulan) {
        selisih_bulan += 1;
    }
