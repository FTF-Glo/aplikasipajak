...
class ObjekPajak extends Pajak {
	...
	public function update_device() {
		...
		if(count($acc_device) != 0){
			$query = sprintf("UPDATE PATDA_HIBURAN_PROFIL 
				SET CPM_DEVICE_ID = '%s' 
		...
		if(count($err_device) != 0){
			$_SESSION['_error'] = 'Update gagal, device ID berikut sudah dipakai '.implode(', ',$err_device).'.';
		}
    ...
    public function delete(){
		...
		if(isset($_POST['CPM_ID'])){
			...
			$query = "select count(*) as total FROM PATDA_HIBURAN_PROFIL A INNER JOIN 
			PATDA_HIBURAN_DOC B ON A.CPM_ID = B.CPM_ID_PROFIL
			WHERE A.CPM_ID = '{$_POST['CPM_ID']}'";
			
			$res = mysqli_query($this->Conn, $query);
			$res_data = mysqli_fetch_assoc($res);
			if($res_data['total'] == 0){
				$query = "DELETE FROM PATDA_HIBURAN_PROFIL WHERE CPM_ID = '{$_POST['CPM_ID']}'";
				$res = mysqli_query($this->Conn, $query);
				if($res){ 
    ...
    public function save() {
        $query = "SELECT * FROM PATDA_HIBURAN_PROFIL 
        ...
        $query = "UPDATE PATDA_HIBURAN_PROFIL SET CPM_AKTIF ='0' WHERE CPM_NPWPD = '{$this->CPM_NPWPD}' AND CPM_NOP='{$this->CPM_NOP}'";
        $res = mysqli_query($this->Conn, $query);

        if($this->CPM_NOP==''){
            // get last nop
            $res_nop = mysqli_query($this->Conn, "SELECT CPM_NOP from PATDA_HIBURAN_PROFIL WHERE LENGTH(CPM_NOP)=12 order by CPM_NOP desc limit 1");
    ...
    public function get_last_profil($npwpd, $nop) {
        ...
        $query = "SELECT 
			...
			FROM PATDA_WP WP 
				LEFT JOIN PATDA_HIBURAN_PROFIL P ON P.CPM_NPWPD = WP.CPM_NPWPD AND P.CPM_AKTIF='1' AND P.CPM_NOP='{$nop}'
    ...
    public function getRekening($kdrek='') {
        return parent::getRekening("4.1.01.08");
    }
    ...
    public function grid_data() {
        try {
            ...
            #count utk pagging
            $query = "SELECT COUNT(*) AS RecordCount FROM {$this->PATDA_HIBURAN_PROFIL}
					WHERE {$where}";
            $result = mysqli_query($this->Conn, $query);
			...
			$query = "SELECT * FROM {$this->PATDA_HIBURAN_PROFIL} WHERE {$where}
	...
	public function check_nop(){
		...
		$query = sprintf("SELECT COUNT(*) AS TOTAL FROM PATDA_HIBURAN_PROFIL 
	...
	public function get_list_nop($npwpd){
		...
		$query = "SELECT PR.*,KEC.CPM_KECAMATAN,KEL.CPM_KELURAHAN FROM {$this->PATDA_HIBURAN_PROFIL} PR
