...
if (empty($value['CPM_KOTA_WP']) && (!isset($value['CPM_ID']) || $value['CPM_ID'] == '')) {
    $value['CPM_KOTA_WP'] = 'Lampung Tengah';
    $readonly_cty = true;
}
...
<form class="cmxform" id="form-wp" method="post" enctype="multipart/form-data" action="function/<?php echo "{$DIR}/{$modul}"; ?>/svc-wp.php?param=<?php echo base64_encode($json->encode(array("a" => $wp->_a, "m" => $wp->_m))) ?>">
    ...
    <table class="main" width="900">
        ...
        <tr>
            ...
            <td>:
                <label><input type="radio" name="WP[CPM_LUAR_DAERAH]" class="CPM_LUAR_DAERAH" value="0" <?php echo (isset($value['CPM_LUAR_DAERAH'])? ($value['CPM_LUAR_DAERAH'] == 0 ? 'checked' : '') : 'checked') ?> <?php echo (!empty($value['CPM_USER']) ? "onclick='javascript:return true'" : '') ?>> Daerah Kab Lampung Tengah</label> <br>&nbsp;
                <label><input type="radio" name="WP[CPM_LUAR_DAERAH]" class="CPM_LUAR_DAERAH" value="1" <?php echo (isset($value['CPM_LUAR_DAERAH'])? ($value['CPM_LUAR_DAERAH'] == 1 ? 'checked' : '') : '') ?> <?php echo (!empty($value['CPM_USER']) ? "onclick='javascript:return true'" : '') ?>> Luar Daerah Kab Lampung Tengah</label>
            </td>
        </tr>
        <tr>
