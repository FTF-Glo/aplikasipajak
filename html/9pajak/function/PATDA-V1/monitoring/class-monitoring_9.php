...
class MonitoringPajak extends Pajak {
	...
	public function filtering_status_bayar($id) {
		...
		if($id=="1"){
			...
			$bank = "Bank : <br/><select id=\"bank-{$id}\" name=\"bank-{$id}\">{$opt_bank}</select>";
			$lbl_date1 = "Tgl Bayar awal";
			$lbl_date2 = "Tgl Bayar akhir";
		}
		
		if($id=="1"){
			$operator .= "Operator : <br/><input type=\"text\" name=\"operator-{$id}\" id=\"operator-{$id}\" />";
		}
		...
		$opt_jenis_lapor = "<option value=\"\">Semua</option>";
		...
		$html = "<div class=\"filtering\">
					...
					<form>
						<table width=\"1100\" border=\"0\" cellspacing=\"0\" cellpadding=\"2\" class=\"monitoring\">
								<tr>
									<td>Kode Bayar : <br/><input type=\"text\" name=\"payment_code-{$id}\" id=\"payment_code-{$id}\" /></td>                                    
								
									<td width=\"150\">{$lbl_date1} : <br/><input type=\"text\" name=\"expired_date1\" id=\"expired_date1-{$id}\" class=\"datepicker\" size=\"10\" /></td>
									<td width=\"150\">{$lbl_date2} : <br/><input type=\"text\" name=\"expired_date2\" id=\"expired_date2-{$id}\" class=\"datepicker\" size=\"10\" /></td>
									<td width=\"150\">
										Jenis Pajak : <br/><select id=\"jenis-{$id}\" name=\"jenis-{$id}\">{$opt_jenis_pajak}</select>
									</td>
									...
									<td width=\"200\">
										<table border=\"0\" width=\"200\">
											<tr>
												<td width=\"110\">
													Tahun Pajak : <br/><select name=\"simpatda_tahun_pajak-{$id}\" id=\"simpatda_tahun_pajak-{$id}\">{$opt_tahun}</select>
												</td>
												<td  width=\"40\">
													Bulan Pajak : <select name=\"simpatda_bulan_pajak-{$id}\" id=\"simpatda_bulan_pajak-{$id}\">{$opt_bulan}</select>
												</td>
											</tr>
										</table>
									</td>
									
									<td width=\"".((!empty($bank))?"100" : "300")."\">
										Jenis Pelaporan : <br/><select id=\"jenis_lapor-{$id}\" name=\"jenis_lapor-{$id}\">{$opt_jenis_lapor}</select>
								...
								<tr>
									<td>No. Pelaporan / NPWPD : <br/><input type=\"text\" name=\"sptpd-{$id}\" id=\"sptpd-{$id}\" /></td>                                    
									<td>Nama WP / Tempat Usaha : <br/><input type=\"text\" name=\"wp_nama\" id=\"wp_nama-{$id}\" /></td>                                    
									
									<td> 
										Alamat : <br/><input type=\"text\" name=\"wp_alamat\" id=\"wp_alamat-{$id}\"/>
									</td>
									
									<td>
										Nilai Tagihan : <br/><select id=\"simpatda_dibayar-{$id}\" name=\"simpatda_dibayar-{$id}\">
									...
									<td colspan=\"2\">
										...
										<input type=\"submit\" name=\"button2\" id=\"cari-{$id}\" value=\"Tampilkan\"/>
										<input type=\"button\" name=\"buttonToExcel\" id=\"buttonToExcel\" value=\"Ekspor ke xls\" onClick=\"toExcel({$id})\"/>
					...
					$(document).ready(function() {
						...
						$('#laporanPajak-{$this->_i}').jtable({
							...
							fields: {
								...
								jenis: {title: 'Jenis Pajak',width: '10%',edit :false},
		...
		try {
			...
			if(!empty($_REQUEST['jenis'])){
				...
				if(empty($_REQUEST['jenis_lapor'])){
					...
					$where .= " AND simpatda_type='{$jenis}'";
				}
			}elseif(!empty($_REQUEST['jenis_lapor'])){
				$where .= ($_REQUEST['jenis_lapor'] == 1)? " AND simpatda_type<=12 " : "AND simpatda_type>12 ";
			}
			...
			if ($_REQUEST['simpatda_dibayar'] != 0) {
				$arr_dibayar = array(1 => " (simpatda_dibayar < 5000000) ",
						...
						<form>
							<table border=\"0\" cellspacing=\"0\" cellpadding=\"2\" class=\"monitoring\">
								<tr>
									<td>Tahun Pajak </td>
									<td> :
										<select name=\"CPM_TAHUN_PAJAK-{$id}\" id=\"CPM_TAHUN_PAJAK-{$id}\">{$opt_tahun}</select>   
										<select name=\"CPM_MASA_PAJAK-{$id}\" id=\"CPM_MASA_PAJAK-{$id}\">{$opt_bulan}</select>
									</td>
									<td></td>
									<td>NPWPD</td>
									<td>: <input type=\"text\" name=\"CPM_NPWPD-{$id}\" id=\"CPM_NPWPD-{$id}\"></td>
									<td></td>
									<td>Kecamatan</td>
								...
								<tr>
									<td>Jenis Pajak</td>
									<td>: <select id=\"CPM_JENIS_PAJAK-{$id}\" name=\"CPM_JENIS_PAJAK-{$id}\">{$opt_jenis_pajak}</select>                                        
									</td>   
									<td></td>
									<td>Nama WP / Tempat Usaha</td>
									<td>: <input type=\"text\" name=\"CPM_NAMA_WP-{$id}\" id=\"CPM_NAMA_WP-{$id}\"></td>
									<td></td>
									<td>Kelurahan</td>
									<td>: <select name=\"kelurahan-{$id}\" id=\"kelurahan-{$id}\" style=\"width:92%\">{$opt_kelurahan}</select></td>
									<td></td>
									<td colspan=\"2\">
										<input type=\"button\" name=\"button2\" id=\"cari-{$id}\" value=\"Tampilkan\"/>
										<input type=\"button\" name=\"buttonToExcel\" id=\"buttonToExcel\" value=\"Ekspor ke xls\" onClick=\"javascript:toExcelBelumLapor({$id})\"/>
										<span id=\"loadlink-{$id}\" style=\"font-size: 10px; display: none;\">Loading...</span>
									</td>
								</tr>
							</table>
	...
	public function download_excel_status_bayar() {
		...
		if($_REQUEST['jenis']==29){
			$this->download_excel_status_bayar_mineral();exit;
		}
		$arr_config = $this->get_config_value($this->_a);
		...
		$objPHPExcel->getActiveSheet()->getStyle('A'.($row+3).':Q'.($row+3).'')->applyFromArray(
				array(
					'font' => array(
						'bold' => true
					),
	...
	public function download_pdf_status_bayar() {
		if($_REQUEST['jenis']==29){
			$this->download_pdf_status_bayar_mineral();exit;
		}
		global $sRootPath;
		$arr_config = $this->get_config_value($this->_a);
		...
		$query = "select jenis,op_nama,op_alamat, sptpd, npwpd,wp_nama, wp_alamat, simpatda_tahun_pajak,
		...
		$pdf->Image($sRootPath . "view/Registrasi/configure/logo/" . $LOGO_CETAK_PDF, 20, 5, 25, '', '', '', '', false, 300, '', false);
