...
class Payment extends Pajak{
	...
	public function inquiry(){
		...
		if($row>0){
			if($data = mysqli_fetch_object($res)){				
				if($data->patda_kurangbayar == 0){/*jika tagihan kurang bayar*/
					$data->RC = '88';
					$data->MSG = 'Tagihan Tidak Kurang Bayar.';
					...
					// echo "$banyak_bulan * 2/100 * ".$data->simpatda_dibayar;
					// exit();
					...
					$data->patda_sudah_dibayar = $data->patda_total_bayar;
					$data->patda_total_bayar = $data->patda_kurangbayar;
					$data->patda_total_sudahdibayar = $data->patda_collectible + $data->patda_denda;
					$data->RC = '00';
	...
	public function bayar(){
		...
		$his = date("H:i:s");
		$data->payment_kurangbayar = $data->payment_paid .' '. $his;
		$data->patda_kurangbayar = $data->patda_kurangbayar - $data->patda_total_bayar;
		$data->patda_total_bayar = $data->patda_sudah_dibayar + $data->patda_total_bayar;
		//var_dump($data->patda_kurangbayar, $data->patda_sudah_dibayar, $data->patda_total_bayar);die;
			...
			if($dataSettlement = mysqli_fetch_object($res)){
				...
				$query = sprintf("
					UPDATE SIMPATDA_GW SET
						...
						patda_total_bayar = '%s',
						patda_kurangbayar = '%s',
						payment_paid_kurangbayar = '%s'
					WHERE id_switching = '%s'",
					
					$data->simpatda_dibayar,
					$data->patda_denda,
					$data->patda_total_bayar,
					$data->patda_kurangbayar,
					$data->payment_kurangbayar,
					$inq->id_switching
				);
