...
class LaporPajak extends Pajak {
    ...
    public function save() {
		...
		$arr_rekening = $this->getRekening("4.1.1.1");
		...
		$NotAdmitReason = '';
		$Status = 0;
		$JumlahKamar = str_replace(",", "", $this->JumlahKamar); 
		$JumlahHari = str_replace(",", "", $this->JumlahHari); 
		$Golongan = $this->CPM_REKENING. ' - '.$arr_rekening['ARR_REKENING'][$this->CPM_REKENING]['nmrek'];
		$JenisKamar = $this->JenisKamar.' - '.$arr_jenis_kamar[$this->JenisKamar]->Jenis;
		$TarifKamar = str_replace(",", "", $this->TarifKamar);  
		...
		if($res){
			$ID = c_uuid();
			$query = sprintf("INSERT INTO TRANSACTION_ATR_HOTEL
			(ID, TransactionNumber, Golongan, NamaWP, NamaOP, JenisKamar, TarifKamar, JumlahKamar, JumlahHari, TransactionAmount, TarifPajak, 
			TaxAmount, Status)
			VALUES('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s','%s')",
				$ID, $TransactionNumber, $Golongan, $NamaWP, $NamaOP, $JenisKamar, $TarifKamar, $JumlahKamar, $JumlahHari, $TransactionAmount, 
				$TarifPajak, $TaxAmount, $Status
			);
			$res = mysql_query($query, $conn) or die(mysql_error());
		}
