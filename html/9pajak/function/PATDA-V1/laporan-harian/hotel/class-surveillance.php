...
class TransaksiSurveillance extends Pajak {
	...
	public function grid_data_pembanding_detail() {
		try {
            ...
            while ($data = mysqli_fetch_assoc($result)) {
                ...
                $dataTapbox['TarifKamar'] = number_format($data['TarifKamar'],2);
	...
	public function deleteRecord($TransactionNumber){		
		try {
			if($this->getRecordTran($TransactionNumber,"delete")){
				...
				if($result){
					$query = "DELETE FROM TRANSACTION_ATR_HOTEL WHERE TransactionNumber = '{$TransactionNumber}' ";		
					$result = mysqli_query($this->connSv, $query) or die(mysqli_error($this->connSv));
				}
    ...
    public function filtering_pembanding_detail($id) {
		...
		$url = "view/{$DIR}/laporan-harian/hotel/svc-download-tapbox.xls.php";
					...
					<div class=\"contentcontainer\">
						<table border=\"0\" width=\"500\">
							...
							<tr>
								...
								<td width=\"380\">: <span id=\"label-JenisKamar-{$this->_i}\"></span></td>
							</tr>
							<tr>
								<td width=\"120\">Tarif Kamar</td>
								<td width=\"380\">: <span id=\"label-TarifKamar-{$this->_i}\"></span></td>
							</tr>
							<tr>
								<td width=\"120\">Jumlah Kamar</td>
								<td width=\"380\">: <span id=\"label-JumlahKamar-{$this->_i}\"></span></td>
							</tr>
							<tr>
								<td width=\"120\">Jumlah Hari</td>
								<td width=\"380\">: <span id=\"label-JumlahHari-{$this->_i}\"></span></td>
							</tr>
							<tr>
        ...
        $html = "
                ...
                <script type=\"text/javascript\">
					...
					function viewData(json){
						...
						$('#label-TarifKamar-{$this->_i}').html(data.TarifKamar);
						$('#label-TarifPajak-{$this->_i}').html(data.TarifPajak+' %');
						$('#label-JumlahKamar-{$this->_i}').html(data.JumlahKamar+' buah');
						$('#label-JumlahHari-{$this->_i}').html(data.JumlahHari+' hari');
    ...
    function download_tapbox_xls() {
		...
		$query = "select * FROM TRANSACTION A INNER JOIN TRANSACTION_ATR_HOTEL B ON A.TransactionNumber = B.TransactionNumber
					WHERE {$where} ORDER BY A.TransactionDate DESC LIMIT {$offset}, {$total}";
