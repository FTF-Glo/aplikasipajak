...
$(function () {
	...
	$("input.btn-submit").click(function () {
		...
		} else if (action == "pembatalan_denda") {
			if (form.valid()) {
				res = confirm("Apakah anda yakin untuk membatalkan denda pada laporan ini?");
			}
...
function rekDetail(no) {
  ...
  function hitung_total() {
	...
	if (bulans < bulan_sekarangs || tahuns < tahun_sekarangs) {
		if (selisih_bulan > 1) {
			...
			if ($('#editable_terlambat_lap').val() == 1) {
				...
				//tamabahan var sanksi = omzet * (selisih_bulan -2) * 2 / 100;
				var sanksi = omzet * (selisih_bulan) * 2 / 100;
				sanksi = Math.round(parseFloat(sanksi));
				$('#CPM_DENDA_TERLAMBAT_LAP').autoNumeric('set', sanksi);
			}
		} else {
			var sanksi = 0;
		...
		if (selisih_bulan2 > 1) {
			// alert(selisih_bulan);w

			if ($('#editable_terlambat_lap').val() == 1) {
				var sanksi = omzet * (selisih_bulan2) * 2 / 100;
				var sanksi = $('#CPM_DENDA_TERLAMBAT_LAP').autoNumeric('get');
			} else {
				//tamabahan var sanksi = omzet * (selisih_bulan -2) * 2 / 100;
				var sanksi = omzet * (selisih_bulan2) * 2 / 100;
	...
	sanksi = Math.round(parseFloat(sanksi));
	var total = eval(omzet) + eval(kurangLebih);
	// console.log(sanksi);
