...
class LaporPajak extends Pajak
{
	...
	public function filtering($id)
	{
        ...
        <option value="2">Reklame Kain / Reklame Papan / Billboard / Videotron / Megatron</option>
        ';
                    ...
                    <form>
						<table>
							<tr valign=\"bottom\">
								...
								<td>Rekening :<br><select style=\"width:100px; height:30px;\" class=\"form-control\" name=\"CPM_KODE_REKENING-{$id}\" style=\"max-width:200px\" id=\"CPM_KODE_REKENING-{$id}\">{$opt_rekening}</select></td>
								</td>
							</tr>
						</table>
						<table>
                            <tr valign=\"bottom\">
                                ...
                                <td><br><button type=\"button\" style=\"width:150px; height:30px;\" class=\"btn btn-sm btn-secondary\" onclick=\"javascript:download_excel('{$id}','function/PATDA-V1/svc-download-bentang-panjang-rekl.xls.php')\">Cetak Bentang Panjang</button> </td>             
                            </tr>
                        </table>
					</form>
                </div> ";
	...
	public function grid_data()
	{
		try {
			...
			$where .= (isset($_REQUEST['CPM_JENIS_PJK']) && $_REQUEST['CPM_JENIS_PJK'] != "") ? " AND pj.CPM_TYPE_PAJAK='{$_REQUEST['CPM_JENIS_PJK']}' " : "";
	...
	public function print_skpd($type = "")
	{
		...
		foreach ($pajak_atr as $no => $atr) {
			...
			$html .= "<tr>
											...
											<td align=\"left\">
												...
												" . number_format($atr['CPM_ATR_PANJANG'], 2) . "M x {$atr['CPM_ATR_LEBAR']}M x " . intval($atr['CPM_ATR_TINGGI']) . " Tinggi x " . intval($atr['CPM_ATR_SISI']) . " Buah x " . intval($DATA['CPM_MASA_PAJAK']) . " {$DATA['CPM_JNS_MASA_PAJAK']} x <br>Rp. " . number_format($atr['CPM_ATR_TOTAL']) . " / {$DATA['CPM_JNS_MASA_PAJAK']} / M<sup>2</sup>
											</td>
		...
		$html .= "<tr>
										...
										<tr>
											...
											<td align=\"right\">
												<b>" . number_format(($DATA['CPM_TOTAL_OMZET'] + $DATA['CPM_DENDA_TERLAMBAT_LAP']), 0) . "</b>
											</td>
										</tr>
										<tr>
...
*/
	...
	public function print_sspd()
	{
		...
		$html .= " <tr>
					...
					<span style=\"font-size:24px\"><i>BPPRD PESAWARAN {$tgl_cetak}</i></span>
		...
		foreach ($pajak_atr as $no => $atr) {
			$row_atr .= "<tr>
											...
											<tr>
												...
												<td>{$atr['CPM_ATR_LEBAR']} M x {$atr['CPM_ATR_PANJANG']} M x {$atr['CPM_ATR_TINGGI']} Tinggi x {$atr['CPM_ATR_SISI']} Buah x {$DATA->CPM_MASA_PAJAK} {$DATA->CPM_JNS_MASA_PAJAK}</td>
	...
	public function get_hargadasar($params = array())
	{
		...
		// $muka = $muka > 3 ? 4 : $muka;
		$alkohol_rokok = (isset($_POST["alkohol_rokok"]) && $_POST["alkohol_rokok"] == 1) ? true : false;
		...
		if ($x == 'false') {
			...
			if ($gedung == 'DALAM') {
				...
				$hitung_total = "<b>Total Pajak sdasd</b><br>
    ...
    <tr>
        ...
        <td align="left" width="240" valign="top">
            <select name="PAJAK_ATR[CPM_TIPE_PAJAK][]" tabindex="' . $idx . '" id="CPM_NOP-' . $no . '" class="CPM_NOP"  style="max-width:260px">' . $tipe_pajak . '</select>
        </td>
        <td width="110" align="left" valign="top">Panjang <b class="isi">*</b></td>
        <td width="130" align="center" valign="top">
            <label id="load-type-' . $no . '"></label>
            <input name="PAJAK_ATR[CPM_ATR_PANJANG][]" type="text" class="form-control number" tabindex="' . ($idx + 4) . '" id="CPM_ATR_PANJANG-' . $no . '" size="11" maxlength="11" placeholder="Panjang"  />
        </td>
        <td rowspan="3" align="center" valign="top">
            <input style="margin-top:8px;"  name="PAJAK_ATR[CPM_ATR_SISI][]" type="text" class="form-control number" tabindex="' . ($idx + 7) . '" id="CPM_ATR_SISI-' . $no . '" value="" size="11" maxlength="11" placeholder="Jumlah"  />
            <b class="isi">*</b>
        </td>
        <td rowspan="3" align="center" valign="top">
    ...
    <tr>
        ...
        <td align="left" width="240" valign="top">
            <select name="PAJAK_ATR[CPM_ATR_NOP][]" tabindex="' . $idx . '" id="CPM_NOP-' . $no . '" class="CPM_NOP"  style="max-width:260px">' . $opt_nop . '</select>
        </td>
        <td align="left" valign="top">Lebar <b class="isi">*</b></td>
        <td align="center" valign="top">
            <input name="PAJAK_ATR[CPM_ATR_LEBAR][]" tabindex="' . ($idx + 5) . '" type="text" class="form-control number" id="CPM_ATR_LEBAR-' . $no . '" size="11" maxlength="11" placeholder="Lebar"  />
        </td>
    </tr>
    <tr>
        ...
        <td valign="top">
            <!--<select id="CPM_ATR_SUDUT_PANDANG-' . $no . '"  name="PAJAK_ATR[CPM_ATR_SUDUT_PANDANG][]">' . $opt_sudut_pandang . '</select>-->
            <input name="PAJAK_ATR[CPM_ATR_TINGGI][]" tabindex="' . ($idx + 6) . '" type="text" id="CPM_ATR_TINGGI-' . $no . '" class="form-control number" size="11" maxlength="11" value="" placeholder="Tinggi" />
        </td>
    </tr>
    <tr>
        ...
        <td class="ID_JAM-' . $no . '" align="left" valign="top">Jam <b class="isi">*</b></td>
        <td class="ID_JAM-' . $no . '" align="center" valign="top">
            <input class="form-control" style="width:150px;height:30px;display:inline-block;font-size:small;" name="PAJAK_ATR[CPM_ATR_JAM][]" tabindex="3" type="text" class="number" id="CPM_ATR_JAM-' . $no . '" size="11" maxlength="11" value="' . $DATA['pajak_atr'][0]['CPM_ATR_JAM'] . '" ' . ($lapor->_s == 1 || $lapor->_s == "" || ($lapor->_s == "4" && $lapor->_mod == "pel") ? "" : "readonly") . ' placeholder="Jam" />
        </td>
    </tr>
    <tr>
        <td>Jenis Waktu Pemakaian </td>
        <td>
            <select class="form-control" id="CPM_ATR_TYPE_MASA-' . $no . '" tabindex="' . ($idx + 2) . '"  name="PAJAK_ATR[CPM_ATR_TYPE_MASA][]">' . $opt_type_masa . '</select>
        </td>
        <td align="left" valign="top">Sudut Pandang<b class="isi">*</b></td>
        <td valign="top">
            <select class="form-control" style="height:30px;" id="CPM_ATR_SUDUT_PANDANG-' . $no . '"  name="PAJAK_ATR[CPM_ATR_SUDUT_PANDANG][]">' . $opt_sudut_pandang . '</select>
        </td>
        <td >
            <select name="PAJAK_ATR[CPM_ATR_GEDUNG][]" class="form-control CPM_GEDUNG-'.$no.'">
                <option value="LUAR" ' . ($DATA['pajak_atr'][0]['CPM_ATR_GEDUNG'] == 'LUAR' ? 'selected' : '') . ' ' . ($lapor->_s == 1 || $lapor->_s == "" || ($lapor->_s == "4" && $lapor->_mod == "pel") ? '' : 'disabled') . '>Luar Gedung</option>
                <option value="DALAM" ' . ($DATA['pajak_atr'][0]['CPM_ATR_GEDUNG'] == 'DALAM' ? 'selected' : '') . ' ' . ($lapor->_s == 1 || $lapor->_s == "" || ($lapor->_s == "4" && $lapor->_mod == "pel") ? '' : 'disabled') . '>Dalam Gedung</option>
            </select>
        </td>
    </tr>
    <tr>
        ...
        <td>
            <select class="form-control" style="height:30px;" id="CPM_ATR_KAWASAN-' . $no . '" tabindex="' . ($idx + 3) . '"  name="PAJAK_ATR[CPM_ATR_KAWASAN][]">' . $opt_kawasan_1 . '</select>
        </td>
		<td></td>
        <td >
            <label>Alkohol/Rokok </label>
				<select name="PAJAK_ATR[CPM_ATR_ALKOHOL_ROKOK][]" class="form-control CPM_ALKOHOL_ROKOK-'.$no.'">
					<option value="1" ' . ($DATA['pajak_atr'][0]['CPM_ATR_ALKOHOL_ROKOK'] == '1' ? 'selected' : '') . '>Ya</option>
					<option value="0" ' . ($DATA['pajak_atr'][0]['CPM_ATR_ALKOHOL_ROKOK'] == '0' ? 'selected' : '') . '>Tidak</option>
				</select>
        </td>
        <td>
            <label>Jalan Tol : </label>
			<select name="PAJAK_ATR[CPM_ATR_TOL][]" class="form-control CPM_TOL-'.$no.'">
				<option value="1" ' . ($DATA['pajak_atr'][0]['CPM_ATR_TOL'] == '1' ? 'selected' : '') . '>Ya</option>
				<option value="0" ' . ($DATA['pajak_atr'][0]['CPM_ATR_TOL'] == '0' ? 'selected' : '') . '>Tidak</option>
			</select>
        </td>
    </tr>
    <tr>
        ...
        <td>
            <select class="form-control" id="CPM_ATR_JALAN-' . $no . '" tabindex="' . ($idx + 3) . '"  name="PAJAK_ATR[CPM_ATR_JALAN][]">' . $opt_jalan . '</select>
        </td>
		<td></td>
		<td colspan="2">
			<button  type="button" onclick="hitungDetail('.$no.')" class="form-control btn btn-success" id="berhitung-'.$no.'">
					 HITUNG
			</button>
		</td>
    </tr>
    ...
    <tr>
        ...
        <td>
            <input name="PAJAK_ATR[CPM_ATR_HARGA_DASAR_UK][]" style="width: 260px;" placeholder="Harga Dasar Ukuran" type="text" class="number" id="CPM_ATR_HARGA_DASAR_UK-' . $no . '" readonly />
        </td>
    </tr>
    <tr>
        <td>Harga Dasar Ketinggian</td>
        <td>
            <input name="PAJAK_ATR[CPM_ATR_HARGA_DASAR_TIN][]" style="width: 260px;" placeholder="Harga Dasar Ketinggian" type="text" class="number" id="CPM_ATR_HARGA_DASAR_TIN-' . $no . '" readonly />
        </td>
    </tr>
    <tr>
    ...
    <tr>
        <td colspan="6" align="right" valign="top">
            ...
            <button type="button" onclick="delRow(' . $no . ')" >Hapus</button>
        </td>
    </tr>
</table>';
}
