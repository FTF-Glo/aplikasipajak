...
class LaporPajak extends Pajak {
	...
	public function get_previous_pajak($npwpd, $nop) {
        ...
        $query = "
			...
			FROM PATDA_REKLAME_DOC AS DOC
			INNER JOIN PATDA_REKLAME_PROFIL PR ON DOC.CPM_ID_PROFIL = PR.CPM_ID
        ...
        if(empty($pajak['CPM_ID'])){
			...
			$query = "SELECT atr.CPM_ATR_JENIS, atr.CPM_ATR_JUDUL, atr.CPM_ATR_LOKASI, atr.CPM_ATR_PANJANG, atr.CPM_ATR_LEBAR,
			...
			atr.CPM_ATR_REKENING, atr.CPM_ATR_TYPE_MASA, atr.CPM_ATR_KAWASAN, atr.CPM_ATR_PERHITUNGAN,
			atr.CPM_CEK_PIHAK_KETIGA, atr.CPM_NILAI_PIHAK_KETIGA,
			atr.CPM_ATR_NJOP, atr.CPM_ATR_NILAI_STRATEGIS,
			per.nmrek ,atr.CPM_ATR_TARIF, atr.CPM_ATR_JUMLAH_TAHUN,  atr.CPM_ATR_JUMLAH_BULAN,  atr.CPM_ATR_JUMLAH_MINGGU, atr.CPM_ATR_JUMLAH_HARI,
			per.type_masa, per.nmrek
			from PATDA_REKLAME_DOC_ATR AS atr
			INNER JOIN PATDA_REKLAME_DOC AS doc  ON doc.CPM_ID = atr.CPM_ATR_REKLAME_ID
			INNER JOIN {$this->PATDA_REK_PERMEN13} AS per ON per.kdrek = atr.CPM_ATR_REKENING
			WHERE atr.CPM_ATR_REKLAME_ID = '{$pajak['CPM_ID']}'";
        ...
        if ($validasi['result'] == true || ($validasi['data']['CPM_TRAN_STATUS'] == '4')) {
            ...
            if ($res) {
                ...
                for ($x = 0; $x < $j; $x++) {
					...
					//
					$res_hargadasar = $this->get_hargadasar(array(
							'panjang' => $panjang,
							'lebar' => $lebar,
							'muka' => $muka,
							'sisi' => $sisi,
					...
					$perhitungan = mysqli_real_escape_string($res_hargadasar['rumus_hitung']);
                    ...
                    $query = sprintf("INSERT INTO {$this->PATDA_REKLAME_DOC_ATR}
                            ...
                            CPM_ATR_NILAI_STRATEGIS, CPM_ATR_KAWASAN, CPM_ATR_JALAN, CPM_ATR_SUDUT_PANDANG, CPM_ATR_NJOP, CPM_ATR_PERHITUNGAN,CPM_CEK_PIHAK_KETIGA,CPM_NILAI_PIHAK_KETIGA,CPM_ATR_SISI, CPM_ATR_ID_PROFIL)
                            VALUES ('%s','%s',%f,%f,'%s',
                                    %f,%f,%f,%f,%f,
                                    %f,'%s','%s','%s','%s','%s',
                                    '%s','%s','%s','%s','%s','%s',
                                    '%s','%s','%s','%s','%s','%s','%s','%s')", $this->CPM_ID, $judul, $biaya, $harga, $lokasi, $lebar, $panjang, $jumlah, $jari, $muka, $tarif, $jumlah_tahun, $jumlah_hari, $jumlah_minggu, $jumlah_bulan, $batas_awal, $batas_akhir, $total,
                                    $norekening, $type_masa, $nilai_strategis, $kawasan, $jalan, $sudut_pandang, $njop, $perhitungan,$cek_pk, $nilai_pk, $sisi, $nop);
        ...
        if ($validasi['result'] == true) {
            ...
            for ($x = 0; $x < $j; $x++) {
                ...
                if($atr_id==''){
					...
					$query = sprintf("UPDATE {$this->PATDA_REKLAME_DOC_ATR} SET CPM_ATR_JUDUL='%s', CPM_ATR_BIAYA='%s', CPM_ATR_HARGA='%s',
						...
						WHERE CPM_ATR_ID='%s'
						", $judul, $biaya, $harga, $lebar, $panjang, $jumlah, $jari, $muka, $tarif, $jumlah_tahun, $jumlah_hari, $jumlah_minggu, $jumlah_bulan,
						$batas_awal, $batas_akhir, $total, $norekening, $type_masa, $lokasi,
						$nilai_strategis, $kawasan, $jalan, $sudut_pandang, $njop, $perhitungan, $cek_pk, $nilai_pk,
						$sisi,
						$atr_id
					);
				}
                        ...
                        <td colspan=\"2\"><table width=\"710\" border=\"1\" cellpadding=\"3\">
                                <tr>
                                    <td width=\"200\" valign=\"top\" align=\"center\">
										...
										<br/><br/>
										<br/>
                                        <b><br/>" . $JENIS_PEMERINTAHAN.' '.strtoupper($NAMA_PEMERINTAHAN)."</b>
                                    </td>
								...
								<tr>
									...
									<td width=\"310\"><table width=\"310\" class=\"header\" border=\"0\">
										<tr class=\"first\">
											<td>
												Kepada Yth. <br/>
												KEPALA BADAN PENGELOLAAN PAJAK DAN RETRIBUSI DAERAH<br/>
					...
					<tr>
						<td colspan=\"2\" align=\"center\">
							<table width=\"100%\" border=\"0\" align=\"left\" cellpadding=\"5s\">
								<tr>
									<td colspan=\"2\">Dengan menyadari sepenuhnya akan segala akibatnya termasuk sanksi-sanksi sesuai ketentuan perundang-undangan yang berlaku, saya meberitahukan bahwa apa yang telah saya beritahukan diatas beserta lampiran-lampirannya adalah benar, lengkap, jelas dan bersyarat.</td>
								</tr>
								<tr>
									<td width=\"355\"></td>
									<td align=\"center\">
...
*/
		...
		$html = "<table width=\"1015\" class=\"main\" border=\"1\">
				...
				<table width=\"1015\" border=\"0\" class=\"child\" cellpadding=\"0\" cellspacing=\"0\" style=\"font-size:28px\">
								<tr>
									...
									$html .= $row_atr;
									$html .= "<tr>
												<td colspan=\"6\" align=\"right\" style=\"border:none\"><b>DENDA/BIAYA ADMIN</b></td>
	...
	public function get_hargadasar($params = array()){
		...
		$tarif = $tarif/100;
		$luas = round($panjang * $lebar * $muka * $jumlah, 2);
		// $muka = $muka > 3 ? 4 : $muka;
		$response = array(
			'luas'=>$luas,
			'njop'=>0,
		...
		/* $query = sprintf("SELECT CPM_NILAI_FUNGSI FROM PATDA_REKLAME_NSPR
						WHERE CPM_NAMA_FUNGSI = '%s'
						AND CPM_INDEX_NILAI = 'Fungsi Ruang'",
						$kawasan);
		$res_nfr = mysqli_fetch_assoc(mysqli_query($this->Conn, $query));
		$NFR = $res_nfr['CPM_NILAI_FUNGSI'];

		$query = sprintf("SELECT CPM_NILAI_FUNGSI FROM PATDA_REKLAME_NSPR
						WHERE CPM_NAMA_FUNGSI = '%s'
						AND CPM_INDEX_NILAI = 'Fungsi Jalan'",
						$jalan);
		$res_nfj = mysqli_fetch_assoc(mysqli_query($this->Conn, $query));
		$NFJ = $res_nfj['CPM_NILAI_FUNGSI'];

		$query = sprintf("SELECT CPM_NILAI_FUNGSI FROM PATDA_REKLAME_NSPR
						WHERE CPM_NAMA_FUNGSI = '%s'
						AND CPM_INDEX_NILAI = 'Sudut Pandang'",
						$sudut_pandang);
		$res_nfp = mysqli_fetch_assoc(mysqli_query($this->Conn, $query));
		$NFS = $res_nfp['CPM_NILAI_FUNGSI']; */

		$arr_label = array('Bulan'=>4, 'Tahun'=>1);
		$tmasa = $arr_label[$durasi_label];

		$list_tarif = $this->list_tarif($kdrek, $kawasan, $tmasa);
		// echo "<pre>";
		// var_dump($list_tarif);exit();
		$response['harga'] = $list_tarif['tarif'];
		// echo'<pre>',print_r($list_tarif);exit;

		/* if ($kdrek == '4.1.1.4.02.1') {
			$data_nilai_strategis = 20000;
		}else{
			$query_nilai_strategis = sprintf("
				SELECT CPM_NJOP
				FROM PATDA_REKLAME_TARIF_NJOP
				WHERE CPM_LUAS_AWAL <= %s
				AND CPM_LUAS_AKHIR >= %s",
				$luas, $luas);
			$res_nilai_strategis = mysqli_fetch_assoc(mysqli_query($this->Conn, $query_nilai_strategis));
			$data_nilai_strategis = $res_nilai_strategis['CPM_NJOP'];
		} */

		if($x=='false'){

			$rumus = "1. NSPR (Nilai Strategis Pajak Reklame) = Bobot x Scor x Titik Simpul<br>
			2. Biaya Pemeliharaan = 5% (Per Seratus) x Biaya Pembuatan<br>
			3. NJOPR (Nilai Jual Objek Pajak Reklame) = Biaya Pembuatan + Biaya Pemeliharaan<br>
			4. Besaran Pajak = (NSPR + NJOPR) x Tarif";


			$biaya_pemeliharaan = ($list_tarif['njopr']['biaya_pemeliharaan']/100) * $list_tarif['njopr']['biaya_pembuatan'];
			$nspr = ($list_tarif['nspr']['bobot']/100) * $list_tarif['nspr']['scor'] * $list_tarif['nspr']['titik'];
			$njopr = $list_tarif['njopr']['biaya_pembuatan'] + $biaya_pemeliharaan;

			$total = $luas * $durasi * $list_tarif['tarif'];

			$hitung = "1. {$list_tarif['nspr']['bobot']}% x {$list_tarif['nspr']['scor']} x ".number_format($list_tarif['nspr']['titik'])." = ".number_format($nspr)."<br>
			2. {$list_tarif['njopr']['biaya_pemeliharaan']}% x ".number_format($list_tarif['njopr']['biaya_pembuatan'])." = ".number_format($biaya_pemeliharaan)."<br>
			3. ".number_format($list_tarif['njopr']['biaya_pembuatan'])." + ".number_format($biaya_pemeliharaan)." = ".number_format($njopr)."<br><br>
			<b>Total Pajak</b><br>= $luas M<sup>2</sup> x $durasi $durasi_label x Rp. ".number_format($list_tarif['tarif'])."<br>= ".number_format($total);
			// $njopr = 0;
			// $nspr = 0;
			// $total = 0;

			/* if ($kdrek == '4.1.1.4.02.1') {
				$rumus = "<b>1. NJOPR</b><br/>";
				$rumus .= "A. Panjang x Lebar = Luas<br/>";
				$rumus .= "B. Luas x Harga Dasar Ukuran x Masa Pajak = Dasar Ukuran Reklame<br/>";
				$rumus .= "C. Tinggi x Harga Dasar Ketinggian = Dasar Ketinggian Reklame<br/>";
				$rumus .= "D. Dasar Ukuran Reklame + Dasar Ketinggian Reklame = Jumlah NJOPR<br/>";
				$rumus .= "<b>2. NSPR</b><br/>";
				$rumus .= "A. NFR + NFJ + NSP = Jumlah Index<br/>";
				$rumus .= "B. Jumlah Index x Harga Nilai Strategis = Jumlah NSPR<br/>";

				$luas = floatval($panjang) * floatval($lebar);
				$dur = floatval($luas) * floatval($harga_dasar_uk) * floatval($durasi);
				$dkr = floatval($muka) * floatval($harga_dasar_tin);
				$jml_njopr = floatval($dur) + floatval($dkr);
				$jml_index = floatval($NFR) + floatval($NFJ) + floatval($NFS);
				$jml_nspr = floatval($jml_index) * floatval($data_nilai_strategis);

				$hitung = "<b>1. NJOPR</b><br/>";
				$hitung .= "A. ".number_format($panjang,2)." x ".number_format($lebar,2)." = ".number_format($luas,2)."<br/>";
				$hitung .= "B. ".number_format($luas,2)." x ".number_format($harga_dasar_uk,2)." x ".number_format($durasi,2)." = ".number_format($dur,2)."<br/>";
				$hitung .= "C. ".number_format($muka,2)." x ".number_format($harga_dasar_tin,2)." = ".number_format($dkr,2)."<br/>";
				$hitung .= "D. ".number_format($dur,2)." x ".number_format($dkr,2)." = ".number_format($jml_njopr,2)."<br/>";
				$hitung .= "<b>2. NSPR</b><br/>";
				$hitung .= "A. ".number_format($NFR,2)." + ".number_format($NFJ,2)." + ".number_format($NFS,2)." = ".number_format($jml_index,2)."<br/>";
				$hitung .= "B. ".number_format($jml_index,2)." x ".number_format($data_nilai_strategis,2)." = ".number_format($jml_nspr,2)."<br/>";
			} else {
				$rumus = "<b>1. NJOPR</b><br/>";
				$rumus .= "A. Panjang x Lebar = Luas<br/>";
				$rumus .= "B. Luas x Harga Dasar Ukuran = Dasar Ukuran Reklame<br/>";
				$rumus .= "C. Tinggi x Harga Dasar Ketinggian = Dasar Ketinggian Reklame<br/>";
				$rumus .= "D. Dasar Ukuran Reklame + Dasar Ketinggian Reklame = Jumlah NJOPR<br/>";
				$rumus .= "<b>2. NSPR</b><br/>";
				$rumus .= "A. NFR + NFJ + NSP = Jumlah Index<br/>";
				...
				$luas = floatval($panjang) * floatval($lebar);
				$dur = floatval($luas) * floatval($harga_dasar_uk);
				$dkr = floatval($muka) * floatval($harga_dasar_tin);
				$jml_njopr = floatval($dur) + floatval($dkr);
				$jml_index = floatval($NFR) + floatval($NFJ) + floatval($NFS);
				$jml_nspr = floatval($jml_index) * floatval($data_nilai_strategis);
				...
				$hitung = "<b>1. NJOPR</b><br/>";
				$hitung .= "A. ".number_format($panjang,2)." x ".number_format($lebar,2)." = ".number_format($luas,2)."<br/>";
				$hitung .= "B. ".number_format($luas,2)." x ".number_format($harga_dasar_uk,2)." = ".number_format($dur,2)."<br/>";
				$hitung .= "C. ".number_format($muka,2)." x ".number_format($harga_dasar_tin,2)." = ".number_format($dkr,2)."<br/>";
				$hitung .= "D. ".number_format($dur,2)." x ".number_format($dkr,2)." = ".number_format($jml_njopr,2)."<br/>";
				$hitung .= "<b>2. NSPR</b><br/>";
				$hitung .= "A. ".number_format($NFR,2)." + ".number_format($NFJ,2)." + ".number_format($NFS,2)." = ".number_format($jml_index,2)."<br/>";
			...
			if ($sudut_pandang == "2 Arah") {
				$rumus .= "<b>3. Jumlah Pajak Yang Harus Dibayarkan</b><br/>";
				$rumus .= "A. Jumlah NJOPR + Jumlah NSPR = NSR<br/>";
				$rumus .= "B. NSR x Muka x Tarif Pajak = Jumlah Pajak Yang Harus Dibayarkan<br/>";

				$jml_nrs = floatval($jml_njopr) + floatval($jml_nspr);
				$total = floatval($jml_nrs) * floatval($sisi) * floatval($tarif);

				$hitung .= "<b>3. Jumlah Pajak Yang Harus Dibayarkan</b><br/>";
				$hitung .= "A. ".number_format($jml_njopr,2)." + ".number_format($jml_nspr,2)." = ".number_format($jml_nrs,2)."<br/>";
				$hitung .= "B. ".number_format($jml_nrs,2)." x {$sisi} x ".number_format($tarif,2)." = ".number_format($total,2)."<br/>";
			}else{
				$rumus .= "<b>3. Jumlah Pajak Yang Harus Dibayarkan</b><br/>";
				$rumus .= "A. Jumlah NJOPR + Jumlah NSPR = NSR<br/>";
				$rumus .= "B. NSR x Muka x Tarif Pajak = Jumlah Pajak Yang Harus Dibayarkan<br/>";
 ...
 */
			$total = round($total);
			// if ($kdrek == '4.1.1.4.03.1' || $kdrek == '4.1.1.4.04.1' || $kdrek == '4.1.1.4.08.1') {
			// 	$query_nilai_strategis = sprintf("
			// 		SELECT CPM_NILAI
			// 		FROM PATDA_REKLAME_NILAI_STRATEGIS
			// 		WHERE CPM_REKENING = '%s'
			// 		AND CPM_KAWASAN = '%s'
			// 		AND CPM_UKURAN_AWAL <= %s
			// 		AND CPM_UKURAN_AKHIR >= %s",
			// 		$kdrek, $kawasan, $luas, $luas);
			// }else{
			// 	$query_nilai_strategis = sprintf("
			// 		SELECT CPM_NILAI
			// 		FROM PATDA_REKLAME_NILAI_STRATEGIS
			...
			// 		$kdrek, $kawasan);
			// }
			// $res_nilai_strategis = mysqli_query($this->Conn, $query_nilai_strategis);
			// $data_nilai_strategis = mysqli_fetch_assoc($res_nilai_strategis);

			// extract($response);
			// $nilai_strategis = $data_nilai_strategis['CPM_NILAI'];
			...
			// $total = ($luas * $nilai_strategis * $durasi * $tarif) * $jumlah;
			// $rumus = "(Luas x Nilai Strategis x Durasi x Tarif pajak) x Jumlah (Qty)<br/>";
			// $hitung = "(".number_format($luas,2)." x ".number_format($nilai_strategis,2)." x
			// ".number_format($durasi,2)." x ".number_format($tarif,2).") x ".number_format($jumlah,0);
			...
			$html .= "Luas Reklame : $luas M<sup>2</sup> <br/>";
			// $html .= 'Nilai Strategis : ' . number_format($nilai_strategis,2) . " <br/>";
			$html .= 'Durasi : ' . $durasi.' '.$durasi_label . " <br/>";
			$html .= 'Tarif Pajak : ' . number_format($tarif*100,0) . "% <br/>";
			$html .= 'Besaran Pajak : Rp. ' . number_format($list_tarif['tarif'],0) . " / $durasi_label <br/>";
			$html .= "</td></tr></table>";
			$html .= "</div>";
        ...
        // echo $x."else";exit();
            ...
            $html .= "<br><font color=\"blue\">Total Pajak : (".number_format($npk)." X ".intval($tarif*100)."%)<font>";
            $html .= "<br><font color=\"blue\">Total Pajak : ".number_format($total)."<font>";
			$rumus = $html;
		}
		$response['total'] = $total;
		$response['html'] = $html;
		$response['rumus_hitung'] = $rumus.'<br><br>'.$hitung;
