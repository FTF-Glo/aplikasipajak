...
class LaporPajak extends Pajak {
    ...
    public function get_pajak($npwpd='', $nop='') {
        ...
        if(empty($pajak['CPM_ID'])){
			...
			atr.CPM_ATR_SISI, atr.CPM_ATR_TINGGI, atr.CPM_ATR_HARGA_DASAR_UK, atr.CPM_ATR_HARGA_DASAR_TIN, atr.CPM_ATR_GEDUNG, atr.CPM_ATR_ALKOHOL_ROKOK, atr.CPM_ATR_TOL
    ...
    private function save_pajak($cpm_no='') {
        ...
        if ($validasi['result'] == true || ($validasi['data']['CPM_TRAN_STATUS'] == '4')) {
            ...
            $query = sprintf("INSERT INTO {$this->PATDA_REKLAME_DOC}
                    ...
                    $this->CPM_TAHUN_PAJAK, $this->CPM_TOTAL_OMZET, $this->CPM_TOTAL_PAJAK, $this->CPM_TARIF_PAJAK,
                ...
                for ($x = 0; $x < $j; $x++) {
                    ...
                    $query = sprintf("INSERT INTO {$this->PATDA_REKLAME_DOC_ATR}
                            ...
                            VALUES ('%s','%s',%f,%f,'%s',
                                    ...
                                    %f,'%s','%s','%s','%s','%s',
                                    '%s','%s','%s','%s','%s','%s',
                                    '%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')", $this->CPM_ID, $judul, $biaya, $harga, $lokasi, $lebar, $panjang, $jumlah, $jari, $muka, $tarif, $jumlah_tahun, $jumlah_hari, $jumlah_minggu, $jumlah_bulan, $batas_awal, $batas_akhir, $total,
                                    $norekening, $type_masa, $nilai_strategis, $kawasan, $jalan, $sudut_pandang, $njop, $perhitungan,$cek_pk, $nilai_pk, $sisi, $nop,
									$hd_ukuran, $hd_ketinggian, $tinggi, $gedung, $alkohol_rokok, $tol);
...
*/
	...
	public function get_hargadasar($params = array()){
		...
		if($x=='false'){
			...
			if($kdrek=='4.1.01.09.01.002'){ // Vidiotron/megatron 
				$total_njopr = (($luas * $harga_dasar->ukuran) + ($tinggi * $harga_dasar->ketinggian)) * 16 * $durasi_hari;
				...
				$hitung_njopr = "<b>NJOPR</b><br>
					...
					= ({$panjang} x {$lebar} x ".number_format($harga_dasar->ukuran).") + ({$tinggi} x ".number_format($harga_dasar->ketinggian).")  x 16 jam x {$durasi_hari} Hari<br>
					= (".number_format($panjang * $lebar * $harga_dasar->ukuran).") + (".number_format($tinggi * $harga_dasar->ketinggian).")  x 16 jam x {$durasi_hari} Hari<br>
			...
			}elseif($kdrek=='4.1.01.09.03' || // Melekat/Stiker: jumlah x harga
					$kdrek=='4.1.01.09.04' // Selebaran: jumlah x harga
				){
				...
				$val_tinggi = $sisi;
				$sisi = 1;
			}elseif($kdrek=='4.1.01.09.08'){ // Suara: jumlah x harga
				$rumus = "NJOPR = (Jumlah x Harga Dasar) x Durasi<br>
					NSPR = (NFR + NFJ + NSP) x Harga Dasar NSPR <br>
					NSR = NJOPR + NSPR<br>
					Total Pajak = NSR x Tarif Pajak";
				...
				$label_tinggi = 'Jumlah';
				$val_tinggi = $sisi;
			}elseif($kdrek=='4.1.01.09.06' || // Udara: jumlah x harga
					$kdrek=='4.1.01.09.07'){ // Apung
				...
				$hitung_nsr = "<b>NSR</b><br>
					=  NJOPR<br>
					= Rp. ".number_format($total_nsr);
			}elseif($kdrek=='4.1.01.09.01.006'){ // Wall Painting dan Sejenisnya
