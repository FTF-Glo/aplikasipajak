...
function hitungDetail(id) {
	var rek = $("#CPM_ATR_REKENING-" + id),
	 data = rek.find("option:selected").data();
 
	var params = {
		kdrek: rek.val(),
		panjang: $("#CPM_ATR_PANJANG-" + id).val(),
		lebar: $("#CPM_ATR_LEBAR-" + id).val(),
		muka: $("#CPM_ATR_MUKA-" + id).val(),
		tinggi: $("#CPM_ATR_TINGGI-" + id).val(),
		waktu: $("#CPM_ATR_WAKTU-" + id).val(),
		sisi: Number($("#CPM_ATR_SISI-" + id).val()),
		sudut_pandang: $("#CPM_ATR_SUDUT_PANDANG-" + id).val(),
		biaya: $("#CPM_ATR_BIAYA-" + id).val(),
		tarif: data.tarif,
		kawasan: $("#CPM_ATR_KAWASAN-" + id).val(),
		jalan: $("#CPM_ATR_JALAN-" + id).val(),
		durasi: $("#CPM_MASA_PAJAK").val(),
		durasi_label: $("#CPM_ATR_TYPE_MASA option:selected").text(),
		durasi_hari: $("#CPM_ATR_JUMLAH_HARI").val(),
		jumlah: $("#CPM_ATR_JUMLAH-" + id).val(),
		x: document.getElementById("CPM_CEK_PIHAK_KETIGA").checked,
		npk: $("#CPM_NILAI_PIHAK_KETIGA").val(),
		gedung: $(".CPM_GEDUNG-" + id).val(),
		alkohol_rokok: $(".CPM_ALKOHOL_ROKOK-" + id).val(),
		tol: $(".CPM_TOL-" + id).val(),
		jam: $("#CPM_ATR_JAM-" + id).val(),
		function: "get_hargadasar",
	};
	console.log("Nilai tol:", params.tol);
	if (
		params.panjang == "" ||
		params.lebar == "" ||
		params.tinggi == "" ||
	...
	$.ajax({
		...
		async: false,
		success: function (res) {
			$("#CPM_ATR_TARIF-" + id).val(res.tarif);
			$("#area_perhitungan-" + id).html(res.html);
			$("#CPM_ATR_TOTAL-" + id).autoNumeric("init");
			$("#CPM_ATR_TOTAL-" + id).autoNumeric("set", res.total);
			hitung_total();
		},
	});
}
...
function hitungDetail22(no) {
	...
	var params = {
	  ...
	  x: document.getElementById("CPM_CEK_PIHAK_KETIGA").checked,
	  npk: $("#CPM_NILAI_PIHAK_KETIGA").val(),
	  gedung			: $(".CPM_GEDUNG option:selected").val(),
	  alkohol_rokok		: $(".CPM_ALKOHOL_ROKOK option:selected").val(),
	  tol				: $(".CPM_TOL option:selected").val(),
...
function calculation() {
    ...
    // Memeriksa apakah isi dari elemen dengan ID 'jangka-waktu' kosong
    switch(true) {
    case $.trim($('#jangka-waktu').html()) 	=== '':
    case $('#CPM_ATR_BATAS_AWAL').val()  === '':
    case $('#CPM_ATR_BATAS_AKHIR').val() === '':
        alert('Silakan Isi Masa Pajak');
        return;
    case $('#CPM_ATR_REKENING').val() 	 === '' 	|| 
		 $('#CPM_ATR_REKENING').val() 	 === 0.00:
        alert('Harap Pilih Rekening');
        return;
    case $('#CPM_ATR_PANJANG').val() 	 === '' 	|| 
		 $('#CPM_ATR_PANJANG').val() 	 === 0.00:
        alert('Harap isi panjang');
        return;
    case $('#CPM_ATR_LEBAR').val() 		 === '' 	|| 
		 $('#CPM_ATR_LEBAR').val() 		 === 0.00:
        alert('Harap isi lebar');
        return;
    case $('#CPM_ATR_TINGGI').val() 	 === '' 	|| 
		 $('#CPM_ATR_TINGGI').val() 	 === 0.00:
        alert('Harap isi tinggi');
        return;
    case $('#CPM_ATR_SISI').val() 		 === '' 	|| 
		 $('#CPM_ATR_SISI').val()  		 === 0.00:
        alert('Harap isi QTY');
        return;
    case $('.CPM_GEDUNG').val()		 	 === '':
        alert('Harap isi gedung');
        return;
    case $('.CPM_ALKOHOL_ROKOK').val() 	 === '':
        alert('Harap isi alkohol');
        return;
    case $('.CPM_TOL').val() === '':
        alert('Harap isi jalan');
        return;
    default:
        // Kode untuk eksekusi jika tidak ada kasus yang cocok
	break;
}
// Panggil fungsi get_hargadasar()
    get_hargadasar();
}
...
function get_hargadasar(){
	...
	var params= {
		...
		tarif 			: data.tarif, 
		kawasan 		: $('#CPM_ATR_KAWASAN option:selected').val(), 
		jalan 			: $('#CPM_ATR_JALAN option:selected').val(), 
		durasi 			: $('#CPM_MASA_PAJAK').val(), 
		durasi_label 	: $('#CPM_ATR_TYPE_MASA option:selected').text(), 
		durasi_hari 	: $('#CPM_ATR_JUMLAH_HARI').val(), 
...
$(function() {
    ...
    $(".btn-tambah").click(function() {
        var no = parseInt($("#count").val()),
            ...
            jamnya = $("#CPM_MASA_PAJAK").val(),
            waktu = $("#CPM_MASA_PAJAK").val() +
