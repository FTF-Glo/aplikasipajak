...
class BerkasSKPDKB extends Pajak {
    ...
    public function get_berkas() {
        ...
        $query = "SELECT * FROM PATDA_BERKAS WHERE CPM_ID = '{$this->_id}' AND CPM_SKPDKB='1'";
    ...
    public function save() {
        ...
        $query = "SELECT * FROM PATDA_BERKAS WHERE CPM_NO_SPTPD ='{$this->CPM_NO_SPTPD}' AND CPM_SKPDKB='1'";
        $result = mysqli_query($this->Conn, $query);
        if (mysqli_num_rows($result) == 0) {
            $query = sprintf("INSERT INTO PATDA_BERKAS 
                    ...
                    CPM_AUTHOR,CPM_STATUS, CPM_SKPDKB)
                    VALUES ( '%s','%s','%s','%s','%s',
                             '%s','%s','%s','%s','%s',
    ...
    public function filtering($id) {
        ...
        $html = "<div class=\"filtering\">
                    <form><table><tr valign=\"bottom\">
                        ...
                        Jenis Pajak :<br><select name=\"CPM_JENIS_PAJAK-{$id}\" id=\"CPM_JENIS_PAJAK-{$id}\"class=\"form-control\" style=\"height: 32px; width: 128\" >{$opt_jenis_pajak}</select></td>
                        <td>Tahun Pajak :<br><select name=\"CPM_TAHUN_PAJAK-{$id}\" id=\"CPM_TAHUN_PAJAK-{$id}\" class=\"form-control\" style=\"height: 32px; width: 128\">{$opt_tahun}</select></td>
                        <td>Masa Pajak :<br><select name=\"CPM_MASA_PAJAK-{$id}\" id=\"CPM_MASA_PAJAK-{$id}\" class=\"form-control\" style=\"height: 32px; width: 128\">{$opt_bulan}</select></td>
                        <td>No. SKPDKB/T :<br><input type=\"text\" name=\"CPM_NO_SKPDKB-{$id}\" id=\"CPM_NO_SKPDKB-{$id}\" class=\"form-control\" style=\"height: 32px; width: 128px\" ></td>
                        <td>NPWPD :<br><input type=\"text\" name=\"CPM_NPWPD-{$id}\" id=\"CPM_NPWPD-{$id}\" class=\"form-control\" style=\"height: 32px; width: 128px\" > </td>
                        <td>Tanggal Lapor :<br><input type=\"text\" name=\"CPM_TGL_LAPOR1-{$id}\" id=\"CPM_TGL_LAPOR1-{$id}\" readonly size=\"10\" class=\"date form-control\" style=\"height: 32px; width: 96px; display: inline-block\" ><button type=\"button\" value=\"x\" onclick=\"javascript:$('#CPM_TGL_LAPOR1-{$id}').val('');\" class=\"btn btn-danger\" style=\"padding: 4px 8px\">x</button> s.d <input type=\"text\" name=\"CPM_TGL_LAPOR2-{$id}\" id=\"CPM_TGL_LAPOR2-{$id}\" size=\"10\" class=\"date form-control\" style=\"height: 32px; width: 96px; display: inline-block\" ><button type=\"button\" value=\"x\" onclick=\"javascript:$('#CPM_TGL_LAPOR2-{$id}').val('');\" class=\"btn btn-danger\" style=\"padding: 4px 8px\">x</button></td>
                        <td>
                            ...
                            <button type=\"button\" onclick=\"javascript:download_excel('{$id}','function/PATDA-V1/svc-download-berkas.xls.php','skpdkb')\" class=\"btn btn-success lm-btn\" style=\"font-size: 0.7rem !important;\" ><i class=\"fa fa-download\"></i> Export to xls</button>
                        </td>
                    </tr></table></form>
                </div> ";
        return $html;
    ...
    private function grid_data_berkas_masuk() {
        ...
        $query = "SELECT CPM_ID, CPM_JENIS_PAJAK, CPM_NO_SKPDKB, CPM_VERSION, 
                    ...
                    CPM_TAHUN_PAJAK, 
                    CPM_NPWPD, CPM_AUTHOR, CPM_STATUS, DATE(CPM_TGL_INPUT) as CPM_TGL_INPUT, CPM_TRAN_READ
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $row['CPM_JENIS_PAJAK'] = isset($this->arr_pajak[$row['CPM_JENIS_PAJAK']])? $this->arr_pajak[$row['CPM_JENIS_PAJAK']] : $row['CPM_JENIS_PAJAK'];
    ...
    private function grid_data_pajak_disetujui() {
        ...
        $query = "SELECT s.CPM_ID, s.CPM_JENIS_PAJAK, s.CPM_NO_SKPDKB, s.CPM_VERSION, s.CPM_MASA_PAJAK, s.CPM_TAHUN_PAJAK, s.CPM_TAMBAHAN,  
                    ...
                    FROM PATDA_SKPDKB s INNER JOIN PATDA_SKPDKB_TRANMAIN tr ON
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $row = array_merge($row, array("NO" => ++$no));
            $row['READ'] = strpos($row['CPM_TRAN_READ'], ";{$_SESSION['uname']};") === false ? 0 : 1;
            $no_skpdkb[$row['CPM_NO_SKPDKB']] = $row['CPM_NO_SKPDKB'];
            
            $base64 = "a={$this->_a}&m={$this->_m}&s={$row['CPM_TRAN_STATUS']}&mod={$this->_mod}&i={$this->_i}&flg=1&f={$this->_f}&type={$row['CPM_JENIS_PAJAK']}&id={$row['CPM_ID']}&tambahan={$row['CPM_TAMBAHAN']}&idtran={$row['CPM_TRAN_ID']}&read={$row['READ']}";
            $url = "main.php?param=" . base64_encode($base64);
            $row['CPM_NO_SKPDKB'] = "<a href=\"{$url}\" title=\"Klik untuk detail\">{$row['CPM_NO_SKPDKB']}</a>";
            $row['CPM_JENIS_PAJAK'] = $this->arr_pajak[$row['CPM_JENIS_PAJAK']];
		...
		$res = mysqli_query($Conn_gw, "SELECT sptpd,payment_code from SIMPATDA_GW WHERE sptpd IN('".implode("','", $no_skpdkb)."')");
		while($gw=mysqli_fetch_assoc($res)){
			$no_skpdkb[$gw['sptpd']] = $gw['payment_code'];
		}

		// masukkan kode bayar by no sptpd ke data
		foreach($rows as $i=>$row){
			$rows[$i]['kode_verifikasi'] = $no_skpdkb[strip_tags($row['CPM_NO_SKPDKB'])];
		}
    ...
    public function print_buktiterima() {
        ...
        $html = "<table border=\"1\" cellpadding=\"5\">
                    <tr>
                        ...
                        <td align=\"center\" width=\"72%\" colspan=\"2\">
                            <br>
                            PEMERINTAH KOTA " . strtoupper($KOTA) . "<br/>DINAS PENGELOLAAN KEUANGAN<br/>DAN KEKAYAAN DAERAH<br/><br/>
                    ...
                    <tr>
                        ...
                        <td colspan=\"3\">
                            ...
                            <table border=\"0\" cellpadding=\"1\" cellspacing=\"5\">
                                ...
                                <tr>
                                    <td width=\"125\">Nomor S</td><td width=\"10\">:</td>
                                    <td width=\"180\">{$DATA['CPM_NO_SKPDKB']}</td>
                                </tr>
                                <tr>
        ...
        $pdf->Image('../../../../view/Registrasi/configure/logo/' . $LOGO_CETAK_PDF, 6, 11, 20, '', '', '', '', false, 300, '', false);
        $pdf->SetAlpha(0.3);
    ...
    public function print_disposisi() {
        ...
        $html = "<table border=\"1\" cellpadding=\"10\">
                    <tr>
                        ...
                        <td align=\"center\" width=\"60%\">
                            <!-- <font size=\"+4\"> --> PEMERINTAH KOTA " . strtoupper($KOTA) . "<br/>DINAS PENDAPATAN DAERAH<br />
                            <!-- </font> -->
                        </td>
                        <!--KOSONG-->
                            ...
                            <table border=\"0\" cellpadding=\"2\" cellspacing=\"7\">
                                ...
                                <tr>
                                    ...
                                    <td width=\"auto\" cellspacing=\"5\"><table>
                                            <tr><td>{$radio_lampiran[1]} SKPDKB</td></tr>
                                            <tr><td>{$radio_lampiran[2]} SKPDKBT</td></tr>
                                        </table>
                                    </td>
                                </tr>
                            </table>					
        ...
        $pdf->AddPage('P', 'A4');
        $pdf->writeHTML($html, true, false, false, false, '');
        $pdf->Image('../../../../view/Registrasi/configure/logo/' . $LOGO_CETAK_PDF, 15, 18, 18, '', '', '', '', false, 300, '', false);
        $pdf->SetAlpha(0.3);
    ...
    public function read_dokumen_notif() {
        ...
        if (in_array("masuk", $arr_tab)) {
            $q = "SELECT count(CPM_ID) as total FROM PATDA_BERKAS WHERE CPM_STATUS='0' AND CPM_SKPDKB='1' AND 
                    (CPM_TRAN_READ not like '%;{$_SESSION['uname']};%' OR CPM_TRAN_READ is null)";
            $result = mysqli_query($this->Conn, $q);
            if ($data = mysqli_fetch_assoc($result))
                $notif['masuk'] = $data['total'];
        }        
        if (in_array("diterima", $arr_tab)) {
            $q = "SELECT count(CPM_ID) as total FROM PATDA_BERKAS WHERE CPM_STATUS='1' AND CPM_SKPDKB='1' AND 
                    (CPM_TRAN_READ not like '%;{$_SESSION['uname']};%' OR CPM_TRAN_READ is null)";
            $result = mysqli_query($this->Conn, $q);
            if ($data = mysqli_fetch_assoc($result))
                $notif['diterima'] = $data['total'];
        }
