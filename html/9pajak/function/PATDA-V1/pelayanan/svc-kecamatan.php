...
require_once($sRootPath . "inc/PATDA-V1/dbMonitoring.php");
// echo "string";
...
//if(stillInSession($DBLink,$json,$sdata)){
	if ($p) {
		if ($kel==0) {
			$query = "SELECT * FROM PATDA_MST_KECAMATAN WHERE CPC_TKC_KKID ='".$p."' order by CPC_TKC_URUTAN ASC";
			...
			$query = "SELECT * FROM PATDA_MST_KELURAHAN WHERE CPM_KEL_ID like '".$p."%'  order by CPM_KEL_ID ASC";
			// echo $query;
			$res = mysqli_query($DBLink,$query);
			...
			if (intval($num)>0){
				while ($row = mysqli_fetch_assoc($res)) {
					$data[$i]["id"] = $row["CPM_KEL_ID"];
					$data[$i]["name"] = $row["CPM_KELURAHAN"];
					$i++;
				}
			}
		}
