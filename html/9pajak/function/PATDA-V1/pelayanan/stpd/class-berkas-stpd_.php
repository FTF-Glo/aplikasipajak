...
class BerkasSTPD extends Pajak {
    ...
    public function filtering($id) {
        ...
        $html.= "</select> TAHUN : <select name=\"CPM_TAHUN_PAJAK-{$id}\" id=\"CPM_TAHUN_PAJAK-{$id}\"><option value=''>All</option>";
        ...
        $html.= "</select>    
                        No. STPD : <input type=\"text\" name=\"CPM_NO_STPD-{$id}\" id=\"CPM_NO_STPD-{$id}\"><br/><br/><hr style=\"border-top:1px #CCC solid\"/><br/>  
                        ...
                        Tanggal Lapor : <input type=\"text\" name=\"CPM_TGL_LAPOR1-{$id}\" id=\"CPM_TGL_LAPOR1-{$id}\" readonly class=\"date\" ><input type=\"button\" value=\"x\" onclick=\"javascript:$('#CPM_TGL_LAPOR1-{$id}').val('');\"> s.d
                          <input type=\"text\" name=\"CPM_TGL_LAPOR2-{$id}\" id=\"CPM_TGL_LAPOR2-{$id}\" class=\"date\" ><input type=\"button\" value=\"x\" onclick=\"javascript:$('#CPM_TGL_LAPOR2-{$id}').val('');\">
                        <button type=\"submit\" id=\"cari-{$id}\">Cari</button>
                        <button type=\"button\" onclick=\"javascript:download_excel('{$id}','function/PATDA-V1/svc-download-berkas.xls.php','stpd')\">Export to xls</button>        
                    </form>
                </div> ";
        return $html;
    }
    ...
    private function grid_data_berkas_masuk() {
        ...
        $query = "SELECT CPM_ID, CPM_JENIS_PAJAK, CPM_NO_STPD, CPM_VERSION, 
                    ...
                    CPM_AUTHOR, STR_TO_DATE(CPM_TGL_INPUT,'%d-%m-%Y') as CPM_TGL_INPUT, CPM_STATUS, CPM_TRAN_READ
    ...
    public function print_buktiterima() {
        ...
        require_once(/*$_SERVER['DOCUMENT_ROOT']*/."/inc/payment/tcpdf/tcpdf.php");
        ...
        $pdf->Image(/*$_SERVER['DOCUMENT_ROOT']*/.'/view/Registrasi/configure/logo/' . $LOGO_CETAK_PDF, 6, 11, 20, '', '', '', '', false, 300, '', false);
        $pdf->SetAlpha(0.3);
    ...
    public function print_disposisi() {
        ...
        $pdf->AddPage('P', 'A4');
        $pdf->writeHTML($html, true, false, false, false, '');
        $pdf->Image(/*$_SERVER['DOCUMENT_ROOT']*/.'/view/Registrasi/configure/logo/' . $LOGO_CETAK_PDF, 10, 21, 30, '', '', '', '', false, 300, '', false);
        $pdf->SetAlpha(0.3);
