...
class Pajak {
	...
	public function get_payment_code($conn, $id = '', $config){
		$payment_code = '';
		...
		if(!empty($id)){
			...
			$year = date('y');
			$kode_prefix = '4';
			$length = isset($config['PATDA_PAYMENT_CODE_LENGTH'])? $config['PATDA_PAYMENT_CODE_LENGTH'] : 7; 
			$query = "SELECT MAX(SUBSTRING(payment_code,4, {$length} )) nomor FROM SIMPATDA_GW WHERE DATE_FORMAT(saved_date, '%y') = '{$year}'";
			$res = mysql_query($query, $conn);
			...
			$payment_code = $kode_prefix.$year.str_pad($nomor, $length, '0', STR_PAD_LEFT);
		}
    ...
    private function get_op_reklame($id){
        $res = mysql_query("SELECT * from PATDA_REKLAME_PROFIL WHERE CPM_ID='{$id}'", $this->Conn);
        if($data = mysql_fetch_assoc($res)){
            return $data;
        }
        return;
    ...
    protected function save_gateway($jns, $arr_config) {
        ...
        $payment_code = $this->get_payment_code($Conn_gw,'',$arr_config);
        ...
        $res = mysql_query($query, $Conn_gw) or die(mysql_error());
        mysql_close($Conn_gw);
    ...
    public function search_npwpd() {
        ...
        if ($this->CPM_JENIS_PAJAK == 1) {
            ...
            if (mysql_num_rows($result) > 0) {
                $dataProfil = mysql_fetch_assoc($result);
    ...
    public function get_val_tapbox() {
        ...
        $Conn_gw = @mysql_connect($dbHost, $dbUser, $dbPwd, true) or die($this->getError("Error Koneksi Data Pembanding"));
        mysql_select_db($dbName, $Conn_gw);
        ...
        $result = mysql_query($query, $Conn_gw) or die($this->getError(mysql_error()));
    ...
    public function save_ket_tapbox(){
        ...
        return mysql_query($query, $Conn_gw);
...
// end of SayInIndonesian
    ...
    public function laporan_resume($p) {
        ...
        $result = mysql_query($query, $Conn_gw) or die(mysql_error());
        $data = mysql_fetch_assoc($result);
	...
	public function print_sspd() {
								...
								<tr>
									<td><table width=\"700\" border=\"0\" cellpadding=\"5\">
											...
											<tr>
												...
												<td width=\"230\">:<i>{$DATA['pajak']['CPM_MASA_PAJAK1']} - {$DATA['pajak']['CPM_MASA_PAJAK2']}</i></td>
												<td width=\"70\"><i>Tahun</i></td>
												<td width=\"225\">: <i>{$DATA['pajak']['CPM_TAHUN_PAJAK']}</i></td>
											</tr>
											<tr>
												<td>Cara Pembayaran</td>
												<td colspan=\"3\">:  [_] Bank</td>                                        
											</tr> 
											<tr>
                                            ...
                                            <tr>
                                                <td>Kode Bayar (BANK)</td>
                                                <td style=\"colspan:3;font-size:40px;font-weight:bold;\">: {$PAYMENT_CODE}</td>
                                            </tr>
											<tr>
												<td>Uraian</td>
                                ...
                                <tr>
                                    <td><table width=\"900\" border=\"1\" cellpadding=\"3\">
                                            ...
                                            <tr>
                                                ...
                                                <td align=\"right\">" . number_format($DATA['pajak']['CPM_BAYAR_TERUTANG'], 2) . "</td>
                                            </tr>
        ...
        $pdf->AddPage('P', 'A4');
        $pdf->writeHTML($html, true, false, false, false, '');
        $pdf->Image("{$sRootPath}view/Registrasi/configure/logo/{$LOGO_CETAK_PDF}", 8, 16, 14, '', '', '', '', false, 300, '', false);
        $pdf->SetAlpha(0.3);
	...
	public function getWP(){
		...
		if($row = mysql_fetch_assoc($res)){
			...
			if(!empty($data['CPM_GOL'])){
				...
				$res = mysql_query($query, $this->Conn);
				if($permen = mysql_fetch_assoc($res)){
	...
	public function get_bank_payment(){
		...
		$qry = mysql_query($sql,$Conn_gw);
		while($bank = mysql_fetch_object($qry)){
