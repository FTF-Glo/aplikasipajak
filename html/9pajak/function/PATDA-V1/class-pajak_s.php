...
class Pajak {
	...
	function check_role($id){
			...
			$query_check = "select b.CTR_RM_ID from central_user a
inner join central_user_to_app b ON a.CTR_U_ID = b.CTR_USER_ID where a.CTR_U_UID = '{$id}'";
			$res1 = mysqli_query($this->Conn, $query_check);
			//$check_status = mysqli_num_rows($res1);
			$data = array();
			$data = mysqli_fetch_object($res1);
    ...
    protected function save_gateway($jns, $arr_config) {
        ...
        // var_dump($KODE_PAJAK,$this->CPM_TIPE_PAJAK, $jns);die;
...
private function download_pajak_xls_bentang_panjang() {
        ...
        while($row = mysqli_fetch_assoc($res2)){
			...
			$res_wp = mysqli_query($this->Conn, $query_wp);
            // echo "<pre>";
            // var_dump(mysqli_fetch_assoc($res_wp));
            // die;
			while($row = mysqli_fetch_assoc($res_wp)){
				$data_wp[$row['CPM_NPWPD']]['CPM_NPWPD'] = $row['CPM_NPWPD'];
				$data_wp[$row['CPM_NPWPD']]['CPM_NAMA_WP'] = $row['CPM_NAMA_WP'];
			...
			// border
			$objPHPExcel->getActiveSheet()->getStyle('A7:Q'.$row)->applyFromArray(array(
				'borders' => array(
					'allborders' => array(
						'style' => PHPExcel_Style_Border::BORDER_THIN
						)
...
private function download_pajak_xls_bentang_panjangV2() {
       ...
       $objPHPExcel->getSheet(1)->mergeCells("F7:R7");
        ...
        // border
        $objPHPExcel->getSheet(0)->getStyle('A6:S'.($row+1))->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                    )
        ...
        $objPHPExcel->getSheet(1)->getStyle('A6:S'.($row+1))->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                    )
	...
	private function download_pajak_xls_pat() {
        ...
        // border
        $objPHPExcel->getActiveSheet()->getStyle('A6:J'.$lastRow)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                    )
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('D'.($row+1), "Kepala Badan Pengelola Pajak dan Retribusi Daerah");
        $objPHPExcel->getActiveSheet()->setCellValue('D'.($row+2), "Kabupaten Lampung Selatan");
	...
	private function download_pajak_xls_non14() {
        ...
        if($periode_bulan!=''){
			...
			if($bul != 1){
				...
				$query11 = "SELECT SUM(a.CPM_TOTAL_OMZET) as CPM_TOTAL_OMZET, SUM(a.CPM_TOTAL_PAJAK) as CPM_TOTAL_PAJAK from (SELECT pj.CPM_TOTAL_OMZET, pj.CPM_TOTAL_PAJAK
							...
							INNER JOIN PATDA_{$JENIS_PAJAK}_PROFIL{$this->SUFIKS} pr ON pj.CPM_ID_PROFIL = pr.CPM_ID
							INNER JOIN PATDA_{$JENIS_PAJAK}_DOC_TRANMAIN{$this->SUFIKS} tr ON pj.CPM_ID = tr.CPM_TRAN_{$JENIS_PAJAK}_ID
							WHERE tr.CPM_TRAN_STATUS = '5' AND DATE_FORMAT(STR_TO_DATE(CPM_TGL_LAPOR,\"%d-%m-%Y\"), '%Y-%m')>='$tahtah' AND DATE_FORMAT(STR_TO_DATE(CPM_TGL_LAPOR,\"%d-%m-%Y\"), '%Y-%m')<='$bulan_lalu' {$where2} group by pj.CPM_ID) a";
        ...
        $objPHPExcel->getActiveSheet()->mergeCells("A{$row}:H{$row}");
        $objPHPExcel->getActiveSheet()->setCellValue('A'.$row, "JUMLAH S/D BULAN LALU");
        $objPHPExcel->getActiveSheet()->setCellValue('K'.$row, number_format($total_omzet_bulan_lalu+0));
        $objPHPExcel->getActiveSheet()->setCellValue('M'.$row, number_format($total_pajak_bulan_lalu+0));
        $row++;
        ...
        $objPHPExcel->getActiveSheet()->mergeCells("A{$row}:H{$row}");
        $objPHPExcel->getActiveSheet()->setCellValue('A'.$row, "JUMLAH S/D BULAN INI");
        $objPHPExcel->getActiveSheet()->setCellValue('K'.$row, '=K'.($row-2).'+J'.($row-1));
        $objPHPExcel->getActiveSheet()->setCellValue('M'.$row, '=M'.($row-2).'+L'.($row-1));
	}else{
        $objPHPExcel->getActiveSheet()->mergeCells("A{$row}:H{$row}");
        $objPHPExcel->getActiveSheet()->setCellValue('A'.$row, "JUMLAH BULAN INI");
        $objPHPExcel->getActiveSheet()->setCellValue('J'.$row, '=SUM(J6:J'.($row-1).')');
        ...
        // border
        $objPHPExcel->getActiveSheet()->getStyle('A6:L'.$lastRow)->applyFromArray(array(
            'borders' => array(
                'allborders' => array(
                    'style' => PHPExcel_Style_Border::BORDER_THIN
                    )
        ...
        if ($this->_idp == 8) {
            // border
            $objPHPExcel->getActiveSheet()->getStyle('N6:N'.$lastRow)->applyFromArray(array(
                'borders' => array(
                    'allborders' => array(
                        'style' => PHPExcel_Style_Border::BORDER_THIN
                        )
            ...
            $objPHPExcel->getActiveSheet()->getStyle('M6:M'.$lastRow)->applyFromArray(array(
                'borders' => array(
                    'allborders' => array(
                        'style' => PHPExcel_Style_Border::BORDER_THIN
                        )
            ...
            if ($this->_i==4) {
                $objPHPExcel->getActiveSheet()->getStyle('O6:O'.$lastRow)->applyFromArray(array(
                    'borders' => array(
                        'allborders' => array(
                            'style' => PHPExcel_Style_Border::BORDER_THIN
                            )
        ...
        // $objPHPExcel->getActiveSheet()->setCellValue('A'.($row+1), "Kepala Bidang");
        // $objPHPExcel->getActiveSheet()->setCellValue('A'.($row+2), "Pengembangan dan Penetapan BPPRD");
        // $objPHPExcel->getActiveSheet()->setCellValue('A'.($row+6), "YUYUN MAYA SAPHIRA, SE");
        // $objPHPExcel->getActiveSheet()->setCellValue('A'.($row+7), "NIP. 19780708 200312 2 008");

        $objPHPExcel->getActiveSheet()->setCellValue('A' . ($row + 2), "Kepala Bidang Pengembangan dan Penetapan,");
        //$objPHPExcel->getActiveSheet()->setCellValue('A' . ($row + 2), "Ub. Kasubbid Perhitungan dan Penetapan");
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('I'.($row+2), "dan Penetapan BPPRD");
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('E'.($row+1), "Kepala Badan Pengelola Pajak");
	...
	public function print_sspd() {
                        ...
                        <td><table width=\"710\" border=\"1\" cellpadding=\"10\">
                                <tr valign=\"top\">
                                    ...
                                    <th width=\"260\">
										...
										<br/>
										<b>Tahun &#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;: {$DATA['pajak']['CPM_TAHUN_PAJAK']}<br/>
                                ...
                                <tr>
                                    <td><table width=\"700\" border=\"0\" cellpadding=\"2\">
									        <tr>
                                                ...
                                                <td colspan=\"3\">: {$DATA['pajak']['CPM_NO_SSPD']}</td>
                                            </tr>
											<tr>
                                                <td width=\"180\">Tanggal Jatuh Tempo</td>
                                                <td colspan=\"3\">: {$this->formatDateForDokumen($tgl_jth_tempo)}</td>
                                            </tr>
                                            <tr style=\"vertical-align: top\">
                                                <td>Tanggal</td>
                                                <td colspan=\"3\">: {$this->formatDateForDokumen($TGL_PENETAPAN)}</td>
                                            </tr>
                                            ...
                                            <tr>
                                                <td>Masa Pajak</td>
                                                <td width=\"230\">: {$DATA['pajak']['CPM_MASA_PAJAK1']} - {$DATA['pajak']['CPM_MASA_PAJAK2']}</td>
                                            </tr>
											<tr>
                                                <td>Tahun</td>
                                                <td colspan=\"3\">: {$DATA['pajak']['CPM_TAHUN_PAJAK']}</td>
                                            </tr> 
                                            <tr>
                                            ...
                                            <tr>
                                                ...
                                                <td colspan=\"3\">: Pajak ".$this->arr_pajak[$this->id_pajak]."</td>
                                            </tr>
                                ...
                                <tr>
                                    <td><table width=\"900\" border=\"1\" cellpadding=\"3\">
                                            <tr>
                                                ...
                                                <th width=\"330\" align=\"center\">Jenis Pajak Daerah</th>
                                            ...
                                            <tr>
                                                ...
                                                <td align=\"right\">" . number_format($DATA['pajak']['CPM_DPP'], 2) . " {$tarif_pajak}</td>
                                            </tr>
        ...
        $pdf->AddPage('P', 'A4');
        $pdf->writeHTML($html, true, false, false, false, '');
        $pdf->Image("{$sRootPath}view/Registrasi/configure/logo/{$LOGO_CETAK_PDF}", 15, 17, 17, '', '', '', '', false, 300, '', false);
        $pdf->SetAlpha(0.3);
    ...
    public function get_kawasan(){
        ...
        //$query = "SELECT CPM_NAMA FROM PATDA_REKLAME_PARAM WHERE CPM_TYPE = 'KAWASAN'";
    ...
    public function get_jalan(){
        ...
        //$query = "SELECT CPM_KELAS FROM PATDA_REKLAME_TARIF_NSPR GROUP BY CPM_KELAS ";
		//$res = mysqli_query($this->Conn, $query);
		...
		$query = "SELECT CPM_NAMA FROM PATDA_REKLAME_PARAM_NILAI WHERE CPM_GRUP = 'NFJ'";
		$res = mysqli_query($this->Conn, $query);
	...
	public function validasi_piutang(){
		if($this->check_piutang(1) == false || $this->check_piutang(2) == false || $this->check_piutang(3) == false || $this->check_piutang(4) == false ||
