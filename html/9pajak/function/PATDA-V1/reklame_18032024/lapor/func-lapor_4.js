...
function get_hargadasar() {
	...
	$.ajax({
		...
		success: function (res) {
			...
			// new aldes, gw ga ngerti kanapa gini tapi ya ini bisa..
			// $("#CPM_ATR_TOTAL").autoNumeric('set', total);
			$("#CPM_ATR_TOTAL").autoNumeric('set', eval(omzet));
...
$(function () {
	...
	// #CPM_DENDA_TERLAMBAT_LAP,

	// $('#CPM_DENDA_TERLAMBAT_LAP').keyup(function() {
	// 	let sanksi = $('#CPM_DENDA_TERLAMBAT_LAP').autoNumeric('get');
	// 	let total = $('#CPM_TOTAL_PAJAK').autoNumeric('get');

	// 	let newTotal = sanksi + total;
	// 	$('#CPM_TOTAL_PAJAK').autoNumeric('set', newTotal);
	// })
	$('#CPM_DENDA_TERLAMBAT_LAP, #CPM_ATR_JUMLAH, #CPM_ATR_PANJANG, #CPM_ATR_LEBAR, #CPM_ATR_MUKA, #CPM_ATR_SISI, #CPM_ATR_TINGGI, #CPM_ATR_JARI, #CPM_DISCOUNT,  #CPM_ATR_JUMLAH').keyup(function () {
		if ($(this).attr('readonly') === 'readonly')
			return false;
		get_hargadasar();
		calculation();
	...
	// new aldes
	$('[name="PAJAK_ATR[CPM_ATR_JUMLAH][]"]').trigger('keyup');
		calculation();
