...
a.CPM_BAYAR_TERUTANG as CPM_JUMLAH_TUNGGAKAN
	...
	public function grid_table_new_2() {
        $DIR = "PATDA-V1";
        $modul = "surat-teguran-real";
        $html = "<link href=\"inc/{$DIR}/jtable/themes/jtable.min.css\" rel=\"stylesheet\" type=\"text/css\" />
                <script src=\"inc/{$DIR}/jtable/jquery.jtable.min.js\" type=\"text/javascript\"></script>
    ...
    public function grid_data() {
        try {
            ...
            $query = "SELECT *
						...
						INNER JOIN simpatda_gw q ON t.CPM_ID = q.id_switching
                        WHERE {$where}
	...
	public function grid_data_new_2() {
        try {
    ...
    public function update() {
        ...
        if ($validasi['result'] == true) {
			...
			if($this->tipe_teguran == 4){
				...
				$queryU = sprintf("UPDATE simpatda_gw set                
						teguran_status = '%s'
						WHERE id_switching = '%s'", 2, $this->CPM_ID
				);
				//echo $queryU;exit;
				mysqli_query($this->Conn, $queryU);
				...
				$query = sprintf("UPDATE PATDA_TEGURAN set                
                    ...
                    CPM_LAMPIRAN = '%s',
					CPM_TGL_INPUT = '%s'
                    WHERE CPM_ID = '%s'", $this->CPM_NPWPD, $this->CPM_NAMA_OP, $this->CPM_ALAMAT_OP, $this->CPM_KECAMATAN_OP, $this->CPM_NAMA_WP, $this->CPM_NO_SURAT, $this->CPM_JENIS_PAJAK, $this->CPM_MASA_PAJAK, $this->CPM_TAHUN_PAJAK, $this->CPM_NO_SKPD, $this->CPM_TGL_SKPD, $this->CPM_JATUH_TEMPO, $this->CPM_JUMLAH_TUNGGAKAN, $this->CPM_TERBILANG, '2', $this->CPM_SIFAT, $this->CPM_LAMPIRAN, $this->CPM_TGL_INPUT, $this->CPM_ID
				);
				#echo $query;exit;
				return mysqli_query($this->Conn, $query);
			}else{
                        ...
                        <tr>
                            <td width=\"710\" colspan=\"2\" align=\"center\"><table width=\"100%\" border=\"0\" align=\"left\">
									...
									<tr>
										...
										<th width=\"90%\" colspan=\"2\">&nbsp; b. Peraturan Daerah Kabupaten Lampung Selatan Nomor 12 Tahun 2011 tentang Pajak {$this->arr_pajak[(int) $DATA['pajak']['CPM_JENIS_PAJAK']]}</th>
                                    </tr>
									<tr>
                                        <th width=\"10%\"   colspan=\"2\"></th>
									...
									<tr>
                                        <th width=\"10%\"   colspan=\"2\"></th>
										<th width=\"90%\" colspan=\"2\">&nbsp; d. Laporan Realisasi Penerimaan Pajak Minerba Tahun {$tahun_berjalan}</th>
                                    </tr>
                                </table>            
                            </td>
                        </tr>
