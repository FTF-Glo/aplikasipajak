...
$list_pejabat_surat = $lapor->get_pejabat_surat();

foreach ($list_pejabat_surat as $list_pejabat_surat) {
    $opt_pejabat .= "<option value=\"{$list_pejabat_surat['NIP']}\">{$list_pejabat_surat['NIP']} - {$list_pejabat_surat['Nama']}</option>";
}
...
$ttd_penanggung_jawab .= "
            <div class=\"row\" class=\"child\">
                <div class=\"col-md-12\">
                    <div class=\"lm-subtitle-md\">TTD Penangung Jawab</div>
                    <hr />
                </div>
            </div>
            <div class=\"row\">
...
<form class="cmxform" id="form-lapor" method="post" action="function/<?php echo "{$DIR}/{$modul}"; ?>/svc-teguran.php?param=<?php echo base64_encode($json->encode(array("a" => $lapor->_a, "m" => $lapor->_m, "mod" => $lapor->_mod, "f" => $lapor->_f))) ?>">
    <input type="hidden" name="tipe_teguran" id="tipe_teguran" value="2">
