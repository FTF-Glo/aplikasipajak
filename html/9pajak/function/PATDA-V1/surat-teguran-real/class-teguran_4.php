...
class Teguran extends Pajak {
	...
	public function get_pajak_new() {
        ...
        //$query = "SELECT * FROM PATDA_TEGURAN WHERE CPM_ID = '{$this->_id}'";
		if($jns == 7){
			$query = "SELECT a.CPM_ID, b.npwpd as CPM_NPWPD, b.op_nama as CPM_NAMA_OP, b.op_alamat as CPM_ALAMAT_OP, c.CPM_KECAMATAN as CPM_KECAMATAN_OP, b.wp_nama as CPM_NAMA_WP, d.CPM_NO as CPM_JENIS_PAJAK, e.CPM_ATR_TYPE_MASA as CPM_MASA_PAJAK, a.CPM_TAHUN_PAJAK, b.sptpd as CPM_NO_SKPD, DATE_FORMAT(STR_TO_DATE(a.CPM_TGL_LAPOR,'%d-%m-%Y'), '%d/%m/%Y')  as CPM_TGL_SKPD, DATE_FORMAT(a.CPM_TGL_JATUH_TEMPO,'%d/%m/%Y') as CPM_JATUH_TEMPO,
a.CPM_BAYAR_TERUTANG as CPM_JUMLAH_TUNGGAKAN, f.CPM_KELURAHAN as CPM_KELURAHAN_OP
		FROM {$table} a 
...
a.CPM_BAYAR_TERUTANG as CPM_JUMLAH_TUNGGAKAN, f.CPM_KELURAHAN as CPM_KELURAHAN_OP
        ...
        if (mysqli_num_rows($result) > 0) {
			...
			$query = "SELECT a.CPM_ID, b.npwpd as CPM_NPWPD, b.op_nama as CPM_NAMA_OP, b.op_alamat as CPM_ALAMAT_OP, b.kecamatan_op as CPM_KECAMATAN_OP, b.wp_nama as CPM_NAMA_WP, d.CPM_NO as CPM_JENIS_PAJAK, a.CPM_MASA_PAJAK, a.CPM_TAHUN_PAJAK, b.sptpd as CPM_NO_SKPD, DATE_FORMAT(STR_TO_DATE(a.CPM_TGL_LAPOR,'%d-%m-%Y'), '%d/%m/%Y')  as CPM_TGL_SKPD, DATE_FORMAT(a.CPM_TGL_JATUH_TEMPO,'%d/%m/%Y') as CPM_JATUH_TEMPO,
	...
	public function grid_data_new() {
        try {
			$select = "(SELECT MAX(a.CPM_VERSION), a.CPM_ID, a.CPM_NO, STR_TO_DATE(a.CPM_TGL_LAPOR,'%d-%m-%Y') as CPM_TGL_LAPOR, a.CPM_BAYAR_TERUTANG, a.CPM_AUTHOR, a.CPM_MASA_PAJAK, a.CPM_TAHUN_PAJAK, c.id_sw as TIPE_PAJAK, b.npwpd as CPM_NPWPD";
            ...
            $where.= (isset($_REQUEST['CPM_TAHUN_PAJAK']) && $_REQUEST['CPM_TAHUN_PAJAK'] != "") ? " AND CPM_TAHUN_PAJAK = \"{$_REQUEST['CPM_TAHUN_PAJAK']}\" " : "";
            $where.= (isset($_REQUEST['CPM_MASA_PAJAK']) && $_REQUEST['CPM_MASA_PAJAK'] != "") ? " AND CPM_MASA_PAJAK = \"" . (int) $_REQUEST['CPM_MASA_PAJAK'] . "\" " : "";
    ...
    public function update() {
        ...
        if ($validasi['result'] == true) {
			...
			if($this->tipe_teguran == 4){
				...
				$queryU = sprintf("UPDATE simpatda_gw set                
						teguran_status = '%s'
						WHERE id_switching = '%s'", 2, $this->CPM_ID_DOC
				);
				//echo $queryU;exit;
            ...
            $query = sprintf("INSERT INTO PATDA_TEGURAN
                    ...
                    VALUES ( '%s','%s','%s','%s','%s','%s','%s',
							 ...
							 2, $this->CPM_SIFAT, $this->CPM_LAMPIRAN, $this->CPM_ID_DOC
            );
			//var_dump($query);die;
            return mysqli_query($this->Conn, $query);
                        ...
                        <tr>
                            <td width=\"710\" colspan=\"2\"><table width=\"100%\" border=\"0\">
                                    ...
                                    <tr>
                                        ...
                                        <td align=\"left\">Kalianda, " . date("d") . " {$this->arr_bulan[(int) date("m")]} " . date("Y") . "<br/>{$JABATAN}<br/>" . strtoupper($KOTA) . "<br/><br/></td>
                                    </tr>
                                    <tr>
                                        <td colspan=\"3\">&nbsp;</td>
                                    </tr>  
                                    <tr>
                                        <td align=\"left\"></td>
                                        <td align=\"left\"></td>
                                        <td align=\"left\"><u>{$NAMA_TTD}</u><br/>Pembina Utama Muda<br/>NIP. {$NIP}</td>
                                    </tr>
                                </table>
                                </td>
                            </tr>
                            <tr>
                                <td>Tembusan :<br/>
                                    ...
                                    3. Bupati {$JENIS_PEMERINTAHAN} {$NAMA_PEMERINTAHAN}<br/>
