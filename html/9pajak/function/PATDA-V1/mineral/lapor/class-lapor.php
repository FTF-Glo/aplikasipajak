...
class LaporPajak extends Pajak
{
    ...
    public function get_masa_panen()
    {
        return array('Bulan' => 'Setiap Bulan', 'Triwulan' => 'Tiga Bulan Sekali', '6 Bulan' => 'Enam Bulan Sekali',);
    }

    public function get_pajak($npwpd = '', $nop = '')
    {
        $Op = new ObjekPajak();
        $arr_rekening = $this->getRekening("4.1.01.14");
        $pajak_atr = array();
        $list_nop = array();

        $query = "
			SELECT DOC.*, DATE_FORMAT(DOC.CPM_TGL_JATUH_TEMPO,'%d-%m-%Y') as CPM_TGL_JATUH_TEMPO
			FROM PATDA_MINERAL_DOC AS DOC WHERE DOC.CPM_ID = '{$this->_id}' LIMIT 0,1";
        ...
        if (empty($pajak['CPM_ID'])) {
            ...
            $pajak['CPM_REKENING'] = '4.1.1.11.06 - Pajak Mineral Bukan Logam dan Batuan';
            ...
            $query = "SELECT atr.*,rek.kdrek, rek.nmrek
                FROM PATDA_MINERAL_DOC pjk
                INNER JOIN PATDA_MINERAL_DOC_ATR atr ON pjk.CPM_ID = atr.CPM_ATR_MINERAL_ID
                INNER JOIN PATDA_REK_PERMEN13 rek ON atr.CPM_ATR_NAMA=rek.kdrek
    ...
    public function filtering($id)
    {
        ...
        // 				}
        //                 $html .= "<td>Pilih Jenis :<br><select style=\"width:100px; height:30px;\" class=\"form-control\" name=\"CPM_PIUTANG-{$id}\" id=\"CPM_PIUTANG-{$id}\">{$opt_pilih}</select></td>
        ...
        // 					<button type=\"button\"  style=\"width:150px; height:30px;\" class=\"btn btn-sm btn-secondary\" onclick=\"javascript:download_excel('{$id}','function/PATDA-V1/cetakpanjang/svc-download-bentang-panjang-min.xls.php')\">Cetak Bentang Panjang</button>            
        //                 </td>
        //             </tr></table></form>
        //         </div> ";
    ...
    private function save_pajak($cpm_no = '')
    {
        ...
        if ($validasi['result'] == true || ($validasi['data']['CPM_TRAN_STATUS'] == '4')) {
            ...
            // tambahan aldes
            $this->CPM_BAYAR_LAINNYA = (!empty($this->CPM_BAYAR_LAINNYA)) ? str_replace(",", "", $this->CPM_BAYAR_LAINNYA) : 0;
            ...
            $this->JENIS_TIPE_MASA = $this->CPM_MASA_TIPE;
            // echo $this->JENIS_TIPE_MASA ."<br>";
            ...
            $query = sprintf(
                "INSERT INTO {$this->PATDA_MINERAL_DOC}
                    ...
                    CPM_MASA_PAJAK,CPM_TAHUN_PAJAK,CPM_TIPE_PAJAK,
                    CPM_TOTAL_OMZET,CPM_TOTAL_PAJAK,CPM_TARIF_PAJAK,
                    CPM_TGL_JATUH_TEMPO,CPM_KETERANGAN,CPM_VERSION,
                    CPM_AUTHOR,CPM_ID_TARIF,CPM_BAYAR_LAINNYA,
                    CPM_DPP, CPM_BAYAR_TERUTANG, CPM_NO_SSPD,
                    CPM_MASA_PAJAK1,CPM_MASA_PAJAK2,CPM_DENDA_TERLAMBAT_LAP, JENIS_TIPE_MASA, CPM_PIUTANG)
                    VALUES ( '%s','%s','%s',
                             '%s','%s','%s',
                             '%s','%s','%s',
                             '%s','%s','%s',
                ...
                $this->JENIS_TIPE_MASA,
                $this->CPM_PIUTANG
            );
            // echo $query;exit;
            $res = mysqli_query($this->Conn, $query);

            if ($res && $this->CPM_PIUTANG == 0) {
    ...
    private function update_tgl_input()
    {
        ...
        $query = "UPDATE {$this->PATDA_MINERAL_DOC} SET CPM_TGL_INPUT = '{$tgl_input}'
    ...
    private function update_tgl_lapor()
    {
        ...
        $query = "UPDATE {$this->PATDA_MINERAL_DOC} SET CPM_TGL_LAPOR = '{$tgl_input}'
                  WHERE CPM_ID ='{$this->CPM_ID}'";
    ...
    private function update_tgl_lapor_ditolak($cpm_no, $tgl_lapor, $tgl_input)
    {
        ...
        if ($tgl_input == 'NULL') {
            $query = "UPDATE {$this->PATDA_MINERAL_DOC} SET CPM_TGL_LAPOR = '{$tgl_lapor}'
                  WHERE CPM_NO ='{$cpm_no}'";
        } else {
            $query = "UPDATE {$this->PATDA_MINERAL_DOC} SET CPM_TGL_LAPOR = '{$tgl_lapor}', CPM_TGL_INPUT = '{$tgl_input}'
                  WHERE CPM_NO ='{$cpm_no}'";
        }
    ...
    public function update()
    {
        ...
        if ($validasi['result'] == true) {
            ...
            $query = sprintf(
                "UPDATE {$this->PATDA_MINERAL_DOC} SET
                    CPM_TOTAL_OMZET = '%s',
                    CPM_TOTAL_PAJAK = '%s',
                    CPM_TARIF_PAJAK = '%s',
                    CPM_BAYAR_LAINNYA = '%s',
            ...
            if ($res) {
                ...
                $del = sprintf("DELETE FROM {$this->PATDA_MINERAL_DOC_ATR} WHERE CPM_ATR_MINERAL_ID = '%s'", $this->CPM_ID);
                ...
                foreach ($PAJAK_ATR['CPM_ATR_NAMA'] as $nama) {
                    ...
                    if ($nama != "") {
                        $query = sprintf(
                            "INSERT INTO {$this->PATDA_MINERAL_DOC_ATR}
                            (CPM_ATR_MINERAL_ID, CPM_ATR_NAMA,
                             CPM_ATR_VOLUME, CPM_ATR_HARGA)
                             VALUES ('%s','%s','%s','%s')",
                            $this->CPM_ID,
    ...
    public function print_sptpd()
    {
                        ...
                        <td colspan=\"2\"><table width=\"710\" border=\"1\" cellpadding=\"3\">
                                <tr>
                                    ...
                                    <td width=\"310\" valign=\"top\" align=\"center\">
                                        ...
                                        <b style=\"font-size:55px\">PAJAK MINERAL BUKAN LOGAM DAN BATUAN</b><br/>
                                ...
                                <tr>
                                    ...
                                    <td align=\"left\" width=\"270\" colspan=\"3\">
										<table width=\"700\" align=\"center\" cellpadding=\"1\" border=\"1\" cellspacing=\"0\">
											<tr>
												<td width=\"30\">No.</td>
												<td width=\"379\">Jenis Mineral Bukan Logam dan Batuan</td>
												<td width=\"130\">Volume /<br/> Tonase (m <sup>3</sup>)</td>
												<td width=\"150\">Harga pasar/<br/> nilai standar (m <sup>3</sup>)</td>
											</tr>";
        $xx = 0;
        foreach ($DATA['pajak_atr'] as $pajak_atr) {
            if (isset($pajak_atr['kdrek'])) {
                ...
                $page1 .= "<tr>
													<td width=\"30\">" . (++$xx) . ".</td>
													<td width=\"379\" align=\"left\" > {$pajak_atr['nmrek']}</td>
													<td align=\"right\" width=\"130\"> {$pajak_atr['CPM_ATR_VOLUME']} </td>
													<td align=\"right\" width=\"150\">Rp. " . number_format($pajak_atr['CPM_ATR_HARGA'], 2) . "</td>
												</tr>";
            }
        }
        $page1 .= "</table><br/>
				...
				<table width=\"710\" class=\"main\" cellpadding=\"0\" border=\"1\" cellspacing=\"0\">
					<tr>
						<td colspan=\"2\" align=\"center\" style=\"background-color:#CCC\">
							<b>C. PERNYATAAN</b>
						</td>
    ...
    /*public function print_sspd() {
        ...
        if ($DATA['pajak']['CPM_TIPE_PAJAK'] == 2) {
            ...
            $PERIODE = substr($this->CPM_NO, 14, 2)."0" . substr($this->CPM_NO, 0, 8);
        }
        ...
        $html = "<table width=\"710\" class=\"main\" border=\"1\">
                    <tr>
                        <td><table width=\"710\" border=\"1\" cellpadding=\"10\">
                                <tr>
                                    <th valign=\"top\" width=\"450\" align=\"center\"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>
                                        ".strtoupper($JENIS_PEMERINTAHAN)." " . strtoupper($NAMA_PEMERINTAHAN) . "<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                        ".strtoupper($NAMA_PENGELOLA)."<br /><br />  </b>
                    ...
                    <tr>
                        <td><table width=\"960\" border=\"0\" cellpadding=\"5\">
                                ...
                                <tr>
                                    <td width=\"230\">Kode Bayar (BENDAHARA)</td>
                                    <td>: <strong>{$PAYMENT_CODE}</strong></td>
                                </tr>
                                <tr>
                                    ...
                                    <td><table width=\"900\" border=\"1\" cellpadding=\"3\">
                                            ...
                                            ".($gw->payment_flag == 1?
												...
												<tr>
													...
													<td align=\"right\">Rp. " . number_format($gw->patda_total_bayar, 2) . "</td>
												</tr>"
												:
												"<tr>
													<td>4.</td>
        ...
        $pdf->AddPage('P', 'A4');
        $pdf->writeHTML($html, true, false, false, false, '');
        $pdf->Image("{$sRootPath}view/Registrasi/configure/logo/{$LOGO_CETAK_PDF}", 10, 20, 10, '', '', '', '', false, 300, '', false);
        $pdf->SetAlpha(0.3);
    ...
    public function get_val_tracking()
    {
        global $DIR;
        ...
        $query = "SELECT SUM(REPLACE(CPM_VOLUME, ',', '')) as TOTAL
                 FROM PATDA_MINERAL_AUDITTRAIL WHERE
                 CPM_TRUCK_ID IN ('{$id}') AND
                 DATE_FORMAT(STR_TO_DATE(CPM_DATE_SENT,'%d-%m-%Y'),'%Y')='{$this->CPM_TAHUN_PAJAK}' AND
                 DATE_FORMAT(STR_TO_DATE(CPM_DATE_SENT,'%d-%m-%Y'),'%m')='{$this->CPM_MASA_PAJAK}'";
        $result = mysqli_query($this->Conn, $query) or die($this->getError(mysqli_error($this->Conn)));
        ...
        if ($data = mysqli_fetch_assoc($result)) {
            ...
            $row = array();
            $row['CPM_TRUCK_ID'] = str_replace("','", ";", $id);
            $row['MASA_PAJAK'] = $this->CPM_MASA_PAJAK;
            $row['TAHUN_PAJAK'] = $this->CPM_TAHUN_PAJAK;
            $row['AMOUNT'] = empty($data['TOTAL']) ? 0 : $data['TOTAL'];
            $row['CPM_NPWPD'] = $this->CPM_NPWPD;

            $json = base64_encode($this->Json->encode($row));
            $q = base64_encode("{'a':'{$this->_a}', 'm':'{$this->_m}','u':'{$_SESSION['uname']}','i':'6','url':'function/{$DIR}/tracking/mineral/svc-download-tracking.xls.php'}");
            $link = "<a href='javascript:void(0)' onclick=\"javascript:get_detail_tracking('{$q}','{$json}')\">" . number_format($data['TOTAL'], 0) . "m<sup>3</sup></a>";

            $respon['amount'] = ($data['TOTAL'] == 0) ? "0" : $link;
        }
        mysqli_close($this->Conn);
        echo $this->Json->encode($respon);
    }
    ...
    public function get_previous_pajak($npwpd, $nop)
    {
        ...
        $query = "
			SELECT DOC.*, DATE_FORMAT(DOC.CPM_TGL_JATUH_TEMPO,'%d-%m-%Y') as CPM_TGL_JATUH_TEMPO
			FROM PATDA_MINERAL_DOC AS DOC
			INNER JOIN PATDA_MINERAL_PROFIL PR ON DOC.CPM_ID_PROFIL = PR.CPM_ID
        ...
        if (empty($pajak['CPM_ID'])) {
            ...
            for ($z = $x; $z < $max_rek; $z++) {
                $pajak_atr[$z] = array('CPM_ATR_NAMA' => '', 'CPM_ATR_VOLUME' => 0, 'CPM_ATR_HARGA' => 0);
            }
        }
