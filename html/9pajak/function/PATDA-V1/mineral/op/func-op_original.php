...
<form class="cmxform" autocomplete="off" id="form-op" method="post" action="function/<?php echo "{$DIR}/{$modul}"; ?>/op/svc-op.php?param=<?php echo base64_encode($json->encode(array("a" => $op->_a, "m" => $op->_m))) ?>">
	...
	<table class="main" width="900">
		<tr>
			<td colspan="2" align="center" class="subtitle"><b>PROFIL PAJAK MBLB</b></td>
		</tr>
		<?php if (!empty($npwpd)): ?>
			<tr>
		...
		<?php if (isset($npwpd)): ?>
			...
			<?php if (isset($nop)): ?>
				...
				<tr>
					<td>ID Angkutan <b class="isi">*</b></td>
					<td>:
						<select name="CPM_TRUCK_ID[]" id="CPM_TRUCK_ID" multiple="multiple" style="width: 600px;">
							<option></option>
							<?php
							if (count($angkutan) > 0) {
