...
class ObjekPajak extends Pajak {
    ...
    public function update() {
		$truck_id = implode(';',$_POST['CPM_TRUCK_ID']);
		
        #update profil
        $query = sprintf("UPDATE PATDA_MINERAL_PROFIL SET
                    CPM_NPWPD = '%s',
                    CPM_NAMA_WP = '%s',
                    CPM_ALAMAT_WP = '%s',
                    CPM_KELURAHAN_WP = '%s',
                    ...
                    $this->CPM_TELEPON_WP, $this->CPM_TELEPON_OP, $truck_id);
    ...
    public function delete(){
		...
		if(isset($_POST['CPM_ID'])){
			...
			$query = "select count(*) as total FROM PATDA_MINERAL_PROFIL A INNER JOIN 
			PATDA_MINERAL_DOC B ON A.CPM_ID = B.CPM_ID_PROFIL
			WHERE A.CPM_ID = '{$_POST['CPM_ID']}'";
			
			$res = mysqli_query($this->Conn, $query);
			$res_data = mysqli_fetch_assoc($res);
			if($res_data['total'] == 0){
				$query = "DELETE FROM PATDA_MINERAL_PROFIL WHERE CPM_ID = '{$_POST['CPM_ID']}'";
				$res = mysqli_query($this->Conn, $query);
				if($res){ 
    ...
    public function save() {
        ...
        $query = "SELECT * FROM PATDA_MINERAL_PROFIL 
        ...
        $query = "UPDATE PATDA_MINERAL_PROFIL SET CPM_AKTIF ='0' WHERE CPM_NPWPD = '{$this->CPM_NPWPD}' AND CPM_NOP='{$this->CPM_NOP}'";
        $res = mysqli_query($this->Conn, $query);

        if($this->CPM_NOP==''){
            // get last nop
            $res_nop = mysqli_query($this->Conn, "SELECT CPM_NOP from PATDA_MINERAL_PROFIL WHERE LENGTH(CPM_NOP)=12 order by CPM_NOP desc limit 1");
            ...
            // echo $this->CPM_NOP;exit();
        }

        #insert profil baru
        $query = sprintf("INSERT INTO PATDA_MINERAL_PROFIL 
                    ...
                    CPM_APPROVE, CPM_AUTHOR, CPM_NOP, CPM_TELEPON_WP, CPM_TELEPON_OP, CPM_TRUCK_ID, longitude, latitude)
                    VALUES ( '%s','%s','%s',
                             '%s','%s','%s',
                             '%s','%s',
                             '%s','%s',
                             ...
                             '%s','%s','%s','%s','%s','%s','%s','%s')",
                              ...
                              $this->CPM_AUTHOR, $this->CPM_NOP, $this->CPM_TELEPON_WP, $this->CPM_TELEPON_OP, $truck_id, $this->longitude, $this->latitude
    ...
    public function get_last_profil($npwpd, $nop) {
        ...
        $query = "SELECT 
			...
			FROM PATDA_WP WP 
				LEFT JOIN PATDA_MINERAL_PROFIL P ON P.CPM_NPWPD = WP.CPM_NPWPD AND P.CPM_AKTIF='1' AND P.CPM_NOP='{$nop}'
    ...
    public function getRekening($kdrek='') {
        return parent::getRekening("4.1.01.14");
    }
    ...
    public function grid_data() {
        try {
            ...
            #count utk pagging
            $query = "SELECT COUNT(*) AS RecordCount FROM {$this->PATDA_MINERAL_PROFIL}
					WHERE {$where}";
            $result = mysqli_query($this->Conn, $query);
			...
			$query = "SELECT * FROM {$this->PATDA_MINERAL_PROFIL} WHERE {$where}
	...
	public function check_nop(){
		...
		$query = sprintf("SELECT COUNT(*) AS TOTAL FROM PATDA_MINERAL_PROFIL 
	...
	public function get_list_nop($npwpd){
		...
		$query = "SELECT PR.*,KEC.CPM_KECAMATAN,KEL.CPM_KELURAHAN FROM {$this->PATDA_MINERAL_PROFIL} PR
