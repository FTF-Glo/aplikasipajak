...
$angkutan = $pajak->get_list_angkutan();
?>
			...
			<div class="col-md-6">
				<div class="form-group">
					...
					<?php if (!empty($npwpd)): ?>
						...
						?>
							<input type="button" value="Cari NPWPD Lainnya" class="btn btn-info lm-btn" style="margin-top: 10px" onclick="location.href='<?php echo $prm ?>'">
						<?php endif; ?>
					<?php else: ?>
		...
		<?php if (isset($npwpd)) : ?>
			<?php if (isset($nop)) : ?>
				...
				<div class="row">
					<div class="col-md-12">
						<div class="form-group">
							<label>ID Angkutan <b class="isi">*</b></label>
							<select name="CPM_TRUCK_ID[]" id="CPM_TRUCK_ID" multiple="multiple" class="form-control" style="width: 100%;">
								<option></option>
								<?php
								if (count($angkutan) > 0) {
									$arr_angkutan = explode(';', $value['CPM_TRUCK_ID']);
									foreach ($angkutan as $ang) {
										echo '<option value="' . $ang->CPM_TRUCK_ID . '" ' . (in_array($ang->CPM_TRUCK_ID, $arr_angkutan) ? 'selected' : '') . '>' . $ang->CPM_NOPOL . '</option>';
									}
								}
								?>
							</select>
