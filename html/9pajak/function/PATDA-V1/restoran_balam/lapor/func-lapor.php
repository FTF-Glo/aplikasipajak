...
<script type="text/javascript" src="function/<?php echo "{$DIR}/{$modul}"; ?>/lapor/func-lapor.js?=v.0.2.3.js"></script>
<script type="text/javascript" src="function/<?php echo $DIR ?>/op.js?=v.0.0.3.js"></script>
...
<form class="cmxform" autocomplete="off" id="form-lapor" method="post" action="function/<?php echo "{$DIR}/{$modul}"; ?>/lapor/svc-lapor.php?param=<?php echo base64_encode($json->encode(array("a" => $lapor->_a, "m" => $lapor->_m, "mod" => $lapor->_mod, "f" => $lapor->_f))) ?>">
    ...
    <table class="main" width="900">
        ...
        <?php 
    if (!empty($DATA['profil']['CPM_ID'])){
    ?>
        <tr>
            <td colspan="2" align="center" class="subtitle"><b>TAGIHAN 6 BULAN TERAKHIR</b></td>
        </tr>
                ...
                <tbody>
                    ...
                    foreach ($tagihan as $row) {
                        ...
                        if ($row['status_bayar'] == "LUNAS") {
                            ...
                            $total = $row['simpatda_dibayar'];
                        }
                    
                            echo "<tr>";
                            echo "<td>" . $row['sspd'] . "</td>";
                            echo "<td>" .  date("d-m-Y", strtotime($row['masa_pajak_awal'])) ." - " . date("d-m-Y", strtotime($row['masa_pajak_akhir'])) ."</td>";
                            echo "<td>" . $lapor->arr_bulan2[$row['simpatda_bulan_pajak']] . "</td>" ;
                            echo "<td>" .  $row['simpatda_tahun_pajak'] . "</td>";
                            echo "<td> Rp. " . number_format($row['simpatda_dibayar']) . "</td>";
                            echo "<td> Rp. " . number_format($row['patda_denda']) . "</td>";
            ...
            <td colspan="2">
                ...
                <table width="950" border="0" align="center" class="child">
                    ...
                    <tr valign="top">
                        ...
                        <td width="200"> : <?php echo ($DATA['pajak']['CPM_NO'] != "") ? "<input class=\"form-control\" style=\"width:150px;height:30px;display:inline-block;\" type=\"text\" name=\"PAJAK[CPM_NO]\" id=\"CPM_NO\" maxlength=\"25\" value=\"{$DATA['pajak']['CPM_NO']}\" readonly>" : "..." ?></td>
                        ...
                        <td width="400" rowspan="9">Keterangan :</br>
                            <textarea class="form-control" name="PAJAK[CPM_KETERANGAN]" id="CPM_KETERANGAN" style="display:block" rows="10" <?php echo ($lapor->_s == 1 || $lapor->_s == "" || ($lapor->_s == "4" && $lapor->_mod == "pel")) ? "" : "readonly"; ?> placeholder="Keterangan"><?php echo $DATA['pajak']['CPM_KETERANGAN'] ?></textarea>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td> :
                            <select class="form-control" style="width:150px;height:30px;display:inline-block;" name="PAJAK[CPM_TAHUN_PAJAK]" id="CPM_TAHUN_PAJAK">
                                <?php
                                if (in_array($lapor->_mod, array("pel", ""))) {
                                    if (!in_array($lapor->_i, array(1, 3, ""))) {
                    ...
                    <tr>
                        <td>Bulan Pajak</td>
                        <td> :
                            <select class="form-control" style="width:150px;height:30px;display:inline-block;" id="CPM_MASA_PAJAK">
                                <?php
                                if (in_array($lapor->_mod, array("pel", ""))) {
                                    if (!in_array($lapor->_i, array(1, 3, ""))) {
                    ...
                    <tr>
                        ...
                        <td colspan="2"> : <input type="text" name="PAJAK[CPM_MASA_PAJAK1]" <?php echo ($lapor->_s == 1 || $lapor->_s == "" || ($lapor->_s == "4" && $lapor->_mod == "pel")) ? "id=\"CPM_MASA_PAJAK1\"" : ""; ?> style="width: 120px;" readonly value="<?php echo $DATA['pajak']['CPM_MASA_PAJAK1']; ?>" placeholder="Masa Awal">
                            s.d
                    ...
                    <tr>
                        ...
                        <td> : <input type="text" name="PAJAK[CPM_TARIF_PAJAK]" id="CPM_TARIF_PAJAK" style="width: 50px;" readonly value="<?php echo $DATA['pajak']['ARR_REKENING']['4.1.01.07.01']['tarif'] ?>" placeholder="Tarif Pajak"> %
                        </td>
                        <?php if ($DATA['profil']['CPM_REKENING'] == "4.1.01.07.07") { ?>
                            <td width="300" colspan="2">Pelaksana kegiatan : <input type="text" name="PAJAK[PELAKSANA_KEGIATAN]" id="PELAKSANA_KEGIATAN" maxlength="17" value="<?php echo $DATA['pajak']['PELAKSANA_KEGIATAN'] ?>" placeholder="Pelaksanan Kegiatan" <?php echo ($lapor->_s == 1 || $lapor->_s == "" || ($lapor->_s == "4" && $lapor->_mod == "pel")) ? "" : "readonly"; ?>>
                            </td>
                        <?php } ?>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td> : <input type="text" name="PAJAK[CPM_DENDA_TERLAMBAT_LAP]" id="CPM_DENDA_TERLAMBAT_LAP" readonly <?php echo ($lapor->_s == 1 || $lapor->_s == "" || ($lapor->_s == "4" && $lapor->_mod == "pel")) ? ($editable_terlambat_lap == 1 ? "" : "readonly")  : "readonly"; ?> class="SUM number" value="<?php echo $DATA['pajak']['CPM_DENDA_TERLAMBAT_LAP'] ?>" placeholder="Sanksi Telat Lapor"></td>
                    </tr>
...
<script>
    /* codingan untuk gabisa input kalo nunggak
    ...
    */
    ...
    // Panggil set_tarif_pajak dengan nilai default pada halaman load
    $(document).ready(function() {
      var defaultYear = $('#CPM_TAHUN_PAJAK').val(); // Ambil nilai default dari elemen select
      set_tarif_pajak(defaultYear);
    });

    document.getElementById('CPM_TAHUN_PAJAK').addEventListener('change', function() {
      var selectedYear = this.value;
      // Kirim nilai tahun menggunakan Ajax
      set_tarif_pajak(selectedYear);
      // console.log(selectedYear);
    });

    function set_tarif_pajak(selectedYear) {
        
      var kdrek = $('#CPM_REKENING').val();

      $.ajax({
          type: "POST",
          url: "function/PATDA-V1/svc-class-pajak.php", // Menggunakan PHP_SELF untuk mengarahkan ke file yang sama
          data: { tahun: selectedYear, kdrek : kdrek  },
          success: function(response) {
              // Handle response dari server jika diperlukan
              $('#CPM_TARIF_PAJAK').val(response);
