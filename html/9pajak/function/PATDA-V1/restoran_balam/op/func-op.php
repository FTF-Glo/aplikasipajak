		...
		<?php if (isset($npwpd)) : ?>
			...
			<?php if (isset($nop)) : ?>
				...
				<tr valign="top">
					...
					<td>:
						<select name="PROFIL[CPM_REKENING]" id="CPM_REKENING">
							<?php
				...
				<tr>
					<td>Menggunakan Cash Register <b class="isi">*</b></td>
					<td>:
						<label><input type="radio" name="PROFIL[CPM_CASHREGISTER]" value="1" <?php echo $value['CPM_CASHREGISTER'] == '1' ? 'checked' : '' ?> /> Ya</label>
						<label><input type="radio" name="PROFIL[CPM_CASHREGISTER]" value="2" <?php echo $value['CPM_CASHREGISTER'] == '2' ? 'checked' : '' ?> /> Tidak</label>
					</td>
				</tr>
				<tr>
					<td>Menggunakan Pembukuan / Pencatataan <b class="isi">*</b></td>
					<td>:
						<label><input type="radio" name="PROFIL[CPM_PEMBUKUAN]" value="1" <?php echo $value['CPM_PEMBUKUAN'] == '1' ? 'checked' : '' ?> /> Ya</label>
						<label><input type="radio" name="PROFIL[CPM_PEMBUKUAN]" value="2" <?php echo $value['CPM_PEMBUKUAN'] == '2' ? 'checked' : '' ?> /> Tidak</label>
					</td>
				</tr>
				<tr>
					<td>Longtitude <b class="isi"></b></td>
					<td>:
						<input type="text" name="PROFIL[longitude]" id="longitude" style="width: 200px;" value="<?php echo $value['longitude'] ?>" placeholder="Longtitude">
						<label for="tapping1"><input type="radio" name="PROFIL[tapping]" id="tapping1" value="1" checked> Menggunakan Tappingbox &nbsp;</label>
						<label for="tapping2"><input type="radio" name="PROFIL[tapping]" id="tapping2" value="0"> Tidak Menggunakan Tappingbox</label>
					</td>
				</tr>
				<tr>
				...
				<tr>
					<td colspan="2" align="center" id="detail_rinci">
						<table width="900" class="profil_detail">
							<tr>
								<th width="30%">Jumlah Meja</th>
								<th width="30%">Jumlah Kursi</th>
								<th width="30%">Jumlah Pengunjung Per Hari</th>
								<th width="10%">Aksi</th>
							</tr>
							<?php
							if ($jml_detail > 0) {
								foreach ($value['detail'] as $i => $det) {
									$i++;
									echo "<tr>
								<td>
									<input type=\"hidden\" id=\"CPM_ID-{$i}\" name=\"PROFIL_DETAIL[CPM_ID][]\" value=\"{$det['CPM_ID']}\" />
									<input type=\"number\" class=\"number\" id=\"CPM_JUMLAH_MEJA-{$i}\" name=\"PROFIL_DETAIL[CPM_JUMLAH_MEJA][]\" value=\"{$det['CPM_JUMLAH_MEJA']}\" style=\"display:block;width:100%\" />
								</td>
								<td><input type=\"number\" class=\"number\" id=\"CPM_JUMLAH_KURSI-{$i}\" name=\"PROFIL_DETAIL[CPM_JUMLAH_KURSI][]\" value=\"{$det['CPM_JUMLAH_KURSI']}\" style=\"display:block;width:100%\" /></td>
								<td><input type=\"number\" class=\"number\" id=\"CPM_JUMLAH_PENGUNJUNG-{$i}\" name=\"PROFIL_DETAIL[CPM_JUMLAH_PENGUNJUNG][]\" value=\"{$det['CPM_JUMLAH_PENGUNJUNG']}\" style=\"display:block;width:100%\" /></td>
								<td align=\"center\"><a href=\"#\" onclick=\"hapus_detail({$i});return false;\">[Hapus]</a></td>
							</tr>";
								}
							}
							?>
						</table>
						<input type="hidden" id="num_detail" value="<?php echo $jml_detail ?>" />
						<input class="add button" type="button" value="Tambah" />
					</td>
				</tr>
