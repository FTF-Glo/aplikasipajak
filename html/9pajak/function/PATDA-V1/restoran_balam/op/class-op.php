...
class ObjekPajak extends Pajak {
    ...
    public function update() {
        #update profil
        $query = sprintf("UPDATE PATDA_RESTORAN_PROFIL SET
                    CPM_NPWPD = '%s',
                    CPM_NAMA_WP = '%s',
                    CPM_ALAMAT_WP = '%s',
                    CPM_KELURAHAN_WP = '%s',
                    ...
                    $this->CPM_TELEPON_WP, $this->CPM_TELEPON_OP, $this->CPM_CASHREGISTER, $this->CPM_PEMBUKUAN);
        

                    # update profil detail
        foreach($_REQUEST['PROFIL_DETAIL']['CPM_ID'] as $x=>$id){
            $jml_meja = $_REQUEST['PROFIL_DETAIL']['CPM_JUMLAH_MEJA'][$x];
            $jml_kursi = $_REQUEST['PROFIL_DETAIL']['CPM_JUMLAH_KURSI'][$x];
            $jml_pengunjung = $_REQUEST['PROFIL_DETAIL']['CPM_JUMLAH_PENGUNJUNG'][$x];
            if($jml_meja!='' && $jml_kursi!='' && $jml_pengunjung!=''){ // hanya save data yg diisi semua
                if($id=='')
                    $sql_detail = "INSERT INTO PATDA_RESTORAN_PROFIL_DETAIL(CPM_NPWPD, CPM_JUMLAH_MEJA, CPM_JUMLAH_KURSI, CPM_JUMLAH_PENGUNJUNG) values('{$this->CPM_NPWPD}', '{$jml_meja}', '{$jml_kursi}', '{$jml_pengunjung}')";
                else
                    $sql_detail = "UPDATE PATDA_RESTORAN_PROFIL_DETAIL SET CPM_JUMLAH_MEJA='{$jml_meja}', CPM_JUMLAH_KURSI='{$jml_kursi}', CPM_JUMLAH_PENGUNJUNG='{$jml_pengunjung}' WHERE CPM_ID='{$id}'";
                mysqli_query($this->Conn, $sql_detail);
            }
        }
    ...
    public function update_device() {
		...
		if(count($acc_device) != 0){
			$query = sprintf("UPDATE PATDA_RESTORAN_PROFIL 
				SET CPM_DEVICE_ID = '%s' 
    ...
    public function delete(){
		...
		if(isset($_POST['CPM_ID'])){
			...
			$query = "select count(*) as total FROM PATDA_RESTORAN_PROFIL A INNER JOIN 
			PATDA_RESTORAN_DOC B ON A.CPM_ID = B.CPM_ID_PROFIL
			WHERE A.CPM_ID = '{$_POST['CPM_ID']}'";
			
			$res = mysqli_query($this->Conn, $query);
			$res_data = mysqli_fetch_assoc($res);
			if($res_data['total'] == 0){
				$query = "DELETE FROM PATDA_RESTORAN_PROFIL WHERE CPM_ID = '{$_POST['CPM_ID']}'";
				$res = mysqli_query($this->Conn, $query);
				if($res){ 
    ...
    public function save() {
        ...
        $this->CPM_NPWPD = str_replace(array('.',' '), '', $this->CPM_NPWPD);

        $query = "SELECT * FROM PATDA_RESTORAN_PROFIL 
        ...
        $query = "UPDATE PATDA_RESTORAN_PROFIL SET CPM_AKTIF ='0' WHERE CPM_NPWPD = '{$this->CPM_NPWPD}' AND CPM_NOP='{$this->CPM_NOP}'";
        $res = mysqli_query($this->Conn, $query);

        if($this->CPM_NOP==''){
            // get last nop
            $res_nop = mysqli_query($this->Conn, "SELECT CPM_NOP from PATDA_RESTORAN_PROFIL WHERE LENGTH(CPM_NOP)=12 order by CPM_NOP desc limit 1");
        ...
        #insert profil baru
        $query = sprintf(
            "INSERT INTO PATDA_RESTORAN_PROFIL 
                    ...
                    CPM_CASHREGISTER, CPM_PEMBUKUAN,
                    CPM_APPROVE, CPM_AUTHOR, CPM_NOP, CPM_TELEPON_WP, CPM_TELEPON_OP, longitude, latitude,tapping)
                    VALUES ( '%s','%s','%s',
                             '%s','%s','%s',
                             '%s','%s',
                             '%s','%s',
                             ...
                             '%s','%s','%s','%s','%s','%s','%s','%s')",
                              ...
                              $this->CPM_KECAMATAN_OP,$this->CPM_AKTIF, $this->CPM_REKENING, 
                              $this->CPM_CASHREGISTER, $this->CPM_PEMBUKUAN,
                              $this->CPM_APPROVE, $this->CPM_AUTHOR, $this->CPM_NOP, $this->CPM_TELEPON_WP, $this->CPM_TELEPON_OP, $this->longitude, $this->latitude,$this->tapping
        );
        // var_dump($query);die;
        $save_op = mysqli_query($this->Conn, $query);

        if($save_op){
            # insert profil detail
            foreach($_REQUEST['PROFIL_DETAIL']['CPM_ID'] as $x=>$id){
                $jml_meja = $_REQUEST['PROFIL_DETAIL']['CPM_JUMLAH_MEJA'][$x];
                $jml_kursi = $_REQUEST['PROFIL_DETAIL']['CPM_JUMLAH_KURSI'][$x];
                $jml_pengunjung = $_REQUEST['PROFIL_DETAIL']['CPM_JUMLAH_PENGUNJUNG'][$x];
                if($jml_meja!='' && $jml_kursi!='' && $jml_pengunjung!=''){ // hanya save data yg diisi semua
                    mysqli_query($this->Conn, "INSERT INTO PATDA_RESTORAN_PROFIL_DETAIL(CPM_NPWPD, CPM_JUMLAH_MEJA, CPM_JUMLAH_KURSI, CPM_JUMLAH_PENGUNJUNG) values('{$jml_meja}', '{$jml_kursi}', '{$jml_pengunjung})");
                }
            }
    ...
    public function del_detail(){
        $del = mysqli_query($this->Conn, "DELETE FROM PATDA_RESTORAN_PROFIL_DETAIL WHERE CPM_ID='{$_REQUEST['DATA_ID']}'");
        mysqli_close($this->Conn);
        if($del){
            echo json_encode(array('status'=>1, 'message'=>'Data berhasil dihapus'));
        }else{
            echo json_encode(array('status'=>0, 'message'=>'Data gagal dihapus. Silahkan coba lagi!'));
        }
    }
    ...
    public function get_last_profil($npwpd, $nop) {
        ...
        $query = "SELECT 
			...
			FROM PATDA_WP WP 
				LEFT JOIN PATDA_RESTORAN_PROFIL P ON P.CPM_NPWPD = WP.CPM_NPWPD AND P.CPM_AKTIF='1' AND P.CPM_NOP='{$nop}'
        ...
        # query get profil detail
        $query2 = mysqli_query($this->Conn, "SELECT * from PATDA_RESTORAN_PROFIL_DETAIL WHERE CPM_NPWPD='{$npwpd}'");
        ...
        $data = array_merge($data, $arr_rekening, $detail); 
    ...
    public function getRekening($kdrek='') {
        return parent::getRekening("4.1.01.07");
    }
                    ...
                    $(document).ready(function() {
                        $('#laporanPajak-{$this->_i}').jtable({
                            ...
                            fields: {
                                ...
                                tapping  : {title: 'Tapping',width: '10%'},
                            },
    ...
    public function grid_data() {
        try {
            ...
            #count utk pagging
            $query = "SELECT COUNT(*) AS RecordCount FROM {$this->PATDA_RESTORAN_PROFIL}
					WHERE {$where}";
            $result = mysqli_query($this->Conn, $query);
			...
			$query = "SELECT *, 
                        IF(tapping = 1, 'Menggunakan Tappingbox', 'Tidak Menggunakan Tappingbox') as tapping 
                        FROM {$this->PATDA_RESTORAN_PROFIL} WHERE {$where}
	...
	public function check_nop(){
		...
		$query = sprintf("SELECT COUNT(*) AS TOTAL FROM PATDA_RESTORAN_PROFIL 
	...
	public function get_list_nop($npwpd){
		...
		$query = "SELECT PR.*,KEC.CPM_KECAMATAN,KEL.CPM_KELURAHAN FROM {$this->PATDA_RESTORAN_PROFIL} PR
