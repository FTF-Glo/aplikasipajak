...
<form class="cmxform" id="form-profil" method="post" action="function/<?php echo "{$DIR}/{$modul}"; ?>/svc-profil.php?param=<?php echo base64_encode($json->encode(array("a" => $profil->_a, "m" => $profil->_m))) ?>">
    ...
    <input type="hidden" name="PROFIL[CPM_DEVICE_ID]" id="CPM_DEVICE_ID" value="<?php echo base64_decode($value['CPM_DEVICE_ID']) ?>">
    ...
    <table class="main" width="900">        
        ...
        <tr>
            <td>Kelurahan Restoran <b class="isi">*</b></td>
            <td>: 
        ...
        <tr valign="top">
            <td>Golongan Restoran <b class="isi">*</b></td>
            <td>: <select name="PROFIL[CPM_GOL_RESTORAN]" id="CPM_GOL_RESTORAN">
                    <?php
                    foreach ($value['CPM_REKENING'] as $gol) {
                        echo ($value['CPM_GOL_RESTORAN'] == $gol['kdrek'])? "<option value='{$gol['kdrek']}' selected>{$gol['kdrek']} - {$gol['nmrek']}</option>" : "<option value='{$gol['kdrek']}'>{$gol['kdrek']} - {$gol['nmrek']}</option>";
                    }
                    ?>
                </select>
            </td>
