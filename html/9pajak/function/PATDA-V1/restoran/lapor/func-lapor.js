$(document).ready(function () {
    ...
    form.validate({
        rules: {
            ...
            "PAJAK[CPM_GOL_RESTORAN]": "required",
            "PAJAK[CPM_MASA_PAJAK1]": "required",
            "PELAKSANA_KEGIATAN": "required",
            "PELAKSANA_KEGIATAN1": "required",
        ...
        messages: {
            ...
            "PAJAK[CPM_GOL_RESTORAN]": "harus diisi",
            "PAJAK[CPM_MASA_PAJAK1]": "harus diisi",
            "PELAKSANA_KEGIATAN": "harus diisi",
            "PELAKSANA_KEGIATAN1": "harus diisi",
        }
    });
    ...
    /*FUNGSI PADA PELAPORAN DI PELAYANAN*/
    ...
    $("#CPM_NPWPD").keyup(function () {
        ...
        $("#CPM_NAMA_OP").val('');
        $("#CPM_ALAMAT_OP").val('');
        $("#CPM_GOL_RESTORAN option").prop('selected', false).attr('disabled', 'disabled');
    ...
    $("#btn-search-npwpd").click(function () {
        ...
        $.ajax({
            type: "POST",
            data: "PAJAK[CPM_NPWPD]=" + npwpd + "&function=search_npwpd&PAJAK[CPM_JENIS_PAJAK]=8",
            ...
            success: function (res) {
                if (res.result == 1) {
                    ...
                    $("#CPM_GOL_RESTORAN option[value='" + res.CPM_GOL_RESTORAN + "']").prop('selected', true).removeAttr('disabled');
                    ;
                    $('#CPM_TARIF_PAJAK').val($("#CPM_GOL_RESTORAN option[value='" + res.CPM_GOL_RESTORAN + "']").attr('tarif'));
                    function_getval_tapbox();
                } else {
    ...
    //setting AUTO COMPLATE
    if ($('#CPM_REKENING').val() === '4.1.01.07.07') {
        var tipe = 2;
        //   console.log(tipe);
        $('#CPM_TIPE_PAJAK').val(tipe);
        // $('#CPM_TIPE_PAJAK').prop('readonly', true);
        $('#CPM_TIPE_PAJAK option[value="2"]').prop('disabled', false);
        $('input[name="PAJAK[CPM_METODE_HITUNG]"][value="Non DPP"]').prop('checked', false);
        $('input[name="PAJAK[CPM_METODE_HITUNG]"][value="DPP"]').prop('checked', true);
        ...
        // $('#CPM_TIPE_PAJAK').val(1);
        // $('#CPM_TIPE_PAJAK').prop('readonly', true);
        $('#CPM_TIPE_PAJAK option[value="1"]').prop('disabled', false);

        $('input[name="PAJAK[CPM_METODE_HITUNG]"][value="Non DPP"]').prop('checked', true);
        $('input[name="PAJAK[CPM_METODE_HITUNG]"][value="DPP"]').prop('checked', false);
        setTipePajak(tipe)
    }
...
function getDetTranTapbox(q, json) {
    ...
    $.ajax({
        data: "i=6&p=" + json + "&q=" + q,
        type: "post",
        url: "view/PATDA-V1/restoran/svc-list-pembanding-detail.php",
...
function function_sum(obj) {
    ...
    if (metode_hitung == 'DPP') {
        ...
        console.log(masa_pajak_akhir, selisih_bulan, sanksi);
        // var sanksi = selisih_bulan2 * dpp;
        // console.log(selisih_bulan2, dpp);
        $('#CPM_DENDA_TERLAMBAT_LAP').autoNumeric('set', sanksi);
...
//     var total = ((eval(dpp))) + eval(sanksi);
//     total = Math.ceil(total);
//     $('#CPM_TOTAL_PAJAK').autoNumeric('set', total);
...
//     })
// }
// function ini untuk menghitung selisih bulan setelah tanggal akhir bulan
// function selisihBulan_ori(awal_bulan) {
