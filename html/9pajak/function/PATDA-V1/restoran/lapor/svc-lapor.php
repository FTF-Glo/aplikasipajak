...
$json = new Services_JSON(SERVICES_JSON_SUPPRESS_ERRORS);
if ($_GET['search']) {
    //$a = $json->decode(base64_decode($_GET['term']));
    $lapor->get_kegiatan($_GET['search']);
    // var_dump($lapor->get_kegiatan($_GET['search']));
    // die;
}
...
if (isset($_GET['param'])) {
    if (!in_array($_POST['function'], array("print_sptpd", "print_sspd", "print_skpd"))) {
        $lapor->redirect();
    }
}
