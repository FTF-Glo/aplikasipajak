...
class ObjekPajak extends Pajak
{
    ...
    public function update()
    {
        #update profil
        $query = sprintf(
            "UPDATE PATDA_REKLAME_PROFIL SET
                    CPM_NPWPD = '%s',
                    CPM_NAMA_WP = '%s',
                    CPM_ALAMAT_WP = '%s',
                    CPM_KELURAHAN_WP = '%s',
    ...
    public function delete()
    {
        ...
        if (isset($_POST['CPM_ID'])) {
            ...
            $query = "select count(*) as total FROM PATDA_REKLAME_PROFIL A INNER JOIN 
			PATDA_REKLAME_DOC B ON A.CPM_ID = B.CPM_ID_PROFIL
			WHERE A.CPM_ID = '{$_POST['CPM_ID']}'";

            $res = mysqli_query($this->Conn, $query);
            $res_data = mysqli_fetch_assoc($res);
            if ($res_data['total'] == 0) {
                $query = "DELETE FROM PATDA_REKLAME_PROFIL WHERE CPM_ID = '{$_POST['CPM_ID']}'";
                $res = mysqli_query($this->Conn, $query);
                if ($res) {
    ...
    {
        $query = "SELECT * FROM PATDA_REKLAME_PROFIL 
        ...
        $query = "UPDATE PATDA_REKLAME_PROFIL SET CPM_AKTIF ='0' WHERE CPM_NPWPD = '{$this->CPM_NPWPD}' AND CPM_NOP='{$this->CPM_NOP}'";
        $res = mysqli_query($this->Conn, $query);

        if ($this->CPM_NOP == '') {
            // get last nop
            $res_nop = mysqli_query($this->Conn, "SELECT CPM_NOP from PATDA_REKLAME_PROFIL WHERE LENGTH(CPM_NOP)=12 order by CPM_NOP desc limit 1");
    ...
    public function get_last_profil($npwpd, $nop)
    {
        ...
        $query = "SELECT 
			...
			FROM PATDA_WP WP 
				LEFT JOIN PATDA_REKLAME_PROFIL P ON P.CPM_NPWPD = WP.CPM_NPWPD AND P.CPM_AKTIF='1' AND P.CPM_NOP='{$nop}'
    ...
    public function get_last_profil_perpanjangan($npwpd, $nop)
    {
        #query untuk mengambil data relasi user dan data profil
        $query = "SELECT 
				P.*, WP.*,gw.sspd, 
			...
			FROM PATDA_WP WP 
				...
				LEFT JOIN PATDA_MST_KELURAHAN AS KEL ON P.CPM_KELURAHAN_OP = KEL.CPM_KEL_ID 
				LEFT JOIN PATDA_MST_KECAMATAN AS KEC ON P.CPM_KECAMATAN_OP = KEC.CPM_KEC_ID
                LEFT JOIN simpatda_gw gw ON WP.CPM_NPWPD = gw.npwpd
			WHERE WP.CPM_NPWPD = '{$npwpd}'";
        // echo $query . '<br><br>';
        $result = mysqli_query($this->Conn, $query);
        $data = $this->get_field_array($result);
    ...
    {
        return parent::getRekening("4.1.01.09");
    }
    ...
    public function get_profil_bywp($npwpd)
    {
        #query untuk mengambil data relasi user dan data profil
        $query = "SELECT  * FROM PATDA_WP WHERE CPM_NPWPD = '{$npwpd}' limit 1";
        // echo $query;
        $result = mysqli_query($this->Conn, $query);
        $data = $this->get_field_array($result);
                    ...
                    $(document).ready(function() {
                        $('#laporanPajak-{$this->_i}').jtable({
                            ...
                            fields: {
                                ...
                                CPM_ALAMAT_OP: {title: 'Alamat OP',width: '10%'},longitude : {title: 'Longtitude',width: '10%'},
    ...
    public function grid_data()
    {
        try {
            ...
            #count utk pagging
            $query = "SELECT COUNT(*) AS RecordCount FROM {$this->PATDA_REKLAME_PROFIL}
					WHERE {$where}";
            $result = mysqli_query($this->Conn, $query);
            ...
            $query = "SELECT * FROM {$this->PATDA_REKLAME_PROFIL} WHERE {$where}
    ...
    public function check_nop()
    {
        ...
        $query = sprintf(
            "SELECT COUNT(*) AS TOTAL FROM PATDA_REKLAME_PROFIL 
    ...
    public function get_list_nop($npwpd)
    {
        ...
        $query = "SELECT PR.*,KEC.CPM_KECAMATAN,KEL.CPM_KELURAHAN FROM {$this->PATDA_REKLAME_PROFIL} PR
