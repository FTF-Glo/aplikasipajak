...
class LaporPajak extends Pajak
{
	...
	public function filtering($id)
	{
		...
		$opt_bulan = '<option value="">All</option>';
		foreach ($this->arr_bulan as $x => $y) {
			$bulan_dua_digit = sprintf("%02d", $x); // Mengonversi nilai bulan menjadi dua digit
			$opt_bulan .= "<option value='{$bulan_dua_digit}'>{$y}</option>";
		}
		...
		$list_jenis = $this->get_list_jenis_penerimaan();
		$jenis_penerimaan = "<option value=\"\">All</option>";
		foreach ($list_jenis as $k => $v) {
			$jenis_penerimaan .= "<option value=\"{$k}\">{$v->jenis_penerimaan}</option>";
		}

		$list_jenis_ret = $this->get_list_jenis_retribusi();
		$nama_pendapatan = "<option value=\"\">All</option>";
		foreach ($list_jenis_ret as $k => $v) {
			$nama_pendapatan .= "<option value=\"{$k}\">{$v->nama_pendapatan}</option>";
		}
							...
							<table>
								<div class=\"row\">
									...
									<div class=\" form-group col-md-3\"> 
										...
										<select class=\"form-control\" name=\"JENIS_PENERIMAAN-{$id}\" id=\"JENIS_PENERIMAAN-{$id}\">{$jenis_penerimaan}</select>
									</div>
									<div class=\" form-group col-md-3\"> 
										<label>Nama Jenis Retribusi</label>
										<select class=\"form-control\" name=\"JENIS_RETRIBUSI-{$id}\" id=\"JENIS_RETRIBUSI-{$id}\">{$nama_pendapatan}</select>
									</div>
				
									<div class=\" form-group col-md-3\"> 
										<label>Bulan</label>
										<select class=\"form-control\" name=\"BULAN-{$id}\" id=\"BULAN-{$id}\">{$opt_bulan}</select>
									</div>

									<div class=\" form-group col-md-3\"> 
										<label>Tahun</label>
										<select class=\"form-control\" name=\"TAHUN-{$id}\" id=\"TAHUN-{$id}\">{$opt_tahun}</select>
									</div>
									...
									<div class=\" form-group col-md-12\"> 
										...
										<button type=\"button\" class=\"btn btn-success\" onclick=\"javascript:download_excel('{$id}','function/PATDA-V1/svc-download-retribusi.xls.php')\"><i class=\"fa fa-download\"></i> Export to xls</button>  
									</div>
								</div>
							</table>
						</form>
                    ...
                    $(document).ready(function() {
                        ...
                        $('#laporanPajak-{$this->_i}').jtable({
                            ...
                            fields: {
                                ...
                                jenis_penerimaan: {title: 'Jenis Penerimaan',width: '10%'},
                                nama_pendapatan: {title: 'Jenis Retribusi',width: '10%'},
                                rekening: {title: 'Rekening',width: '10%'},
                                anggaran: {title: 'Anggaran',width: '10%'},
                                target: {title: 'Target',width: '10%'},
                                bulan: {title: 'Bulan',width: '10%'},
                                tahun: {title: 'Tahun',width: '10%'},
                                jumlah_realisasi: {title: 'Jumlah Realisasi',width: '10%'}
                        ...
                        $('#cari-{$this->_i}').click(function (e) {
                            e.preventDefault();
                            $('#laporanPajak-{$this->_i}').jtable('load', {
                                JENIS_RETRIBUSI : $('#JENIS_RETRIBUSI-{$this->_i}').val(),
                                JENIS_PENERIMAAN : $('#JENIS_PENERIMAAN-{$this->_i}').val(),
                                BULAN : $('#BULAN-{$this->_i}').val(),
	...
	public function grid_data_pelayanan()
	{
		...
		try {
			...
			#count utk pagging
			$query = "SELECT COUNT(DISTINCT doc.CPM_ID) AS RecordCount FROM patda_retribusi_doc doc
					INNER JOIN rekening_retribusi rek ON doc.jenis_retribusi_id = rek.id
					INNER JOIN jenis_penerimaan_retribusi jen ON doc.jenis_penerimaan_id = jen.id
                    WHERE {$where} ";
			...
			#query select list data
			$query = "SELECT doc.CPM_ID,doc.rekening,doc.jumlah_realisasi,doc.bulan,doc.tahun,doc.target,doc.anggaran,rek.nama_pendapatan,jen.jenis_penerimaan    
                      FROM patda_retribusi_doc doc
					  INNER JOIN rekening_retribusi rek ON doc.jenis_retribusi_id = rek.id
					  INNER JOIN jenis_penerimaan_retribusi jen ON doc.jenis_penerimaan_id = jen.id
                      WHERE {$where}
                      ORDER BY doc.CPM_ID DESC {$_GET["jtSorting"]} LIMIT {$_GET["jtStartIndex"]},{$_GET["jtPageSize"]}";
								// var_dump($query);
								// die;
			$result = mysqli_query($this->Conn, $query);
			...
			while ($row = mysqli_fetch_assoc($result)) {
				...
				$row['bulan'] = $this->arr_bulann[$row['bulan']];
				$row['anggaran'] = number_format($row['anggaran'], 2);
				$row['target'] = number_format($row['target'], 2);
				$row['jumlah_realisasi'] = number_format($row['jumlah_realisasi'], 2);
				// $row['CPM_NPWPD'] = Pajak::formatNPWPD($row['CPM_NPWPD']);
	...
	private function validasi_pajak($input = 1)
	{
		...
		$query = "SELECT * FROM patda_retribusi_doc WHERE 
				  bulan = '{$_POST['bulan']}' AND 
				  tahun = '{$_POST['tahun']}' AND
				  jenis_penerimaan_id = {$_POST['jenis_penerimaan_id']} AND
				  jenis_retribusi_id = {$_POST['jenis_retribusi']}";
		...
		if ($this->notif == true) {
			if (mysqli_num_rows($res)) {	
				...
				$pesan = "<span style='color: red;'>Gagal disimpan, Data realisasi bulan {$this->arr_bulann[$_POST['bulan']]} Tahun {$_POST['tahun']} sudah di laporkan sebelumnya!</span>";
			
				$this->Message->setMessage($pesan);
			} 
		}
...
// var_dump($respon['result']);die;
	...
	public function save_final()
	{
		...
		if ($this->save_pajak($this->CPM_NO)) {
			...
			if ($res) {
				$_SESSION['_success'] = 'Data retribusi berhasil diinput';
				// header("Location: ../../../../{$urlberkas}");
				// exit();
			} else {
				$_SESSION['_error'] = 'Data Pajak gagal diinput';
			}
		}
	}
						...
						<td colspan=\"2\" align=\"center\" style=\"font-size:24px;\">
							<table width=\"100%\" border=\"0\" align=\"left\" cellpadding=\"10\">
								<tr>
									...
									<td align=\"center\"><b>
										Pesawaran, " . $tgl[0] . " {$this->arr_bulan[(int)$tgl[1]]} " . $tgl[2] . " {$tgl_approve} <br/>
		...
		if ($x == 'false') {
			...
			// print_r($NSL);exit;
				...
				if ($_POST['rumus'] == 'RMS1' || $rms == 'RMS1' ) {
					...
					$hitung_pajak = "<br><b>Nilai Strategis NSPR x NJOPR ya</b><br>
					= NSPR x NJOPR <br>
					...
					$hitung_pajak = "<br><b>Nilai Strategis  NSPR x NJOPR ga</b><br>
						= " . number_format($tarif) . "% x " . number_format($subTotal_nsr) . "<br>
						= Rp. " .  number_format($total_terpasang, 2, '.', ',');
...
// var_dump($_POST['rumus']);die;
	...
	private function save_pajak($cpm_no = '')
	{
		...
		// var_dump($_POST['jenis_penerimaan']);die;
		// die;
...
// var_dump($validasi);die;
		if ($validasi['result'] == true || ($validasi['data']['CPM_TRAN_STATUS'] == '4')) {
			...
			$query = sprintf(
				"INSERT INTO patda_retribusi_doc
                    (jenis_penerimaan_id, jenis_retribusi_id, rekening, anggaran, target, bulan, jumlah_realisasi,tahun)
                    VALUES ( '%s','%s','%s',%f,%f,'%s','%s','%s')",
				$_POST['jenis_penerimaan'],
				$_POST['jenis_retribusi'],
				$_POST['rekening'],
				$_POST['anggaran'],
				$_POST['target'],
				$_POST['bulan'],
				$_POST['jumlah_realisasi'],
				$_POST['tahun']
