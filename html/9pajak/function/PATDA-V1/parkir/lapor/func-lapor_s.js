$(document).ready(function () {
    ...
    // setTimeout(function(){
    //     var value_lokasi = $(".CPM_LOKASI").val();
    //     console.log(value_lokasi);
    //     if (value_lokasi) {tarif_pajak_parkit(value_lokasi);}
    // },3000)
    ...
    form.validate({
        ...
        messages: {
            ...
            "PAJAK[CPM_GOL_PARKIR]": "harus diisi",
    ...
    $("#CPM_NPWPD").keyup(function () {
        ...
        $("#CPM_NAMA_OP").val('');
        $("#CPM_ALAMAT_OP").val('');
        $("#CPM_GOL_PARKIR option").prop('selected', false).attr('disabled', 'disabled');
    ...
    $("#btn-search-npwpd").click(function () {
        ...
        $.ajax({
            type: "POST",
            data: "PAJAK[CPM_NPWPD]=" + npwpd + "&function=search_npwpd&PAJAK[CPM_JENIS_PAJAK]=5",
            url: "function/PATDA-V1/parkir/lapor/svc-lapor.php",
            dataType: "json",
            success: function (res) {
                ...
                if (res.result == 1) {
                    ...
                    $("#CPM_GOL_PARKIR option[value='" + res.CPM_GOL_PARKIR + "']").prop('selected', true).removeAttr('disabled');
                    // $('#CPM_TARIF_PAJAK').val($("#CPM_GOL_PARKIR option[value='" + res.CPM_GOL_PARKIR + "']").attr('tarif'));
                    function_getval_tapbox();
                } else {
    ...
    $('.CPM_LOKASI').change(function () {
        var value_lokasi = $(this).val();
        tarif_pajak_parkit(value_lokasi);
    });

    function tarif_pajak_parkit(param) {
        if (param == 0) {
            $('#CPM_TARIF_PAJAK').val(10);
            function_sum();
        } else if (param == 1) {
            $('#CPM_TARIF_PAJAK').val(30);
            function_sum();
        }
    }
...
function getDetTranTapbox(q, json) {
    ...
    $.ajax({
        data: "i=6&p=" + json + "&q=" + q,
        type: "post",
        url: "view/PATDA-V1/parkir/svc-list-pembanding-detail.php",
