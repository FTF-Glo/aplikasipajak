...
class LaporPajak extends Pajak
{
    ...
    {
        $Op = new ObjekPajak();
        $arr_rekening = $this->getRekening("4.1.01.11");
        $list_nop = array();

        $query = "
			SELECT DOC.*, DATE_FORMAT(DOC.CPM_TGL_JATUH_TEMPO,'%d-%m-%Y') as CPM_TGL_JATUH_TEMPO
			FROM PATDA_PARKIR_DOC AS DOC WHERE DOC.CPM_ID = '{$this->_id}' LIMIT 0,1";
    ...
    public function filtering($id)
    {
		...
		// 				}
        //                 $html .= "<td>Pilih Jenis :<br><select style=\"width:85px; height:30px;\" class=\"form-control\" name=\"CPM_PIUTANG-{$id}\" id=\"CPM_PIUTANG-{$id}\">{$opt_pilih}</select></td>
        //                 <td bgcolor=\"#ffff00\">
		...
		// 					<button type=\"button\"  style=\"width:150px; height:30px;\" class=\"btn btn-sm btn-secondary\" onclick=\"javascript:download_excel('{$id}','function/PATDA-V1/cetakpanjang/svc-download-bentang-panjang.xls.php')\">Cetak Bentang Panjang</button>            
        //                 </td>
        //             </tr></table></form>
        //         </div> ";
                            ...
                            <form>
                                    ...
                                    $html .=  "<div class=\" form-group col-md-3\"> 
                                        ...
                                        <div class=\" form-group col-md-12\">    
                                            ...
                                            <button type=\"button\"  class=\"btn btn-success\" onclick=\"javascript:download_excel('{$id}','function/PATDA-V1/cetakpanjang/svc-download-bentang-panjang.xls.php')\"><i class=\"fa fa-print\"></i> Cetak Bentang Panjang</button>            
                                        </div>
                                    </div>
                            </form>
                        </div>
    ...
    public function grid_data()
    {
        try {
            ...
            #count utk pagging
            $query = "SELECT COUNT(*) AS RecordCount FROM {$this->PATDA_PARKIR_DOC} pj
                            INNER JOIN {$this->PATDA_PARKIR_PROFIL} pr ON pj.CPM_ID_PROFIL = pr.CPM_ID
                            INNER JOIN {$this->PATDA_PARKIR_DOC_TRANMAIN} tr ON pj.CPM_ID = tr.CPM_TRAN_PARKIR_ID
                            WHERE {$where}";
            $result = mysqli_query($this->Conn, $query);
            ...
            $query = "SELECT pj.CPM_ID, pj.CPM_NO, pj.CPM_TAHUN_PAJAK, pr.CPM_NAMA_OP,
                            ...
                            tr.CPM_TRAN_READ, tr.CPM_TRAN_ID, pj.CPM_PIUTANG,gw.payment_flag
                            FROM {$this->PATDA_PARKIR_DOC} pj INNER JOIN {$this->PATDA_PARKIR_PROFIL} pr ON pj.CPM_ID_PROFIL = pr.CPM_ID
                            INNER JOIN {$this->PATDA_PARKIR_DOC_TRANMAIN} tr ON pj.CPM_ID = tr.CPM_TRAN_PARKIR_ID
                            INNER JOIN simpatda_gw gw ON pj.CPM_ID = gw.id_switching
                            WHERE {$where}
    ...
    public function grid_data_pembanding()
    {
        try {
            ...
            #count utk pagging
            $query = "SELECT CPM_DEVICE_ID, CPM_NPWPD from PATDA_PARKIR_PROFIL WHERE CPM_AKTIF='1' {$where}";
            $result = mysqli_query($this->Conn, $query);
            $dataAll = array();
            $x = 0;
            while ($data = mysqli_fetch_assoc($result)) {
    ...
    {
        $query = "SELECT * FROM {$this->PATDA_PARKIR_DOC_TRANMAIN} WHERE CPM_TRAN_PARKIR_ID='{$this->CPM_ID}' AND CPM_TRAN_FLAG='0'";
        $res = mysqli_query($this->Conn, $query);
        $data = mysqli_fetch_assoc($res);

        return $data['CPM_TRAN_PARKIR_VERSION'];
    }
    ...
    private function save_pajak($cpm_no = '')
    {
        ...
        if ($validasi['result'] == true || ($validasi['data']['CPM_TRAN_STATUS'] == '4')) {
            ...
            #update profil baru
            $query = "UPDATE {$this->PATDA_PARKIR_PROFIL} SET CPM_APPROVE ='1' WHERE CPM_NPWPD = '{$this->CPM_NPWPD}' AND CPM_AKTIF='1'";
            ...
            $arr_config = $this->get_config_value($this->_a);
            $query = sprintf(
                "INSERT INTO {$this->PATDA_PARKIR_DOC}
                    ...
                    CPM_MASA_PAJAK1,CPM_MASA_PAJAK2,CPM_TIPE_PAJAK,CPM_DENDA_TERLAMBAT_LAP,CPM_LOKASI, CPM_PIUTANG)
                    VALUES ( '%s','%s','%s',
                             '%s','%s','%s',
                             '%s','%s','%s',
                             '%s','%s','%s',
    ...
    private function save_tranmain($param)
    {
        #insert tranmain
        $CPM_TRAN_ID = c_uuid();
        $CPM_TRAN_PARKIR_ID = $this->CPM_ID;

        $query = "UPDATE {$this->PATDA_PARKIR_DOC_TRANMAIN} SET CPM_TRAN_FLAG = '1' WHERE CPM_TRAN_PARKIR_ID = '{$CPM_TRAN_PARKIR_ID}'";
        $res = mysqli_query($this->Conn, $query);

        $query = sprintf(
            "INSERT INTO {$this->PATDA_PARKIR_DOC_TRANMAIN}
                    (CPM_TRAN_ID, CPM_TRAN_PARKIR_ID, CPM_TRAN_PARKIR_VERSION, CPM_TRAN_STATUS, CPM_TRAN_FLAG, CPM_TRAN_DATE,
                    CPM_TRAN_OPR, CPM_TRAN_OPR_DISPENDA,CPM_TRAN_INFO)
                    VALUES ( '%s','%s','%s','%s','%s',
                             '%s','%s','%s','%s')",
            $CPM_TRAN_ID,
            $CPM_TRAN_PARKIR_ID,
            $param['CPM_TRAN_PARKIR_VERSION'],
    ...
    private function update_tgl_input()
    {
        ...
        $query = "UPDATE {$this->PATDA_PARKIR_DOC} SET CPM_TGL_INPUT = '{$tgl_input}'
    ...
    private function update_tgl_lapor()
    {
        ...
        $query = "UPDATE {$this->PATDA_PARKIR_DOC} SET CPM_TGL_LAPOR = '{$tgl_input}'
                  WHERE CPM_ID ='{$this->CPM_ID}'";
    ...
    private function update_tgl_lapor_ditolak($cpm_no, $tgl_lapor, $tgl_input)
    {
        ...
        if ($tgl_input == 'NULL') {
            $query = "UPDATE {$this->PATDA_PARKIR_DOC} SET CPM_TGL_LAPOR = '{$tgl_lapor}'
                  WHERE CPM_NO ='{$cpm_no}'";
        } else {
            $query = "UPDATE {$this->PATDA_PARKIR_DOC} SET CPM_TGL_LAPOR = '{$tgl_lapor}', CPM_TGL_INPUT = '{$tgl_input}'
                  WHERE CPM_NO ='{$cpm_no}'";
        }
    ...
    public function save()
    {
        ...
        if ($this->save_pajak($this->CPM_NO)) {
            $param = array();
            $param['CPM_TRAN_PARKIR_VERSION'] = "1";
    ...
    public function new_version()
    {
        ...
        if ($this->save_pajak($this->CPM_NO)) {
            ...
            $query = "UPDATE {$this->PATDA_PARKIR_DOC_TRANMAIN} SET CPM_TRAN_FLAG ='1' WHERE CPM_TRAN_PARKIR_ID='{$id}'";
            mysqli_query($this->Conn, $query);

            $param['CPM_TRAN_PARKIR_VERSION'] = $new_version;
            $param['CPM_TRAN_STATUS'] = "1";
            $param['CPM_TRAN_FLAG'] = "0";
            $param['CPM_TRAN_DATE'] = date("d-m-Y");
            $param['CPM_TRAN_OPR'] = $this->CPM_AUTHOR;
    ...
    public function update_final()
    {
        ...
        if ($this->update()) {
            $param['CPM_TRAN_PARKIR_VERSION'] = $this->CPM_VERSION;
            $param['CPM_TRAN_STATUS'] = "2";
            $param['CPM_TRAN_FLAG'] = "0";
            $param['CPM_TRAN_DATE'] = date("d-m-Y");
            $param['CPM_TRAN_OPR'] = $this->CPM_AUTHOR;
    ...
    public function update()
    {
        ...
        if ($validasi['result'] == true) {
            ...
            $query = sprintf(
                "UPDATE {$this->PATDA_PARKIR_DOC} SET
                    CPM_TOTAL_OMZET = '%s',
                    CPM_TOTAL_PAJAK = '%s',
                    CPM_TARIF_PAJAK = '%s',
                    CPM_BAYAR_LAINNYA = '%s',
    ...
    {
        $query = "DELETE FROM {$this->PATDA_PARKIR_DOC} WHERE CPM_ID ='{$this->CPM_ID}'";
        $res = mysqli_query($this->Conn, $query);
        if ($res) {
            $query = "DELETE FROM PATDA_PARKIR_DOC_TRANMAIN WHERE CPM_TRAN_PARKIR_ID ='{$this->CPM_ID}'";
            mysqli_query($this->Conn, $query);
        }
    }
    ...
    // {
    //     $query = "UPDATE {$this->PATDA_PARKIR_DOC} SET CPM_TGL_JATUH_TEMPO = {$expired_date}
    //               WHERE CPM_ID ='{$this->CPM_ID}'";
    //     return mysqli_query($this->Conn, $query);
    // }
    ...
    public function print_sptpd_base()
    {
        ...
        $BAG_VERIFIKASI_NIP = $config['BAG_VERIFIKASI_NIP'];

        $html = "<table width=\"710\" class=\"main\" border=\"1\" cellpadding=\"5\">
                    <tr>
                        <td colspan=\"2\" width=\"480\"><table width=\"470\" border=\"0\">
                                <tr class=\"first\">
                                    <th valign=\"top\" align=\"center\" class=\"headerplk parkir headertext\" width=\"470\">
                                    <font class=\"normal\">" . strtoupper($JENIS_PEMERINTAHAN) . " " . strtoupper($NAMA_PEMERINTAHAN) . "</font><br />
                                        <b>" . strtoupper($NAMA_PENGELOLA) . "</b><br /><br />
                        ...
                        <td width=\"230\"><table border=\"0\" cellspacing=\"0\" cellpadding=\"0\">
                                <tr>
                                    <th width=\"80\"> NO.SPTPD </th><th width=\"140\">: {$DATA['pajak']['CPM_NO']}</th>
                                </tr>
                                <tr>
                                    <th width=\"80\"> NPWPD </th><th>: " . Pajak::formatNPWPD($DATA['profil']['CPM_NPWPD']) . "</th>
                                </tr>
                            </table>
                        </td>
                    </tr>
                    <tr>
                        <td colspan=\"3\"><table width=\"700\" class=\"header\" border=\"0\">
                                <tr class=\"first\">
                                ...
                                <tr>
                                    ...
                                    <td width=\"335\">: {$DATA['profil']['CPM_NAMA_WP']}</td>
                                    <td width=\"90\">Masa Pajak</td>
                    ...
                    <tr>
                        <td colspan=\"3\" align=\"center\"><table width=\"100%\" border=\"0\" align=\"left\">
                                <tr>
                                    <td colspan=\"2\"></td>
                                </tr>
                                <tr>
                                    <td width=\"150\">&nbsp;&nbsp;Nama Objek Pajak</td>
                                    <td width=\"550\">: {$DATA['profil']['CPM_NAMA_OP']}</td>
                                </tr>
                                <tr>
                    ...
                    <tr>
                        <td width=\"710\" colspan=\"2\"><table width=\"700\" align=\"center\" cellpadding=\"1\" border=\"1\" cellspacing=\"0\">
                                ...
                                <tr>
                                    <td align=\"left\" width=\"270\">&nbsp;&nbsp;&nbsp;b. Pembayaran Pemakaian</td>
                                ...
                                <tr>
                                    <td align=\"left\" width=\"270\">&nbsp;&nbsp;&nbsp;g. Sanksi Administrasi Telat Lapor ({$persen_terlambat_lap}%)</td>
                                    <td width=\"30\" align=\"left\">Rp.</td>
                    ...
                    <tr>
                        <td>&nbsp;&nbsp;<b>PERHATIAN :</b><ol><li>Apabila Setelah 7 (tujuh) hari Surat Pemberitahuan Pajak Daerah (SPTPD) diterima oleh Wajib Pajak, dan tidak ada keberatan maka dianggap wajib pajak telah menyetujui besarnya nilai pajak yang terutang.</li></ol>
                        </td>
                        <td align=\"center\">Tanggal " . date("d") . " {$this->arr_bulan[(int) date("m")]} " . date("Y") . "<br/>Diterima Oleh<br/><br/><br/><br/><br/><br/><u>{$BAG_VERIFIKASI_NAMA}</u><br/>NIP.{$BAG_VERIFIKASI_NIP}</td>
                        <td align=\"center\">{$KOTA}," . str_pad("", 25, " .", STR_PAD_RIGHT) . " <br/>Kepala Dinas Pendapatan Daerah<br/>Kota {$KOTA}<br/><br/><br/><br/><br/><u>{$KEPALA_DINAS}</u><br/>NIP.{$NIP}</td>
                    </tr>
                </table>";
        ...
        $pdf->Image("{$sRootPath}view/Registrasi/configure/logo/{$LOGO_CETAK_PDF}", 13, 18, 25, '', '', '', '', false, 300, '', false);
        $pdf->SetAlpha(0.3);

        $pdf->Output('sptpd-parkir.pdf', 'I');
    }
    ...
    public function print_sptpd()
    {
                        ...
                        <td colspan=\"2\"><table width=\"710\" border=\"1\" cellpadding=\"3\">
                                <tr>
                                    ...
                                    <td width=\"310\" valign=\"top\" align=\"center\">
                                        ...
                                        <b style=\"font-size:55px\">PAJAK PARKIR</b><br/>
					...
					<tr style=\"font-size:32px\">
                        <td width=\"710\" colspan=\"2\" align=\"center\"><table width=\"700\" align=\"center\" cellpadding=\"1\" border=\"0\" cellspacing=\"0\">
                                <tr>
                                    ...
                                    <td align=\"left\" width=\"270\">Golongan Parkir</td>
        ...
        $pdf->Image("{$sRootPath}view/Registrasi/configure/logo/{$LOGO_CETAK_PDF}", 27, 10, 8, '', '', '', '', false, 300, '', false);
    ...
    public function read_dokumen()
    {
        if (isset($_REQUEST['read']) && $_REQUEST['read'] == 0) {
            ...
            $select = "SELECT CPM_TRAN_READ FROM {$this->PATDA_PARKIR_DOC_TRANMAIN} WHERE CPM_TRAN_ID='{$idtran}'";
            $result = mysqli_query($this->Conn, $select);
            $data = mysqli_fetch_assoc($result);

            $read = $data['CPM_TRAN_READ'];
            $read = (trim($read) == "") ? ";{$_SESSION['uname']};" : "{$read};{$_SESSION['uname']};";
            $query = "UPDATE {$this->PATDA_PARKIR_DOC_TRANMAIN} SET CPM_TRAN_READ = '{$read}' WHERE CPM_TRAN_ID='{$idtran}'";
            mysqli_query($this->Conn, $query);
        }
    }
    ...
    public function grid_data_pembanding_detail()
    {
        try {
            ...
            $query = "select
                        ...
                        from {$dbTable}
                        WHERE {$where} ";
            $result = mysqli_query($this->Conn, $Conn_gw, $query) or die(mysqli_error($this->Conn));
    ...
    public function get_previous_pajak($npwpd, $nop)
    {
        ...
        $query = "
			SELECT DOC.*, DATE_FORMAT(DOC.CPM_TGL_JATUH_TEMPO,'%d-%m-%Y') as CPM_TGL_JATUH_TEMPO
			FROM PATDA_PARKIR_DOC AS DOC
			INNER JOIN PATDA_PARKIR_PROFIL PR ON DOC.CPM_ID_PROFIL = PR.CPM_ID
