...
class ProfilPajak extends Pajak{
    ...
    public function update() {
        ...
        $query = sprintf("UPDATE PATDA_PARKIR_PROFIL SET
                    ...
                    ", $this->CPM_NPWPD, $this->CPM_NAMA_WP, $this->CPM_ALAMAT_WP, $this->CPM_NAMA_OP, $this->CPM_ALAMAT_OP, $this->CPM_KELURAHAN_OP, $this->CPM_KECAMATAN_OP, $this->CPM_KELURAHAN_WP, $this->CPM_KECAMATAN_WP, $this->CPM_AUTHOR, $this->CPM_NOP,$this->CPM_GOL_PARKIR);                    
    ...
    public function save() {
        $query = "SELECT * FROM PATDA_PARKIR_PROFIL WHERE CPM_NPWPD = '{$this->CPM_NPWPD}' ORDER BY CPM_TGL_UPDATE DESC LIMIT 0,1";
        ...
        $query = "UPDATE PATDA_PARKIR_PROFIL SET CPM_AKTIF ='0' WHERE CPM_NPWPD = '{$this->CPM_NPWPD}'";
        $res = mysql_query($query, $this->Conn);  
        ...
        $query = sprintf("INSERT INTO PATDA_PARKIR_PROFIL 
                    ...
                    CPM_AKTIF, CPM_APPROVE, CPM_DEVICE_ID,
                    CPM_AUTHOR, CPM_NOP,CPM_GOL_PARKIR)
                    VALUES ( '%s','%s','%s',
                             '%s','%s','%s',
                             '%s','%s',
                             '%s','%s',
                             '%s','%s','%s',
                             '%s','%s','%s')", 
                $this->CPM_ID, $this->CPM_NPWPD, $this->CPM_NAMA_WP, $this->CPM_ALAMAT_WP, $this->CPM_NAMA_OP, $this->CPM_ALAMAT_OP, $this->CPM_KELURAHAN_OP, $this->CPM_KECAMATAN_OP, $this->CPM_KELURAHAN_WP, $this->CPM_KECAMATAN_WP, $this->CPM_AKTIF, $this->CPM_APPROVE, $this->CPM_DEVICE_ID,$this->CPM_AUTHOR, $this->CPM_NOP,$this->CPM_GOL_PARKIR
        );
    ...
    public function update_tapbox() {
        $query = "UPDATE PATDA_PARKIR_PROFIL SET CPM_DEVICE_ID ='{$this->CPM_DEVICE_ID}' WHERE CPM_NPWPD = '{$this->CPM_NPWPD}' AND CPM_ID='{$this->CPM_ID}'";
    ...
    public function get_last_profil($user) {
        ...
        $query = "SELECT 
			...
			FROM PATDA_WP WP 
				LEFT JOIN PATDA_PARKIR_PROFIL P ON P.CPM_NPWPD = WP.CPM_NPWPD AND P.CPM_AKTIF='1' 
    ...
    public function getRekening(){
        return parent::getRekening("4.1.1.7");
    }
    ...
    public function rollback() {
        $query = "DELETE FROM PATDA_PARKIR_PROFIL WHERE CPM_ID='{$this->CPM_ID}'";
        mysql_query($query, $this->Conn);
        ...
        $query = "UPDATE PATDA_PARKIR_PROFIL SET CPM_AKTIF='1' WHERE CPM_ID='{$data['CPM_ID']}'";
        $result = mysql_query($query, $this->Conn);
    }
