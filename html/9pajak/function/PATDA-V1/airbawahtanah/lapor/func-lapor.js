$(document).ready(function () {
    ...
    $('input[type="checkbox"]#HITUNG_PENGURANGAN').on('change', function () {
        var v = $(this);
        var pengurangan = $('#CPM_TARIF_PENGURANGAN');
        if (v.prop('checked')) {
    ...
    $('#CPM_TARIF_PENGURANGAN').autoNumeric('init');
    ...
    $('#CPM_TAHUN_PAJAK').change(function () {
...
function function_sum(obj) {
    ...
    //                 var sanksi = terutang * (selisih_bulan - 1) * 1 / 100;
    ...
    //         if (selisih_bulan2 > 1) {
    //             if ($('#editable_terlambat_lap').val() == 1) {
    //                 var sanksi = terutang * (selisih_bulan2 - 1) * 1 / 100;
    //                 var sanksi = $('#CPM_DENDA_TERLAMBAT_LAP').autoNumeric('get');
    //             } else {
    //                 var sanksi = terutang * (selisih_bulan2 - 1) * 1 / 100;
    //             }
    ...
    // var maksimumSanksi = terutang * 48 / 100;
    // if (sanksi > maksimumSanksi) {
    //     sanksi = maksimumSanksi;
    // }
    ...
    // aktifkan kalo ada pengurangannya
    // if (pengurangan > 0) {
    //     total = total - (total * pengurangan);
    // }

    total = Math.ceil(total);

    // var total = ((eval(terutang) + eval(kurangLebih)) * pengurangan) + eval(sanksi);
    // total = Math.ceil(total);
...
function hitungNPA(id) {

    $.ajax({
        type: "POST",
        data: { function: "hitung_npa", CPM_PERUNTUKAN: $('#CPM_PERUNTUKAN').val(), CPM_VOLUME: $('#CPM_ATR_VOLUME-' + id).val() },
        url: "function/PATDA-V1/airbawahtanah/lapor/svc-lapor.php",
        dataType: 'json',
        success: function (res) {
            $('#CPM_ATR_TOTAL-' + id).val(res.total);
            $('#CPM_ATR_PERHITUNGAN-' + id).val(res.html);
            $('#tabel_perolehan-' + id).html(res.html);
            var total = eval($('#CPM_ATR_TOTAL-1').val()) + eval($('#CPM_ATR_TOTAL-2').val() || 0) + eval($('#CPM_ATR_TOTAL-3').val() || 0);
            $('#CPM_TOTAL_OMZET').autoNumeric('set', total);
            $('#NPA_DETAIL').html(JSON.stringify(res.NPA));
