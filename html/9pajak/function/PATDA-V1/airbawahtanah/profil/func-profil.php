...
$value = $profil->get_last_profil(isset($_REQUEST['id'])? $_REQUEST['id'] : $data->uname);
...
<form class="cmxform" id="form-profil" method="post" action="function/<?php echo "{$DIR}/{$modul}"; ?>/svc-profil.php?param=<?php echo base64_encode($json->encode(array("a" => $profil->_a, "m" => $profil->_m))) ?>">
    ...
    <table class="main" width="900">        
        ...
        <tr>
            ...
            <td>: <input type="text" name="PROFIL[CPM_NAMA_WP]" id="CPM_NAMA_WP" style="width: 200px;" value="<?php echo $value['CPM_NAMA_WP'] ?>" readonly></td>
        </tr>
        <tr valign="top">
            <td>Alamat Wajib Pajak <b class="isi">*</b></td>
            <td>: <textarea name="PROFIL[CPM_ALAMAT_WP]" id="CPM_ALAMAT_WP" cols="80" rows="3" readonly><?php echo $value['CPM_ALAMAT_WP'] ?></textarea></td>
        </tr>
		<tr>
            <td>Kecamatan Wajib Pajak <b class="isi">*</b></td>
            <td>: <input type="text" name="PROFIL[CPM_KECAMATAN_WP]" id="CPM_KECAMATAN_WP" style="width: 200px;" value="<?php echo $value['CPM_KECAMATAN_WP'] ?>" readonly>
            </td>
        </tr>
        <tr>
            <td>Kelurahan Wajib Pajak <b class="isi">*</b></td>
            <td>: <input type="text" name="PROFIL[CPM_KELURAHAN_WP]" id="CPM_KELURAHAN_WP" style="width: 200px;" value="<?php echo $value['CPM_KELURAHAN_WP'] ?>" readonly>
            </td>
        </tr>		
        <tr>
       ...
       <tr>
        ...
        <tr>
            <td>Kelurahan Air Bawah Tanah <b class="isi">*</b></td>
            <td>: 
        ...
        <tr valign="top">
            <td>Golongan Air Bawah Tanah <b class="isi">*</b></td>
            <td>: <select name="PROFIL[CPM_GOL_AIRBAWAHTANAH]" id="CPM_GOL_AIRBAWAHTANAH">
                    <?php
                    foreach ($value['CPM_REKENING'] as $gol) {
                        echo ($value['CPM_GOL_AIRBAWAHTANAH'] == $gol['kdrek'])? "<option value='{$gol['kdrek']}' selected>{$gol['kdrek']} - {$gol['nmrek']}</option>" : "<option value='{$gol['kdrek']}'>{$gol['kdrek']} - {$gol['nmrek']}</option>";
                    }
                    ?>
                </select>
            </td>
