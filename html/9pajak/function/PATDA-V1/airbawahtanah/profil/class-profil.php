...
class ProfilPajak extends Pajak {
    ...
    public function update() {
        #update profil
        $query = sprintf("UPDATE PATDA_AIRBAWAHTANAH_PROFIL SET
                    CPM_NPWPD = '%s',
                    CPM_NAMA_WP = '%s',
                    CPM_ALAMAT_WP = '%s',
                    CPM_KELURAHAN_WP = '%s',
                    ...
                    CPM_AUTHOR = '%s',
                    CPM_NOP = '%s',
                    CPM_GOL_AIRBAWAHTANAH = '%s'
                    WHERE CPM_ID = '{$this->CPM_ID}'
                    ", $this->CPM_NPWPD, $this->CPM_NAMA_WP, $this->CPM_ALAMAT_WP, $this->CPM_KELURAHAN_WP, $this->CPM_KECAMATAN_WP, $this->CPM_NAMA_OP, $this->CPM_ALAMAT_OP, $this->CPM_KELURAHAN_OP, $this->CPM_KECAMATAN_OP, $this->CPM_AUTHOR, $this->CPM_NOP, $this->CPM_GOL_AIRBAWAHTANAH);
    ...
    public function save() {
		$query = "SELECT * FROM PATDA_AIRBAWAHTANAH_PROFIL WHERE CPM_NPWPD = '{$this->CPM_NPWPD}' ORDER BY CPM_TGL_UPDATE DESC LIMIT 0,1";
        ...
        $query = "UPDATE PATDA_AIRBAWAHTANAH_PROFIL SET CPM_AKTIF ='0' WHERE CPM_NPWPD = '{$this->CPM_NPWPD}'";
        $res = mysqli_query($this->Conn, $query);

        #insert profil baru
        $query = sprintf("INSERT INTO PATDA_AIRBAWAHTANAH_PROFIL 
					...
					CPM_AKTIF, CPM_GOL_AIRBAWAHTANAH,
                    ...
                    VALUES ( '%s','%s','%s',
                             ...
                             '%s','%s','%s')",
                              ...
                              $this->CPM_KECAMATAN_OP,$this->CPM_AKTIF, $this->CPM_GOL_AIRBAWAHTANAH, $this->CPM_APPROVE, 
                              $this->CPM_AUTHOR, $this->CPM_NOP
        );
    ...
    public function get_last_profil($user) {
        ...
        $query = "SELECT 
			...
			FROM PATDA_WP WP 
				LEFT JOIN PATDA_AIRBAWAHTANAH_PROFIL P ON P.CPM_NPWPD = WP.CPM_NPWPD AND P.CPM_AKTIF='1' 
			...
			WHERE WP.CPM_USER = '{$user}' OR WP.CPM_NPWPD = '{$user}'";
    ...
    public function getRekening() {
        return parent::getRekening("4.1.1.8");
    }
    ...
    public function rollback() {
        $query = "DELETE FROM PATDA_AIRBAWAHTANAH_PROFIL WHERE CPM_ID='{$this->CPM_ID}'";
        ...
        $query = "UPDATE PATDA_AIRBAWAHTANAH_PROFIL SET CPM_AKTIF='1' WHERE CPM_ID='{$data['CPM_ID']}'";
        $result = mysqli_query($this->Conn, $query);
    }
	
	public function cutWords($string, $limit=100, $break=" ", $pad="..."){
