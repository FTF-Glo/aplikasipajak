...
<form class="cmxform" id="form-profil" method="post" action="function/<?php echo "{$DIR}/{$modul}"; ?>/svc-profil.php?param=<?php echo base64_encode($json->encode(array("a" => $profil->_a, "m" => $profil->_m))) ?>">
    ...
    <table class="main" width="900">        
        ...
        <tr valign="top">
            <td>Golongan Walet <b class="isi">*</b></td>
            <td>: <select name="PROFIL[CPM_GOL_WALET]" id="CPM_GOL_WALET">
                    <?php
                    foreach ($value['CPM_REKENING'] as $gol) {
                        echo ($value['CPM_GOL_WALET'] == $gol['kdrek'])? "<option value='{$gol['kdrek']}' selected>{$gol['kdrek']} - {$gol['nmrek']}</option>" : "<option value='{$gol['kdrek']}'>{$gol['kdrek']} - {$gol['nmrek']}</option>";
                    }
                    ?>
                </select>
            </td>
