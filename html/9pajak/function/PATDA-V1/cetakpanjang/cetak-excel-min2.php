...
class Cetak extends Pajak
{
  ...
  public function download_pajak_xls_atb_su()
  {
    ...
    $query2 = "SELECT
          ...
          pr.CPM_KECAMATAN_OP,
          (
    SELECT SUM(atr_inner.CPM_ATR_VOLUME)
    FROM PATDA_AIRBAWAHTANAH_DOC_ATR atr_inner
    JOIN PATDA_AIRBAWAHTANAH_DOC pj_inner ON atr_inner.CPM_ATR_AIRBAWAHTANAH_ID = pj_inner.CPM_ID
    WHERE  pj_inner.CPM_MASA_PAJAK = atr.CPM_ATR_BULAN AND
   atr_inner.CPM_ATR_AIRBAWAHTANAH_ID = pj.CPM_ID 
   GROUP BY atr_inner.CPM_ATR_AIRBAWAHTANAH_ID
  ) as CPM_VOLUME
    ...
    while ($row = mysqli_fetch_assoc($res)) {
      ...
      switch (TRUE) {
        case ($row['CPM_YEAR'] > $row['TAHUN_LAPOR']):
          ...
          break;
        case ($row['CPM_YEAR'] == $row['TAHUN_LAPOR'] || $row['CPM_YEAR'] < $row['TAHUN_LAPOR']):
   ...
   GROUP BY atr_inner.CPM_ATR_AIRBAWAHTANAH_ID
    ...
    foreach ($data_wp as $npwpd => $rowDataWP) {
      ...
      if ($rowDataWP['CPM_KECAMATAN_WP'] != $cek_kecamatan) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('A' . $row, "Jumlah aawal ");
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('R' . $row, $airtriwulan1);
        $objPHPExcel->getActiveSheet()->setCellValue('S' . $row, $airtriwulan2);
        $objPHPExcel->getActiveSheet()->setCellValue('T' . $row, $airtriwulan3);
        $objPHPExcel->getActiveSheet()->setCellValue('U' . $row, $airtriwulan4);
      ...
      // $objPHPExcel->getActiveSheet()->setCellValue('R' . $row,  "masuk sinai mna");
      ...
      // $totalairtriwulan1 += "jumlsh volumr apa ini:>";
      // die;
      ...
      if ($jumlah_data == $jml) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('A' . $row, "Jumlah husna");
        $objPHPExcel->getActiveSheet()->setCellValue('D' . $row, $tahun1);
        $objPHPExcel->getActiveSheet()->setCellValue('E' . $row, $tahun2);
        $objPHPExcel->getActiveSheet()->setCellValue('F' . $row, $tahun3);
        $objPHPExcel->getActiveSheet()->setCellValue('G' . $row, $tahun4);
        ...
        if ($jumlah_data == $jml) {
          ...
          $objPHPExcel->getActiveSheet()->setCellValue('R' . $space, $totalairtriwulan1);
          $objPHPExcel->getActiveSheet()->setCellValue('S' . $space, $totalairtriwulan2);
          $objPHPExcel->getActiveSheet()->setCellValue('T' . $space, $totalairtriwulan3);
          $objPHPExcel->getActiveSheet()->setCellValue('U' . $space, $totalairtriwulan4);
