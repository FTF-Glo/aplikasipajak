...
class LaporPajak extends Pajak
{
	...
	public function get_pajak($npwpd = '', $nop = '')
	{
		...
		if (empty($pajak['CPM_ID'])) {
			...
			atr.CPM_ATR_REKENING, atr.CPM_ATR_TYPE_MASA, atr.CPM_ATR_KAWASAN, atr.CPM_ATR_JALAN, atr.CPM_ATR_JALAN_TYPE ,atr.CPM_ATR_SUDUT_PANDANG, atr.CPM_ATR_PERHITUNGAN,CPM_RUMUS,
		...
		$html = "<table width=\"710\" border=\"1\" cellpadding=\"4\">
				  ...
				  <tr>
					<td colspan=\"4\"><table width=\"100%\" border=\"0\">
					  ...
					  <tr>
						<td align=\"right\">2.</td>
						<td>Apabila SKPD tidak atau kurang dibayar setelah tengat waktu paling lama 30 hari setelah SKPD ini diterima akan dikenakan sanksi administrasi berupa bunga sebesar 1 %</td>
					  </tr>
					</table></td>
				  </tr>
				  <tr>
	...
	public function print_skpd($type = "")
	{
		...
		function tanggal_indo($tanggal) {
			...
			// Ubah format tanggal sesuai format yang diinginkan
			$tanggal_obj = date_create($tanggal);
			$tanggal_bulan = date_format($tanggal_obj, 'n'); // Dapatkan bulan sebagai angka (1-12)
			$tanggal_hari = date_format($tanggal_obj, 'j'); // Dapatkan hari (1-31)
			$tanggal_tahun = date_format($tanggal_obj, 'Y'); // Dapatkan tahun (4 digit)
		
			$bulan_indonesia = $bulan[$tanggal_bulan];
			
			// Bentuk tanggal sesuai format yang diinginkan
			$tanggal_indo = $tanggal_hari . ' ' . $bulan_indonesia . ' ' . $tanggal_tahun;
		...
		// hitung_denda
		$expired_date = $DATA['CPM_TGL_JATUH_TEMPO'];
		...
		// $tgl_1_bulan_setelah = date('Y-m-d', strtotime($tgl_jatuh_tempo_timestamp));
		// var_dump($tgl_1_bulan_setelah);die;
		if ($DATA['CPM_PERPANJANGAN'] == 1) {
			$tgl_jatuh_tempo = $DATA['CPM_TGL_JATUH_TEMPO'];
			$datetime = DateTime::createFromFormat('Y/m/d', $tgl_jatuh_tempo);
			...
			if ($DATA['CPM_START_DENDA'] == 1) {
				...
				if ($gw->payment_flag == 0 && ($DATA['CPM_MASA_PAJAK1'] != '' || $DATA['CPM_MASA_PAJAK1'] != NULL)) {
					...
					$total_pajak = $DATA['CPM_TOTAL_OMZET'];
					$tgl_jatuh_tempo = $DATA['CPM_TGL_JATUH_TEMPO'];
					$tgl_jatuh_tempo_obj = date_create_from_format("Y/m/d", $tgl_jatuh_tempo);
								...
								<tr>
									<td width=\"400\">
										<table width=\"380\" border=\"0\" cellpadding=\"0\">
											<tr>
												<td width=\"150\">Masa Pajak</td>
												<td width=\"550\">: {$masa_pajaks1} s/d {$masa_pajaks2}</td>
											</tr>
											<tr>
												<td width=\"150\">Tahun</td>
												<td width=\"550\">: {$DATA['CPM_TAHUN_PAJAK']}</td>
											</tr>
											<tr>
								...
								<tr>
									<td width=\"400\">
										<table width=\"300\" border=\"0\" cellpadding=\"0\">
											<tr>
												<td>Tanggal Jatuh Tempo</td>
												<td>:  " . tanggal_indo($expired_date) . "</td>
											</tr>
										</table>
									</td>
									<td width=\"400\">
		...
		foreach ($pajak_atr as $no => $atr) {
			...
			$total_semuanya = round($DATA['CPM_TOTAL_OMZET'] + $DATA['CPM_DENDA_TERLAMBAT_LAP']  + $DENDA);
		...
		$html .= "<tr>
											...
											<td align=\"right\">
												" .  number_format($DATA['CPM_TOTAL_OMZET']) . "
											</td>
										</tr>
										<tr>
											<td align=\"left\">
											...
											<td align=\"right\">
												" . number_format($DATA['CPM_DENDA_TERLAMBAT_LAP']) . "
											</td>
										</tr>
										<tr>
											<td align=\"left\">
										...
										<tr>
											...
											<td align=\"right\">
												<b>" .  number_format($total_semuanya) . "</b>
											</td>
										</tr>
										<tr>
											<td align=\"left\" colspan=\"4\">
											Dengan huruf :<br/>
											<b><i>" . ucwords($this->SayInIndonesian($total_semuanya)) . " rupiah</i></b>
											</td>
							...
							<tr>
								<td><table width=\"100%\" border=\"0\" align=\"left\" style=\"font-size:28px;\">
										...
										<tr>
											...
											&nbsp;&nbsp; &nbsp; &nbsp;
											dikenakan sanksi/denda administrasi sebesar 1% per bulan</td>
										</tr>
									</table>
									</td>
								</tr>
						...
						<td colspan=\"2\" align=\"center\" style=\"font-size:24px;\">
							<table width=\"100%\" border=\"0\" align=\"left\" cellpadding=\"10\">
								<tr>
									...
									<td align=\"center\"><b>
									Gedong Tataan, " . $tgl[0] . " {$this->arr_bulan[(int)$tgl[1]]} " . $tgl[2] . " {$tgl_approve} <br/>
										A.n Kepala Badan Pendapatan Daerah <br/> Kabupaten Pesawaran <br/>
										Kepala Bidang Pajak Daerah Lain <br/>		";
		...
		// $strQR = $config['KABID_PENDATAAN_NAMA'];
		// $strQR .= "NIP. {$config['KABID_PENDATAAN_NAMA']}\n";
								...
								<tr>
									<td>&nbsp;&nbsp;&nbsp;<table width=\"700\" cellpadding=\"0\" border=\"0\" cellspacing=\"0\" style=\"font-size:33px;\">
											...
											<tr style=\"font-size:28px;\">
												...
												<td width=\"270\" rowspan=\"6\" align=\"center\">Gedong Tataan, " . date("d") . " {$this->arr_bulan[(int) date("m")]} " . date("Y") . "<br/>
		...
		$pdf->AddPage('P', 'F4');
		$pdf->Image("{$sRootPath}view/Registrasi/configure/logo/{$LOGO_CETAK_PDF}", 8, 11, 15, '', '', '', '', false, 300, '', false);
	...
	public function print_sptpd()
	{
                        ...
                        <td width=\"710\" colspan=\"2\" align=\"center\"><table width=\"100%\" border=\"0\" align=\"left\" cellpadding=\"5\">
							<tr style=\"font-size:28px\">
								<td><table width=\"130%\" border=\"0\" align=\"left\">
										...
										<tr>
											<td>&nbsp;&nbsp;&nbsp;1. Setelah Formulir SPTD ini ditandatangani, wajib diserahkan <br>  &nbsp;&nbsp;&nbsp; kembali Badan Pendapatan Daerah <br> &nbsp;&nbsp;&nbsp;&nbsp; Kabupaten Pesawaran</td>
										</tr>
										<tr>
								...
								<tr>
									...
									<td>
										<table width=\"200\" border=\"0\" align=\"left\">
											<tr>
												<td align=\"center\">Gedong Tataan, {$tgl_pengesahans}<br><br><br><br>{$DATA['profil']['CPM_NAMA_WP']}</td>
											</tr>
										</table>
									</td>
								</tr>
		...
		$pdf->AddPage('P', 'F4');
		//echo $page1;exit;
		$pdf->Image("{$sRootPath}view/Registrasi/configure/logo/{$LOGO_CETAK_PDF}", 7, 8, 14, '', '', '', '', false, 300, '', false);
		$pdf->writeHTML($page1, true, false, false, false, '');
                        ...
                        <td width=\"710\" colspan=\"2\" align=\"center\"><table width=\"100%\" border=\"0\" align=\"left\" cellpadding=\"5\">
							<tr style=\"font-size:28px\">
								<td><table width=\"130%\" border=\"0\" align=\"left\">
										...
										<tr>
											<td>&nbsp;&nbsp;&nbsp;1. Setelah Formulir SPTD ini ditandatangani, wajib diserahkan <br>  &nbsp;&nbsp;&nbsp; kembali Badan Pendapatan Daerah <br> &nbsp;&nbsp;&nbsp;&nbsp; Kabupaten Pesawran</td>
										</tr>
										<tr>
											<td>&nbsp;&nbsp;&nbsp;2. Keterlambatan penyerahan dari tanggal tersebut diatas akan <br> &nbsp;&nbsp;&nbsp;&nbsp; dilakukan penetapan Secara Jabatan.</td>
										</tr>
...
error_reporting(E_ALL);
		...
		// } else {
			if ($DATA['pajak']['CPM_START_DENDA'] == 1) {
				if ($gw->payment_flag == 0 && ($DATA['pajak']['CPM_MASA_PAJAK1'] != '' || $DATA['pajak']['CPM_MASA_PAJAK1'] != NULL)) {
					...
					$tgl_jatuh_tempo = $DATA['pajak']['CPM_TGL_JATUH_TEMPO'];
					$tgl_jatuh_tempo_obj = date_create_from_format("d-m-Y", $tgl_jatuh_tempo);
					// var_dump( $tgl_jatuh_tempo_obj);die;
					$tgl_sekarang = date("Y-m-d");
					// Menghitung selisih bulan dan tahun antara tanggal sekarang dan tanggal jatuh tempo
					$selisih_bulan = 0;
					$selisih_tahun = 0;
		...
		$html = "<table width=\"710\" class=\"main\" border=\"1\">
                    ...
                    <tr>
                        <td><table border=\"0\" cellpadding=\"3\" style=\"font-size:33px\">
								<tr>
									<td width=\"400\"></td>
									<td width=\"310\"><table>
											<tr>
                    ...
                    <tr>
                        <td><table width=\"900\" border=\"0\" class=\"child\" cellpadding=\"0\" cellspacing=\"0\"  style=\"font-size:33px\">
                                <tr>
		...
		$html .= " <tr>
                    ...
                    <tr>
						<td>
							...
							Jatuh tempo : ".date('d-m-Y', strtotime($gw->expired_date))."
							Denda 1% per bulan maksimal 24 bulan
							</font>
						</td>
					</tr>
					<span style=\"font-size:24px\"><i>BAPENDA PESAWARAN {$tgl_cetak}</i></span>
	...
	public function print_nota_hitung_fix()
	{
		global $sRootPath;
		$this->_id = $this->CPM_ID;
		$DATA = $this->get_pajak();
					...
					<tr>
						<td><table width=\"1015\" border=\"1\" cellpadding=\"10\">
								...
								<tr>
									<td><table cellpadding=\"1\" border=\"0\">
									<tr><td width=\"70\">Nama WP</td><td width=\"7\">:</td><td width=\"275\">{$DATA->CPM_NAMA_WP}</td></tr>
									<tr><td>Nama OP</td><td>:</td><td>{$pajak_atr['0']['CPM_NAMA_OP']}</td></tr>
									</table></td>
									<td><table cellpadding=\"1\" border=\"0\"><tr>
		...
		foreach ($pajak_atr as $no => $atr) {
			$row_atr .= "<tr>
											...
											<tr>
												...
												<td>{$atr['CPM_ATR_LEBAR']} M x {$atr['CPM_ATR_PANJANG']} M x {$atr['CPM_ATR_TINGGI']} Tinggi x {$atr['CPM_ATR_MUKA']} Muka x {$DATA->CPM_MASA_PAJAK} {$DATA->CPM_JNS_MASA_PAJAK}</td>
		...
		if ($x == 'false') {
			...
			// print_r($NSL);exit;
				...
				if ($_POST['rumus'] == 'RMS1' || $rms == 'RMS1' ) {
					...
					$total_ketinggian = $tinggi * $harga_ketinggian;
					$subTotal_nsrr = $panjang * $lebar * $muka *  $harga_dasar_ukuran + $total_ketinggian;
					$subTotal_nsr = $subTotal_nsrr * $jumlah * (25 / 100);

					$hitung_nsr = "<b>NJOPR</b><br>
					= (Panjang x Lebar x Muka x Jumlah unit x Harga dasar Ukuran)<br>
					= " . number_format($panjang) . " x (" . number_format($lebar) . " x " . number_format($muka) . ")  x  " . number_format($harga_dasar_ukuran) . "<br>
					= Rp. " . number_format($tinggi) ." x " . number_format($harga_ketinggian) ." <br>
					= Rp. " . number_format($subTotal_nsrr) ." <br><br>

					= " . number_format(25) . "% x ".number_format($jumlah) ." x " . number_format($subTotal_nsrr) . "<br>
					...
					$total_pajak = $subTotal_nsr + $total_terpasang;
					$subTotal_pajak = $total_pajak;
					...
					$total = $total_pajak;
				}elseif ($_POST['rumus'] == 'RMS2' || $rms == 'RMS2') {
					...
					$total_ketinggian = $tinggi * $harga_ketinggian;
					$subTotal_nsr = $panjang * $lebar * $muka * $jumlah * $harga_dasar_ukuran + $total_ketinggian;
					// $subTotal_nsr = $panjang * $lebar * $muka * $jumlah * $harga_dasar_ukuran;
					// $subTotal_nsr = $subTotal_nsrr * (25 / 100);


					$hitung_nsr = "<b>NJOPR</b><br>
					= (Panjang x Lebar x Jumlah unit x Harga dasar Ukuran)<br>
					= " . number_format($panjang) . " x (" . number_format($lebar) . " x " . number_format($muka) . ") x ". number_format($jumlah)." x  " . number_format($harga_dasar_ukuran) . "<br>
					= Rp. " . number_format($tinggi) ." x " . number_format($harga_ketinggian) ." <br>
...
// var_dump($_POST['rumus']);die;
			if ($_POST['rumus'] == 'RMS1' || $rms == 'RMS1') {
				...
				= Rp. " . number_format($total);
						...
						$hitung_total = "<br><b>Total Pajak</b><br>
						= " . number_format($subTotal_nsr, 2, '.', ',') . " + " . number_format($total_terpasang) . "<br>
						{$hitung_alkohol_rokok}
						= Rp. " . number_format($total);
...
// var_dump($this->CPM_TGL_JATUH_TEMPO);die;
			$query = sprintf(
				"INSERT INTO {$this->PATDA_REKLAME_DOC}
                    ...
                    CPM_KETERANGAN, CPM_VERSION, CPM_AUTHOR, CPM_DPP, CPM_BAYAR_TERUTANG, CPM_NO_SSPD, CPM_JNS_MASA_PAJAK,CPM_TGL_JATUH_TEMPO,
                    ...
                    VALUES ( '%s','%s','%s',%f,%f,%f,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',%f,%d,%f,'%s','%s')",
				$this->CPM_ID,
				$this->CPM_ID_PROFIL,
				$this->CPM_NO,
				$this->CPM_MASA_PAJAK,
			...
			if ($res) {
				...
				for ($x = 0; $x < $j; $x++) {
					...
					$jam = (isset($PAJAK_ATR['CPM_ATR_JAM'][$x])) ? $PAJAK_ATR['CPM_ATR_JAM'][$x] : '0';
					$rms = (isset($PAJAK_ATR['RMS'][$x])) ? $PAJAK_ATR['RMS'][$x] : '0';
					// $rms = (isset($PAJAK_ATR['RMS'][$x])) ? $PAJAK_ATR['RMS'][$x] : '0';
					// echo "<pre>";
					// var_dump($kontol);die;
					//
					// var_dump($this->get_hargadasar(array("kontol"=$kontol)));
					// die;
					$res_hargadasar = $this->get_hargadasar(
						array(
							'panjang' => $panjang,
					...
					$nilai_strategis = $res_hargadasar['nilai_strategis'];
					$njop = $res_hargadasar['njop'];
					// $perhitungan = $res_hargadasar['html'];
					$perhitungan = $res_hargadasar['rumus_hitung'];
					$harga = $res_hargadasar['harga'];
					// $harga = $res_hargadasar[''];
	...
	function addRow()
	{
		...
		<tr>
			...
			<td align="center" valign="top"><input name="PAJAK_ATR[CPM_ATR_MUKA][]" tabindex="' . ($idx + 5) . '" type="text" class="number" id="CPM_ATR_MUKA-' . $no . '" size="11" maxlength="11" placeholder="Muka" onkeyup="hitungDetail(' . $no . ')" /></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td colspan="2">
				<select class="form-control text-center" onchange="hitungDetail(' . $no . ')" name="PAJAK_ATR[RMS][' . ($no - 1) . ']" id="RMS-' . $no . '">
