    ...
    $(".btn-tambah").click(function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                // -2
                $('#CPM_PEMBANGKIT-' + (no + 1)).autoNumeric('init');
                $('#CPM_FAKTOR_DAYA-' + (no + 1)).autoNumeric('init');
                $('#CPM_SATUAN-' + (no + 1)).autoNumeric('init');

                $('#CPM_TOTAL_KVA-' + (no + 1)).autoNumeric('init');
                $('#CPM_NYALA-' + (no + 1)).autoNumeric('init');

                $('#CPM_TOTAL_OMZET-' + (no + 1)).autoNumeric('init');

                $('#CPM_BAYAR_LAINNYA-' + (no + 1)).autoNumeric('init');
                $('#CPM_TOTAL_KWH-' + (no + 1)).autoNumeric('init');
                // $('#CPM_TOTAL_KWH2').autoNumeric('init');
                $('#CPM_HARGA_DASAR-' + (no + 1)).autoNumeric('init');
                $('#CPM_DPP-' + (no + 1)).autoNumeric('init');
                $('#CPM_TARIF_PAJAK-' + (no + 1)).autoNumeric('init');
                $('#CPM_BAYAR_TERUTANG-' + (no + 1)).autoNumeric('init');
                $('#CPM_TOTAL_PAJAK-' + (no + 1)).autoNumeric('init');
...
function function_sum(obj) {
    ...
    var atr = Number($('#CPM_ATR_TOTAL').autoNumeric('get'));
...
function hitungaku() {
    var tarif = parseFloat($("#CPM_TARIF_PAJAK").val());
    ...
    if (count == 1) {
        ...
        for (var i = 2; i <= count; i++) {
            var om1 = Number($('#CPM_TOTAL_OMZET-' + i).autoNumeric('get'));
            var lain1 = Number($('#CPM_BAYAR_LAINNYA-' + i).autoNumeric('get'));
            var atr1 = Number($('#CPM_ATR_TOTAL-' + i).autoNumeric('get'));

            // CPM_ATR_TOTAL-2
            var tarif1 = Number($('#CPM_TARIF_PAJAK-' + i).val());
            var dpp1 = eval(om1) + eval(lain1);
            var terutang1 = dpp1 * tarif1 / 100;

            //  $('#CPM_BAYAR_TERUTANG-'+i).autoNumeric('set', terutang1);
            $('#CPM_ATR_TOTAL-' + i).autoNumeric('set', terutang1);
            var atrTotal = Number($('#CPM_ATR_TOTAL-' + i).autoNumeric('get'));
            jum += atrTotal;
    ...
    var total = eval(jumlah1) + eval(kurangLebih) + eval(sanksi);
    total = Math.ceil(total);


    // $("#CPM_TOTAL_OMZET").autoNumeric("set", omzet);
    $("#CPM_TOTAL_PAJAK").autoNumeric("set", total);
    $('#CPM_BAYAR_TERUTANG').autoNumeric('set', jumlah1);
...
function hitungDetail(no) {
    $("#CPM_PEMBANGKIT-" + no).keyup(function () {
        var pembangkit = Number($('#CPM_PEMBANGKIT-' + no).autoNumeric('get'));
        var faktor_daya = Number($('#CPM_FAKTOR_DAYA-' + no).autoNumeric('get'));
        var satuan = Number($('#CPM_SATUAN-' + no).autoNumeric('get'));
        var kwh = Number($('#CPM_TOTAL_KWH-' + no).autoNumeric('get'));
        var total_pemakaian = eval(pembangkit) * eval(faktor_daya) * eval(satuan) * eval(kwh);
        $('#CPM_TOTAL_OMZET-' + no).autoNumeric('set', total_pemakaian);
        $('#CPM_DPP-' + no).autoNumeric('set', total_pemakaian);
        // function_sum();
        //    console.log();
        hitungaku();
    });

    $("#CPM_FAKTOR_DAYA-" + no).keyup(function () {
        var pembangkit = Number($('#CPM_PEMBANGKIT-' + no).autoNumeric('get'));
        var faktor_daya = Number($('#CPM_FAKTOR_DAYA-' + no).autoNumeric('get'));
        var satuan = Number($('#CPM_SATUAN-' + no).autoNumeric('get'));
        var kwh = Number($('#CPM_TOTAL_KWH-' + no).autoNumeric('get'));
    ...
    $("#CPM_TOTAL_KWH-" + no).keyup(function () {
        var pembangkit = Number($('#CPM_PEMBANGKIT-' + no).autoNumeric('get'));
        var faktor_daya = Number($('#CPM_FAKTOR_DAYA-' + no).autoNumeric('get'));
        var satuan = Number($('#CPM_SATUAN-' + no).autoNumeric('get'));
        var kwh = Number($('#CPM_TOTAL_KWH-' + no).autoNumeric('get'));
        var total_pemakaian = eval(pembangkit) * eval(faktor_daya) * eval(satuan) * eval(kwh);
        // console.log(total_pemakaian);
        $('#CPM_TOTAL_OMZET-' + no).autoNumeric('set', total_pemakaian);
        $('#CPM_DPP-' + no).autoNumeric('set', total_pemakaian);
    ...
    $("#CPM_TOTAL_OMZET-" + no).keyup(function () {
        ...
        // var total_pemakaian = eval(pembangkit) * eval(faktor_daya) * eval(satuan) * eval(kwh);
        // console.log(total_pemakaian);
        $('#CPM_TOTAL_OMZET-' + no).autoNumeric('get');
        var apik = $('#CPM_TOTAL_OMZET-' + no).autoNumeric('get');

        $('#CPM_DPP-' + no).autoNumeric('set', apik);
