...
<form class="cmxform" autocomplete="off" id="form-lapor" method="post" enctype="multipart/form-data" action="function/<?php echo "{$DIR}/{$modul}"; ?>/lapor/svc-lapor.php?param=<?php echo base64_encode($json->encode(array("a" => $lapor->_a, "m" => $lapor->_m, "mod" => $lapor->_mod, "f" => $lapor->_f))) ?>">
    ...
    <table class="main" width="900">
                                    ...
                                    if (!in_array($lapor->_i, array(1, 3, ""))) {
                                        ...
                                        if ($DATA['profil']['CPM_PERUNTUKAN'] != "") {
                                            ...
                                            foreach ($list_npa2 as $x => $y) {
                                                ...
                                                echo ($x == $DATA['profil']['CPM_PERUNTUKAN']) ? "<option value='{$x}' class='ini2 hide'>{$y}</option>" : "<option class='ini2 hide' value='{$x}' disabled>{$y}</option>";
                                            }
                                        } else {
                                            foreach ($list_npa as $x => $y) {
        ...
        <td width="200" rowspan="4">Keterangan :</br>
            <textarea name="PAJAK[CPM_KETERANGAN]" id="CPM_KETERANGAN" rows="10" <?php echo ($lapor->_s == 1 || $lapor->_s == "" || ($lapor->_s == "4" && $lapor->_mod == "pel")) ? "" : "readonly"; ?> placeholder="Keterangan"><?php echo $DATA['pajak']['CPM_KETERANGAN'] ?></textarea>
        </td>
        </tr>
        ...
        <tr>
            <td>Type Masa</td>
            <td> :
                <select class="form-control" style="width:150px;height:30px;display:inline-block;font-size:small;" name="PAJAK[CPM_TYPE_MASA]" id="CPM_TYPE_MASA">
                    <?php
                    if (in_array($lapor->_mod, array("pel", ""))) {
                        if (!in_array($lapor->_i, array(1, 3, ""))) {
		...
		<tr>
            <td>Jatuh Tempo <?= $DATA['pajak']['CPM_TGL_JATUH_TEMPO'] ?><b class="isi">*</b></td>
            <td> : <input style="width:150px;height:30px;display:inline-block;font-size:small;" type="date" name="PAJAK[CPM_TGL_JATUH_TEMPO]" class="form-control" id="CPM_TGL_JATUH_TEMPO" 
			<?php if ($lapor->_s <> 2) : echo ' value="' . $DATA['pajak']['CPM_TGL_JATUH_TEMPO'] . '" readonly';
                    endif; ?> >
            </td>
        </tr>
		<?php endif; ?> -->
                    ...
                    <div id="item-perolehan-1" style="border-bottom:1px solid #ccc;padding-bottom:6px">
                        <table>
                            <tr>
                                <td>Bulan</td>
                                <td> :
                                    <span id="bulan-perolehan-1"><?php echo  $bulan; ?></span>
                                    <span id="tahun-perolehan-1"><?php echo  $DATA['pajak']['CPM_TAHUN_PAJAK'] ?></span>
                    ...
                    <div id="item-perolehan-2" style="<?php echo (isset($DATA['pajak_atr'][1]['CPM_ATR_TOTAL']) ? $DATA['pajak_atr'][1]['CPM_ATR_TOTAL'] : '') == '' ? 'display:none;' : '' ?>border-bottom:1px solid #ccc;padding-bottom:6px"">
                            <table>
                                <tr>
                                    <td>Bulan</td>
                                    <td> :
                                        <span id=" bulan-perolehan-2">
                        <?php echo  isset($DATA['pajak_atr'][1]['CPM_ATR_BULAN']) ? $lapor->arr_bulan[$DATA['pajak_atr'][1]['CPM_ATR_BULAN']] : '' ?>
                        </span>
                        <span id="tahun-perolehan-2"><?php echo  $DATA['pajak']['CPM_TAHUN_PAJAK'] ?></span>
