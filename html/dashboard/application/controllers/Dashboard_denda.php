...
class Dashboard_denda extends CI_Controller
{
    ...
    public function getdataapi(){
    	$sembilan_pajak = $this->getdata_9pajak_get();
    	$bphtb = $this->getdata_bphtb_get();
    	$pbb = $this->getdata_pbb_get();
    	$walet = $this->sarang_walet_get();
    	$html = '';
    	$no = 0;
    	$alltarget = 0;
    	$allbulanlalu = 0;
    	...
    	if (!empty($sembilan_pajak)) {
	    	foreach ($sembilan_pajak as $ket => $value) {
	    		...
	    		$html .= "</tr>";
		    		$html .= "<tr>";
		    		$html .= "<td></td>";
		    		$html .= "<td colspan=\"2\">DENDA</td>";

		    		$alldenda_pajak = 0;
					for ($i=1; $i <= $monthnow; $i++) { 
		    			$alldenda_pajak += $this->permonth_dendapajak($jenis_sw[$ket],$i);
						$html .= "<td align='right'>".number_format(intval($this->permonth_dendapajak($jenis_sw[$ket],$i)),0,',','.')."</td>";
					}
		    		$html .= "<td align=\"right\">".number_format(intval($alldenda_pajak),0,',','.')."</td>";
		    		$html .= "<td></td>";
	    		$html .= "</tr>";
	    	}
	    }
    	...
    	if (isset($pbb)) {
	    	foreach ($pbb as $ket => $value) {
				...
				for ($i=1; $i <= $monthnow; $i++) { 
					...
					$alldendaperbulan[$i] += intval($this->permonth_dendapbb($i));
	    		...
	    		$html .= "</tr>";
		    		...
		    		$alldenda_pbb = 0;
					for ($i=1; $i <= $monthnow; $i++) { 
		    			$alldenda_pbb += $this->permonth_dendapbb($i);
						$html .= "<td align='right'>".number_format(intval($this->permonth_dendapbb($i)),0,',','.')."</td>";
					}
		    		$html .= "<td align=\"right\">".number_format(intval($alldenda_pbb),0,',','.')."</td>";
		    		$html .= "<td></td>";
	    		$html .= "</tr>";
	    	}
    	}
	...
	public function getdata_9pajak_get(){
		// data 9 pajak
		$airbawahtanah = $this->db->query("SELECT (SELECT SUM(c.patda_total_bayar) FROM simpatda_gw c
													...
													(SELECT cpm_jumlah FROM patda_target_pajak WHERE cpm_jenis_pajak = 1) AS target")
											->row();
		$hiburan = $this->db->query("SELECT (SELECT SUM(c.patda_total_bayar) FROM simpatda_gw c
						                          ...
						                          (SELECT SUM(c.patda_denda) FROM simpatda_gw c
															...
															AND MONTH(c.payment_paid) = MONTH(CURRENT_DATE)
															AND YEAR(c.payment_paid) = YEAR(CURRENT_DATE)) AS denda_bulan_ini,
													(SELECT cpm_jumlah FROM patda_target_pajak WHERE cpm_jenis_pajak = 2) AS target")
											->row();
		...
		$hotel = $this->db->query("SELECT (SELECT SUM(c.patda_total_bayar) FROM simpatda_gw c
                                      ...
                                      (SELECT SUM(c.patda_denda) FROM simpatda_gw c
															...
															AND MONTH(c.payment_paid) = MONTH(CURRENT_DATE)
															AND YEAR(c.payment_paid) = YEAR(CURRENT_DATE)) AS denda_bulan_ini,
													(SELECT cpm_jumlah FROM patda_target_pajak WHERE cpm_jenis_pajak = 3) AS target")
											->row();
		...
		$minerba = $this->db->query("SELECT (SELECT SUM(c.patda_total_bayar) FROM simpatda_gw c
                                      ...
                                      (SELECT SUM(c.patda_denda) FROM simpatda_gw c
															...
															AND MONTH(c.payment_paid) = MONTH(CURRENT_DATE)
															AND YEAR(c.payment_paid) = YEAR(CURRENT_DATE)) AS denda_bulan_ini,
													(SELECT cpm_jumlah FROM patda_target_pajak WHERE cpm_jenis_pajak = 4) AS target")
											->row();
		...
		$parkir = $this->db->query("SELECT (SELECT SUM(c.patda_total_bayar) FROM simpatda_gw c
                                      ...
                                      (SELECT SUM(c.patda_denda) FROM simpatda_gw c
															...
															AND MONTH(c.payment_paid) = MONTH(CURRENT_DATE)
															AND YEAR(c.payment_paid) = YEAR(CURRENT_DATE)) AS denda_bulan_ini,
													(SELECT cpm_jumlah FROM patda_target_pajak WHERE cpm_jenis_pajak = 5) AS target")
											->row();
		...
		$jalan = $this->db->query("SELECT (SELECT SUM(c.patda_total_bayar) FROM simpatda_gw c
                                      ...
                                      (SELECT SUM(c.patda_denda) FROM simpatda_gw c
															...
															AND MONTH(c.payment_paid) = MONTH(CURRENT_DATE)
															AND YEAR(c.payment_paid) = YEAR(CURRENT_DATE)) AS denda_bulan_ini,
													(SELECT cpm_jumlah FROM patda_target_pajak WHERE cpm_jenis_pajak = 6) AS target")
											->row();
		...
		$reklame = $this->db->query("SELECT (SELECT SUM(c.patda_total_bayar) FROM simpatda_gw c
                                      ...
                                      (SELECT SUM(c.patda_denda) FROM simpatda_gw c
															...
															AND MONTH(c.payment_paid) = MONTH(CURRENT_DATE)
															AND YEAR(c.payment_paid) = YEAR(CURRENT_DATE)) AS denda_bulan_ini,
													(SELECT cpm_jumlah FROM patda_target_pajak WHERE cpm_jenis_pajak = 7) AS target")
											->row();
		...
		$restoran = $this->db->query("SELECT (SELECT SUM(c.patda_total_bayar) FROM simpatda_gw c
                                      ...
                                      (SELECT SUM(c.patda_denda) FROM simpatda_gw c
															...
															AND MONTH(c.payment_paid) = MONTH(CURRENT_DATE)
															AND YEAR(c.payment_paid) = YEAR(CURRENT_DATE)) AS denda_bulan_ini,
													(SELECT cpm_jumlah FROM patda_target_pajak WHERE cpm_jenis_pajak = 8) AS target")
											->row();
		// 9 pajak end
		$result = [
					'HOTEL'			=>	$hotel,
