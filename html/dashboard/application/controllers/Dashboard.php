...
class Dashboard extends CI_Controller
{
	...
	public function getdataapi()
	{
		...
		$arrayBPJT = [];
		$bpjtTargetTerbesar = 0;

		if (!empty($sembilan_pajak)) {
			$z = 0;
			foreach ($sembilan_pajak as $ket => $value) {
				$z++;
				if($ket=='REKLAME' || $ket=='AIR BAWAH TANAH' || $ket=='MINERBA'){
					...
					$html .= "</tr>";
					$x = sprintf("%09d", str_replace('.', '', number_format(($value->target + $z), 2, '.', '')));
					$dataFilter[$x] = $html;
				}else{
					$xx = sprintf("%08d", str_replace('.', '', number_format(($value->target + $z), 2, '.', '')));
					$value->pajak = $ket;
					$arrayBPJT[(int)$xx] = $value;
					$bpjtTargetTerbesar = ($bpjtTargetTerbesar<$value->target) ? $value->target : $bpjtTargetTerbesar;
				}
			}
		}
		...
		$html .= "<td align='center'>{nomorurut}</td>";
		$html .= "<td colspan=6>P B J T</td>";
		$html .= "</tr>";
		
		foreach ($arrayBPJT as $value) {
			$alltarget += $value->target;
			$allbulanlalu += $value->bulan_lalu;
			$allbulanini += $value->bulan_ini;
			$alldendabulanlalu += $value->denda_bulan_lalu;
			$alldendabulanini += $value->denda_bulan_ini;
			
			$html .= "<tr style='background-color:#DCDCDC'>";
			$html .= "<td align='center'> </td>";
			$html .= "<td style='padding-left:3rem'>{$value->pajak}</td>";
			$html .= "<td align='right'>" . number_format($value->target, 0, ',', '.') . "</td>";
			$html .= "<td align='right'>" . number_format($value->bulan_lalu, 0, ',', '.') . "</td>";
			$html .= "<td align='right'>" . number_format($value->bulan_ini, 0, ',', '.') . "</td>";
			$jumlah = $value->bulan_lalu + $value->bulan_ini;
		...
		$x = sprintf("%09d", str_replace('.', '', number_format(($bpjtTargetTerbesar), 2, '.', '')));
		$dataFilter[$x] = $html;
		/// ===========================================================================================
		...
		if (isset($bphtb)) {
			foreach ($bphtb as $ket => $value) {
				...
				$x = sprintf("%06d", str_replace('.', '', number_format($value['target'] + 98, 2, '.', '')));
		...
		if (isset($pbb)) {
			foreach ($pbb as $ket => $value) {
				...
				$x = sprintf("%06d", str_replace('.', '', number_format($value['target'] + 99, 2, '.', '')));

				$dataFilter[$x] = $html;
			}
		}

		// if($_SERVER['HTTP_USER_AGENT']=='Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:122.0) Gecko/20100101 Firefox/122.0'){
		// 	// d3Di_browser
		// 	$alltarget += 0;
		// 	$allbulanlalu += 0;
		// 	$allbulanini += 0;
		...
		// 	$html .= "<td align='center'>{nomorurut}</td>";
		// 	$html .= "<td>BPJT</td>";
		// 	$html .= "<td align='right' data-label='target'>" . number_format(1, 0, ',', '.') . "</td>";
		// 	$html .= "<td align='right' data-label='bulan_lalu'>" . number_format(2, 0, ',', '.') . "</td>";
		// 	$html .= "<td align='right' data-label='bulan_ini'>" . number_format(3, 0, ',', '.') . "</td>";
		// 	$jumlah = 6;
		// 	$html .= "<td align='right' data-label='jumlah'>" . number_format(4, 0, ',', '.') . "</td>";
		// 	$target = 6 != '' ? 5 : 0;
		// 	if ($target == 0 || $jumlah == 0) {
		// 		$total_persen = 0;
		// 	} else {
		// 		$total_persen = (intval($jumlah) / intval($target)) * 100;
		// 	}
		// 	$html .= "<td align='right' data-label='total_persen'>" . str_replace('.',',',(float)round(0.22, 2)) . "%</td>";
		// 	// }
		// 	$html .= "</tr>";

		// 	$x = sprintf("%06d", str_replace('.', '', number_format(144, 2, '.', '')));
