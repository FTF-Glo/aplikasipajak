...
class GetBPHTBService extends REST_Controller {
    ...
    function index_post() {
        ...
        $sql = "SELECT ssb.op_nomor AS NOP,ssb.ntpd AS NTPD FROM ssb WHERE op_nomor='$nop'";
        ...
        $sql = "SELECT ssb.op_nomor AS NOP,ssb.ntpd AS NTPD FROM ssb WHERE ntpd='$ntpd'";
        $sqlRun = $this->db->query($sql)->result();
            if (!$sqlRun) {
        ...
        if($nop == ""){
            ...
            IF(ssb.payment_flag=1, 'Y', 'T') AS `STATUS`,
            DATE_FORMAT(ssb.payment_paid, '%d/%m/%Y') AS TANGGAL_PEMBAYARAN,
            ssb.payment_code as NTPD,
            IF(ssb.payment_flag=1, 'L', 'H') AS JENISBAYAR
            FROM gw_ssb.ssb ssb 
            INNER JOIN sw_ssb.cppmod_ssb_doc doc ON ssb.id_switching = doc.CPM_SSB_ID 
            WHERE doc.CPM_STATUS_NTPD = 1 AND 
            ssb.op_nomor='$nop' AND 
            ssb.ntpd='$ntpd'
            AND ((ssb.payment_flag = '1' AND ssb.bphtb_dibayar>=0) OR (ssb.payment_flag = '0' AND ssb.bphtb_dibayar=0))";
