...
class DbGwMonitor {
    ...
    public function __construct($dbSpec, $kdKotaKab, $attConnection) {
        $this->dbSpec = $dbSpec;
        $this->where(array("OP_KOTAKAB_KODE" => mysqli_real_escape_string($connection, trim($kdKotaKab))));
    ...
    public function order_by($order_by, $opt = "") {
        $order_by = mysqli_real_escape_string($connection, trim($order_by));
        $opt = mysqli_real_escape_string($connection, trim($opt));
    ...
    public function get($field = array()) {
        ...
        if (count($this->aWhere) > 0 || count($this->aLike) > 0) {
            ...
            if (count($this->aWhere) > 0) {
                ...
                foreach ($this->aWhere as $key => $value) {
                    $query .= " $key = '" . mysqli_real_escape_string($connection, trim($value)) . "' ";
                    if ($key != $last_key)
                        $query .= " AND ";
                }
            }
            ...
            if (count($this->aLike) > 0) {
                ...
                foreach ($this->aLike as $key => $value) {
                    $query .= " $key LIKE '%" . mysqli_real_escape_string($connection, trim($value)) . "%' ";
                    if ($key != $last_key)
                        $query .= " AND ";
                }
            }
        ...
        if (count($this->aGroup_by) > 0) {
            ...
            foreach ($this->aGroup_by as $key => $val) {
                $query .= mysqli_real_escape_string($connection, trim($val));
                if ($key != $last_key)
                    $query .= ", ";
            }
        }
