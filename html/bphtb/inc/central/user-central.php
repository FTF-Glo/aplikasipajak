...
class SCANCentralUser
{
	...
	{
		// FIX: mysql escape string
		$sppid = mysqli_real_escape_string($ppid);
		$cid = "";
	...
	public function GetModuleInApp($sUID, $app, &$moduleIds)
	{
		...
		$sQ = "SELECT DISTINCT C.CTR_USER_ID, F.CTR_RM2F_MID, F.CTR_RM2F_PRIV, M.CTR_M_NAME,M.ICON " .
		...
		// NEW: order by
		$sQ .= "ORDER BY M.URUTAN, M.CTR_M_NAME ASC ";
		// echo $sQ;
		// var_dump(mysqli_query($this->DBLink, $sQ));
		// die;
	...
	public function GetModuleInApps($sUID, $app, &$moduleIds2)
	{
		// FIX: mysql escape string
		$sUID = mysqli_real_escape_string($this->DBLink, $sUID);
		$app = mysqli_real_escape_string($this->DBLink, $app);

		CTOOLS_ArrayRemoveAllElement($moduleIds2);
		$bOK = false;
		...
		if ($res = mysqli_query($this->DBLink, $sQ)) {
			...
			if ($nRes > 0) {
				$i = 0;
				while ($row = mysqli_fetch_array($res)) {
					$moduleIds2[$i]["id"] = $row["CTR_RM2F_MID"];
					$moduleIds2[$i]["name"] = $row["CTR_M_NAME"];
					$moduleIds2[$i]["icon"] = $row["ICON"];
					// $moduleIds[$i]["priv"] = $row["CTR_RM2F_PRIV"];
					$i++;
				}
			}
		}
		// print_r($moduleIds);die;
		return $bOK;
	}
	...
	public function GetTerminal($appId, $moduleId, &$terminal)
	{
		...
		if (CTOOLS_IsInFlag($this->iDebug, DEBUG_DEBUG))
			error_log("[" . strftime("%Y%m%d%H%M%S", time()) . "][" . $this->sThisFile . ":" . __LINE__ . "] [DEBUG] sQ [$sQ]\n", 3, $this->sLogFilename);
		if ($res = mysqli_query($this->DBLink, $terminalQuery)) {
			$nRes = mysqli_num_rows($res);
			if ($nRes > 0) {
				$i = 0;
				while ($row = mysqli_fetch_array($res)) {
	...
	public function CheckOTPPass($ppid, $pass, &$status, $OTPDBLink = NULL)
	{
		...
		$ppid = mysqli_real_escape_string($ppid, $dblink);
