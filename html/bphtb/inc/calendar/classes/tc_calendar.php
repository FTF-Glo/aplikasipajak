...
//
//adjust: change php short open tag (<?php echo ) to normal tag (<?php)
...
class tc_calendar{
	...
	function getDayHeaders(){
		if($this->startMonday)
			return array("1"=>"S", "2"=>"S", "3"=>"R", "4"=>"K", "5"=>"J", "6"=>"S","7"=>"M");
		else
			return array("7"=>"M", "1"=>"S", "2"=>"S", "3"=>"R", "4"=>"K", "5"=>"J", "6"=>"S");
	}
	...
	function getMonthNames(){
		return array("Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Augustus", "September", "Oktober", "November", "Desember");		
	}
	...
	function inputTime($inputId,$value) {
		$hour = 0;
		$minute = 0;
		$second = 0;
		if ($value) {
			$expTime = explode(":",$value);
			$hour = intval($expTime[0]);
			$minute = intval($expTime[1]);
			$second = intval($expTime[2]);
		} else {
			$value = '00:00:00';
		}
		
		$iH = "<select id='".$inputId."_hour' name='".$inputId."_hour' onchange=' onChangeTime(\"".$inputId."\")'>\n";
		for($i=0;$i<24;$i++) {
			$iH .= "<option value='".str_pad($i,2,"0",STR_PAD_LEFT)."' ".($i==$hour?"selected='selected'":"").">".str_pad($i,2,"0",STR_PAD_LEFT)."</option>\n";
		}
		$iH.= "</select>";
		$iM = "<select id='".$inputId."_minute' name='".$inputId."_minute' onchange=' onChangeTime(\"".$inputId."\")'>\n";
		for($i=0;$i<60;$i++) {
			$iM .= "<option value='".str_pad($i,2,"0",STR_PAD_LEFT)."' ".($i==$minute?"selected='selected'":"").">".str_pad($i,2,"0",STR_PAD_LEFT)."</option>\n";
		}
		$iM.= "</select>";
		$iS = "<select id='".$inputId."_second' name='".$inputId."_second' onchange=' onChangeTime(\"".$inputId."\")'>\n";
		for($i=0;$i<60;$i++) {
			$iS .= "<option value='".str_pad($i,2,"0",STR_PAD_LEFT)."' ".($i==$second?"selected='selected'":"").">".str_pad($i,2,"0",STR_PAD_LEFT)."</option>\n";
		}
		$iS.= "</select>";
		$hiddenInput =  "<input type='hidden' id='".$inputId."' name='".$inputId."' value='".$value."' />\n";
		return $iH.$iM.$iS.$hiddenInput;
	}
}
?>