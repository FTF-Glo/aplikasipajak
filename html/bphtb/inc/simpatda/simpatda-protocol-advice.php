...
class simpatdaProtocolAdviceRequest extends CISO8583Message
{
	function __construct()
	{
		...
		//set defaults
		$this->SetValueForDataElement(0,2220); //mti
	...
	private function ConstructPrivateData($aPriv)
	{
		...
		$sPriv .= str_pad($aPriv['tax_refnum'],32,"0", STR_PAD_LEFT);
		$sPriv .= str_pad($aPriv['gw_refnum'],32,"0", STR_PAD_LEFT);
		$sPriv .= str_pad($aPriv['sw_refnum'],32,"0", STR_PAD_LEFT);
		$sPriv .= str_pad($aPriv['pay_refnum'],32,"0", STR_PAD_LEFT);
		...
		$sPriv .= str_pad($aPriv['object_name'],25," ",STR_PAD_RIGHT);
	...
	{
		$sPriv2 = '';
		$sPriv2 .= str_pad($aPriv2['minor']       , 1, "0", STR_PAD_LEFT);
		$sPriv2 .= str_pad($aPriv2['total']       , 2, "0", STR_PAD_LEFT);
		$sPriv2 .= str_pad($aPriv2['type']        , 2, "0", STR_PAD_LEFT);
		$sPriv2 .= str_pad($aPriv2['amount']      , 17, "0", STR_PAD_LEFT);
		return $sPriv2;	
	}
...
class simpatdaProtocolAdviceResponse extends CISO8583Parser
{
	...
	private function SplitSecondPrivateData($stream){
		$splitter = new ProtocolGeneric();
		$splitter->SetStream($stream);
		$splitter->SetBlockHeaderLengthArray(array(1, 2, 2, 17));
		$splitter->SetBlockHeaderAssocNameArray(array( 'minor', 'total', 'type', 'amount'));
		$splitter->Extract();
		$this->secondPrivateData = $splitter->GetComponentHeaderArray();
		foreach($this->secondPrivateData as $key => $value) $this->secondPrivateData[$key] = trim($value);
	}
