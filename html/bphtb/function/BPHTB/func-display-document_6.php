...
where b.CPM_SSB_AUTHOR = '$notaris'";
    ...
    function setSubmit($type, $info, $info_disetujui) {
        global $data, $DBLink;
        $this->submit = true;
        $this->getData();
        $idtran = c_uuid();
...
//        #aktifkan validasi berkas
//        if ($type == 1 && ($this->dispenda1) && $status == 3) {
//            $sqlCheck = "select * from cppmod_ssb_berkas where CPM_BERKAS_IDSSB = '{$x}' and CPM_BERKAS_STATUS='1'";
//            $resultCheck = mysqli_query($sqlCheck, $appDbLink);
//            if (mysqli_num_rows($resultCheck) == 0) {
//                echo "Verifikasi gagal! Berkas dokumen harus dilengkapi dahulu sebelum diverifikasi! ";
//                $params = "a=" . $_REQUEST['a'] . "&m=" . $_REQUEST['m'] . "&n=1";
//                $address = $_SERVER['HTTP_HOST'] . "payment/pc/svr/central/main.php?param=" . base64_encode($params);
//                echo " <input type=\"button\" value=\"Kembali\" onclick=\"javascript:window.location ='./main.php?param=" . base64_encode($params) ."'\">";
//                exit;
//            }
//        }
        ...
        if ($status == 5) {
			if($cekGW!=1){
				$this->insertToGateway($x, $opr);
			}else{
				echo "Data Tidak Berhasil disimpan Ke Tagihan, Karena Data Tagihan sudah ada ...! ";
				exit;
			}
        }

        $query = sprintf("UPDATE cppmod_ssb_tranmain SET CPM_TRAN_STATUS='%s', CPM_TRAN_FLAG='%s',
		CPM_TRAN_OPR_NOTARIS='%s' WHERE CPM_TRAN_SSB_ID='%s' AND CPM_TRAN_STATUS='%s' AND CPM_TRAN_FLAG=0", $sts, "1", mysqli_real_escape_string($DBLink, $opr), mysqli_real_escape_string($DBLink, $x), $sts);
        $result = mysqli_query($DBLink, $query);
		...
		CPM_TRAN_DATE,CPM_TRAN_CLAIM,CPM_TRAN_OPR_NOTARIS,CPM_TRAN_OPR_DISPENDA_1,CPM_TRAN_OPR_DISPENDA_2,CPM_TRAN_INFO, CPM_TRAN_INFO_DISETUJUI) 
		VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')", $idtran, $refnum, $x, $version, $status, '0', mysqli_real_escape_string($DBLink, $trdate), '', $opr, $dispenda1, $dispenda2, nl2br($info), nl2br($info_disetujui));
        $result = mysqli_query($DBLink, $query);
...
//print_r($qry); 
    ...
    // $qry = "select * 
	        // from cppmod_ssb_doc A LEFT JOIN cppmod_ssb_tranmain AS B ON A.CPM_SSB_ID = B.CPM_TRAN_SSB_ID
            // where A.CPM_WP_NOKTP = '{$noktp}' and CPM_OP_THN_PEROLEH= '{$tahun}' 
			// AND B.CPM_TRAN_STATUS <> 4 AND B.CPM_TRAN_FLAG <> 1 AND A.CPM_OP_JENIS_HAK <> 30 AND A.CPM_OP_JENIS_HAK <> 31 
			// AND A.CPM_OP_JENIS_HAK <> 32 AND A.CPM_OP_JENIS_HAK <> 33";
// //print_r($qry); 
    // $res = mysqli_query($qry, $DBLink);
    // if ($res === false) {
        // return false;
    // }
