...
class displayDocument
{
	...
	private function insertToGateway($ssbid, $opr = "")
	{
		...
		$bul = $pecahkan[1];
		$tgl = $pecahkan[2];
		// var_dump($pecahkan[2]);die;
		$tahbul = $kode_daerah . $tah . $bul;
		$tahbull = $kode_daerah . $tah . $bul . $tgl;

		/* 	---------------------------------------------------------------------------------
		|	KODE BAYAR 
		*/
		// $query = "SELECT MAX(SUBSTRING(payment_code,9, {$length})) nomor FROM ssb WHERE PAYMENT_CODE LIKE '{$tahbul}%________'";
		$query = "SELECT MAX(CAST(SUBSTRING(payment_code, 9, LENGTH(payment_code) - 10) AS UNSIGNED)) AS nomor FROM ssb WHERE payment_code LIKE '{$tahbul}%'";
		...
		*/
		...
		$query = "SELECT RIGHT(ntpd, 2) AS urut FROM ssb WHERE LEFT(ntpd, 10) = '{$tahbull}' ORDER BY RIGHT(ntpd, 2) DESC LIMIT 1";
		// var_dump($query);die;
		$res = mysqli_query($DBLinkLookUp, $query);
		$ntpd_x = 1;
		if ($data = mysqli_fetch_assoc($res)) {
	...
	//         echo mysqli_error($DBLink);
	//     } else {
	//         echo "Data Berhasil disimpan ke versi terbaru...!";
	...
	function display()
	{
		...
		if ($data->CPM_TRAN_STATUS == '5' || $data->CPM_TRAN_STATUS == '2' || $data->CPM_TRAN_STATUS == '3') {
			...
			if ($this->right != 1 || ($data->CPM_TRAN_STATUS == '3' && $data->CPM_TRAN_FLAG == '0')) {
				$draf = 3;
				if ($data->CPM_PAYMENT_TIPE == 2) {
					$draf = 1;
				}
			}
			if ($data->CPM_TRAN_STATUS == '5' && $data->CPM_TRAN_FLAG == '0' || ($data->CPM_TRAN_STATUS == '2'))
				$setuju = 1;
			...
			if ($setuju == 1 || $draf == 3) {
				if ($data->CPM_PAYMENT_TIPE != 2) {
					...
					$infoReject = "<div align=\"right\" class=\"download\">Print SSPD Kurang Bayar
							...
							title=\"Dokumen PDF\" onclick=\"printToPDFKurangBayar('$param');\" ></div><br>

							<div align=\"right\" class=\"download\">Print SKPDKB
							<img src=\"./image/icon/document-pdf-text.png\" width=\"16px\" height=\"16px\" 
							title=\"Dokumen PDF\" onclick=\"printToPDFSKPDKB('$param');\" ></div>
							";

					$infoKetetapanKB = "<div align=\"right\" style=\"margin-bottom:20px\">Print Ketetapan Kurang Bayar
		  ...
		  <tr>
			...
			<tr>
			  ...
			  <td><table width=\"900\" border=\"0\" cellspacing=\"1\" cellpadding=\"4\">
				  ...
				  <tr>
					<td>Nilai Perolehan Objek Pajak Kena Pajak(NPOPKP)asdasaa</td>
