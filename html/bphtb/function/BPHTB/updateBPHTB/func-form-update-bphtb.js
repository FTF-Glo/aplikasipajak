	...
	for (var i=0, len=els.length; i<len; i++){
		...
		if (x.type == 'text') {			
			...
			if (x.value == '') {
				...
				// if ((i != 30) && (i != 32)  && (i != 38) && (i != 39)&&(i != 3)&&(i != 11)&&(i != 14)&&(i != 48)&&(i != 49)) {
			  	if ((i != 37)  && (i != 38)&& (i != 39)&&(i != 46)&&(i != 47) && (i != 48)) {

					var ge = document.getElementById('err_'+i);

					if(rb==1) {
						if(els.item(i).id!="jsb-choose-number" && els.item(i).id!="jsb-choose-date" && els.item(i).id!="jsb-choose-fraction2"){
							if (!ge) {
								imgEx[i]=document.createElement("img");
								imgEx[i].setAttribute("src","./image/icon/exclamation.png");
...
function NPOPKP () {
	...
	var NPOPTKP = parseFloat(document.getElementById('hd-npoptkp').value);
	var NPOPKP=0;
	if ((s1+s2) > t ) {
	...
	//console.log(s1+'-'+s2+'-'+t+'-'+NPOPTKP+'-'+NPOPKP)
	document.getElementById('tNPOPKP').value = NPOPKP;
...
function loadNPOPTKP() {
    ...
    Ext.Ajax.request({
        ...
        method: 'POST',
        params: {id: d, axx: axx, noktp: noktp,trsid:trsid,nop:nop},
        success: function(result, request) {
            var jsonData = JSON.parse(result.responseText);
            if (jsonData.success) {
