...
class displayDocument {
    ...
    private function insertToGateway($ssbid, $opr = "") {
        ...
        while ($rw = mysqli_fetch_array($rs)) {
            ...
            $ccc = $this->getBPHTBPayment($rw['CPM_OP_LUAS_BANGUN'], $rw['CPM_OP_NJOP_BANGUN'], $rw['CPM_OP_LUAS_TANAH'], $rw['CPM_OP_NJOP_TANAH'], $rw['CPM_OP_HARGA'], $rw['CPM_PAYMENT_TIPE_PENGURANGAN'], $rw['CPM_OP_JENIS_HAK'], $rw['CPM_OP_NPOPTKP'], $rw['CPM_PENGENAAN'], $rw['CPM_APHB'], $rw['CPM_DENDA']);
...
where b.CPM_SSB_AUTHOR = '$notaris'";
        ...
        SCANPayment_ConnectToDB($DBLinkLookUp, $DBConn2, $dbHost, $dbUser, $dbPwd, $dbName);
			...
			$query = "SELECT MAX(SUBSTRING(payment_code,5, {$length})) nomor FROM ssb WHERE PAYMENT_CODE LIKE '{$tahbul}%_____{$search_code}%'";
		...
		if (intval($ccc) == 0){
			...
			payment_flag,op_luas_tanah,op_luas_bangunan,bphtb_npop,bphtb_jenis_hak,bphtb_notaris,wp_npwp,author,payment_paid,payment_bank_code,PAYMENT_SETTLEMENT_DATE,bphtb_collectible, payment_code) VALUES (
			...
			DATE_ADD(DATE(saved_date), INTERVAL {$dbLimit} DAY),'$payment_flag','$luas_tanah','$luas_bangunan','$npop','$hak','$notaris','$npwp', '$author','$payment_paid','$payment_bank_code','$payment_settlement_date','$bphtb_collectible', '$payment_code')";
		}else{
			$query2 = "INSERT INTO $dbTable (wp_nama,wp_alamat,wp_rt,wp_rw,wp_kelurahan,wp_kecamatan,wp_kabupaten,wp_kodepos,op_letak,
			op_rt,op_rw,op_kelurahan,op_kecamatan,op_kabupaten,bphtb_dibayar,op_nomor,saved_date,wp_noktp,id_switching,expired_date,
			payment_flag,op_luas_tanah,op_luas_bangunan,bphtb_npop,bphtb_jenis_hak,bphtb_notaris,wp_npwp,author, payment_code) VALUES (
			'$wp_nama','$wp_alamat','$wp_rt','$wp_rw','$wp_kelurahan','$wp_kecamatan','$wp_kabupaten','$wp_kodepos','$op_letak',
			'$op_rt','$op_rw','$op_kelurahan','$op_kecamatan','$op_kabupaten','$bphtb_dibayar','$op_nomor','$saved_date','$noktp','$id_switching',
			DATE_ADD(DATE(saved_date), INTERVAL {$dbLimit} DAY),'$payment_flag','$luas_tanah','$luas_bangunan','$npop','$hak','$notaris','$npwp', '$author', '$payment_code')";
			
		}
		// echo $query2;die(var_dump($DBLinkLookUp));
    ...
    function setSubmitNewVersion($sts) {
		...
		$info = @isset($_REQUEST['info-Upt']) ? $_REQUEST['info-Upt'] : "";
		...
		CPM_NO_PENDAFTARAN, CPM_SSB_ID,CPM_KPP,
		...
		$query = sprintf("UPDATE cppmod_ssb_berkas SET CPM_INFO_UPDATE_UPT ='%s'
			WHERE CPM_SSB_DOC_ID='%s' ", $info ,$ssb_id);
        $result = mysqli_query($DBLink, $query);
        
		$query = sprintf("UPDATE cppmod_ssb_berkas SET CPM_USER_UPT_UPDATE ='%s'
			WHERE CPM_SSB_DOC_ID='%s' ", mysqli_real_escape_string($DBLink, $nokhd) ,$ssb_id);
        $result = mysqli_query($DBLink, $query);
        ...
        $query = sprintf("UPDATE cppmod_ssb_tranmain SET CPM_TRAN_FERIF_LAPANGAN='%s'
			WHERE CPM_TRAN_ID='%s' ", 99, $_REQUEST['trsid']);
        $result = mysqli_query($DBLink, $query);
        ...
        $query = "INSERT INTO cppmod_ssb_tranmain (CPM_TRAN_ID,CPM_TRAN_REFNUM,CPM_TRAN_SSB_ID,
			...
			CPM_TRAN_OPR_NOTARIS,CPM_TRAN_FERIF_LAPANGAN) VALUES
			 ('" . $tranid . "','" . $refnum . "','" . $iddoc . "','" . $version . "','" . $sts . "','0','" . $trdate . "','0','" . $opr . "',99)";
        //echo $query;
        $result = mysqli_query($DBLink, $query);
        if ($result === false) {
            //handle the error here
				...
				if ($this->approved && $this->claim && (!$this->stsrejected)) {
					$HTML .= "<tr><td>&nbsp;</td><td><table width=\"450\" border=\"0\" cellspacing=\"1\" cellpadding=\"8\" style=\"background-color:#f1c3e2\">
					  ...
					  <tr>
						...
						<td class=\"result-analisis\">
							<label>
							  <input type=\"radio\" name=\"RadioGroup99\" value=\"91\" id=\"RadioGroup99_1\" checked disabled/>
							  Kelengkapan Document
						  	</label>
							<label>
							  <input type=\"radio\" name=\"RadioGroup99\" value=\"92\" id=\"RadioGroup99_2\" disabled/>
							  Verifikasi Lapagan
						  	</label>
					  	</td>
					  </tr>
...
//print_r($qry); 
    ...
    function formSSB($dat, $edit) {
		...
		$btn_upload1 = "<input type='button' id='btn_1' name='btn_1' value='Upload'  onclick=\"upload('file_lamp1',  1);\">";
		$btn_upload2 = "<input type='button' id='btn_2' name='btn_2' value='Upload'  onclick=\"upload('file_lamp2',  2);\">";
		$btn_upload3 = "<input type='button' id='btn_3' name='btn_3' value='Upload'  onclick=\"upload('file_lamp3',  3);\">";
        $msgClaim = "";
		$berkas_berkas = 1;
	...
	$hitungAPHB = $this->getConfigValue("1",'HITUNG_APHB');
        $html = "<link rel=\"stylesheet\" href=\"./function/BPHTB/VerifikasiLpgn/func-detail-upt.css\" type=\"text/css\">\n";
        ...
        $html .= "<script src=\"./function/BPHTB/VerifikasiLpgn/func-new-upt.js?ver=0\"></script>\n";        
...
function checkDukcapil(){
	...
	$qryn = "SELECT * FROM cppmod_ssb_berkas WHERE CPM_SSB_DOC_ID ='".$dat->CPM_SSB_ID."'";
	$id_berkas = '';
	$bukti1 = '';
	$bukti2 = '';
	$bukti3 = '';
	$infofromUpt = '';
	$res = mysqli_query($DBLink, $qryn);
	while($key = mysqli_fetch_assoc($res)){
		$id_berkas = $key['CPM_BERKAS_ID'];
		$infofromUpt = $key['CPM_INFO_UPDATE_UPT'];
		if ($key['CPM_UPLOAD_UPT1']!='') {
			$bukti1="<a href ='function/BPHTB/VerifikasiLpgn/{$key['CPM_UPLOAD_UPT1']}' target='_blank'>Download/view</a>";
		}
		if ($key['CPM_UPLOAD_UPT2']!='') {
			$bukti2="<a href ='function/BPHTB/VerifikasiLpgn/{$key['CPM_UPLOAD_UPT2']}' target='_blank'>Download/view</a>";
		}
		if ($key['CPM_UPLOAD_UPT3']!='') {
			$bukti3="<a href ='function/BPHTB/VerifikasiLpgn/{$key['CPM_UPLOAD_UPT3']}' target='_blank'>Download/view</a>";
		}
	}
	$html .="
	function upload(id_file,i){
		var form_data = new FormData();
		form_data.append('file', document.getElementById(id_file).files[0]);
		$.ajax({
			url:'function/BPHTB/VerifikasiLpgn/upload-bukti.php?id_berkas=".$id_berkas."&nobukti='+i,
			type:'POST',
			data: form_data,
			contentType: false,
			cache: false,
		...
		<div id=\"main-content\"><form id=\"form-notaris\" name=\"form-notaris\" method=\"post\" action=\"\" onsubmit=\"return checkform();\">
				...
				<tr>
				  ...
				  <td><table width=\"900\" border=\"0\" cellspacing=\"1\" cellpadding=\"4\">
					<tr>
					  ...
					  <td width=\"210\"><input type=\"text\" name=\"name2\" id=\"name2\" value=\"" . $dat->CPM_OP_NOMOR . "\" onBlur=\"checkNOP(this);\" maxlength=\"18\" size=\"25\"  " . $readonly . " title=\"NOP PBB\"/></td>
					  <td width=\"100\">Nama WP Lama : </td>
					  <td><input type=\"text\" name=\"nama-wp-lama\" id=\"nama-wp-lama\" value=\"" . $dat->CPM_WP_NAMA_LAMA . "\" size=\"35\" maxlength=\"30\" title=\"Nama WP Lama\" onKeyUp=\"javascript:{this.value = this.value.toUpperCase(); }\"/>
					  </td>
					  </tr>
					<tr valign='top'>
					  <td><div align=\"right\">2.</div></td>
					  <td>Lokasi Objek Pajak</td>
					  <td><textarea  name=\"address2\" id=\"address2\" onKeyUp=\"javascript:{this.value = this.value.toUpperCase(); }\" cols=\"35\" rows=\"4\" title=\"Lokasi Objek Pajak\" " . $readonly . ">" . str_replace("<br />", "\n", $dat->CPM_OP_LETAK) . "</textarea></td>
                                          <td width=\"100\">Nama WP Sesuai Sertifikat : </td>
					  <td><input type=\"text\" name=\"nama-wp-cert\" id=\"nama-wp-cert\" value=\"" . $dat->CPM_WP_NAMA_CERT . "\" size=\"35\" maxlength=\"30\" title=\"Nama WP Sesuai Sertifikat\" onKeyUp=\"javascript:{this.value = this.value.toUpperCase(); }\"/>
					  </td>    
					</tr>
					<tr>
					...
					<tr>
					  ...
					  <td><input type=\"text\" name=\"certificate-number\" id=\"certificate-number\" value=\"" . $dat->CPM_OP_NMR_SERTIFIKAT . "\" onKeyPress=\"return numbersonly(this, event)\"  size=\"50\" maxlength=\"50\" " . $readonly . " title=\"Nomor Sertifikat\"/></td>
					</tr>
				  </table>
				  </td>
				</tr>
				<tr>
				  ...
				  <td width=\"97%\"><table width=\"900\" border=\"0\" cellspacing=\"1\" cellpadding=\"4\">
					<tr>
					  ...
					  <td width=\"\"><input type=\"text\" name=\"name\" id=\"name\" onKeyUp=\"javascript:{this.value = this.value.toUpperCase(); }\" value=\"" . $dat->CPM_WP_NAMA . "\" onkeypress=\"return nextFocus(this,event)\" size=\"60\" maxlength=\"60\" " . $readonly . " title=\"Nama Wajib Pajak\"/></td>
					</tr>
					<tr>
					  <td><div align=\"right\">2.</div></td>
					  <td>NPWP</td>
					...
					<tr>
					  <td><div align=\"right\">3.</div></td>
					  <td>Nomor KTP</td>
					  <td><input type=\"text\" name=\"noktp\" id=\"noktp\" value=\"" . $dat->CPM_WP_NOKTP . "\" onkeypress=\"return nextFocus(this,event)\" size=\"24\" maxlength=\"24\" title=\"No KTP\" onblur=\"checkTransLast();checkTransaksi();\" />&nbsp;&nbsp;<input type=\"button\" name=\"checkKTP\" id=\"checkKTP\" value=\"Ambil Data Dukcapil\" class=\"myButton\" onclick=\"checkDukcapil();checkTransLast();\"><img src=\"./image/icon/loading.gif\" id=\"loaderCek\"><div id=\"newl\"></div></td>
					</tr>
					<tr valign='top'>
					  <td><div align=\"right\">4.</div></td>
			...
			m²</td>
			  ...
			  <tr>
				...
				<br>
		11. Harga Transaksi Rp. <input type=\"text\" name=\"trans-value\" id=\"trans-value\" value=\"" . $dat->CPM_OP_HARGA . "\" onKeyPress=\"return numbersonly(this, event)\"  onchange=\"checkTransaction()\" title=\"Harga Transaksi\"/ onblur=\"loadLaikPasar();\">
			  </td>
				</tr>
...
<tr>
		...
		$upttxt='';
		if ($dat->CPM_TRAN_FERIF_LAPANGAN == 92) {
			$upttxt = "
				<tr>
					<td colspan='2'>Bukti 1</td>
					<td>
						<input onchange='check_extension(this.value)' type='file' name='file_lamp1' id='file_lamp1'>
						".$btn_upload1."
						<span   id='uploaded_image_1'>".$bukti1."</span>
					</td>
				</tr>
				<tr>
					<td colspan='2'>Bukti 2</td>
					<td>
						<input onchange='check_extension(this.value)' type='file' name='file_lamp2' id='file_lamp2'>
						".$btn_upload2."
						<span   id='uploaded_image_2'>".$bukti2."</span>
					</td>
				</tr>
				<tr>
					<td colspan='2'>Bukti 3</td>
					<td>
						<input onchange='check_extension(this.value)' type='file' name='file_lamp3' id='file_lamp3'>
						".$btn_upload3."
						<span   id='uploaded_image_3'>".$bukti3."</span>
					</td>
				</tr>
				<tr>
				  <td align=\"center\" valign=\"top\"></td>
				  <td align=\"right\" valign=\"top\"></td>
				  <td valign=\"top\"><textarea name=\"info-Upt\" id=\"info-Upt\" cols=\"35\" rows=\"5\" title=\"Lain-lain\">{$infofromUpt}</textarea>
				  </td>
				</tr>";
		}
					//echo $qry;exit;
...
$html .="			      </select></td>
				...
				<tr>
				  ...
				  <td align=\"right\" valign=\"top\"></td>
				  <td valign=\"top\">
				  		<!--Khusus untuk waris dan Hibah Pengurangan dihitung sendiri menjadi -->
				  		<input type=\"text\" name=\"jsb-choose-fraction1\" id=\"jsb-choose-fraction1\" size=\"1\" maxlength=\"2\" value=\"".$typePecahan[0]."\" ".$readonly." ".$r5." title=\"pecahan 1\" onkeyup=\"checkTransaction()\" onKeyPress=\"return numbersonly(this, event)\" hidden=\"hidden\"/>
				  		<input type=\"text\" name=\"jsb-choose-fraction2\" id=\"jsb-choose-fraction2\" size=\"1\" maxlength=\"2\" value=\"".$typePecahan[1]."\" ".$readonly." ".$r6." title=\"pecahan 2\" onkeyup=\"checkTransaction()\" onKeyPress=\"return numbersonly(this, event)\" hidden=\"hidden\"/></td>
				</tr>
				".$upttxt."
			  </table></td>
				</tr>
				<tr>
				<td colspan=\"2\" align=\"center\" valign=\"middle\" id=\"jmlBayar\">Jumlah yang dibayarkan : " . number_format($tNPOPKP * 0.05, 0, '.', ',') . "</td>
				</tr>
	...
	function getSPPTInfo($idssb) {
		...
		$query = "SELECT PAYMENT_FLAG, PAYMENT_PAID FROM $DbTable WHERE id_switching = '".$idssb."' ORDER BY saved_date DESC limit 1  "; return $query;
    ...
    function display() {
        ...
        if ($data->CPM_TRAN_STATUS == '5' || $data->CPM_TRAN_STATUS == '2' || $data->CPM_TRAN_STATUS == '3') {
            ...
            if ($setuju == 1 || $draf == 3){
				if ($data->CPM_PAYMENT_TIPE!=2){
				$infoReject = "<div align=\"right\"><label onclick=\"printToPDF('$param');\">Print to PDF
				<img src=\"./image/icon/adobeacrobat.png\" width=\"16px\" height=\"16px\" 
				title=\"Dokumen PDF\"></label></div><br/>";
        ...
        if ($this->approved) {
            $html .= "<script language=\"javascript\" src=\"./function/BPHTB/dispenda/func-display-dispenda.js\" type=\"text/javascript\"></script>";
            if (($this->dispenda1) && (($data->CPM_TRAN_STATUS != '3') && ($data->CPM_TRAN_STATUS != '5'))){
                $addApp = $this->addApprove();
			}
            if (($this->dispenda2) && ($data->CPM_TRAN_STATUS == '3')){
                $addApp = $this->addApprove();
			}
        }
		...
		<form id=\"form-notaris\" name=\"form-notaris\" method=\"post\" action=\"\" onsubmit=\"return checkform();\">
			...
			<tr>
			  ...
			  <td><table width=\"900\" border=\"0\" cellspacing=\"1\" cellpadding=\"4\">
				<tr>
				  ...
				  <td width=\"100\">Nama WP Lama :</td>
				  <td width=\"\" class=\"document-x\">" . $data->CPM_WP_NAMA_LAMA . "</td>
				  </tr>
				<tr>
				  <td><div align=\"right\">2.</div></td>
				  <td>Lokasi Objek Pajak</td>
				  <td class=\"document-x\">" . $data->CPM_OP_LETAK . "</td>
                                  <td width=\"100\">Nama WP Sesuai Sertifikat :</td>
				  <td width=\"\" class=\"document-x\">" . $data->CPM_WP_NAMA_CERT . "</td>    
				</tr>
				<tr>
				  <td><div align=\"right\">3.</div></td>
				  <td>Kelurahan/Desa</td>
		...
		$html .="<tr>
			...
			<tr>
			  ...
			  <td><table width=\"900\" border=\"0\" cellspacing=\"1\" cellpadding=\"4\">
				<tr>
				  <td colspan=\"3\"><strong>Jumlah Setoran tes Berdasarkan</strong> : $typepayment</td>
				</tr>
				<tr>
			...
			" . $addApp . "
			<tr>
				<td colspan=\"2\" align=\"center\" valign=\"middle\" style=\"border-radius: 10px 10px 10px 10px;\" id=\"jmlBayar\">Jumlah yang dibayarkan : " . number_format($this->getBPHTBPayment_all(6), 0, '.', ',') . "</td>
				</tr>
			".$kurang."
		  </table>
		</form></div>
