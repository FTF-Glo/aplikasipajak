...
class PS_Pagination {
	...
	function renderFirst($tag = 'First') {
		...
		if ($this->page == 1) {
			...
			$L_encode= $this->php_self . "?param=" . base64_encode($this->append."&page=1");
			return '<a href="' . $L_encode . '">' . $tag . '</a> ';
		}
	}
	...
	function renderLast($tag = 'Last') {
		...
		if ($this->page == $this->max_pages) {
			...
			$L_encode = $this->php_self . "?param=" . base64_encode($this->append."&page=".$this->max_pages);
			return ' <a href="' . $L_encode . '">' . $tag . '</a>';
		}
	}
	...
	function renderNext($tag = '&gt;&gt;') {
		...
		if ($this->page < $this->max_pages) {
			...
			$L_encode = $this->php_self . "?param=" . base64_encode($this->append."&page=".($this->page + 1));
			return '<a href="' . $L_encode . '">' . $tag . '</a>';
		} else {
			return $tag;
		}
	...
	function renderNav($prefix = '<span class="page_link">', $suffix = '</span>') {
		...
		for($i = $start; $i <= $end; $i ++) {
			if ($i == $this->page) {
				...
				$L_encode= $this->php_self . "?param=" . base64_encode($this->append."&page=".$i);
				$links .= ' ' . $prefix . '<a href="'.$L_encode.'">' . $i . '</a>' . $suffix . ' ';
			}
		}
