...
//echo $dbName . ' ' . $dbHost . ' ' .$dbPwd . ' ' .$dbUser . '<br>';
//$conn = mysql_connect($dbHost, $dbUser, $dbPwd);
$conn = mysqli_connect($dbHost, $dbUser, $dbPwd, $dbName);
if ($conn) {
    // Akses Basis Data
    $qry_config="select CTR_AC_VALUE from central_app_config where CTR_AC_AID='aPBB' and CTR_AC_KEY='tahun_tagihan'";
    ...
    $query = "SELECT A.NOP, A.WP_NAMA, A.WP_ALAMAT, A.WP_KELURAHAN, A.WP_RT, A.WP_RW, A.WP_KECAMATAN, A.WP_KOTAKAB, A.WP_KODEPOS,
            ...
            FROM
                {$db} A
            LEFT JOIN cppmod_pbb_sppt_final B ON A.NOP = B.CPM_NOP
            LEFT JOIN cppmod_pbb_sppt_susulan C ON C.CPM_NOP=A.NOP
            LEFT JOIN cppmod_pbb_sppt D ON D.CPM_NOP = A.NOP
...
cppmod_pbb_sppt_final A LEFT JOIN 
cppmod_pbb_sppt_current B ON A.CPM_NOP = B.NOP LEFT JOIN 
cppmod_pbb_sppt_susulan C ON C.CPM_NOP=A.CPM_NOP LEFT JOIN 
cppmod_pbb_sppt D ON D.CPM_NOP = A.CPM_NOP LEFT JOIN
cppmod_tax_kabkota E ON  E.CPC_TK_ID  = A.CPM_OP_KOTAKAB LEFT JOIN
						cppmod_tax_kecamatan F ON F.CPC_TKC_ID  = A.CPM_OP_KECAMATAN LEFT JOIN
                        cppmod_tax_kelurahan G ON G.CPC_TKL_ID = A.CPM_OP_KELURAHAN 
