...
function updateCounter($nop,$no) {
    global $DBLink;

    $qry = "Update cppmod_ssb_nop_split SET CPM_SSB_SP_NO = '$no' where CPM_SSB_SP_NOP = '$nop'";

    $result = mysqli_query($DBLink, $qry);
}

function insertCounter($nop) {
    global $DBLink;
...
function getNOPSplit($nop) {
    global $DBLink;
    $nop = str_replace("_", "", $nop);

    if (strlen($nop) == 18) {
        $qry = "SELECT * FROM cppmod_ssb_nop_split WHERE CPM_SSB_SP_NOP ='" . $nop . "' limit 1";
        $result = mysqli_query($DBLink, $qry);
		if(mysqli_num_rows($result)>0){
				while ($row = mysqli_fetch_assoc($result)) {
					updateCounter($nop,(strval($row["CPM_SSB_SP_NO"])+1));
...
function formSSB($val = array()) {
	global $DBLink;
    $nopfs = @isset($_REQUEST['nopfs']) ? $_REQUEST['nopfs'] : getConfigValue('PREFIX_NOP');
	
    $vp = "{'vnop':'','vaddres':'','vkelurahan':'','vrt':''
				,'vrw':'','vkecamatan':'','vkabupaten':'','vzip','',
				,'vryear':'','vzip':'','vwplama':'','vwpcert':'','vrlandarea':''
				,'vrlandnjop':'', 'vrbuildingarea':'','vrbuildingnjop':'','vrcert':''}";
	...
	$readonlyvrlandnjop = "";
	$readonlyvrbuildingnjop = "";
	$colornjop = "";
	$getSplitNo="";
	$jpar = json_decode($vpar);
	//echo $vpar;
    if ($jpar->vnop!="") {
        $json = new Services_JSON(SERVICES_JSON_SUPPRESS_ERRORS);
        $jpar = json_decode($vpar);
		$getSplitNo = "getNOPSplit();";
		$readonlyvrlandnjop = "readonly=\"readonly\"";
		$readonlyvrbuildingnjop = "readonly=\"readonly\"";
		$colornjop = " $(\"#land-njop\").css('background-color','#eeeeee');
					   //$(\"#building-njop\").css('background-color','#eeeeee');";
    }
...
<div id=\"main-content\"><form name=\"form-notaris\" id=\"form-notaris\" method=\"post\" action=\"\" >
		  <table width=\"953\" border=\"0\" cellspacing=\"1\" cellpadding=\"8\">
			<tr>
			  <td colspan=\"2\" style=\"border-radius: 10px 10px 0px 0px;\"><strong><font size=\"+2\">Form Surat Setoran Pajak Daerah <br>Bea Perolehan Hak Atas Tanah dan Bangunan <br>(SSPD-BPHTB)</font></strong><br /><br />
			</tr>
			...
			<tr>
			  ...
			  <td><table width=\"900\" border=\"0\" cellspacing=\"1\" cellpadding=\"4\">
				<tr>
				  ...
				  <td width=\"30%\"><input type=\"text\" name=\"name2\" id=\"name2\" value=\"" . $nopfs . "\ onKeyPress=\"return nextFocus(this, event)\" size=\"25\" title=\"NOP PBB\"/>
				  <input type=\"text\" readonly size=\"3\" name=\"nop-split\" id=\"nop-split\"></td>
				  <td>Nama WP Lama : </td>
				...
				<tr>
				  ...
				  <td>&nbsp;</td>
				  <td><input type=\"text\" name=\"op-znt\" id=\"op-znt\" hidden=\"hidden\" /></td>
				</tr>
				<tr>
				...
				<tr>
				  ...
				  <td><input type=\"text\" name=\"certificate-number\" id=\"certificate-number\" size=\"50\" maxlength=\"50\" title=\"Nomor Sertifikat Tanah\" value=\"{$jpar->vrcert}\" /></td>
				</tr>
			  </table>
			<tr>
			  <td width=\"3%\" align=\"center\" valign=\"top\"><div  id='rcorners3'><font size='+1' color=\"white\"><b>B. </b></font></div></td>
			  <td width=\"100%\"><table width=\"900\" border=\"0\" cellspacing=\"1\" cellpadding=\"4\">
				<tr>
				  <td width=\"2%\"><div align=\"right\">1.</div></td>
				  <td>Nomor KTP</td>
				  <td><input type=\"text\" name=\"noktp\" id=\"noktp\" onkeypress=\"return nextFocus(this,event)\" size=\"16\" maxlength=\"24\" title=\"No KTP wajib di isi\" onblur=\"checkTransLast();\"/>&nbsp;&nbsp;<input type=\"button\" name=\"checkKTP\" id=\"checkKTP\" value=\"Ambil Data Dukcapil\" class=\"myButton\" onclick=\"checkDukcapil();checkTransLast();\"><img src=\"./image/icon/loading.gif\" id=\"loaderCek\"><div id=\"newl\"></div></td>
				  <td>&nbsp;</td>
				  <td>&nbsp;</td>
				</tr>
				<tr>
			    ...
			    <tr>
			  </table><table width=\"900\" border=\"1\" cellspacing=\"1\" cellpadding=\"4\" style=\"border: 1px solid black;border-collapse: collapse;\" class=\"pure-table\"><thead>
		  <tr>
...
berdasarkan SPPT PBB terakhir sebelum terjadinya peralihan hak 
			  <input type=\"text\" name=\"right-year\" id=\"right-year\" maxlength=\"4\" size=\"4\" onKeyPress=\"return numbersonly(this, event)\" title=\"Tahun Pajak\" value=\"{$jpar->vryear}\"/></th>
		  ...
		  <tr>
			<td align=\"center\" valign=\"middle\" bgcolor=\"#CCCCCC\"><input type=\"text\" name=\"land-area\" id=\"land-area\" onKeyPress=\"return numbersonly(this, event)\" value=\"{$jpar->vrlandarea}\" onkeyup=\"addSN();addET();checkTransaction();\" title=\"Luas Tanah\"/>
			  m²</td>
			<td align=\"center\" valign=\"middle\" bgcolor=\"#999999\"><input type=\"text\" name=\"land-njop\" id=\"land-njop\" ".$readonlyvrlandnjop." value=\"{$jpar->vrlandnjop}\" onKeyPress=\"return numbersonly(this, event)\"  onkeyup=\"addSN();addET();checkTransaction();\" title=\"NJOP Tanah\"/></td>
		  ...
		  <tr>
			<td align=\"center\" valign=\"middle\" bgcolor=\"#CCCCCC\"><input type=\"text\" name=\"building-area\" id=\"building-area\" onKeyPress=\"return numbersonly(this, event)\" value=\"{$jpar->vrbuildingarea}\" onkeyup=\"addET();addSN();checkTransaction();\" title=\"Luas Bangunan\"/>
		m²</td>
			<td align=\"center\" valign=\"middle\" bgcolor=\"#999999\"><input type=\"text\" name=\"building-njop\" id=\"building-njop\" ".$readonlyvrbuildingnjop." value=\"{$jpar->vrbuildingnjop}\" onKeyPress=\"return numbersonly(this, event)\" onkeyup=\"addET();addSN();checkTransaction();\" title=\"NJOP Bangunan\"/></td>
			...
			<tr>
			  ...
			  <td><table width=\"900\" border=\"0\" cellspacing=\"1\" cellpadding=\"4\">
				<tr>
				  ...
				  <td width=\"188\" id=\"akumulasi\" align=\"right\"></td></td>
				</tr>
			  </table></td>
			</tr>
			<tr>
...
$html .="			      </select></td>
			...
			<td colspan=\"2\" align=\"center\" valign=\"middle\"><input type=\"button\" name=\"btn-save-x\" id=\"btn-save-x\" value=\"Simpan\" onClick = \"submitForm(1);\"  class=\"button-success pure-button\"/>
			  &nbsp;&nbsp;&nbsp;
			  <input type=\"button\" name=\"btn-save-x\" id=\"btn-save-x\" value=\"Simpan dan Finalkan\" onClick = \"submitForm(2);\"  class=\"button-success pure-button\"/>
...
function getNoPel($type) {
    ...
    $jnsPerolehan = str_pad($nomor, 2,"0",STR_PAD_LEFT);
    $tahun = date("Y");
...
function save_berkas($idssb){
	global $DBLink,$data;
	$nop_saja = @isset($_REQUEST['name2']) ? $_REQUEST['name2'] : "";
	$urutan_no = @isset($_REQUEST['nop-split']) ? $_REQUEST['nop-split'] : "";
	$nop=$nop_saja.$urutan_no;
...
function getNOPBPHTB($nop) {
    ...
    $res = mysqli_query($DBLink, $qry, $DBLink);
    if ($res === false) {
        $Ok = false;
    }
...
function save($final) {
    ...
    $data[0] = "-";
    $data[1] = "-";
    $data[2] = @isset($_REQUEST['name']) ? str_replace("'"," ",$_REQUEST['name']) : "Error: Nama Wajib Pajak tidak boleh dikosongkan!";
    ...
    $data[40] = @isset($_REQUEST['nama-wp-lama']) ? str_replace("'"," ",$_REQUEST['nama-wp-lama']) : "Error: Nama WP lama tidak boleh dikosongkan!";
    $data[41] = @isset($_REQUEST['nama-wp-cert']) ? str_replace("'"," ",$_REQUEST['nama-wp-cert']) : "Error: Nama WP Sesuai Sertifikat tidak boleh dikosongkan!";
   ...
   $data[45] = @isset($_REQUEST['op-znt']) ? $_REQUEST['op-znt'] : "1"; 
    ...
    $bphtb_terutang = ($data[36] - $data[29]) * (5 / 100);
	$ccc = getBPHTBPayment($data[24],$data[25],$data[22],$data[23],$data[27],$data[39],$data[26],$data[29], $pengenaan, $aphbt, $denda);
    ...
    if ($result === false) {
        ...
        $vwplama = @isset($_REQUEST['nama-wp-lama']) ? str_replace("'"," ",$_REQUEST['nama-wp-lama']) : "";
        $vwpcert = @isset($_REQUEST['nama-wp-cert']) ? str_replace("'"," ",$_REQUEST['nama-wp-cert']) : "";
		$vrlandarea = @isset($_REQUEST['land-area']) ? $_REQUEST['land-area'] : "";
		$vrlandnjop = @isset($_REQUEST['land-njop']) ? $_REQUEST['land-njop'] : "";
		$vrbuildingarea = @isset($_REQUEST['building-area']) ? $_REQUEST['building-area'] : "";
		$vrbuildingnjop = @isset($_REQUEST['building-njop']) ? $_REQUEST['building-njop'] : "";
		$vrcert = @isset($_REQUEST['certificate-number']) ? $_REQUEST['certificate-number'] : "";
        $vparams = "{'vnop':'{$vnop}','vaddres':'{$vaddres}','vkelurahan':'{$vkelurahan}','vrt':'{$vrt}'
				...
				,'vrlandarea':'{$vrlandarea}','vrlandnjop':'{$vrlandnjop}','vrbuildingarea':'{$vrbuildingarea}'
				,'vrbuildingnjop':'{$vrbuildingnjop}','vrcert':'{$vrcert}'}";

        $params = "a=" . $_REQUEST['a'] . "&m=" . $_REQUEST['m'];
        if ($nxt)
            $params .= "&n=4&f=" . $_REQUEST['f'] . "&nopfs=" . $nfs . "&vpar=" . $vparams;
		if (strlen($vnop) == 18){
			getNOPSplit($vnop);
		 }
        //echo $nfs; 
...
if ($save == 'Simpan') {
    if (!getNOPBPHTB($_REQUEST['name2'] . $_REQUEST['nop-for-split']))
        save(1);
    else {
