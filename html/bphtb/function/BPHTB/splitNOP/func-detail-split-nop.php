...
//	$query = "SELECT * FROM cppmod_ssb_doc A, cppmod_ssb_tranmain B WHERE A.CPM_SSB_ID = B.CPM_TRAN_SSB_ID AND B.CPM_TRAN_STATUS=0 AND A.CPM_SSB_ID='".$id."'";
	...
	$query = sprintf("SELECT *,IF((TIMEDIFF(NOW(), B.CPM_TRAN_CLAIM_DATETIME))<'%s','1','0') AS CLAIM 
					...
					$maxtime,$sts,mysqli_real_escape_string($DBLink, $id));
...
function getnpoptkp($noktp) {
    ...
    $dbName = getConfigValue("1",'BPHTBDBNAME');
    $dbHost = getConfigValue("1",'BPHTBHOSTPORT');
    $dbPwd = getConfigValue("1",'BPHTBPASSWORD');
    $dbTable = getConfigValue("1",'BPHTBTABLE');
    $dbUser = getConfigValue("1",'BPHTBUSERNAME');
    $dbLimit = getConfigValue("1",'TENGGAT_WAKTU');
...
function formSSB($dat, $edit) {
	...
	if($sel_min!=0){
		$option_pengurangan="<option value=\"".$dat->CPM_KODE_PENGURANGAN.".".$dat->CPM_PENGURANGAN."\">Kode ".$dat->CPM_KODE_PENGURANGAN." : ".$da->CPM_PENGURANGAN."%</option>";
	}else{
			...
			<tr>
			  ...
			  <td width=\"97%\"><table width=\"650\" border=\"0\" cellspacing=\"1\" cellpadding=\"4\">
				...
				<tr>
				  <td><div align=\"right\">3.</div></td>
				  <td>Nomor KTP</td>
				  <td><input type=\"text\" name=\"noktp\" id=\"noktp\" value=\"".$dat->CPM_WP_NOKTP."\" onkeypress=\"return nextFocus(this,event)\" size=\"24\" maxlength=\"24\" title=\"No KTP wajib di isi\" onblur=\"checkTransLast();checkTransaksi();\"/></td>
				  <td>&nbsp;</td>
				  <td>&nbsp;</td>
				</tr>
				<tr>
			...
			<tr>
			  ...
			  <td><table width=\"780\" border=\"0\" cellspacing=\"1\" cellpadding=\"4\">
				<tr>
				  ...
				  <td width=\"30%\"><input type=\"text\" name=\"name2\" id=\"name2\" value=\"".$dat->CPM_OP_NOMOR."\" onBlur=\"checkNOP(this);\" onKeyPress=\"return nextFocus(this, event)\" size=\"25\"  readonly=\"readonly\" title=\"NOP PBB\"/></td>
				  <td>Nama WP Lama : </td>
		  ...
		  <tr>
			  ...
			  <tr>
                ...
                <tr>
				  ...
				  <td colspan=\"2\"><select name=\"right-land-build\" id=\"right-land-build\" onchange=\"checkTransLast();\">
				    <option value=\"1\" " . ($typeR == 1 ? "selected=\"selected\"" : "") . ">01. Jual Beli</option>
				    <option value=\"2\" " . ($typeR == 2 ? "selected=\"selected\"" : "") . ">02. Tukar Menukar</option>
				    <option value=\"3\" " . ($typeR == 3 ? "selected=\"selected\"" : "") . ">03. Hibah</option>
				    <option value=\"4\" " . ($typeR == 4 ? "selected=\"selected\"" : "") . ">04. Hibah Wasiat</option>
				    <option value=\"5\" " . ($typeR == 5 ? "selected=\"selected\"" : "") . ">05. Waris</option>
				    <option value=\"6\" " . ($typeR == 6 ? "selected=\"selected\"" : "") . ">06. Pemasukan dalam perseroan</option>
				    <option value=\"7\" " . ($typeR == 7 ? "selected=\"selected\"" : "") . ">07. APHB</option>
				    <option value=\"8\" " . ($typeR == 8 ? "selected=\"selected\"" : "") . ">08. Lelang</option>
				    <option value=\"9\" " . ($typeR == 9 ? "selected=\"selected\"" : "") . ">09. Putusan hakim</option>
				    <option value=\"10\" " . ($typeR == 10 ? "selected=\"selected\"" : "") . ">10. Penggabungan usaha</option>
				    <option value=\"11\" " . ($typeR == 11 ? "selected=\"selected\"" : "") . ">11. Pelebaran usaha</option>
				    <option value=\"12\" " . ($typeR == 12 ? "selected=\"selected\"" : "") . ">12. Pemekaran usaha</option>
				    <option value=\"13\" " . ($typeR == 13 ? "selected=\"selected\"" : "") . ">13. Hadiah</option>
				    <option value=\"14\" " . ($typeR == 14 ? "selected=\"selected\"" : "") . ">14. Jual Beli Khusus Perolehan hak RSS melalui KPR bersubsidi</option>
				    <option value=\"21\" " . ($typeR == 21 ? "selected=\"selected\"" : "") . ">21. Pemberian hak baru sebagai kelanjutan pelepasan hak</option>
				    <option value=\"22\" " . ($typeR == 22 ? "selected=\"selected\"" : "") . ">22. Pemberian hak baru diluar pelepasan hak</option>
			      </select></td>
			    <tr>
				<tr>
				  <td><div align=\"right\">16.</div></td>
			...
			<tr>
			  ...
			  <td><table width=\"650\" border=\"0\" cellspacing=\"1\" cellpadding=\"4\">
				...
				<tr>
				  <td align=\"center\" valign=\"top\"><input type=\"radio\" name=\"RadioGroup1\" value=\"5\" id=\"RadioGroup1_8\"  onclick=\"enableE(this,4);\" ".$c5." /></td>
				  <td align=\"right\" valign=\"top\">e.</td>
				  <td valign=\"top\">Khusus untuk waris dan Hibah Pengurangan dihitung sendiri menjadi <input type=\"text\" name=\"jsb-choose-fraction1\" id=\"jsb-choose-fraction1\" size=\"1\" maxlength=\"2\" value=\"".$typePecahan[0]."\" ".$readonly." ".$r5." title=\"pecahan 1\" onkeyup=\"checkTransaction()\" onKeyPress=\"return numbersonly(this, event)\"/>/<input type=\"text\" name=\"jsb-choose-fraction2\" id=\"jsb-choose-fraction2\" size=\"1\" maxlength=\"2\" value=\"".$typePecahan[1]."\" ".$readonly." ".$r6." title=\"pecahan 2\" onkeyup=\"checkTransaction()\" onKeyPress=\"return numbersonly(this, event)\"/></td>
				</tr>
			  </table></td>
			</tr>
			<tr>
...
*/
		...
		CPM_PAYMENT_TIPE_OTHER='%s',CPM_SSB_CREATED='%s',CPM_SSB_AUTHOR='%s',CPM_SSB_VERSION='%s',CPM_SSB_AKUMULASI='%s',CPM_PAYMENT_TIPE_KHD_NOMOR='%s',CPM_WP_NOKTP='%s',CPM_WP_NAMA_LAMA='%s',
                CPM_WP_NAMA_CERT='%s',CPM_PAYMENT_TIPE_PECAHAN='%s'
		WHERE CPM_SSB_ID ='%s'",
		'','',mysqli_real_escape_string($DBLink, $data[2]),
		mysqli_real_escape_string($DBLink, $data[3]),mysqli_real_escape_string($DBLink, nl2br($data[4])),mysqli_real_escape_string($DBLink, $data[7]),
		mysqli_real_escape_string($DBLink, $data[8]),mysqli_real_escape_string($DBLink, $data[6]),mysqli_real_escape_string($DBLink, $data[9]),mysqli_real_escape_string($DBLink, $data[10]),
		mysqli_real_escape_string($DBLink, $data[11]),mysqli_real_escape_string($DBLink, $data[12]),mysqli_real_escape_string($DBLink, nl2br($data[13])),
		mysqli_real_escape_string($DBLink, $data[16]),mysqli_real_escape_string($DBLink, $data[17]),mysqli_real_escape_string($DBLink, $data[15]),mysqli_real_escape_string($DBLink, $data[18]),
		mysqli_real_escape_string($DBLink, $data[19]),mysqli_real_escape_string($DBLink, $data[20]),mysqli_real_escape_string($DBLink, $data[21]),mysqli_real_escape_string($DBLink, $data[22]),
		mysqli_real_escape_string($DBLink, $data[24]),mysqli_real_escape_string($DBLink, $data[23]),mysqli_real_escape_string($DBLink, $data[25]),mysqli_real_escape_string($DBLink, $data[26]),
		mysqli_real_escape_string($DBLink, $data[27]),mysqli_real_escape_string($DBLink, $data[28]),mysqli_real_escape_string($DBLink, $data[29]),mysqli_real_escape_string($DBLink, $data[30]),
		mysqli_real_escape_string($DBLink, $typeSurat),mysqli_real_escape_string($DBLink, $typeSuratNomor),mysqli_real_escape_string($DBLink, $typeSuratTanggal),mysqli_real_escape_string($DBLink, $data[39]),
		mysqli_real_escape_string($DBLink, $typeLainnya),mysqli_real_escape_string($DBLink, $trdate),mysqli_real_escape_string($DBLink, $opr),mysqli_real_escape_string($DBLink, $version),
		mysqli_real_escape_string($DBLink, $data[36]),mysqli_real_escape_string($DBLink, $nokhd),mysqli_real_escape_string($DBLink, $data[38]),mysqli_real_escape_string($DBLink, $data[40]),mysqli_real_escape_string($DBLink, $data[41]), mysqli_real_escape_string($DBLink, $typePecahan), mysqli_real_escape_string($DBLink, $x));
		...
		CPM_TRAN_OPR_NOTARIS='%s' WHERE CPM_TRAN_ID='%s'","1","1",mysqli_real_escape_string($DBLink, $trdate),
		mysqli_real_escape_string($DBLink, $opr),mysqli_real_escape_string($DBLink, $_REQUEST['trsid']));
		$result = mysqli_query($DBLink, $query);
		...
		VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')",$idtran,$refnum,$x,$version,$final,'0',mysqli_real_escape_string($DBLink, $trdate),
