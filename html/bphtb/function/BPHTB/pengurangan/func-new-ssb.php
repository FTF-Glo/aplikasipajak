  ...
  <form name=\"form-notaris\" id=\"form-notaris\" method=\"post\" action=\"\" onsubmit=\"return checkform();\">
    ...
    berdasarkan SPPT PBB terakhir sebelum terjadinya peralihan hak 
		...
		m²</td>
          ...
          if (!empty($data->CPM_NO_KURANG_BAYAR) || $data->CPM_NO_KURANG_BAYAR != "") {
                $html .= " <tr>
                ...
                <tr>
                  ...
                  <td id=\"harusbayar\" align=\"right\">" . number_format(((strval($data->CPM_BPHTB_BAYAR))), 0, '.', ',') . "</td>

                  <input type=\"hidden\" name=\"sts_kurangbayar\" id=\"sts_kurangbayar\"  value=\"" . $data->CPM_NO_KURANG_BAYAR . "\" onKeyPress=\"return numbersonly(this, event)\"  \" title=\"Nilai Perolehan Objek Pajak Tidak Kena Pajak (NPOPTKP)\"/> 
                </tr>
                <tr>
                ...
                // var_dump($data);die;harusnya 
                $html .= "<tr>
                    <td>Bea Perolehan atas Hak Tanah dan Bangunan Pengurangan </td>
                    <td id=\"xBPHTBT\" align=\"right\"><input type=\"text\" name=\"tBPHTBTU\" id=\"tBPHTBTU\" value=\"" . $data->CPM_BPHTB_BAYAR . "\" onKeyPress=\"return numbersonly(this, event)\"  onkeyup=\"\" title=\"Bea Perolehan atas Hak Tanah dan Bangunan yang terutang\" readonly=\"readonly\"/></td>
                </tr> ";
          }else{
            ...
            <tr>
                <td>Bea Perolehan atas Hak Tanah dan Bangunan yang harusnya dibayar</td>
                <td id=\"harusbayar\" align=\"right\">" . number_format(((strval($data->CPM_OP_HARGA) - strval($data->CPM_OP_NPOPTKP)) * 0.05), 0, '.', ',') . "</td>
...
function save($final, $newVer = false)
{
  ...
  if (validation($data, $err)) {
    ...
    if ($result === false) {
      ...
      $sql = "UPDATE cppmod_ssb_doc SET CPM_BPHTB_BAYAR='$bphtb_bayar', statuspengurangan='1', nilaipengurangan='$bphtb_potongan',nilaipersen='$bphtb_persen' where CPM_SSB_ID ='$rejcs'";
    ...
    if ($newVer) {
      ...
      $query = sprintf("UPDATE cppmod_ssb_tranmain SET CPM_TRAN_SSB_VERSION='%s'
       WHERE CPM_TRAN_SSB_ID='%s' ", $version, $idssb_lama);
      $result = mysqli_query($DBLink, $query);
    }
    ...
    if ($result === false) {
      ...
      $Updatessb = sprintf("UPDATE gw_ssb.ssb SET bphtb_dibayar='%s'
       WHERE payment_code='%s' ", $bphtb_bayar, $kodebayar);
      $result = mysqli_query($DBLink, $Updatessb);
