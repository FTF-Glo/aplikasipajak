...
class displayDocument {
		...
		if(($jh==4)||($jh==5)||($jh==31)){
			...
			if($configAPHB=='1'){
				...
				if($hitungaphb=='1'){
					...
					$hbphtb_aphb = (($nop * $aphb)- strval($NPOPTKP)) * 0.05;
    ...
    private function insertToGateway($ssbid, $opr = "") {
        ...
        SCANPayment_ConnectToDB($DBLinkLookUp, $DBConn2, $dbHost, $dbUser, $dbPwd, $dbName);
			...
			//var_dump($payment_code, $kode_daerah);die;
    ...
    function setSubmitNewVersion($sts) {
		...
		$btnsave = @isset($_REQUEST['btn-save']) ? $_REQUEST['btn-save'] : "";
        ...
        if ($result === false) {
            ...
            if ($btnsave !="Simpan sebagai versi baru") {
		        $getdt = "select * FROM cppmod_ssb_berkas WHERE CPM_SSB_DOC_ID='".$iddoc."'";
    ...
    function display() {
        ...
        if ($data->CPM_TRAN_STATUS == '5' || $data->CPM_TRAN_STATUS == '2' || $data->CPM_TRAN_STATUS == '3') {
            ...
            if ($setuju == 1 || $draf == 3){
				if ($data->CPM_PAYMENT_TIPE!=2){
				$infoReject = "<div align=\"right\"><label onclick=\"printToPDF_view('$param');\">Print to PDF
		...
		$html .="<tr>
				  ...
				  <tr>
					<td><strong>Bea Perolehan atas Hak Tanah dan Bangunan yang harus dibayar</strong></td>
					<!-- <td id=\"tTotal\" class=\"document-x\" align=\"right\">" . number_format($data->CPM_BPHTB_BAYAR, 0, '.', ',') . "</td> -->
