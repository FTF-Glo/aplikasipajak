...
$sRootPath = str_replace('\\', '/', str_replace(DIRECTORY_SEPARATOR . 'function' . DIRECTORY_SEPARATOR . 'BPHTB' . DIRECTORY_SEPARATOR . 'dispenda' . DIRECTORY_SEPARATOR . 'penetapan', '', dirname(__FILE__))) . '/';
...
class displayDocument
{
  ...
  function setSubmit($type, $info, $info_disetujui, $vers)
  {
    ...
    // update NTPD Spn
    $idntpd= $this->jsondata->data[0]->CPM_SSB_ID;
    $updateNtpt="UPDATE cppmod_ssb_doc SET CPM_STATUS_NTPD=0 WHERE CPM_SSB_ID='$idntpd'";
    $result = mysqli_query($DBLink, $updateNtpt);
    // var_dump("ok");
    // die;
  ...
  function display()
  {
    $verifPenetapan= $this->viewVerifPenetapan();
    return $verifPenetapan;
   
  }

  function viewVerifPenetapan(){


    global $data, $DBLink, $sRootPath;

    $this->getData();#untuk menampilkan data pada laporan
    $data = $this->jsondata->data[0];
    $NPOPTKP = $data->CPM_OP_NPOPTKP;

    $addApp="";
    if($data->CPM_TRAN_STATUS != 5){
      $addApp .= $this->addApprove(); #untuk menambahkan form ditolak atau di terima beserta tombol kurang bayar
    }
    // $addApp .= $this->addApprove(); #untuk menambahkan form ditolak atau di terima beserta tombol kurang bayar
    $fieldTambahan = "<tr>
    ...
    $html .= "<script type=\"text/javascript\" src=\"inc/PBB/jquery-ui-1.8.18.custom/js/jquery-1.7.1.min.js\"></script>";
    $html .= "<script type=\"text/javascript\" src=\"inc/PBB/jquery-ui-1.8.18.custom/js/jquery-ui-1.8.18.custom.min.js\"></script>";
    $html .= "<script language=\"javascript\" src=\"./function/BPHTB/dispenda/penetapan/func-display-penetapan.js?N\" type=\"text/javascript\"></script>";
    ...
    $html .= "<form id=\"form-notaris\" name=\"form-notaris\" method=\"post\" action=\"\" onsubmit=\"return checkform(event);\">
		  ...
		  <tr>
			...
			<tr>
			  ...
			  <td><table width=\"900\" border=\"0\" cellspacing=\"1\" cellpadding=\"4\">
				  ...
				  <tr>
					<td>Nilai Perolehan Objek Pajak Tidak Kena Pajak (NPOPTKP) 222</td>
