...
where b.CPM_SSB_AUTHOR = '$notaris'";
        ...
        SCANPayment_ConnectToDB($DBLinkLookUp, $DBConn2, $dbHost, $dbUser, $dbPwd, $dbName);
        	...
        	$cnfg_code = $this->getConfigValue($a,'KODE_DAERAH');
			...
			$query = "SELECT MAX(SUBSTRING(payment_code,9, {$length})) nomor FROM ssb WHERE PAYMENT_CODE LIKE '{$tahbul}%_____{$search_code}%'";
            ...
            $payment_code = $cnfg_code.$tahbul.str_pad($nomor, $length, '0', STR_PAD_LEFT).$jns;
...
//        }
        ...
        $query = sprintf("UPDATE cppmod_ssb_tranmain SET CPM_TRAN_STATUS='%s',
        												CPM_TRAN_FLAG='%s',
        												CPM_TRAN_OPR_NOTARIS='%s',
        												CPM_TRAN_FERIF_LAPANGAN = '%s'
						WHERE CPM_TRAN_SSB_ID='%s' AND CPM_TRAN_STATUS='%s' AND CPM_TRAN_FLAG=0", $sts, "1", mysqli_real_escape_string($DBLink, $opr),
						$sts_lpgn, mysqli_real_escape_string($DBLink, $x), $sts);
        $result = mysqli_query($DBLink, $query);
