...
function inputKodeBayar()
{
	...
	$par1 = $params . "&f=funcKurangBayar&validasikb=1";
	...
	$html .= "<script language=\"javascript\">
				function checkPaymentCode() { 
					...
					$.ajax({
						type: \"post\",
						data: \"kodebayar=\" + jml,
						url: \"./function/BPHTB/notaris/svc-cek-kodebayar.php\",
						success: function(res) {
							...
							if (res==\"Nan\"){
								...
								alert(\"Kode Pembayaran tersebut belum melakukan Pembayaran\");
									...
									window.location = './main.php?param=" . base64_encode($par1) . "'+encodeBase64(\"&idssb=\"+res);
...
function formSSB($val = array())
{
	...
	$html = "
	<script src=\"function/BPHTB/notaris/func-kurang-bayar.js?ver=0\"></script>\n
		...
		m²</td>
		  ...
		  <tr>
		   ...
		   <tr>
			...
			<tr>
			  ...
			  <td><table width=\"900\" border=\"0\" cellspacing=\"1\" cellpadding=\"4\">
				  ...
				  <tr>
					...
					<td id=\"xNJOP\" align=\"right\"><input type=\"text\" name=\"tNPOP\" id=\"tNPOP\"  value=\"" . $data->CPM_OP_NPOP . "\"onKeyPress=\"return numbersonly(this, event)\"  onkeyup=\"checkTransaction();\" title=\"Nilai Perolehan Objek Pajak (NPOP)\"/></td>
				  </tr>
				  <tr>
				  ...
				  <tr>
					...
					<td id=\"tBPHTBTS\" align=\"right\">" . number_format(($data->CPM_OP_NPOP - strval($data->CPM_OP_NPOPTKP)) * 0.05, 0, '.', ',') . "</td>
				  </tr>
				  <tr " . $display_aphb . ">
					<td>APHB &nbsp;&nbsp;</td>
					<td id=\"tAPHB\" align=\"right\">" . number_format(($data->CPM_OP_NPOP - strval($data->CPM_OP_NPOPTKP)) * 0.05 * $tAPHB, 0, '.', ',') . "</td>
				  </tr>
				  ...
				  <tr>
					...
					<td id=\"harusbayar\" align=\"right\">" . number_format(((strval($data->CPM_OP_AKUMULASI) - strval($data->CPM_OP_NPOPTKP)) * 0.05), 0, '.', ',') . "</td>
				  </tr>
				  <tr>
			...
			<!--<tr>
			<td colspan=\"2\" align=\"center\" valign=\"middle\" id=\"jmlBayar\">Jumlah yang dibayarasaskan : </td>
			</tr> -->
...
function save($final, $newVer = false)
{
	...
	// 		// var_dump($_REQUEST['right-land-build'] );die;
	// 		$data[29] = 0;
	// 	} else {
	...
	if (validation($data, $err)) {
		...
		$query = "INSERT INTO cppmod_ssb_doc (
		CPM_SSB_ID,CPM_KPP,
