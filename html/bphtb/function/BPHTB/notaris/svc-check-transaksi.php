...
//    mysql_select_db('VSI_SWITCHER_DEVEL');
    ...
    $sql_pbb = "SELECT * from {$dbTable} WHERE wp_noktp = '{$nop}'";
//echo $sql_pbb;
...
//    }
    if ($dt_pbb = mysqli_fetch_array($res_pbb)) {
        ...
        if ($dt_pbb['payment_flag'] == 0) {
            ...
            $respon['message'] = "Wajib pajak dengan no. KTP {$nop} sudah bertransaksi namun belum dibayar.";
        } else {
            $respon['message'] = "Wajib pajak dengan no. KTP {$nop} sudah bertransaksi dan sudah dibayar.";
        }
        $found = true;
    } else {
