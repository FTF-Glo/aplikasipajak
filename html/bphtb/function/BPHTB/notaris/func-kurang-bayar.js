...
function checkTransaction() {
    ...
    var tNPOPTKP = document.getElementById('tNPOPTKP');
    ...
    if (m <= 0) {
        ...
        npop_kurang_npoptkp = jsbtotalbefore.value - tNPOPTKP.value;
		...
		if((jnsperolehan=="4")||(jnsperolehan=="5")||(jnsperolehan=="31")){
            if(configpengenaan=='1'){
                ...
                //alert(Phibahwaris);
				hbphtbt = ((npop_kurang_npoptkp * 0.05)-hpengenaan);
			...
			if(configaphb=='1'){
				if(hitungaphb=='1'){
					...
					hitbphtb = (npop_kurang_npoptkp * 0.05 * pec[0]/pec[1])+denda;
					...
					hitbphtb = (npop_kurang_npoptkp * 0.05)-(NPOPKP * 0.05 * pec[0]/pec[1])+denda
					...
					denda = (NPOPKP*0.05)*percentdenda/100;
					bphtbt = document.createTextNode(number_format((NPOPKP*0.05)+denda, 0, '.', ','));
					hitbphtb = (npop_kurang_npoptkp*0.05)+denda;
				}
			}else{
					APHB=document.createTextNode(number_format(0, 0, '.', ','));
					denda = (NPOPKP*0.05)*percentdenda/100;
					bphtbt = document.createTextNode(number_format((NPOPKP*0.05)+denda, 0, '.', ','));
					hitbphtb = (NPOnpop_kurang_npoptkpPKP*0.05)+denda;
			}
			 //alert(Phibahwaris);
			bphtbt1= hitbphtb;
			bphtbt = document.createTextNode(number_format(hitbphtb, 0, '.', ','));
			
				ctjpay = document.createTextNode("Jumlah yang dibayarkan : " + number_format(hitbphtb+denda, 0, '.', ','));
		}else{
            pengenaan = document.createTextNode(number_format(0, 0, '.', ','));
			APHB=document.createTextNode(number_format(0, 0, '.', ','));
			bphtbt1= (npop_kurang_npoptkp * 0.05 )+denda;
            console.log(bphtbt1);
			bphtbt = document.createTextNode(number_format((NPOPKP * 0.05)+denda, 0, '.', ','));
    ...
    // console.log(npop_kurang_npoptkp);
    var npopkp = document.createTextNode(number_format(NPOPKP, 0, '.', ','));
    var npopkppp = document.createTextNode(number_format(npop_kurang_npoptkp, 0, '.', ','));
...
function autoNOP(obj) { // get data berdasarkan nop, autofill
    ...
    $.ajax({
        type: "post",
        data: "nop=" + val,
        url: "./function/BPHTB/notaris/svc-auto-nop.php",
        dataType: "json",
        success: function(res) {
