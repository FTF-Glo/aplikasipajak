...
function check_ssb(){
  ...
  $thnnn = date("Y");
  $query = "SELECT * FROM $DbTable WHERE wp_noktp = '" . $noktp . "' and YEAR(saved_date) = {$thnnn} ORDER BY pelaporan_ke DESC limit 1";
  ...
  $query = "SELECT PAYMENT_FLAG, PAYMENT_PAID FROM $DbTable WHERE wp_noktp = '" . $noktp . "' and YEAR(saved_date) = {$thnnn} ORDER BY pelaporan_ke";
  $resBE = mysqli_query($LDBLink, $query);
...
if ($trsid== '' && $noktp != '') {
    ...
    if (isset($ssb['pelaporan_ke']) && $ssb['num_rows'] != 0) {
        ...
        $result['feedback'] = "<td style=\"color:red;font-style:italic;\"><b>PELAPORAN KE ".$pelaporan_ke." PADA KTP ".$noktp." DI TAHUN ".date("Y")."</b></td>";
    }
}
