...
function check_validate(){
  ...
  $qry = "SELECT * FROM cppmod_ssb_doc WHERE cppmod_ssb_doc.cpm_op_nomor = \"{$nop}\" ORDER BY CPM_SSB_CREATED DESC limit 1";
  // echo $qry;
    $res = mysqli_query($DBLink, $qry);
    if ( $res === false ){
      echo $qry ."<br>";
...
function check_ssb($ssb_id){
  ...
  global $DBLink,$nop,$trsid,$a, $LDBLink; 
  ...
  SCANPayment_ConnectToDB($DBLink, $LDBConn, $DbHost, $DbUser, $DbPwd, $DbName, true);
// var_dump($LDBLink);
  ...
  $resBE = mysqli_query($DBLink, $query);
  $return = array();
...
$result = array();
$result['success'] = true;
$result['status_pelaporan'] = 9;
if ($trsid== '') {
  $checkd = check_validate();
  if (isset($checkd['status']) && $checkd['status']!=5) {
