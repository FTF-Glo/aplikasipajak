...
//    $conn = mysql_connect('127.0.0.1', 'sw_user', 'sw_pwd');
//    mysql_select_db('VSI_SWITCHER_DEVEL');
    //$conn = mysql_connect('127.0.0.1', 'sw_user', 'sw_pwd',true);
    $conn = mysql_connect('127.0.0.1', 'sw_user', '',true);
    ...
    $sql_pbb = "SELECT * FROM cppmod_ssb_berkas WHERE CPM_BERKAS_NOP = '{$nop}'";
    
    $res_pbb = mysqli_query($conn, $sql_pbb);
    $row=mysqli_fetch_array($res_pbb);

    if ($row[0] != 0) { 
        if ($row[12]==0)
	$respon['message'] = "NOP {$nop} sudah pernah terpakai dan Berkas belum lengkap";
        else
        $respon['message'] = "NOP {$nop} sudah pernah terpakai dan Berkas sudah lengkap";
	
}
//    if(mysqli_num_rows($res_pbb,$conn)<=0){
