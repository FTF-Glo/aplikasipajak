...
where b.CPM_SSB_AUTHOR = '$notaris'";
        ...
        SCANPayment_ConnectToDB($DBLinkLookUp, $DBConn2, $dbHost, $dbUser, $dbPwd, $dbName);
        		...
        		$qry10 = "SELECT id_ssb FROM `ssb` ORDER BY id_ssb DESC LIMIT 1";
		$rs10 = mysqli_query($DBLinkLookUp, $qry10);
		if($data = mysqli_fetch_assoc($rs10)){
			$idssb_terakhir = $data['id_ssb']+1;
			$payment_code = str_pad($idssb_terakhir,6,'0',STR_PAD_LEFT);
            $payment_code = $payment_code.'11';
		}
...
//        }
		...
		VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')", $idtran, $refnum, $x, $version, $status, '0', mysqli_real_escape_string($DBLink, $trdate), '', $opr, $dispenda1, $dispenda2, nl2br($info));
        $result = mysqli_query($DBLink, $query);
    ...
    private function addApprove() {
        ...
        if ($this->approved && $this->claim && (!$this->stsrejected)) {
            $HTML .= "<tr><td>&nbsp;</td><td><table width=\"450\" border=\"0\" cellspacing=\"1\" cellpadding=\"8\" style=\"background-color:#f1c3e2\">
					  ...
					  <tr>
						<td colspan=\"2\" class=\"result-analisis\">
							<label>
