...
class taxRePrintClass {
	...
	private function executeCekStatusQuery($REFNUM=NULL,$NOPNPWP=NULL,$DATE=NULL) {
		...
		$query = sprintf( "SELECT *	FROM c_registry	WHERE C_R_KEY LIKE '%s'", mysqli_real_escape_string($DBLink, $configCode));
				...
				if ($nRes > 0)
				{
				 ...
				 // $Response['totalrow'] = $nRes;
				  ...
				  {				
					...
					$tot =  $row['CSM_TM_COLLECTIVE_AMOUNT'] +	 $row['CSM_TM_MISC_AMOUNT'] +  $row['CSM_TM_ADMIN_FEE'];	
