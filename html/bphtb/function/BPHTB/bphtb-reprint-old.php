...
class taxRePrintClass {
	...
	private function executeCekStatusQuery($REFNUM=NULL,$NOPNPWP=NULL,$DATE=NULL) {
		...
		$PPIDInfo=$User->getPPIDInfo($data->ppid);
		$myConn = $User->GetDbConnectionFromApp($_REQUEST['a']);
		$configCode = $data->ppid.".PP.bphtb.PC.kabkot";
		
		$query = sprintf( "SELECT *	FROM c_registry	WHERE C_R_KEY LIKE '%s'", mysqli_real_escape_string($myConn, $configCode));
		...
		$retval=null;
		$arResult = LOOKUP_ALL_BPHTB($myConn);
		if (!$arResult) {
			return;
		}
		$LDBLink=null;
				...
				if ($nRes > 0)
				{
				 ...
				 // $Response['totalrow'] = $nRes;
				  ...
				  {				
					...
					$tot =  ($row['CSM_TM_COLLECTIVE_AMOUNT']/pow(10,$row['CSM_TM_MINOR_48'])) +	 $row['CSM_TM_MISC_AMOUNT'] +  $row['CSM_TM_ADMIN_FEE'];	
					$Response[$i]["COLL_BILL_TEXT"] = number_format($row['CSM_TM_COLLECTIVE_AMOUNT'], 2, ',', '.');
					$Response[$i]["MISC_BILL_TEXT"] = number_format($row['CSM_TM_MISC_AMOUNT'], 2, ',', '.');
					$Response[$i]["ADMIN_FEE_TEXT"] = number_format($row['CSM_TM_ADMIN_FEE'], 2, ',', '.');
	...
	private function createHeader($REFNUM,$NOPNPWP,$DATE) {
		...
		$header .= "<script language='javascript' src='function/bphtb/bphtb-reprint.js'></script>\n";
		...
		$header .= "<script language=\"javascript\"> var aU = '".base64_encode($UserName)."';</script>\n"; 
...
if(stillInSession($DBLink,$json,$sdata)){
	...
	if (($nopnpwp != '' && $date != '') || $refnum != '') {
		...
		$bphtb = new taxRePrintClass($printername,$refnum,$nopnpwp,$date);
		
	}
	else {
		$bphtb = new taxRePrintClass($printername);
