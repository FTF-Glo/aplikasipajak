...
if ($data) {
	...
	if(isset($simpan))
	{
		...
		$keterangan="aktif";
		$sqlSimpan="UPDATE tbl_reg_user_bphtb SET status='$status' where userId='$userId'";
		$sqlSimpan2="UPDATE central_user SET CTR_U_BLOCKED='$status' where CTR_U_UID='$userId'";
	...
	if(isset($_REQUEST["dataId"])){
		...
		if($result=mysqli_query($appDbLink, $sqlCari)){
			...
			<form method="POST" action="main.php?param=<?php echo base64_encode("a=$a&m=$m&f=$f")?>" name="formReg" id="formReg" onSubmit="return cekform();">
				...
				<tr>
					<td>Status</td>
					<td><input type="radio" name="status" value="0" <?php echo $dataTampil["status"]==0?"checked":""?>>Aktif&nbsp;<input type="radio" name="status" value="1" <?php echo $dataTampil["status"]==1?"checked":""?>>di Blokir</td>
				</tr>
				<tr>
