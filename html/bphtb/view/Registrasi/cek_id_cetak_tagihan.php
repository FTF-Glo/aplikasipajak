...
if(!$connectDb){
		...
		if(isset($_POST['userId']))//Jika username telah disubmit
		{
			...
			$check_for_username = mysqli_query($koneksi, "SELECT userId FROM TBL_REG_USER_CETAK_TAGIHAN WHERE userId='$username'");
			$check_for_username2 = mysqli_query($koneksi, "SELECT CPC_U_UID FROM cpccore_user WHERE CPC_U_UID='$username'");
