...
function getDocument($sts,&$dat) {
    ...
    while($row=mysqli_fetch_assoc($res)){
		...
		$HTML .= "\t<tr>\n";
		$HTML .= "\t\t<td align=\"center\">".$row['op_nomor']."</td> \n";
		$HTML .= "\t\t<td>".$row['wp_nama']."</td> \n";
		$HTML .= "\t\t<td>".$row['wp_alamat']."</td>
				  <td align=\"center\">".getAUTHOR($row['op_nomor'])."</td>\n";
		$HTML .= "\t\t<td align=\"right\">".number_format($row['bphtb_dibayar'],0,".",",")."</td>\n";
		$HTML .= "\t\t<td align=\"center\">".substr($row['payment_paid'],0,10)."</td> \n";
		$HTML .= "\t</tr>\n";
