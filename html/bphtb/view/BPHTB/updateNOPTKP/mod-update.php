...
function getDataFromSwitcher() {
	...
	$start = $page * $perpage ;
	$sql = "SELECT * FROM cppmod_ssb_doc A ,
(SELECT CPM_OP_NOMOR,MAX(CPM_SSB_VERSION) AS VER FROM cppmod_ssb_doc A  WHERE (A.CPM_WP_NOKTP='{$noktp}' or A.CPM_OP_NOMOR='{$nop}') GROUP BY CPM_OP_NOMOR) B
WHERE A.CPM_OP_NOMOR = B.CPM_OP_NOMOR AND A.CPM_SSB_VERSION = B.VER ";
	//$sql = "SELECT * FROM cppmod_ssb_doc A WHERE (A.CPM_WP_NOKTP='{$noktp}' or  A.CPM_OP_NOMOR='{$nop}') ORDER BY CPM_SSB_CREATED ASC LIMIT ".$start.",".$perpage;
	...
	$qry = "SELECT COUNT(*) AS TOTALROWS FROM cppmod_ssb_doc A ,
(SELECT CPM_OP_NOMOR,MAX(CPM_SSB_VERSION) AS VER FROM cppmod_ssb_doc A  WHERE (A.CPM_WP_NOKTP='{$nop}' or A.CPM_OP_NOMOR='{$nop}') GROUP BY CPM_OP_NOMOR) B
WHERE A.CPM_OP_NOMOR = B.CPM_OP_NOMOR AND A.CPM_SSB_VERSION = B.VER ";
...
function show_body_update() {
	...
	while($row= mysqli_fetch_array($data)) {
		...
		$html .= " <td class={$c_color} align=\"right\">".number_format($row['CPM_OP_HARGA'],0,",",".")."</td>\n";
...
function prosesUpdate(){
	...
	$transaksi = @isset($_REQUEST['trans-value'])? $_REQUEST['trans-value'] : "";
	$noktp = @isset($_REQUEST['noktp'])? $_REQUEST['noktp'] : "";
	...
	$query = sprintf("UPDATE cppmod_ssb_doc SET CPM_WP_NAMA ='%s',CPM_WP_NPWP ='%s',CPM_WP_ALAMAT ='%s',
		...
		CPM_SSB_AKUMULASI='%s',
		CPM_OP_BPHTB_TU='%s', CPM_OP_HARGA='%s', CPM_WP_NOKTP='%s'
		WHERE CPM_SSB_ID ='%s'",
		...
		mysqli_real_escape_string($appDbLink, $f_nilai_pajak),mysqli_real_escape_string($appDbLink, $bphtb),mysqli_real_escape_string($appDbLink, $transaksi),mysqli_real_escape_string($appDbLink, $noktp),
