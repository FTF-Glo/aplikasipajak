...
class modBPHTBApprover
{
	...
	function getSPPTInfo($noktp, $nop, &$paid)
	{
	   ...
	   $paid = "";
		$res = mysqli_query($query, $LDBLink);
		if ( $res === false ){
	...
	function getBPHTBPayment($lb, $nb, $lt, $nt, $h, $p, $jh, $NPOPTKP, $phw, $aphbt, $denda, $ssb)
	{
		$a = "aBPHTB";
		...
		$news___ = $a;
		$hitungaphb = $this->getConfigValue($a, 'HITUNG_APHB');
		$configAPHB = $this->getConfigValue($a, 'CONFIG_APHB');
		$configPengenaan = $this->getConfigValue($a, 'CONFIG_PENGENAAN');
		...
		if (($jh == 4) || ($jh == 5) || ($jh == 31)) {
			...
			// var_dump($hitungaphb);die;
			if ($configAPHB == '1') {
				$pecahan_aphb = explode("/", $aphbt);
				$aphb = $pecahan_aphb[0] / $pecahan_aphb[1];
				$hbphtb_pengenaan = 0;
				if ($hitungaphb == '1') {
					// $hbphtb_aphb = ($npop-strval($NPOPTKP))*0.05 * $aphb;

					// rumus asli tapi salah
	...
	function getDocument($sts, &$dat)
	{
		...
		for ($i = 0; $i < count($data->data); $i++) {
			...
			$ccc = $this->getBPHTBPayment(
				$data->data[$i]->CPM_OP_LUAS_BANGUN,
				$data->data[$i]->CPM_OP_NJOP_BANGUN,
				$data->data[$i]->CPM_OP_LUAS_TANAH,
				$data->data[$i]->CPM_OP_NJOP_TANAH,
				$data->data[$i]->CPM_OP_HARGA,
				$data->data[$i]->CPM_PAYMENT_TIPE_PENGURANGAN,
				$data->data[$i]->CPM_OP_JENIS_HAK,
				$data->data[$i]->CPM_OP_NPOPTKP
			);
		...
		if (($draf) && ($this->userGroup == 1)) {
			...
			<input type=\"button\" value=\"Hapus\" id=\"btn-print\" name=\"btn-delete\" 
				...
				onclick=\"printDataToPDF(1);\"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\n
						Masukan Pencarian Berdasarkan Nama WP <input type=\"text\" id=\"src-approved\" name=\"src-approved\" size=\"60\"/> No. KTP <input type=\"text\" id=\"src-ktp\" name=\"src-ktp\" size=\"20\"/>
			...
			$HTML .= "<input type=\"button\" value=\"Cetak Salinans\" id=\"btn-print\" name=\"btn-print\" 
	...
	{
		$HTML = "<form id=\"form-notaris\" name=\"form-notaris\" method=\"post\" action=\"\" >";
		$HTML .= "Masukan Pencarian Berdasarkan Nama WP <input type=\"text\" id=\"src-approved\" name=\"src-approved\" size=\"60\"/> No. KTP <input type=\"text\" id=\"src-ktp\" name=\"src-ktp\" size=\"20\"/> <input type=\"submit\" value=\"Cari\" id=\"btn-src\" name=\"btn-src\" />\n</form>\n";
		$HTML .= "<table cellpadding=\"4\" cellspacing=\"1\" border=\"0\" width=\"100%\">\n";
		$HTML .= "\t<tr>\n";
		$HTML .= "\t\t<td class=\"tdheader\"> Nomor Objek Pajak</td> \n";
		$HTML .= "\t\t<td class=\"tdheader\"> No. KTP </td> \n";
	...
	{
		$HTML = "<form id=\"form-notaris\" name=\"form-notaris\" method=\"post\" action=\"\" >";
		$HTML .= "Masukan Pencarian Berdasarkan Nama WP <input type=\"text\" id=\"src-approved\" name=\"src-approved\" size=\"60\"/> No. KTP <input type=\"text\" id=\"src-ktp\" name=\"src-ktp\" size=\"20\"/>
				<input type=\"submit\" value=\"Cari\" id=\"btn-src\" name=\"btn-src\" />\n</form>\n";
		$HTML .= "<table cellpadding=\"4\" cellspacing=\"1\" border=\"0\" width=\"100%\">\n";
		$HTML .= "\t<tr>\n";
		$HTML .= "\t\t<td class=\"tdheader\"> Nomor Objek Pajak </td> \n";
	...
	public function headerContentDay($sts)
	{
		$a = $_REQUEST['a'];
		...
		$HTML .= "<table>
					<tr>
						...
						<td> 
							<input type=\"text\" id=\"tanggal\" name=\"tanggal\" size=\"20\" value='' style=\"width:100%\">
						</td>
						<td>
							<input type=\"submit\" value=\"Cari\" id=\"btn-src\" name=\"btn-src\" />
						</td>
					...
					<tr>
						...
						<td style=\"width:250px;\">
							<select name=\"jenis_hk\" id=\"jenis_hk\" style=\"width:100%;\">
							   	<option value=\"\"> </option>
							    <option value=\"1\">1. Jual Beli</option>
							    <option value=\"2\">2. Tukar Menukar</option>
							    <option value=\"3\">3. Hibah</option>
							    <option value=\"4\">4. Hibah Wasiat</option>
							    <option value=\"5\">5. Waris</option>
							    <option value=\"6\">6. Pemasukan Dalam Perseroan atau Badan Hukum Lainnya</option>
							    <option value=\"7\">7. Pemisahan Hak yang Mengakibatkan Peralihan (APHB)</option>
							    <option value=\"8\">8. Peralihan Hak karena Pelaksanaan Putusan Hakim</option>
							    <option value=\"9\">9. Pemberian Hak Baru Atas Tanah Sebagai Kelanjutan dari Pelepasan Hak (PH)</option>
							    <option value=\"10\">10. Pemberian Hak Baru Atas Tanah Diluar Pelepasan Hak</option>
							    <option value=\"11\">11. Penggabungan usaha</option>
							    <option value=\"12\">12. Peleburan usaha</option>
							    <option value=\"13\">13. Pemekaran usaha</option>
							    <option value=\"14\">14. Hadiah</option>
							    <option value=\"15\">15. Penunjukan Pembeli Dalam Lelang</option>
							    <option value=\"16\">16. Waris APHB</option>
							    <option value=\"17\">17. Peningkatan Hak yang berupa LC</option>
								<option value=\"18\">18. Tax Amnesti (TA)</option>
								<option value=\"19\">19. Peningkatan Hak yang berupa Rumah Dinas Untuk WP</option>
						    </select>
						</td>
						<td>
						</td>
	...
	function getDocumentDay($sts, &$dat)
	{
		global $data, $DBLink, $json;
		...
		if ($srcTxt != "") $where .= " WHERE GW_SSB_PEKANBARU.ssb.payment_paid >= '" . $srcTxt . " 00:00:00' AND GW_SSB_PEKANBARU.ssb.payment_paid <= '" . $srcTxt . " 23:59:59'";
		if ($srcTxt2 != "") {
			if ($srcTxt != "") {
				$where2 .= " AND SW_SSB_PEKANBARU.cppmod_ssb_doc.CPM_OP_JENIS_HAK = '" . $srcTxt2 . "'";
			} else {
				$where2 .= " WHERE SW_SSB_PEKANBARU.cppmod_ssb_doc.CPM_OP_JENIS_HAK = '" . $srcTxt2 . "'";
			}
		}
		...
		SCANPayment_ConnectToDB($LDBLink, $LDBConn, $DbHost, $DbUser, $DbPwd, $DbName, true);
		SCANPayment_ConnectToDB($XDBLink, $XDBConn, ONPAYS_DBHOST, ONPAYS_DBUSER, ONPAYS_DBPWD, 'SW_SSB_PEKANBARU', true);
		...
		$query = "SELECT * FROM GW_SSB_PEKANBARU.ssb
					INNER JOIN
			        SW_SSB_PEKANBARU.cppmod_ssb_doc
			   		ON
			        SW_SSB_PEKANBARU.cppmod_ssb_doc.CPM_SSB_ID = GW_SSB_PEKANBARU.ssb.id_switching
			        INNER JOIN
			        SW_SSB_PEKANBARU.cppmod_ssb_jenis_hak
			        ON
			        SW_SSB_PEKANBARU.cppmod_ssb_doc.CPM_OP_JENIS_HAK = SW_SSB_PEKANBARU.cppmod_ssb_jenis_hak.CPM_KD_JENIS_HAK 
			        $where
			        $where2
			        ORDER BY GW_SSB_PEKANBARU.ssb.payment_paid DESC";
	...
	{
		$a = $_REQUEST['a'];
		$srcTxt = @isset($_REQUEST['awal']) ? $_REQUEST['awal'] : "";
		$srcTxt2 = @isset($_REQUEST['jenis_hk']) ? $_REQUEST['jenis_hk'] : "";
		$srcTxt3 = @isset($_REQUEST['akhir']) ? $_REQUEST['akhir'] : "";
		$srcTxt4 = @isset($_REQUEST['tagihan_awal']) ? $_REQUEST['tagihan_awal'] : "";
		$srcTxt5 = @isset($_REQUEST['tagihan_akhir']) ? $_REQUEST['tagihan_akhir'] : "";

		if ($srcTxt == '' && $srcTxt2 == '' && $srcTxt3 == '' && $srcTxt4 == '' && $srcTxt5 == '') {
			$srcTxt = date('Y-m-d');
		}

		$j = base64_encode("{'sts':'" . $sts . "','app':'" . $a . "','src':'" . $srcTxt . "','src2':'" . $srcTxt2 . "','src3':'" . $srcTxt3 . "','src4':'" . $srcTxt4 . "','src5':'" . $srcTxt5 . "'}");
		$HTML = "<form id=\"form-notaris\" name=\"form-notaris\" method=\"post\" action=\"\" >";
		$HTML .= "<table>
					<tr>
						<td>
						...
						<td> 
							<input type=\"text\" id=\"awal\" name=\"awal\" size=\"20\" value=''>
							&nbsp;&nbsp; s/d &nbsp;&nbsp;\n
							<input type=\"text\" id=\"akhir\" name=\"akhir\" size=\"20\" value=''>
						</td>
						<td>
							<input type=\"submit\" value=\"Cari\" id=\"btn-src\" name=\"btn-src\" />
						</td>
					...
					<tr>
						...
						<td>
							<input type=\"text\" id=\"tagihan_awal\" name=\"tagihan_awal\" size=\"20\" value='' placeholder='contoh: 500000000'>
							&nbsp;&nbsp; s/d &nbsp;&nbsp;\n
							<input type=\"text\" id=\"tagihan_akhir\" name=\"tagihan_akhir\" size=\"20\" value='' placeholder='contoh: 500000000'>
						</td>
						<td>
						</td>
						<td>
	...
	function getDocumentRecap($sts, &$dat)
	{
		...
		if ($srcTxt != "") $where .= " WHERE GW_SSB_PEKANBARU.ssb.payment_paid >= '" . $srcTxt . " 00:00:00'";

		if ($srcTxt3 != "") {
			if ($srcTxt != "") {
				$where2 .= " AND GW_SSB_PEKANBARU.ssb.payment_paid <= '" . $srcTxt3 . " 23:59:59'";
			} else {
				$where2 .= " WHERE GW_SSB_PEKANBARU.ssb.payment_paid <= '" . $srcTxt3 . " 23:59:59'";
			}
		}

		if ($srcTxt2 != "") {
			if ($srcTxt == "" && $srcTxt3 == "") {
				$where3 .= " WHERE SW_SSB_PEKANBARU.cppmod_ssb_doc.CPM_OP_JENIS_HAK = '" . $srcTxt2 . "'";
			} else {
				$where3 .= " AND SW_SSB_PEKANBARU.cppmod_ssb_doc.CPM_OP_JENIS_HAK = '" . $srcTxt2 . "'";
			}
		}

		if ($srcTxt4 != "") {
			if ($srcTxt == "" && $srcTxt3 == "" && $srcTxt2 == "") {
				$where4 .= " WHERE SW_SSB_PEKANBARU.cppmod_ssb_doc.CPM_OP_HARGA >= '" . $srcTxt4 . "'";
			} else {
				$where4 .= " AND SW_SSB_PEKANBARU.cppmod_ssb_doc.CPM_OP_HARGA >= '" . $srcTxt4 . "'";
			}
		}
		...
		if ($srcTxt5 != "") {
			if ($srcTxt == "" && $srcTxt3 == "" && $srcTxt2 == "" && $srcTxt4 == "") {
				$where5 .= " WHERE SW_SSB_PEKANBARU.cppmod_ssb_doc.CPM_OP_HARGA <= '" . $srcTxt5 . "'";
			} else {
				$where5 .= " AND SW_SSB_PEKANBARU.cppmod_ssb_doc.CPM_OP_HARGA <= '" . $srcTxt5 . "'";
			}
		}
		...
		$query = "SELECT SW_SSB_PEKANBARU.cppmod_ssb_doc.CPM_OP_JENIS_HAK, SW_SSB_PEKANBARU.cppmod_ssb_jenis_hak.CPM_JENIS_HAK, SUM(SW_SSB_PEKANBARU.cppmod_ssb_doc.CPM_OP_HARGA) AS CPM_OP_HARGA FROM GW_SSB_PEKANBARU.ssb
					INNER JOIN
			        SW_SSB_PEKANBARU.cppmod_ssb_doc
			   		ON
			        SW_SSB_PEKANBARU.cppmod_ssb_doc.CPM_SSB_ID = GW_SSB_PEKANBARU.ssb.id_switching
			        ...
			        $where4
			        $where5
			        GROUP BY SW_SSB_PEKANBARU.cppmod_ssb_doc.CPM_OP_JENIS_HAK, SW_SSB_PEKANBARU.cppmod_ssb_jenis_hak.CPM_KD_JENIS_HAK
			        ORDER BY GW_SSB_PEKANBARU.ssb.payment_paid DESC";
		// ORDER BY GW_SSB_PEKANBARU.ssb.id_ssb DESC LIMIT ".$this->page.",".$this->perpage; 
		// print_r($query);
		$res = mysqli_query($query);
