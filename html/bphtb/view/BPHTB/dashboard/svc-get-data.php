...
for($c=0;$c<12;$c++){
	 ...
	 for ($f=0;$f<$num_rowsy;$f++){
			 ...
			 if ($m ==($c+1)) $mony[$c] = floatval($daty[$f]["bayar"]);
	 }
}
...
// add by d3Di
// get collectible Penerimaan Pajak
$query2 = "SELECT SUM(bphtb_collectible) TOTALBAYAR FROM ssb WHERE payment_flag=1 AND (DATE(payment_paid) BETWEEN '".date('Y')."-01-01' AND '".date('Y-m-d')."')"; 
$jml = mysqli_query($LDBLink, $query2);
if ($jml === false){
	print_r(mysqli_error($LDBLink).$query2);
	return "Tidak Ditemukan"; 
}
$ret['data']['trs_collectible'] = 0;
while ($rowd=mysqli_fetch_array($jml)) {
	$ret['data']['trs_collectible'] = $rowd['TOTALBAYAR'];
}

// NEW TAUFIQ
$query_sdhbyr = "SELECT count(*) sdh_bayar FROM $DbTable WHERE PAYMENT_FLAG = 1 AND YEAR(payment_paid) = $year"; 
$resh = mysqli_query($LDBLink, $query_sdhbyr);
if ( $resh === false ){
	print_r(mysqli_error($LDBLink).$query_sdhbyr);
	return "Tidak Ditemukan"; 
}
while ($rowh=mysqli_fetch_array($resh)) {
	$ret['data']['total_sdh_bayar'] = $rowh['sdh_bayar'];
}

$query_blmbyr = "SELECT count(*) blm_byr FROM $DbTable WHERE PAYMENT_FLAG = 0 AND YEAR(saved_date) = $year"; 
$resh = mysqli_query($LDBLink, $query_blmbyr);
if ( $resh === false ){
	print_r(mysqli_error($LDBLink).$query_blmbyr);
	return "Tidak Ditemukan"; 
}
while ($rowh=mysqli_fetch_array($resh)) {
	$ret['data']['total_blm_bayar'] = $rowh['blm_byr'];
}
///
