...
class stafDispenda extends modBPHTBApprover {
    ...
    function getAllDocument(&$dat) {
        global $data, $DBLink, $json, $a, $m, $find, $find_notaris,$find_nop,$find_kcmtn;
        $srcTxt = $find;
        $where = "";
        ...
        if ($find_kcmtn != "")
            $where .= " AND A.CPM_WP_KECAMATAN LIKE '%" . $find_kcmtn . "%'";
    ...
    function getDocumentInfoText($sts, &$dat) {
        ...
        if ($find_kcmtn != "")
            $where .= " AND cppmod_ssb_doc.CPM_WP_KECAMATAN LIKE '%" . $find_kcmtn . "%'";
		if($sts == 4){
					if ($srcTxt != "")
    ...
    function getDocument($sts, &$dat) {
        global $DBLink, $json, $a, $m, $find, $find_notaris, $page, $find_nop, $find_kcmtn;
        $srcTxt = $find;
        $where = "";
        ...
        $query = "SELECT * FROM cppmod_ssb_doc A, cppmod_ssb_tranmain B WHERE A.CPM_SSB_ID = B.CPM_TRAN_SSB_ID AND B.CPM_TRAN_STATUS=" .
                ...
                // die(var_dump($query));
		if($sts ==5){
		$query = "SELECT * FROM cppmod_ssb_doc JOIN cppmod_ssb_tranmain ON cppmod_ssb_doc.CPM_SSB_ID=cppmod_ssb_tranmain.CPM_TRAN_SSB_ID JOIN cppmod_ssb_log ON cppmod_ssb_doc.CPM_SSB_ID=cppmod_ssb_log.CPM_SSB_ID WHERE cppmod_ssb_tranmain.CPM_TRAN_STATUS IN (3)  AND cppmod_ssb_log.CPM_SSB_LOG_ACTION = 6 $where ORDER BY cppmod_ssb_tranmain.CPM_TRAN_DATE DESC LIMIT " . $this->page . "," . $this->perpage;}
    ...
    public function headerContentAll() {
        ...
        $HTML .= "Pilih Tanggal Cetak
                ...
                &nbsp;&nbsp;\n
                <b>Kecamatan :</b>
                <!-- <input type=\"text\" id=\"src-kcmtn-$s\" name=\"src-kcmtn\" size=\"30\" value=\"$find_kcmtn\"/>\n -->
    ...
    function dropdown_kecamatan($sts){
        ...
        $qry = "SELECT cppmod_ssb_doc.CPM_OP_KECAMATAN FROM cppmod_ssb_doc WHERE (cppmod_ssb_doc.CPM_OP_KECAMATAN NOT LIKE \"%tes%\"AND 
                cppmod_ssb_doc.CPM_OP_KECAMATAN != 1) GROUP BY cppmod_ssb_doc.CPM_OP_KECAMATAN";
        $res = mysqli_query($DBLink, $qry);
        if ($res === false) {
            echo mysqli_error($DBLink);
        }
        $return = "<select name=\"src-kcmtn\" id=\"src-kcmtn-{$sts}\" style=\"width:200px;height:26px;\">";

        $return .= "<option value=\"\">Pilih Kecamatan</option>";
        while ($row = mysqli_fetch_assoc($res)) {
            if ($find_kcmtn==$row['CPM_OP_KECAMATAN']) {
                $selected = 'selected';
            }
            else{
                $selected = '';
            }
            $return .= "<option value=\"".$row['CPM_OP_KECAMATAN']."\" $selected>".$row['CPM_OP_KECAMATAN']."</option>";
        }
        $return .= "</select>";
        return $return;
    }
