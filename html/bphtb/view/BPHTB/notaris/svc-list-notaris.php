...
class BPHTBService extends modBPHTBApprover
{
    ...
    function getSPPTInfo($noktp, $nop, &$paid)
    {
        ...
        $query = "SELECT id_switching, PAYMENT_FLAG, PAYMENT_PAID, expired_date FROM $DbTable WHERE id_switching = '" . $noktp . "' ORDER BY saved_date DESC limit 1  ";
        // die($query);
        $paid = "";
        ...
        for ($i = 0; $i < count($data->data); $i++) {
            ...
            return $data->data[$i]->PAYMENT_FLAG == 1 ? "Sudah Dibayar" : "Siap Dibayar";
        }
    ...
    function getDocument($sts, &$dat)
    {
        ...
        for ($i = 0; $i < count($data->data); $i++) {
            ...
            // var_dump($data->data[$i]->CPM_SSB_ID);die;
            if ($data->data[$i]->CPM_TRAN_READ == "")
                $class = "tdbodyNew";
            if (
                $data->data[$i]->CPM_SSB_ID == '0a47f6b79fddd134857b2d3ebcb7b2f6' ||
                $data->data[$i]->CPM_SSB_ID == '7adcc40a17081f14b9763cc8decfc3d8' ||
                $data->data[$i]->CPM_SSB_ID == '4b665c11967f1934e96ce403caa06b69' ||
                $data->data[$i]->CPM_SSB_ID == '0649878e22b159e4b996d11a133bb1bc' ||
            ...
            // var_dump($data->data[$i]->CPM_PAYMENT_TIPE);
            // die;
            $HTML .= "<div><tr>";
            ...
            if ($adaqris && $statusBayar != 'Sudah Dibayar') {
                ...
                if (($ccc > 0 && $ccc <= 10000000 && $statusBayar != 'Sudah Dibayar' && $PAYMENT_CODE != '' && $dateexp >= $datenow)) {
                    ...
                    $parPOST = "'$ssbid','$sha1'";
                    $imgQRIS = '<div id="divico' . $ssbid . '"><a href="javascript:;" onclick="getQRCode(' . $parPOST . ')"><img id="idico' . $ssbid . '" src="./image/icon/qr_disable.png" width="20px" height="20px"></a>';
                } else {
                    $imgQRIS = '';
                }
            }
            ...
            $HTML .= "<td class=$class align=cente>" . trim(strip_tags($data->data[$i]->CPM_WP_ALAMAT)) . "</td>";
            $HTML .= "<td class=$class>" . $data->data[$i]->CPM_OP_KECAMATAN . "</td>";
            $HTML .= "<td class=$class>" . $data->data[$i]->CPM_OP_KELURAHAN . "</td>";
            $HTML .= "<td class=$class align=center>" . number_format($ccc, 0, ",", ".") . "</td>";
            ...
            if ($sts == 2)
                if ($data->data[$i]->CPM_TRAN_STATUS == 2)
                    // $statusSPPT = "Tertunda Verifikasi";
                     $HTML .= "<td class=$class align=center><span class=\"badge badge-primary\"> Menunggu Verifikasi Pemberkasan</span></td>";
    ...
    function getAllDocument(&$dat)
    {
        ...
        $query = "SELECT * 
                FROM sw_ssb.cppmod_ssb_doc A, sw_ssb.cppmod_ssb_tranmain B 
                WHERE 
                    A.CPM_SSB_ID = B.CPM_TRAN_SSB_ID 
                    AND B.CPM_TRAN_FLAG=0 AND   (A.sdelete !='1' or A.sdelete IS NULL)
                    $where 
        ...
        // die;
        // $query = "SELECT * 
        //         FROM sw_ssb.cppmod_ssb_doc A, sw_ssb.cppmod_ssb_tranmain B, gw_ssb.ssb G   
        //         WHERE A.CPM_SSB_ID = B.CPM_TRAN_SSB_ID AND G.id_switching = A.CPM_SSB_ID
        //         GROUP BY A.CPM_SSB_ID  
        //         ORDER BY B.CPM_TRAN_DATE 
        //         DESC LIMIT " . $hal . "," . $this->perpage;
        $res = mysqli_query($DBLink, $query);
        if ($res === false) {
            return false;
        }
        ...
        $qry = "SELECT COUNT(*) AS TOTALROWS FROM cppmod_ssb_doc A, cppmod_ssb_tranmain B WHERE A.CPM_SSB_ID = B.CPM_TRAN_SSB_ID 
        AND  B.CPM_TRAN_FLAG=0 AND  (A.sdelete !='1' or A.sdelete IS NULL) ";
        // AND B.CPM_TRAN_OPR_NOTARIS ='" . $this->user . "' $where";
        // echo $qry;
        ...
        for ($i = 0; $i < count($data->data); $i++) {
            ...
            if (
                $data->data[$i]->CPM_SSB_ID == '0a47f6b79fddd134857b2d3ebcb7b2f6' ||
                $data->data[$i]->CPM_SSB_ID == '1014e5a9e5d9d0441104b94a02ffb127' ||
                $data->data[$i]->CPM_SSB_ID == '7adcc40a17081f14b9763cc8decfc3d8' ||
                $data->data[$i]->CPM_SSB_ID == '4b665c11967f1934e96ce403caa06b69' ||
                $data->data[$i]->CPM_SSB_ID == '0649878e22b159e4b996d11a133bb1bc' ||
                $data->data[$i]->CPM_SSB_ID == '8ebb877e961c42c4fd38bc9517a40155'
            ...
            $HTML .= "<div><tr>";
            $HTML .= "<td width=9 class=$class align=center>" . (++$no) . ".</td>";
            $HTML .= "<td class=$class><a href=\"main.php?param=" . base64_encode($par1) . "\">" . $data->data[$i]->CPM_OP_NOMOR . "</a></td>";
            $HTML .= "<td class=$class><a href=\"main.php?param=" . base64_encode($par1) . "\">" . $data->data[$i]->CPM_WP_NOKTP . "</a></td>";
            $HTML .= "<td class=$class><a href=\"main.php?param=" . base64_encode($par1) . "\">" . $data->data[$i]->CPM_WP_NAMA . "</a></td>";
                        ...
                        <div class=\"form-filtering\">
                            <div class=\"row \">
                                ...
                                <div class=\"form-group col-md-4\" >
                                    ...
                                    <input type=\"text\" class=\"form-control\"  id=\"src-approved-{$sts}\" name=\"src-approved-{$sts}\" value=\"{$find}\" />
                                </div>
    ...
    public function headerContent($sts)
    {
                                ...
                                <div class=\"collapse\" id=\"collapsFilter-$sts\">
                                    <div class=\"form-filtering\">
                                        <div class=\"row \">
                                            ...
                                            <div class=\" form-group col-md-12\"> 
                                                <input type=\"button\" class=\"btn btn-success\" value=\"Cari\" id=\"btn-src\" name=\"btn-src\"  onclick=\"setTabs ({$sts});\" />
                                                <input type=\"button\" class=\"btn btn-success\" value=\"Cetak SSPD\" id=\"btn-print\" name=\"btn-print\" onclick=\"printDataToPDF(1);\"/>
        ...
        $HTML .= "<td class=tdheader>Qris.</td>";
        $HTML .= "<td class=tdheader>No.</td>";
        $HTML .= "<td class=tdheader>Nomor Objek Pajak</td>";
        $HTML .= "<td class=tdheader>No. KTP</td>";
        $HTML .= "<td class=tdheader>Wajib Pajak</td>";
                            ...
                            <div class=\"collapse\" id=\"collapsFilter-$sts\">
                                <div class=\"form-filtering\">
                                    <div class=\"row \">
                                        ...
                                        <div class=\"form-group col-md-4\" >
                                            <label>Nama WP/NOP/NIK</label>
                                            <input type=\"text\" class=\"form-control\" id=\"src-approved-{$sts}\" name=\"src-approved\" maxlength=\"18\" value=\"{$find}\"/>
                                        </div>
    ...
    public function headerContentApprove($sts, $draf = false)
    {
        ...
        if ($draf) {
                                ...
                                <div class=\"collapse\" id=\"collapsFilter-$sts\">
                                    <div class=\"form-filtering\">
                                        <div class=\"row \">
                                            ...
                                            <div class=\" form-group col-md-12\"> 
                                                <input type=\"button\" class=\"btn btn-success\" value=\"Cari\" id=\"btn-src\" name=\"btn-src\"  onclick=\"setTabs ({$sts});\" />
                                                <input type=\"button\" class=\"btn btn-success\" value=\"Cetak Sementara\" id=\"btn-print\" name=\"btn-print\"onclick=\"printDataToPDF(1);\"/>
                                                <input type=\"hidden\" id=\"hidden-delete\" name=\"hidden-delete\" />
            ...
            <!-- <input type=\"button\" value=\"Hapus\" id=\"btn-print\" name=\"btn-delete\" onclick=\"deleteChekedNotaris();\"/>&nbsp;&nbsp;-->";
        } else {
            $HTML .= "
                            ...
                            <div class=\"collapse\" id=\"collapsFilter-$sts\">
                                <div class=\"form-filtering\">
                                    <div class=\"row \">
                                        ...
                                        <div class=\" form-group col-md-12\"> 
                                            <input type=\"button\" class=\"btn btn-success\" value=\"Cari\" id=\"btn-src\" name=\"btn-src\"  onclick=\"setTabs ({$sts});\" />
                                            <input type=\"button\" class=\"btn btn-success\"  value=\"Cetak Salinan\" id=\"btn-print\" name=\"btn-print\"onclick=\"printDataToPDF(2);\"/>
                                            <input type=\"hidden\" value=\"Cetak Pengantar Bayar\" id=\"btn-print-cpb\" name=\"btn-print-cpb\" onclick=\"printBeritaAcara(2);\"/>
                                        </div>
    ...
    public function displayDataNotaris($sts)
    {
        ...
        echo "<div id=\"notaris-main-content\" style=\"overflow: auto;\">";
        echo "<div id=\"notaris-main-content-inner\" style=\"overflow: auto;\">";
