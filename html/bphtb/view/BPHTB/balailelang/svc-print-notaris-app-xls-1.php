...
$query = sprintf("SELECT * , DATE_FORMAT(DATE_ADD(B.CPM_TRAN_DATE, INTERVAL %s DAY), '%%d-%%m-%%Y') as EXPIRED
                            ...
                            A.CPM_SSB_ID in (%s) ORDER BY B.CPM_TRAN_DATE DESC", getConfigValue("1", 'TENGGAT_WAKTU'), $iddoc);
#echo $query;
#exit;
...
while ($rowData = mysqli_fetch_assoc($res)) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('D' . $row, $rowData['CPM_WP_NOKTP']);
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('J' . $row, $rowData['CPM_OP_NOMOR']);
...
$objPHPExcel->getActiveSheet()->getStyle('A1:AD1')->getFill()->getStartColor()->setRGB('E4E4E4');
