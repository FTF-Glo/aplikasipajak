...
foreach ($data as $item) {
	...
	$queryDftr = "SELECT * FROM sw_ssb.cppmod_ssb_doc where CPM_SSB_ID = '$id' ";
	$resDftr = mysqli_query($DBLink, $queryDftr);
	$rowDftr = mysqli_fetch_assoc($resDftr);

	$CPM_NO_PENDAFTARAN = $rowDftr['CPM_NO_PENDAFTARAN'];
	//	$jam = date('');
	// var_dump($reason);die;
	$query = "UPDATE sw_ssb.cppmod_ssb_doc SET  sdelete = '1', delete_by='$pengguna' ,delete_at = NOW() , keterangan = '$reasonn'  WHERE CPM_NO_PENDAFTARAN = '$CPM_NO_PENDAFTARAN'";
	...
	if ($res != false) {
		$query1 = "INSERT INTO sw_ssb.cppmod_ssb_doc_hapus 
			SELECT *
			FROM sw_ssb.cppmod_ssb_doc where CPM_NO_PENDAFTARAN='$CPM_NO_PENDAFTARAN'";
			// var_dump($query1);die;
		// $query2 = "INSERT INTO gw_ssb.ssb_hapus 
		// 	SELECT *
		// 	FROM gw_ssb.ssb where id_switching = '$id'";
		...
		if ($res1 !== false) {
			...
			// $res4 = mysqli_query($DBLink, $query4);
			if ($res3 === false) {
				echo $query3 . "<br>";
	...
	// return $dt;
	// echo "ID: " . $item['id'] . "<br>";
	// echo "Draf: " . $item['draf'] . "<br>";
	// echo "Axx: " . $item['axx'] . "<br>";
	// echo "Uname: " . $item['uname'] . "<br>";
	//var_dump($item['id']);
}
