...
class modBPN extends modBPHTBApprover
{
    ...
    function getDocument($sts, &$dat)
    {
        ...
        $query = "SELECT * FROM gw_ssb.ssb WHERE  " . $status1 . " ORDER BY saved_Date DESC LIMIT " . $this->page . "," . $this->perpage;
        // echo $query;
        $res = mysqli_query($DBLink, $query);
        //var_dump($DBLink);
        if ($res === false) {
        ...
        $this->totalRows = mysqli_num_rows($res);
        ...
        //         GROUP BY A.CPM_SSB_ID";
        $qry = "select count(*) AS n from gw_ssb.ssb WHERE  " . $status1;
        $resCount = mysqli_query($DBLink, $qry);
        $bar = mysqli_fetch_object($resCount);
        $this->totalRows = $bar->n;
        //$qry =  $this->totalRows->n;
        // var_dump($this->totalRows);
    ...
    public function headerContentAll($sts)
    {
        ...
        $HTML .= "Pencarian berdasarkan <b>No. KTP</b> : <input type=\"text\" name=\"src-noktp\" size=\"40\" value='" . $noktp . "'/> 
				<select name='opsi'>
                    <option " . $selectedand . " value='and'>And</option>
                    <option " . $selectedor . " value='or'>Or</option>
                </select>
                ...
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                &nbsp;&nbsp;&nbsp;
                <b>Nama Wp</b> : <input type=\"text\" name=\"src-nama-wp\" size=\"40\" value='" . $nm_wp . "'/> 
                <b>User</b> : <input type=\"text\" name=\"src-nama-notaris\" size=\"40\" value='" . $findnotaris . "'/> 
                \n</form>\n";
