...
function getDocument($sts,&$dat) {
	...
	for ($i=0;$i<count($data->data);$i++) {
		...
		$dataRow[$tw]['CPM_OP_NJOP_TANAH'] = $data->data[$i]->CPM_OP_NJOP_TANAH;
		...
		$dataRow[$tw]['CPM_OP_NMR_SERTIFIKAT'] = $data->data[$i]->CPM_OP_NMR_SERTIFIKAT;
		$tw ++;
	}
	//totalRows = $tw;
	$dat = $dataRow;
...
if (getDocument($sts, $dat)) {
	...
	foreach ($dat as $row) {
		...
		$worksheet1->write_string($baris,14,$row['CPM_OP_LUAS_BANGUN'],$fBiasa);
		$worksheet1->write_number($baris,15,$row['NJOP'],$fDtlNumber);
		$worksheet1->write_number($baris,16,$row['CPM_OP_NJOP_TANAH'],$fDtlNumber);
		$worksheet1->write_number($baris,17,$row['PAID'],$fDtlNumber);
		$worksheet1->write_string($baris,18,$row['CPM_OP_HARGA'],$fBiasa);
		$worksheet1->write_string($baris,19,$row['JENIS_HAK'],$fBiasa);
		

		$total_paid = $total_paid + $row['PAID'];
		$total_denda = $total_denda + $row['DENDA'];
		$total_njop = $total_njop + $row['NJOP'];
	...
	$worksheet1->write_string(3,14,"LUAS BANGUNAN",$fDtlHead);
	$worksheet1->write_string(3,15,"NJOP",$fDtlHead);
	$worksheet1->write_string(3,16,"NJOP PER/METER",$fDtlHead);
	$worksheet1->write_string(3,17,"BPHTB",$fDtlHead);
	$worksheet1->write_string(3,18,"TRANSAKSI",$fDtlHead);
	$worksheet1->write_string(3,19,"KETERANGAN",$fDtlHead);
	...
	$worksheet1->write_string($baris,15,$total_njop,$fDtlNumber);
	$worksheet1->write_string($baris,16,$total_njopperm,$fDtlNumber);
	$worksheet1->write_string($baris,17,$total_paid,$fDtlNumber);
	$worksheet1->write_string($baris,18,$total_trans,$fDtlNumber);
