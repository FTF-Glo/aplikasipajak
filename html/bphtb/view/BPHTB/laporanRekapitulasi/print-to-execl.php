...
function getDocument($sts, &$dat)
{
	...
	$i = 1;
	while ($row = mysqli_fetch_assoc($res)) {
		$dataNotaris = getDataNotaris($row['CPM_SSB_AUTHOR']);
		$dataRow[$tw]['NO'] = $i++;
		$dataRow[$tw]['PAYMENT_PAID'] = $row['payment_paid'];
		$dataRow[$tw]['SAVED_DATE'] = $row['saved_date'];
		//$dataRow[$tw]['ID_SSB'] = str_pad($row['id_ssb,8,'0',STR_PAD_LEFT);
		$dataRow[$tw]['ID_SSB'] = $row['payment_code'];
		//edit
		$dataRow[$tw]['OP_NOMOR'] = $row['op_nomor'];
		$dataRow[$tw]['WP_NAMA'] = $row['wp_nama'];
		$dataRow[$tw]['WP_ALAMAT'] = $row['wp_alamat'];
		$dataRow[$tw]['WP_NOKTP'] = $row['wp_noktp'];
		$dataRow[$tw]['WP_NAMA_LAMA'] = $row['CPM_WP_NAMA_LAMA'];
		$dataRow[$tw]['CPM_SSB_AUTHOR'] = $row['CPM_SSB_AUTHOR'];
		$dataRow[$tw]['ALAMAT_NOTARIS'] = $dataNotaris['almt_jalan'];
		$dataRow[$tw]['CPM_OP_LETAK'] = $row['CPM_OP_LETAK'];
		$dataRow[$tw]['CPM_OP_KECAMATAN'] = $row['CPM_OP_KECAMATAN'];
		$dataRow[$tw]['CPM_OP_LUAS_TANAH'] = $row['CPM_OP_LUAS_TANAH'];
		$dataRow[$tw]['CPM_OP_LUAS_BANGUN'] = $row['CPM_OP_LUAS_BANGUN'];
		$dataRow[$tw]['NJOP_TANAH'] = (($row['CPM_OP_LUAS_TANAH'] * $row['CPM_OP_NJOP_TANAH']));
		$dataRow[$tw]['NJOP_BANGUN'] = ($row['CPM_OP_LUAS_BANGUN'] * $row['CPM_OP_NJOP_BANGUN']);
		$dataRow[$tw]['CPM_OP_NJOP_TANAH'] = $row['CPM_OP_NJOP_TANAH'];
		$dataRow[$tw]['CPM_OP_NJOP_BANGUN'] = $row['CPM_OP_NJOP_BANGUN'];
		$dataRow[$tw]['PAID'] = $row['bphtb_dibayar'];
		$dataRow[$tw]['CPM_OP_HARGA'] = $row['CPM_OP_HARGA'];
		$dataRow[$tw]['JENIS_HAK'] = $row['CPM_JENIS_HAK'];
		$dataRow[$tw]['TANGGAL'] = $srcTxt;
		//tamabahan no. sertifikat
		$dataRow[$tw]['CPM_OP_NMR_SERTIFIKAT'] = $row['CPM_OP_NMR_SERTIFIKAT'];
		$tw++;
	}
	//totalRows = $tw;
	$dat = $dataRow;
...
if (getDocument($sts, $dat)) {
	...
	foreach ($dat as $row) {
		...
		$worksheet1->write_string($baris, 1, $row['PAYMENT_PAID'], $fDtlCenter);
		$worksheet1->write_string($baris, 2, $row['SAVED_DATE'], $fDtlCenter);
		$worksheet1->write_string($baris, 3, $row['ID_SSB'], $fDtlCenter);
		//edit
		$worksheet1->write_string($baris, 4, $row['OP_NOMOR'], $fDtlCenter);
		$worksheet1->write_string($baris, 5, $row['WP_NAMA'], $fBiasa);
		$worksheet1->write_string($baris, 6, $row['WP_ALAMAT'], $fBiasa);
		$worksheet1->write_string($baris, 7, $row['WP_NOKTP'], $fDtlCenter);
		$worksheet1->write_string($baris, 8, $row['WP_NAMA_LAMA'], $fBiasa);
		$worksheet1->write_string($baris, 9, $row['CPM_SSB_AUTHOR'], $fBiasa);
		$worksheet1->write_string($baris, 10, $row['CPM_OP_NMR_SERTIFIKAT'], $fBiasa);
		$worksheet1->write_string($baris, 11, $row['CPM_OP_LETAK'], $fBiasa);
		$worksheet1->write_string($baris, 12, $row['CPM_OP_KECAMATAN'], $fDtlCenter);
		$worksheet1->write_string($baris, 13, $row['CPM_OP_LUAS_TANAH'], $fDtlCenter);
		$worksheet1->write_number($baris, 14,  $row['NJOP_TANAH'], $fDtlNumber);
		$worksheet1->write_string($baris, 15, $row['CPM_OP_NJOP_TANAH'], $fBiasa);
		$worksheet1->write_number($baris, 16, $row['CPM_OP_LUAS_BANGUN'], $fDtlNumber);
		$worksheet1->write_number($baris, 17, $row['NJOP_BANGUN'], $fDtlNumber);
		$worksheet1->write_number($baris, 18, $row['CPM_OP_NJOP_BANGUN'], $fDtlNumber);
		$worksheet1->write_number($baris, 19, $row['PAID'], $fDtlNumber);
		$worksheet1->write_string($baris, 20, $row['CPM_OP_HARGA'], $fBiasa);
		$worksheet1->write_string($baris, 21, $row['JENIS_HAK'], $fBiasa);


		$total_paid = $total_paid + $row['PAID'];
		$total_denda = $total_denda + $row['DENDA'];
		$total_njop = $total_njop + $row['NJOP_TANAH'];
		$total_njopperm = $total_njopperm + $row['CPM_OP_NJOP_TANAH'];
		// $total_njopbang = 0;
		// $total_njoppermbang = 0;
		$total_njopbang = $total_njopbang + $row['NJOP_BANGUN'];
	...
	$worksheet1->write_string(3, 1, "TGL. BAYAR", $fDtlHead);
	$worksheet1->write_string(3, 2, "TGL. VERIFIKASI", $fDtlHead);
	$worksheet1->write_string(3, 3, "KODE BAYAR", $fDtlHead);
	$worksheet1->write_string(3, 4, "No. OBJEK PAJAK", $fDtlHead);
	$worksheet1->write_string(3, 5, "NAMA PEMBELI", $fDtlHead);
	$worksheet1->write_string(3, 6, "ALAMAT PEMBELI", $fDtlHead);
	$worksheet1->write_string(3, 7, "KTP PEMBELI", $fDtlHead);
	$worksheet1->write_string(3, 8, "NAMA PENJUAL", $fDtlHead);
	$worksheet1->write_string(3, 9, "NOTARIS/PPAT", $fDtlHead);
	$worksheet1->write_string(3, 10, "NO. SERTIFIKAT", $fDtlHead);
	$worksheet1->write_string(3, 11, "LETAK OP", $fDtlHead);
	$worksheet1->write_string(3, 12, "KECAMATAN OP", $fDtlHead);
	$worksheet1->write_string(3, 13, "LUAS TANAH", $fDtlHead);
	$worksheet1->write_string(3, 14, "NJOP TANAH", $fDtlHead);
	$worksheet1->write_string(3, 15, "NJOP PER/METER TANAH", $fDtlHead);
	$worksheet1->write_string(3, 16, "LUAS BANGUNAN", $fDtlHead);
	$worksheet1->write_string(3, 17, "NJOP BANGUNAN", $fDtlHead);
	$worksheet1->write_string(3, 18, "NJOP PER/METER BANGUNAN", $fDtlHead);
	$worksheet1->write_string(3, 19, "BPHTB", $fDtlHead);
	$worksheet1->write_string(3, 20, "TRANSAKSI", $fDtlHead);
	$worksheet1->write_string(3, 21, "KETERANGAN", $fDtlHead);
	...
	$worksheet1->write_string($baris, 14, $total_njop, $fDtlNumber);
	$worksheet1->write_string($baris, 15, $total_njopperm, $fDtlNumber);
	// $worksheet1->write_string($baris, 16, $total_njopperm, $fDtlNumber);
	$worksheet1->write_string($baris, 17, $total_njopbang, $fDtlNumber);
	$worksheet1->write_string($baris, 18, $total_njoppermbang, $fDtlNumber);
	$worksheet1->write_string($baris, 19, $total_paid, $fDtlNumber);
	$worksheet1->write_string($baris, 20, $total_trans, $fDtlNumber);
	...
	$worksheet1->write_string($baris, 14, 'TOTAL BPHTB + DENDA', $fBiasa);
	$worksheet1->write_string($baris, 17, $total_all, $fDtlNumber);
}
