function sendInquiry(){
    
	if($('#nop_npwp').val() == '') alert('Silahkan Isi Nomor NOP !');
	else{
		Ext.Ajax.request({
		   url: 'function/BPHTB/pembayaran/loadData.php',
		   params:{nop:$('#nop_npwp').val(),mode:$('#mode').val(),tgl:$('#tgl-bayar').val()},
		   timeout: 80000,
		   success: function(res){setData(res.responseText)},
		   failure: function(){alert('Connection Error !!'); clearData();}
		});
	}
}
function todaydate(){
	var today = new Date();
	var dd = today.getDate();

	var mm = today.getMonth()+1; 
	var yyyy = today.getFullYear();
	if(dd<10) 
	{
		dd='0'+dd;
	} 

	if(mm<10) 
	{
		mm='0'+mm;
	} 
	today = yyyy+'-'+mm+'-'+dd;
	
	return today;
}
function setData(json){
		var today = todaydate();
        if(json=='[]'){
		alert('Data tagihan tidak ada !'); 
		clearData();
	}
	else{
		
		$data = jQuery.parseJSON(json);
		//alert($data[0].payment_flag+"  "+$('#mode').val());
                if(($data[0].payment_flag==1) && ($('#mode').val()!='cetak_ulang')){
			alert('Pelaporan untuk Nop ini Sudah Dibayar !!!'); 
			clearData();
		}
		else if($data[0].expired_date<today && $('#mode').val()!='cetak_ulang'){
			alert('Pelaporan sudah melewati tanggal expire !!!'); 
			clearData();		
		}
		else if($data[0].payment_flag==0 && $('#mode').val()=='cetak_ulang'){
			alert('Pelaporan Belum Dibayar !!!'); 
			clearData();		
		}
		else if($('#mode').val()=='cetak_ulang'){
			var totalTagihan = $data[0].bphtb_dibayar;
			
			$('#wp-name').html($data[0].wp_nama);
			$('#wp-duedate').html($data[0].expired_date);
			$('#wp-address').html($data[0].wp_alamat);
			$('#wp-kelurahan').html($data[0].wp_kelurahan);
			$('#wp-rtRw').html($data[0].wp_rt+'/'+$data[0].wp_rw);
			$('#wp-kecamatan').html($data[0].wp_kecamatan);
			$('#wp-kabupaten').html($data[0].wp_kabupaten);
			$('#wp-kdPos').html($data[0].wp_kodepos);
			
			$('#op-name').html($data[0].op_nama);
			
			$('#op-address').html($data[0].op_letak);
			$('#op-kelurahan').html($data[0].op_kelurahan);
			$('#op-rtRw').html($data[0].op_rt+'/'+$data[0].op_rw);
			$('#op-kecamatan').html($data[0].op_kecamatan);
			$('#op-kabupaten').html($data[0].op_kabupaten);
			$('#op-kdPos').html($data[0].op_kodepos);
			
			$('#wp-amount').html($data[0].bphtb_dibayar);	
			
			$('#wp-admin').html(0);
			$('#wp-totalamount').html($data[0].bphtb_dibayar);
			
			$('#jml-bayar').val($data[0].bphtb_dibayar);
			$('#jml-uang').val($data[0].bphtb_dibayar);
			$('#jml-kembali').val(0);
                        if($data[0].payment_paid.length > 9){
                            //2015-04-10
                            //alert($data[0].PAYMENT_PAID.substring(8, 10)+'-'+$data[0].PAYMENT_PAID.substring(5, 7)+'-'+$data[0].PAYMENT_PAID.substring(0, 4));
                            $('#tgl-bayar').val($data[0].payment_paid.substring(8, 10)+'-'+$data[0].payment_paid.substring(5, 7)+'-'+$data[0].payment_paid.substring(0, 4));
                        }else $('#tgl-bayar').val($data[0].payment_paid);
			
			document.getElementById("payment").disabled = false; 
		}
		else {
			var totalTagihan = $data[0].bphtb_dibayar;
			
			$('#wp-name').html($data[0].wp_nama);
			$('#wp-duedate').html($data[0].expired_date);
			$('#wp-address').html($data[0].wp_alamat);
			$('#wp-kelurahan').html($data[0].wp_kelurahan);
			$('#wp-rtRw').html($data[0].wp_rt+'/'+$data[0].wp_rw);
			$('#wp-kecamatan').html($data[0].wp_kecamatan);
			$('#wp-kabupaten').html($data[0].wp_kabupaten);
			$('#wp-kdPos').html($data[0].wp_kodepos);
			
			$('#op-name').html($data[0].op_nama);
			
			$('#op-address').html($data[0].op_letak);
			$('#op-kelurahan').html($data[0].op_kelurahan);
			$('#op-rtRw').html($data[0].op_rt+'/'+$data[0].op_rw);
			$('#op-kecamatan').html($data[0].op_kecamatan);
			$('#op-kabupaten').html($data[0].op_kabupaten);
			$('#op-kdPos').html($data[0].op_kodepos);
			
			$('#wp-amount').html($data[0].bphtb_dibayar);	
			
			$('#wp-admin').html(0);
			$('#wp-totalamount').html($data[0].bphtb_dibayar);
			
			$('#jml-bayar').val($data[0].bphtb_dibayar);
			$('#jml-uang').val(0);
			$('#jml-kembali').val(0);
			
			document.getElementById("payment").disabled = false; 
		}
	}
}


function clearData(){
	$('#wp-name').html('-');
	$('#wp-duedate').html('-');
	$('#wp-address').html('-');
	$('#wp-kelurahan').html('-');
	$('#wp-rtRw').html('-');
	$('#wp-kecamatan').html('-');
	$('#wp-kabupaten').html('-');
	$('#wp-kdPos').html('-');

	$('#wp-amount').html(0);	
	$('#wp-penalty').html(0);
	$('#wp-admin').html(0);
	$('#wp-totalamount').html(0);
	
	$('#jml-bayar').val(0);
	$('#jml-uang').val(0);
	$('#jml-kembali').val(0);
	
	document.getElementById("payment").disabled = true; 
}
function sendBayar() {
	var form = document.createElement("form"); 
	form.setAttribute("method", "post"); 
	form.setAttribute("action", "function/BPHTB/pembayaran/printData.php"); 

	var FN = document.createElement("input"); 
	FN.setAttribute("type", "hidden"); 
	FN.setAttribute("name", "driver"); 
	FN.setAttribute("value", $('#driver').val());
	form.appendChild(FN);  

	var FN = document.createElement("input"); 
	FN.setAttribute("type", "hidden"); 
	FN.setAttribute("name", "mode"); 
	FN.setAttribute("value", $('#mode').val());
	form.appendChild(FN);  

	var FN = document.createElement("input"); 
	FN.setAttribute("type", "hidden"); 
	FN.setAttribute("name", "uname"); 
	FN.setAttribute("value", $('#uname').val());
	form.appendChild(FN);  

	var FN = document.createElement("input"); 
	FN.setAttribute("type", "hidden"); 
	FN.setAttribute("name", "tgl"); 
	FN.setAttribute("value", $('#tgl-bayar').val());
	form.appendChild(FN);  

	var FN = document.createElement("input"); 
	FN.setAttribute("type", "hidden"); 
	FN.setAttribute("name", "tgltrans"); 
	FN.setAttribute("value", todaydate());
	form.appendChild(FN);  

	var FN = document.createElement("input"); 
	FN.setAttribute("type", "hidden"); 
	FN.setAttribute("name", "nop"); 
	FN.setAttribute("value", $('#nop_npwp').val());
	form.appendChild(FN);  
	
	document.body.appendChild(form);

	form.submit();
}
function sendBayar1(){
    var v = document.getElementById('jml-kembali').value;
    var n = v.search("-"); 
    var y = document.getElementById('jml-uang');
    var z = clearFormat(y.value,y);
	var tgltrans = todaydate();
    if (parseInt(z) > 0 && n == '-1') {
        
        Ext.Ajax.request({
	    url: 'function/BPHTB/pembayaran/printData.php',
	    params:{
			nop: $('#nop_npwp').val(),
			driver: $('#driver').val(),
			mode: $('#mode').val(),
			uname: $('#uname').val(),
            tgl:$('#tgl-bayar').val(),
			tgltrans : tgltrans
			
		},
	    timeout: 80000,
	    success: function(res){
	        alert('Mencetak Struk');
                var data   = res.responseText;
			var applet = document.jZebra;
			if (applet != null) {
				applet.append64(data);
				applet.print();
			}
			clearData();	
	    },
	    failure: function(){alert('Connection Error !!'); clearData();}
	});
    }
    else alert('Tidak bisa melakukan pembayaran.\nJumlah uang yang dibayar kurang dari jumlah tagihan yang harus dibayar.');
        
}
function jml() {
    var v = document.getElementById('jml-bayar');
    var x = clearFormat(v.value,v);
    var b = document.getElementById('jml-uang');
    var c = clearFormat(b.value,b);
    
    document.getElementById('jml-kembali').value = formatNumber(parseInt(c) - parseInt(x),0,'.','');
} 
function clearFormat (input, milSep){
    var strCheck = '0123456789';
    var clstr = "";
    if (milSep.value != undefined)
        len = milSep.value.length;
    else len = 0;
    for(i=0; i < len; i++)
        if (strCheck.indexOf(milSep.value.charAt(i))!=-1) clstr += milSep.value.charAt(i);
    return clstr;
} 
function filterInput(input, e){
    if(e != undefined){
    var strCheck = '0123456789';
    var whichCode = (window.Event) ? e.which : e.keyCode;
    if (whichCode == 8) return true; // Delete
    //VALIDATION
    key = String.fromCharCode(whichCode); // Get key value from key code
    if (strCheck.indexOf(key) == -1) return false; // Not a valid key
    }
    if (input.value.charAt(0) == '0') return false; // block 0 input
    }
    function applySeparator(str, milSep){
    st = "";
    length = str.length;
    j = 0;
    for (i = length - 1; i >= 0; i--)
    {
    j++;
    if (j > 3)
    {
    st = milSep + st;
    j = 1;
    }
    ch = str.charAt(i);
    st = ch + st;
    }
    return st;
}
// When using this function at onChange event, use filterInput in onKeyPress
// Default using at onBlur
function currencyFormatIC (input, milSep) {
    if (input.value.charAt(0) == '0') return false; // block 0 input
    aux = clearFormat(input,milSep);
    input.value = applySeparator(aux, milSep);
} 
// OnKeyPress, execute before character code displayed
function currencyFormatI(input, milSep, e) {
var sep = 0;
var key = '';
var i = j = 0;
var len = len2 = 0;
var strCheck = '0123456789';
var aux = aux2 = '';
var whichCode = (window.Event) ? e.which : e.keyCode;
len = input.value.length;
//VALIDATION
key = String.fromCharCode(whichCode); // Get key value from key code
if (whichCode == 8) { //Backspace
if(input.value.length != 0){ // Fix all delimiters place before backspace
for(i=0; i < len; i++) // Trim character other than number
if (strCheck.indexOf(input.value.charAt(i))!=-1) aux += input.value.charAt(i);
len = aux.length;
aux2 = '';
j = 0;
first=true;
for (i = len; i >= 0; i--) {
if ((j == 5) && (first==true)) {
first = false;
aux2 += milSep;
j = 0;
}
if ((j == 3) && (first==false)) {
aux2 += milSep;
j = 0;
}
aux2 += aux.charAt(i);
j++;
}
//console.log("aux2:"+ aux2);
input.value = '';
len2 = aux2.length;
for (i = len2; i >= 0; i--)
input.value += aux2.charAt(i);
}
return true;
}
if (input.value.charAt(0) == '0') return false; // block 0 input
if (strCheck.indexOf(key) == -1) return false; // not a valid key
len = input.value.length; // check maxLength
maxlen = input.maxLength;
if (maxlen == len) return false;
if (maxlen % 4 == 0){
if(len == maxlen-1)
return false;
}
aux = '';
for(i=0; i < len; i++)
if (strCheck.indexOf(input.value.charAt(i))!=-1) aux += input.value.charAt(i);
len = aux.length;
aux2 = '';
j = 0; k = 0;
for (i = len; i >= 0; i--) {
if (j == 3) {
aux2 += milSep;
j = 0;
}
aux2 += aux.charAt(i);
j++;
k++;
}
input.value = '';
len2 = aux2.length;
for (i = len2 - 1; i >= 0; i--)
input.value += aux2.charAt(i);
}
function onSelectClearFormat(input, milSep, e){
input.value = clearFormat(input, milSep);
return filterInput(input, e);
}

function openListTag(url){
    window.open(url,'_blank');
}