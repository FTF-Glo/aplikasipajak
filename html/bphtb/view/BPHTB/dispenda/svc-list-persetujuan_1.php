...
class stafDispenda extends modBPHTBApprover {
    ...
    function getDocumentInfoText($sts, &$dat) {
		...
		if ($noktp != "")
            $where .= " AND (cppmod_ssb_doc.CPM_WP_NOKTP LIKE '%" . $noktp . "%')";
		}else{
		        if ($srcTxt != "")
            $where .= " AND (A.CPM_WP_NAMA LIKE '" . $srcTxt . "%' OR A.CPM_OP_NOMOR LIKE '" . $srcTxt . "%')";
        if ($find_notaris != "")
        ...
        for ($i = 0; $i < count($data->data); $i++) {
			...
			if ($sts == 5){
				if($statusSPPT!="Sudah Dibayar"){
					$HTML .= "\t\t<td class=\"" . $class . "\" align=\"center\"><input type=button value='Batalkan' onclick=\"updatelaporan('".$data->data[$i]->CPM_SSB_ID."')\"></td>\n";
				}else{
    ...
    public function headerContent($sts) {
        global $find, $find_notaris, $noktp;
					...
					if(confirm(\"Apakah Anda Yakin Ingin Membatalkan Persetujuan untuk Data ini ?\")){
							...
							$.ajax({
								...
								success: function(res) {
									...
									if(res==1){
										alert('Pembatalan Persetujuan Berhasil, Data dikembalikan ke Pelaporan Sementara');
										location.reload();
									}else{
    ...
    public function headerContentAll() {
        global $s, $find, $find_notaris,$noktp;
        $HTML = "<form id=\"form-notaris\" name=\"form-notaris\" method=\"post\" action=\"\" >";
        $HTML .= "Pencarian Berdasarkan 
				<b>Nama WP :</b>
