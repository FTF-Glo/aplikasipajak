...
require_once($sRootPath."view/BPHTB/laporanHarianBaru/svc-bpn-lookup.php");
...
class modBPHTBApprover  {
	...
	function getBPHTBPayment($lb,$nb,$lt,$nt,$h,$p,$jh,$NPOPTKP,$phw,$aphbt) {
		//$a = $_REQUEST['a'];
		...
		if(($jh==4)||($jh==5)||($jh==3)||($jh==31)){
			$jmlByr= ($npop-strval($NPOPTKP))*0.05 * $phw * 0.01;
		}else if(($jh==33)||($jh==7)){
			$p=explode("/",$aphbt);
			$aphb=$p[0]/$p[1];
			$jmlByr= ($npop-strval($NPOPTKP))*0.05 * $aphb;
		}
		$tp = strval($p);
