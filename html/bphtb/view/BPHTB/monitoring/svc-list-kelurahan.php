...
$query = "select REPLACE(CPM_OP_KELURAHAN, ' ','') AS CPM_OP_KELURAHAN from SW_SSB.cppmod_ssb_doc  
          where REPLACE(CPM_OP_KELURAHAN, ' ','') like '". mysqli_real_escape_string($LDBLink, $_REQUEST['term']) ."%' 
          group by CPM_OP_KELURAHAN order by CPM_OP_KELURAHAN asc limit 0,10";
// echo "$query";
...

{
	...
	{
		$data[] = array(
			'label' => $row['CPM_OP_KELURAHAN'],
			'value' => $row['CPM_OP_KELURAHAN']
		);
	}
}
