...
class BPHTBService extends modBPHTBApprover {
    ...
    function getDocumentPembayaran($sts, &$dat) {
        global $json, $a, $m, $find, $find_notaris, $tgl1, $tgl2;
        $srcTxt = $find;
        ...
        if ($srcTxt != "")
            $where .= " AND wp_nama LIKE '" . mysqli_escape_string($srcTxt) . "%'";
		if($sts == 2){
			if ($find_notaris != "")
        ...
        if ($tgl1 != "" && $tgl2 != "")
            $where .= " AND  (payment_paid between '" . mysqli_escape_string($tgl1) . "' and '" . mysqli_escape_string($tgl2) . " 23:59:59')";
        elseif ($tgl1 != "")
		...
		//echo $query;
        #untuk total                 
        $dataSelect = mysqli_query($qry, $LDBLink);
    ...
    function getDocumentApproval($sts, &$dat) {
        ...
        if ($tgl1 != "" && $tgl2 != "")
            $where .= " AND  (b.CPM_TRAN_DATE between '" . mysqli_escape_string($tgl1) . "' and '" . mysqli_escape_string($tgl2) . " 23:59:59')";
        elseif ($tgl1 != "")
