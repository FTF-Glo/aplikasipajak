...
class TaxService {
    ...
    function getDocument(&$dat) {
        ...
        $srcTxt = $find;
        $where = " WHERE 1=1 ";
        " WHERE CPM_BERKAS_NOP = 0 ";
        if ($srcNama != "")
            $where .= " AND CPM_BERKAS_NAMA_WP LIKE '%" . $srcNama . "%' ";
        if ($srcNomor != "")
            $where .= " AND CPM_BERKAS_NOP LIKE '%" . $srcNomor . "%' ";
        if ($srcNoPel != "")
            $where .= " AND CPM_BERKAS_NOPEL LIKE '%" . $srcNoPel . "%' ";
        if ($srcTglAwal != "")
            $where .= " AND STR_TO_DATE(CPM_BERKAS_TANGGAL, '%d-%m-%Y') >= STR_TO_DATE('" . convertDate($srcTglAwal) . "', '%Y-%m-%d') ";
        if ($srcTglAkhir != "")
            $where .= " AND STR_TO_DATE(CPM_BERKAS_TANGGAL, '%d-%m-%Y') <= STR_TO_DATE('" . convertDate($srcTglAkhir) . "', '%Y-%m-%d') ";
		 if ($srcNoKTP != "")
            $where .= " AND CPM_BERKAS_NPWP LIKE '%" . $srcNoKTP . "%' ";

        $query = "";
        $hal = (($page - 1) > 0 ? ($page - 1) : 0) * $perpage;

        $query = "SELECT * FROM cppmod_ssb_berkas
                  $where ORDER BY CPM_BERKAS_ID DESC LIMIT " . $hal . "," . $perpage;

        $qry = "SELECT COUNT(*) AS TOTALROWS FROM cppmod_ssb_berkas $where ";
		//echo $query;
        $res = mysqli_query($DBLink, $query);
        if ($res === false) {
        ...
        if (count($data->data) > 0) {
            for ($i = 0; $i < count($data->data); $i++) {
                ...
                if ($tab == '0' || $tab == '1')
                    $HTML .= "<input id=\"\" name=\"check-all" . $tab . "[]\" type=\"checkbox\" value=\"" . $data->data[$i]->CPM_BERKAS_ID . "\" />";
                $HTML .= "</td>\n";
                $HTML .= "\t\t<td width=\"20\" class=\"" . $class . "\" align=\"center\">" . (++$no) . ".</td>\n";

                $HTML .= "\t\t<td class=\"" . $class . "\" align=\"center\"><a style='text-decoration: underline;' href=\"main.php?param=" . base64_encode($params . "&svcid=" . $data->data[$i]->CPM_BERKAS_ID) . "\">" . $data->data[$i]->CPM_BERKAS_NOP . "</a></td> \n";
                $HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_BERKAS_NAMA_WP . "</td> \n";
                $HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_BERKAS_NPWP . "</td> \n";
                $HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_BERKAS_NOPEL . "</td> \n";
                $HTML .= "\t\t<td class=\"" . $class . "\" align=\"center\">" . convertDate($data->data[$i]->CPM_BERKAS_TANGGAL) . "</td> \n";
                $status = $data->data[$i]->CPM_BERKAS_STATUS == 1 ? "Lengkap" : "Tidak Lengkap";
                $HTML .= "\t\t<td class=\"" . $class . "\" align=\"center\">{$status}</td> \n";
                $HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_BERKAS_PETUGAS . "</td> \n";

                if ($tab == 1) {
                    if ($isAdminLoket) {
                        if ($data->data[$i]->CPM_BERKAS_STATUS == '0') {
                            $HTML .= "\t\t<td class=\"" . $class . "\"><input type=\"button\" value=\"Hapus\" title=\"Hapus data berkas\" onclick=\"kembalikanKeLoket('" . $data->data[$i]->CPM_BERKAS_NOP . "');\"/></td> \n";
                        }
                        else
        ...
        $HTML .= "
            <div style='overflow:auto;'>
                <div style='float:left;'>
                    $startLink<input type=\"button\" style=\"background:#fc8621;color:white !important;\" value=\"Tambah\" id=\"btn-add\" name=\"btn-add\"/>$endLink
                    <!-- <input type=\"button\" value=\"Hapus\" id=\"btn-delete\" name=\"btn-delete\" style=\"background:#337ab7;color:white !important;\"/> -->
                    <!-- <input type=\"button\" value=\"Cetak\" id=\"btn-print" . $tab . "\" name=\"btn-print\"  style=\"background:#2c917f;color:white !important;\"/>-->
                    <input type=\"button\" value=\"Cetak Disposisi\" id=\"btn-disposisi" . $tab . "\" name=\"btn-disposisi\"  style=\"background:#2c917f;color:white !important;\"/>
                </div>
                <div style='float:left;margin-top:20px'>
                    ...
                    <input type=\"text\" onKeydown=\"Javascript: if (event.keyCode==13) setTabs(" . $tab . ");\" id=\"srcNomor-" . $tab . "\" name=\"srcNomor\" size=\"30\" value=\"" . $srcNomor . "\" placeholder=\"NOP\"/>
                    <input type=\"text\" onKeydown=\"Javascript: if (event.keyCode==13) setTabs(" . $tab . ");\" id=\"srcNoPel-" . $tab . "\" name=\"srcNoPel\" size=\"30\" value=\"" . $srcNoPel . "\" placeholder=\"Nomor Pelayanan\"/>
                    ...
                    <input type=\"text\" onKeydown=\"Javascript: if (event.keyCode==13) setTabs(" . $tab . ");\" id=\"srcNoKTP-" . $tab . "\" name=\"srcNoKTP\" size=\"30\" value=\"" . $srcNoKTP . "\" placeholder=\"No. KTP/NPWP Wajib Pajak\"/>
					<input type=\"button\" style=\"background:#2c917f;color:white !important;height:30px;width:50px\" value=\"Cari\" id=\"btn-src\" name=\"btn-src\" onclick=\"setTabs(" . $tab . ")\"/>\n
                </div>
            </div>
            </form>";

        $HTML .= "<br><table border=\"0\" width=\"100%\" class=\"table-fill\"><thead>";
        $HTML .= "<tr>";
        $HTML .= "<th class=\"text-center\"><div>
                  <div class=\"all\"><input name=\"checkHapusAll" . $tab . "\" id=\"checkHapusAll" . $tab . "\" type=\"checkbox\" value=\"\"/></div></div></th>";
        $HTML .= "\t\t<th class=\"text-center\"> No.</th> \n";

        $HTML .= "<th class=\"text-center\"> No Objek Pajak</th> ";
        $HTML .= "<th class=\"text-center\"> Nama WP </th> ";
        $HTML .= "<th class=\"text-center\"> NPWP </th> ";
        $HTML .= "<th class=\"text-center\"> Nomor Pelayanan </th> ";
        $HTML .= "<th class=\"text-center\"> Tanggal Terima </th>";
