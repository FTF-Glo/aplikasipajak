...
echo "<link rel=\"stylesheet\" href=\"view/BPHTB/dispenda/mod-dispenda2.css\" type=\"text/css\">\n";
...
class pejabatDispenda extends modBPHTBApprover {
	...
	function getAllDocument(&$dat) {
		...
		for ($i=0;$i<count($data->data);$i++) {
			...
			$HTML .= "\t\t<td class=\"".$class."\">".trim(strip_tags($data->data[$i]->CPM_OP_LETAK))."</td><td class=\"".$class."\" align=\"center\">".
					number_format(intval($ccc),0,".",",")."</td><td class=\"".$class."\">".$data->data[$i]->CPM_SSB_AUTHOR."</td><td class=\"".$class."\">".$data->data[$i]->CPM_TRAN_OPR_DISPENDA_1."</td>\n";
	...
	function getDocumentInfoText($sts,&$dat) {
		...
		for ($i=0;$i<count($data->data);$i++) {
			...
			$HTML .= "\t\t<td class=\"".$class."\" align=\"right\">".number_format(intval($ccc),0,".",",")."</td><td class=\"".$class."\">".$data->data[$i]->CPM_SSB_AUTHOR."</td><td class=\"".$class."\">".$data->data[$i]->CPM_TRAN_OPR_DISPENDA_1."</td>\n";
	...
	function getDocument($sts,&$dat) {
		...
		for ($i=0;$i<count($data->data);$i++) {
			$par1 = $params."&f=f340-func--dispenda-pjb&idssb=".$data->data[$i]->CPM_SSB_ID;
			$class = $i%2==0 ? "tdbody1":"tdbody2";
			if ($this->userGroup==3) if (($data->data[$i]->CPM_TRAN_READ == "") ||($data->data[$i]->CPM_TRAN_OPR_DISPENDA_1 == "")) $class = "tdbodyNew";
			$ccc = $this->getBPHTBPayment($data->data[$i]->CPM_OP_LUAS_BANGUN,$data->data[$i]->CPM_OP_NJOP_BANGUN,$data->data[$i]->CPM_OP_LUAS_TANAH,
			$data->data[$i]->CPM_OP_NJOP_TANAH,$data->data[$i]->CPM_OP_HARGA,$data->data[$i]->CPM_PAYMENT_TIPE_PENGURANGAN,$data->data[$i]->CPM_OP_JENIS_HAK,$data->data[$i]->CPM_OP_NPOPTKP);
			...
			$HTML .= "\t\t<td class=\"".$class."\"><a href=\"main.php?param=".base64_encode($par1)."\">".$data->data[$i]->CPM_OP_NOMOR."</a></td> \n";
			$HTML .= "\t\t<td class=\"".$class."\"><a href=\"main.php?param=".base64_encode($par1)."\">".$data->data[$i]->CPM_WP_NAMA."</a></td> \n";
			$HTML .= "\t\t<td class=\"".$class."\">".trim(strip_tags($data->data[$i]->CPM_OP_LETAK))."</td><td class=\"".$class."\">".$data->data[$i]->CPM_SSB_AUTHOR."</td><td class=\"".$class."\">".$data->data[$i]->CPM_TRAN_OPR_DISPENDA_1."</td><td class=\"".$class."\" align=\"right\">".
	...
	function getFinal ($c,&$dat) {
		...
		if ($srcTxt != "") $where = " WHERE (wp_noktp LIKE '".$srcTxt."%' OR op_nomor LIKE '".$srcTxt."%' OR wp_nama LIKE '".$srcTxt."%')";
		
		
		$query = "SELECT id_switching,wp_noktp,op_nomor,wp_nama,wp_alamat,op_letak,bphtb_dibayar,expired_date,
		PAYMENT_FLAG, PAYMENT_PAID,approved FROM $DbTable $where ORDER BY saved_date DESC LIMIT ".$this->page.",".$this->perpage;
		...
		for ($i=0;$i<count($data->data);$i++) {
			$app = "";
			$par1 = $params."&f=f340-func--dispenda-pjb&idssb=".$data->data[$i]->id_switching;
			$p = base64_encode($data->data[$i]->id_switching);
			$class = $i%2==0 ? "tdbody1":"tdbody2";	
			
			if ($this->getFromGateway ($data->data[$i]->id_switching)) {
				$app = "Disetujui";
			} else {
				$app = "<input type=\"button\" value=\"Setujui\" onclick=\"approve('{$p}','{$a}','{$i}');\"></input>";
			}
			
			if ($data->data[$i]->PAYMENT_FLAG == '0') {
				$app = "Belum Dibayar";
			} 
			$HTML .= "\t<div class=\"container\"><tr>\n";
			$HTML .= "\t\t<td class=\"".$class."\" width=\"160px\">".$data->data[$i]->wp_noktp."</td> \n";
			$HTML .= "\t\t<td class=\"".$class."\" width=\"140px\">".$data->data[$i]->op_nomor."</td> \n";
			...
			$HTML .= "\t\t<td class=\"".$class."\" align=\"right\" width=\"100px\">".number_format(intval($data->data[$i]->bphtb_dibayar),0,".",",")."</td> \n";	
			$HTML .= "\t\t<td class=\"".$class."\" width=\"60px\" align=\"center\">".$data->data[$i]->expired_date."</td> \n";		
			$HTML .= "\t\t<td class=\"".$class."\" width=\"110px\" align=\"center\">".$data->data[$i]->PAYMENT_PAID."</td> \n";	
			$HTML .= "\t\t<td class=\"{$class}\" align=\"center\" width=\"80px\"><span id=\"appr-{$i}\">{$app}</span></td> \n";	
			$HTML .= "\t</tr></div>\n";
		}
		$dat = $HTML;
		return true;
	...
	public function headerFinal($a) {
		$HTML = "<form id=\"form-notaris\" name=\"form-notaris\" method=\"post\" action=\"\" >";
		$HTML .= "Masukan Pencarian Berdasarkan Nama WP/NOP/NOKTP <input type=\"text\" id=\"src-approved\" name=\"src-approved\" size=\"60\"/> 
				<input type=\"submit\" value=\"Cari\" id=\"btn-src\" name=\"btn-src\" />\n</form>\n";
		$HTML .= "<table cellpadding=\"4\" cellspacing=\"1\" border=\"0\" width=\"100%\">\n";
		$HTML .= "\t<tr>\n";
		$HTML .= "\t\t<td class=\"tdheader\">Nomor KTP</td> \n";
		$HTML .= "\t\t<td class=\"tdheader\">Nomor Objek Pajak</td> \n";
		$HTML .= "\t\t<td class=\"tdheader\">Wajib Pajak </td> \n";
		$HTML .= "\t\t<td class=\"tdheader\">Alamat</td> \n";
		$HTML .= "\t\t<td class=\"tdheader\">Letak</td><td class=\"tdheader\">BPHTB yang harus dibayar</td>";
		$HTML .= "\t\t<td class=\"tdheader\" >Tgl. Kadaluarsa</td>\n";
		//$HTML .= "\t\t<td class=\"tdheader\" >Status Bayar</td>\n";
		$HTML .= "\t\t<td class=\"tdheader\" >Tgl. Bayar</td>\n";
		$HTML .= "\t\t<td class=\"tdheader\" >Disetujui</td>\n";
		$HTML .= "\t</tr>\n";

		if ($this->getFinal($a,$dat)) {
			$HTML .= $dat;
		} else {
			$HTML .= "Data Kosong !";
		}
	...
	public function getContent() {
		...
		switch ($this->status) {
			//case 102 :
			//	$HTML .= $this->headerFinal(1);
			//	break;
			case 101 :
				$HTML .= $this->headerFinal(0);
				break;
			case 100 :
...
$par6 = $params."&n=5&s=101";
//$par7 = $params."&n=6&s=102";
...
$a = $_REQUEST['a'];
if ($modNotaris->getConfigValue($a,'TYPE_PROSES')=='1') $modNotaris->addMenu("Persetujuan Final","all-menu",base64_encode($par6));
//$modNotaris->addMenu("Disetujui Final","all-menu",base64_encode($par7));
