...
function displayMenu ($selected) {
	...
	echo "\t<ul>\n";
	echo "\t\t<a href=\"main.php?param=".base64_encode($par1)."\"><li ".$selected1."><span id=\"app-menu\">Disetujui</span></li></a>\n";
	echo "\t\t<a href=\"main.php?param=".base64_encode($par2)."\"><li ".$selected2."><span id=\"rej-menu\">Ditolak</span></li></a>\n";
	echo "\t\t<a href=\"main.php?param=".base64_encode($par3)."\"><li ".$selected3."><span id=\"dil-menu\">Tertunda</span></li></a>\n";
	echo "\t\t<a href=\"main.php?param=".base64_encode($par4)."\"><li ".$selected4."><span id=\"tmp-menu\">Sementara</span></li></a>\n";
	echo "\t\t<a href=\"\" onclick=\"printNFPDF();\"><li><span id=\"pdf-menu\">Form SSB(PDF)</span></li></a>\n";
	echo "\t</ul>\n";
	echo "</div>\n";
}
...
function getDocument($sts,&$dat) {
	...
	$srcTxt = $_REQUEST['src-approved']?$_REQUEST['src-approved']:"";
	$where = "";
	if ($srcTxt != "") $where = " AND A.CPM_WP_NAMA LIKE '".$srcTxt."%'";
	$query = "SELECT * FROM cppmod_ssb_doc A, cppmod_ssb_tranmain B WHERE A.CPM_SSB_ID = B.CPM_TRAN_SSB_ID AND B.CPM_TRAN_STATUS=".$sts." AND  B.CPM_TRAN_FLAG=0 AND
			B.CPM_TRAN_OPR_NOTARIS ='".$data->uname."' $where ORDER BY B.CPM_TRAN_DATE DESC LIMIT 0,200";
	...
	for ($i=0;$i<count($data->data);$i++) {
		...
		if (($sts==4) || ($sts==5)) $HTML .= "\t\t<td width=\"20\" class=\"".$class."\" align=\"center\">
		...
		$HTML .= "\t\t<td class=\"".$class."\">".$data->data[$i]->CPM_WP_ALAMAT."</td><td class=\"".$class."\" align=\"center\">".
				$data->data[$i]->CPM_TRAN_SSB_VERSION."</td>\n";
...
function contentNotarisApproved() {
...
function displayContent($selected) {
	...
	if ($selected == 1) {
		echo contentNotarisApproved();
	}
	if ($selected == 2) {
		echo contentNotarisReject();
	}
	if ($selected == 3) {
		echo contentNotarisDelay();
	}
	if ($selected == 4) {
		echo contentNotarisTemp();
	}
	echo "\t</div>\n";
