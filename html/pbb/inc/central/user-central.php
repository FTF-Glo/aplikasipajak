...
class SCANCentralUser {
	...
	public function GetModuleInApp($sUID, $app, &$moduleIds) {
		// FIX: mysql escape string
		// var_dump( $moduleIds);die;
		$sUID = mysqli_real_escape_string($this->DBLink, $sUID);
		$app = mysqli_real_escape_string($this->DBLink, $app);
		// var_dump($sUID,$app);die;
		CTOOLS_ArrayRemoveAllElement($moduleIds);
		$bOK = false;
		
		$sQ = "SELECT DISTINCT C.CTR_USER_ID, F.CTR_RM2F_MID, F.CTR_RM2F_PRIV, M.CTR_M_NAME,M.CTR_M_SUBMENU, M.CTR_M_ICON_SUBMENU " .
		...
		// NEW: order by
		$sQ .= "ORDER BY M.URUTAN ASC ";
		// echo $sQ;exit;
		
		if (CTOOLS_IsInFlag($this->iDebug, DEBUG_DEBUG))
			error_log ("[".strftime("%Y%m%d%H%M%S", time())."][".$this->sThisFile.":".__LINE__."] [DEBUG] sQ [$sQ]\n", 3, $this->sLogFilename);
			// var_dump($this->DBLink);die;
			// var_dump(mysqli_query($this->DBLink, $sQ));die;
		if ($res = mysqli_query($this->DBLink, $sQ)) {
			$bOK = true;
			$nRes = mysqli_num_rows($res);
			if ($nRes > 0) {
				$i = 0;
				while ($row = mysqli_fetch_array($res)) {
					// var_dump($row["CTR_M_ICON_SUBMENU"]);
					$moduleIds[$i]["id"] = $row["CTR_RM2F_MID"];
					$moduleIds[$i]["name"] = $row["CTR_M_NAME"];
					$moduleIds[$i]["submenu"] = $row["CTR_M_SUBMENU"];
					$moduleIds[$i]["icon"] = $row["CTR_M_ICON_SUBMENU"];
	...
	public function GetDbConnection($dbId) {
		...
		if ($res = mysqli_query($this->DBLink, $sQ)) {
			...
			if ($nRes > 0) {
				...
				SCANPayment_ConnectToDB($dbLink, $dbConn, $hostport, $user, $pwd, $schema,true);
	...
	public function GetTerminal($appId, $moduleId, &$terminal) {
		...
		if (CTOOLS_IsInFlag($this->iDebug, DEBUG_DEBUG))
			error_log ("[".strftime("%Y%m%d%H%M%S", time())."][".$this->sThisFile.":".__LINE__."] [DEBUG] sQ [$sQ]\n", 3, $this->sLogFilename);
		if ($res = mysqli_query($dbLink, $terminalQuery)) {
			$nRes = mysqli_num_rows($res);
			if ($nRes > 0) {
				$i = 0;
				while ($row = mysqli_fetch_array($res)) {
	...
	public function IsAccessible($uid, $appId, $terminalId, $terminalColumn) {
		...
		if ($dbApp != null) {
			...
			if ($res = mysqli_query($dbApp, $query)) {
				$nRes = mysqli_num_rows($res);
				if ($nRes > 0) {
					$accessible = true;
				}
    ...
    // aldes
    public function GetPBBUser($uid) {
		$uid = mysqli_real_escape_string($this->DBLink, $uid);
		$sQ = "SELECT * FROM tbl_reg_user_pbb WHERE ctr_u_id = '{$uid}'";
		if (CTOOLS_IsInFlag($this->iDebug, DEBUG_DEBUG))
			error_log ("[".strftime("%Y%m%d%H%M%S", time())."][".$this->sThisFile.":".__LINE__."] [DEBUG] sQ [$sQ]\n", 3, $this->sLogFilename);
		if (false !== $res = mysqli_query($this->DBLink, $sQ)) {
	...
	public function GetLoggedInUser() {
		$cData = isset($_COOKIE['centraldata']) ? $_COOKIE['centraldata'] : null;
		if ($cData !== null) {
			$cData = base64_decode($cData);
			if ($cData) {
				$cData = json_decode($cData);
				if (isset($cData->uid)) {
					return $this->GetPBBUser($cData->uid);
				}
			}
		}
		return false;
