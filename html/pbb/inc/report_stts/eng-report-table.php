...
Class ReportEngineTable extends ReportEngine{
	...
	var $LastErrorMsg;
	var $currentDate;
	function ReportEngineTable($rt,$HValues,$BValues,$FValues, $printerType = null) {
		if (!file_exists($rt)){
			$this->LastError=true;
			...
			$this->ReportEngine($rt, $printerType);
	...
	function PrintHTMLPreview($startPage=1,$endPage=1){
		if($this->LastError){
			...
			for($page=$spage;$page<=$epage;$page++){
				...
				// COLUMN
				$html = "";
				if($this->ReportObject->column) {
						$this->procesLineHTMLPreview($this->ReportObject->column->line, null);
						$strHTML.=$html;
				}
		...
		if($this->LastError){
			...
			for($page=$spage;$page<=$epage;$page++){
				...
				*/
				...
				$html = "";
				if($this->ReportObject->column) {
						$this->procesLineHTML($this->ReportObject->column->line, null, $html);
						$strHTML.=$html;
				}
	...
	function procesLine2File($lines,$values){
		...
		foreach ($lines as $line){
			if(isset($line["space"])){
				switch($line["space"]){
					case "0":
						$strbuffer .= $this->PrinterDriver->command("lineSpacing7/72"); // 7/72 inch line space
						break;
					case "1":
						$strbuffer .= $this->PrinterDriver->command("lineSpacing1/8"); // 1/8 inch line space
					default:
						$strbuffer .= $this->PrinterDriver->command("lineSpacing1/6"); // 1/6 inch line space
				}
			}else
				$strbuffer .= $this->PrinterDriver->command("lineSpacing1/8"); // 1/8 inch line space
			foreach ($line->item as $item){
	...
	function procesLine2OnpaysTxt($lines,$values){
		$strbuffer='';
		$i=0;
		foreach ($lines as $line){
				$j=0;
				//if($i>0) $strbuffer.="~l/";
				foreach ($line->item as $item){
					$itemtext=$this->ApplyTemplateValue($item["text"],$values);
					if($itemtext!=""){
						if($j>0) $strbuffer.="~i/";
						if($item["bold"]=="true"){//0
							$strbuffer.="1~~";
			...
			if($handle==FALSE){
				...
				for($page=$spage;$page<=$epage;$page++){
					...
					*/
					...
					// COLUMN HEADER
					if($this->ReportObject->column) {
						$strbuffer.=$this->procesLine2File($this->ReportObject->column->line, null);
					}
					//proses body
					if ($page == 1) {
						$startRec = 0;
	...
	function Print2TXT(&$strTXT, $startPage=1,$endPage=1,$FFatEnd=1){
		...
		if($this->LastError){
			...
			for($page=$spage;$page<=$epage;$page++){
				$strTXT .= $this->PrinterDriver->command("init"); // reset
				$strTXT .= $this->PrinterDriver->command("cpi10"); // 10cpi, init with (comm)
				$strTXT .= $this->PrinterDriver->command("controlPaper"); // select paper feeder
				if(isset($this->ReportObject["space"])){
					switch($this->ReportObject["space"]){
						case "0":
							$strTXT .= $this->PrinterDriver->command("lineSpacing7/72"); // 7/72 inch line space
							break;
						case "1":
							$strTXT .= $this->PrinterDriver->command("lineSpacing1/8"); // 1/8 inch line space
						default:
							$strTXT .= $this->PrinterDriver->command("lineSpacing1/6"); // 1/6 inch line space
					}
				}else
					$strTXT .= $this->PrinterDriver->command("lineSpacing1/8"); // 1/8 inch line space
				if($this->ReportObject["length"]>0 and $this->ReportObject["length"]<=127){
					$strTXT .= ($this->PrinterDriver->command("setPageLength", chr(intval($this->ReportObject["length"])))); //page length 22 line
					// $strTXT.=(chr(27)."C".chr(intval($this->ReportObject["length"]))); //page length 22 line
				}
				$strTXT .= ($this->PrinterDriver->command("setLeftMargin", chr(intval($this->ReportObject["lmargin"])))); // left margin line
				$strTXT .= ($this->PrinterDriver->command("setRightMargin", chr(intval($this->ReportObject["rmargin"])))); // left margin line
				...
				*/
				...
				// COLUMN HEADER
				if($this->ReportObject->column) {
						$strTXT.=$this->procesLine2File($this->ReportObject->column->line, null);
				}
				...
				if ($FFatEnd==1) {
					$strTXT .= ($this->PrinterDriver->command("formFeed") . str_repeat(chr(13),1)); //FF
					// $strTXT.=(chr(12).str_repeat(chr(13),1)); //FF
				}
				$strTXT .= $this->PrinterDriver->command("init"); // reset
				// $strTXT.=(chr(27)."@"); //reset
			}
	...
	function Print2OnpaysTXT(&$strTXT, $startPage=1,$endPage=1,$FFatEnd=1){
		$strTXT = "";
		if($this->LastError){
			return FALSE;
		}
			...
			for($page=$spage;$page<=$epage;$page++){
				...
				if($page==1){
					if($this->ReportObject->header){
						$strTXT .= $this->procesLine2OnpaysTxt($this->ReportObject->header->line,$this->ReportHeaderValues);
					}
				}
				$strTXT .="~h/";
				...
				// COLUMN HEADER
				if($this->ReportObject->column) {
						$strTXT.=$this->procesLine2OnpaysTxt($this->ReportObject->column->line, null);
				}
				...
				for($i=$startRec;$i<=$endRec;$i++){
					$this->ReportBodyValues[$i]['RECNO']=$i+1;
					$strTXT .= $this->procesLine2OnpaysTxt($this->ReportObject->body->line,$this->ReportBodyValues[$i]);
				...
				$this->ReportFooterValues['CURRENTDATE']=$this->currentDate;
				if($this->ReportObject->footer){
					$strTXT .= $this->procesLine2OnpaysTxt($this->ReportObject->footer->line,$this->ReportFooterValues);
				}
				$strTXT .="~f/";
				$strTXT .="~p/";
