...
function SCANPayment_ConnectToDB(&$DBLink, &$DBConn, $sDBHost, $sDBUser, $sDBPwd, $sDBName, $newLink = false)
{
  ...
  // var_dump($explodeHost);
  if (count($explodeHost) > 1) {
    $host = $explodeHost[0];
    $port = $explodeHost[1];
  }

  $sThisFile = basename(__FILE__);
  // var_dump($sDBHost, $sDBUser, $sDBPwd, $sDBName);
  if ($DBLink = mysqli_connect($host, $sDBUser, $sDBPwd, $sDBName, $port)) {
    /*if ($DBConn = mysqli_select_db($sDBName, $DBLink))
    {
    }
    else
    {
