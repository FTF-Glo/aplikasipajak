...
{
  return (is_numeric($iDebug) && is_numeric($iFlag) && ($iDebug & $iFlag) == $iFlag);
} // end of IsInFlag
...
{
  while (is_array($aRemove) && sizeof($aRemove) > 0) array_shift($aRemove);
  return $aRemove;
...
function CTOOLS_ExecCommand($sCmd, $sIn = "", &$sOut = "", &$sErr = "")
{
  ...
  if (is_resource($Process)) {
    ...
    $sOut = '';
    while (!feof($aPipes[1])) $sOut .= fread($aPipes[1], 1024);
    fclose($aPipes[1]);
    //echo "out [$sOut]\n";

    $sErr = '';
    while (!feof($aPipes[2])) $sErr .= fread($aPipes[2], 1024);
    fclose($aPipes[2]);
    //echo "err [$sErr]\n";
