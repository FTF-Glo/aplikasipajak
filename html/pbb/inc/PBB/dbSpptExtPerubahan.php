...
class DbSpptExtPerubahan {
	...
	public function get($id, $vers, $num="", $filter="") {
		...
		$query = "SELECT * FROM cppmod_pbb_service_change_ext ";
...
//		echo $query;
	...
	public function add ($id, $vers, $num, $aValue) {
		...
		$query = "INSERT INTO cppmod_pbb_service_change_ext (CPM_SPPT_DOC_ID, CPM_SPPT_DOC_VERSION, CPM_OP_NUM, ";
		$tmpVals = "'$id', '$vers', '$num', ";
	...
	public function edit($id, $vers, $num, $aValue) {
		...
		$last_key = end(array_keys($aValue));
		$query = "UPDATE cppmod_pbb_service_change_ext SET ";
	...
	public function isExist($id, $vers, $num) {
		...
		$query = "SELECT * FROM cppmod_pbb_service_change_ext WHERE CPM_SPPT_DOC_ID='$id' AND CPM_SPPT_DOC_VERSION='$vers' AND CPM_OP_NUM='$num'";
