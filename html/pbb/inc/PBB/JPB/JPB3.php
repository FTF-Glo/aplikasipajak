<?php class JPB3 {
    ...
    public static function get($nop, $thn, $data, $conn) {
        ...
        $tinggiKolom    = (float)$data->CPM_JPB3_TINGGI_KOLOM;
        $lebarBentang   = (float)$data->CPM_JPB3_LEBAR_BENTANG;
        $kelilingDinding= (float)$data->CPM_JPB3_KELILING_DINDING;
        $luasMezzanine  = (float)$data->CPM_JPB3_LUAS_MEZZANINE;
        $dayaDukung     = (float)$data->CPM_JPB3_DAYA_DUKUNG_LANTAI;

        $atap           = sprintf("%02d", (int)$data->CPM_OP_ATAP);
        $lantai         = sprintf("%02d", (int)$data->CPM_OP_LANTAI);
        $langit         = sprintf("%02d", (int)$data->CPM_OP_LANGIT);
        ...
        while ($obj = mysqli_fetch_object($res)) {
            $typeKonstruksi = (float)$obj->CPM_TYPE_KONSTRUKSI;
        }

        $sql = "SELECT CPM_NILAI_DBKB_JPB3 
                FROM cppmod_pbb_dbkb_jpb3 
                WHERE 
                    CPM_KD_PROPINSI = '$kdPropinsi' 
                    AND CPM_KD_DATI2 = '$dati2' 
                    AND CPM_THN_DBKB_JPB3 = '$thn' 
                    AND CPM_LBR_BENT_MIN_DBKB_JPB3 <= $lebarBentang 
                    AND CPM_LBR_BENT_MAX_DBKB_JPB3 >= $lebarBentang 
                    AND CPM_TINGGI_KOLOM_MIN_DBKB_JPB3 <= $tinggiKolom 
                    AND CPM_TINGGI_KOLOM_MAX_DBKB_JPB3 >= $tinggiKolom";
        $res = mysqli_query($conn, $sql);
        while ($obj = mysqli_fetch_object($res)) {
            $nilaiKomponenUtama = (float)$obj->CPM_NILAI_DBKB_JPB3;
        }
        ...
        $nilaiMaterial = ($nilaiAtap + $nilaiLantai + $nilaiLangit) * 1.3;
        
        $totalNilaiDinding = $kelilingDinding * $tinggiKolom * $nilaiDinding * 1.666667;
        $totalNilaiDinding = $totalNilaiDinding * 1.3;
        $totalNilaiDinding = round($totalNilaiDinding, 0);
        ...
        $nilaiFasilitas = FasilitasSusutLuas::get($nop, $thn, $data, $conn, $kdJpb="03", $klsBintang);

        $nilaiTotalPerM2 = $nilaiKomponenUtama + $nilaiMaterial + $nilaiDayaDukung + $nilaiFasilitas;

        $nilaiTotalKaliLuas = $nilaiTotalPerM2 * $luasBng;
        ...
        $mNilaiTotalMezzanine = $luasMezzanine * $nilaiMezzanine;
        
        $nilaiFasilitasSusut = FasilitasSusut::get($nop, $thn, $data, $jmlLantai, $conn);

        $nilaiSebelumSusut = $nilaiTotalKaliLuas + $totalNilaiDinding + $mNilaiTotalMezzanine + $nilaiFasilitasSusut;
