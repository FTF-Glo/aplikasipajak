...
class DbSpptTran
{
	...
	public function getDetail($id = "", $filter = [], $custom = "", $jumhal, $perpage, $page)
	{
		...
		$whereClause .= "AND CPM_TRAN_SPPT_DOC_ID <> '' AND CPM_TRAN_SPPT_DOC_ID IS NOT NULL";
		$queryCount .= $whereClause;
		$this->dbSpec->sqlQueryRow($queryCount, $total);
		$this->totalrows = isset($total[0]['TOTAL']) && $total[0]['TOTAL'] != null ? $total[0]['TOTAL'] : 0;
		// $this->totalrows = $this->dbSpec->sqlQueryRows($queryCount);
