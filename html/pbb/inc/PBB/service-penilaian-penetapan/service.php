...
class Service {
	...
	private function doMainProcess($data){
		...
		foreach($data as $row){
			...
			$total = 0;
			try {
				$sRequestStream = "{\"PAN\":\"TP\",\"TAHUN\":\"".$tahun."\",\"KELURAHAN\":\"".$row['CPC_TKL_ID']."\",\"TIPE\":\"1\",\"NOP\":\"".$nop."\",\"SUSULAN\":\"".$susulan."\",\"TANGGAL\":\"".$tanggal."\",\"USER\":\"masal\"}";
				$bOK = GetRemoteResponse($ServerAddress, $ServerPort, $ServerTimeOut, $sRequestStream, $sResp);
				if ($bOK == 0) {
					$sResp = rtrim($sResp, END_OF_MSG);
					$r = explode(",",$sResp);
			...
			echo "[".date('d-m-Y H:i:s')."][Penetapan : {$status}][{$total} NOP] {$nomor}. {$kdKel} - {$nmKel}\n";
			/*END PENETAPAN*/
	...
	private function confirm($data){
		...
		echo "Ketik 'ya' untuk melanjutkan proses Penilaian dan Penetapan : ";
