...
class dbMonitoring
{
	...
	{
		if ($this->dbConn = mysqli_connect($this->dbHost . ":" . $this->dbPort, $this->dbUser, $this->dbPassword, $this->dbName)) {
			//if (mysqli_select_db($this->dbName, $this->dbConn)) return true;
			return true;
		}
	...
	public function pembayaran2($sts, $nop, $tahun, $uname, $tgl)
	{
		if ($sts == '0') {
			$sql = "SELECT 
					...
					FROM {$this->dbTable} {$this->dbWhere}";

			$this->mysql($errMsg) or die(mysqli_error($this->dbConn));
			...
			$query = "Update " . $this->dbTable . " set payment_flag = '1', PBB_DENDA= {$denda}, payment_paid = '{$tgl}', PBB_TOTAL_BAYAR = {$total} " . $this->dbWhere;
			$statusPembayaran = 1;
		} else {
			$query = "Update " . $this->dbTable . " set payment_flag = '3', payment_paid = '', PBB_TOTAL_BAYAR = '0' " . $this->dbWhere;
			$statusPembayaran = 3;
	...
	public function query_result($query)
	{
		...
		else $query = $query . $this->dbTable . $this->dbWhere . " ORDER BY sppt_tahun_pajak DESC, SPPT_TANGGAL_CETAK desc " . $this->paging_query();
	...
	public function query_result_summary()
	{
		...
		//else $query = "Select COALESCE(SUM(A.SPPT_PBB_HARUS_DIBAYAR),0) as pokok, COALESCE(SUM({$a}),0) as denda,  COALESCE(SUM(A.SPPT_PBB_HARUS_DIBAYAR+(COALESCE({$a}, 0))),0) as total " . $this->dbTable . $this->dbWhere;

		$query = "Select COALESCE(SUM(A.SPPT_PBB_HARUS_DIBAYAR),0) as pokok, COALESCE(SUM({$a}),0) as denda,  COALESCE(SUM(A.SPPT_PBB_HARUS_DIBAYAR+(COALESCE({$a}, 0))),0) as total " . $this->dbTable . $this->dbWhere;
	...
	private function getFieldFromMysql()
	{
		...
		for ($j = 0; $j < $i; $j++) {
			$this->fields[$j]["name"] = mysqli_fetch_field_direct($this->result, $j)->name;
		}
	}
	...
	public function showHTMLMundur()
	{
		$this->export = false;
		$this->query_result_mundur($this->dbQuery);
		$html = "<div id=\"frame-tbl-monitoring\" class=\"tbl-monitoring\"><table width=\"100%\" border=\"0\" cellspacing=\"1\" cellpadding=\"4\">";
	...
	public function showHTMLsearchMundur($uid)
	{
		$this->export = false;
		$this->query_result_mundur($this->dbQuery);
	...
	public function query_result_mundur($query)
	{
		$result = array();

		if ($this->export) $query = $query . $this->dbTable . $this->dbWhere . " ORDER BY cpm_nop DESC ";
		else $query = $query . $this->dbTable . $this->dbWhere . " ORDER BY cpm_nop DESC " . $this->paging_query();
		// echo "---------- <br>".$query."<br>";exit;
		if ($this->dbType == 1) {
			if ($this->postgres($errMsg)) {
				$this->result = pg_query($this->dbConn, $query);
							...
							if ($this->dbTitles->data[$x]->field == $this->fields[$i]["name"]) {
								...
								if ($format == "optional") {
									if ($row[$this->fields[$x]["name"]] == 1)
										...
										$opt[$row[$this->fields[$x]["name"]]] = "<input type=\"button\" name=\"btn-bayar\" id=\"btn-bayar\" value=\"Bayar\" onclick=\"bayar_tgl('" . $row['nop'] . "'," . $row['sppt_tahun_pajak'] . ",'0','" . $uid . "')\">";
									}
								}
							}
						}
...
?>
			<script type="text/javascript">
				function bayar_tgl(nop, thnpjk, sts, uid) {
					var tgl = document.getElementById('tgl-bayar').value;
					tgl_pembayaran(nop, thnpjk, sts, uid, tgl + ' 00:00:00');
				}
			</script>
<?php
		}
