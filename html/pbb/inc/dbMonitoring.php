...
//		return $query;               
	...
	private function paging_query() {
		...
		if ($this->dbType == 1) { 
			$html = " LIMIT ".$this->rowPerpage." OFFSET {$p_num}";
		}
		return $html;	
	}
	...
	private function paging() {
		...
		if($page_amount != "0"){
			...
			if($page != "0"){
				$prev = $page-1;
				if ($p_num >1)
				$txtpp = "<span id=\"prev-page\" name=\"prev-page\" onclick=\"setPage({$page_prev})\"></span>";
			}
			/*for ( $counter = 0; $counter <= $page_amount; $counter += 1) {
				$html .="<a href=\"section.php?q=$section&p=$counter\">";
				$html .=$counter+1;
				$html .="</a>";
			}*/
			if($lp< $tot){
				$next = $page+1;
				//if ($lp < $tot)
				$txtnp = "<span id=\"next-page\" name=\"next-page\" onclick=\"setPage({$page_next})\"></span>";
			} else {
				$lp = $tot;
			}
	...
	public function query_result ($query) {
		...
		if ($this->export) $query = $query.$this->dbTable.$this->dbWhere." ORDER BY wp_nama ASC ";
		else $query = $query.$this->dbTable.$this->dbWhere." ORDER BY wp_nama ASC ".$this->paging_query();
		echo "---------- <br>".$query."<br>";
		if ($this->dbType == 1) {
			if ($this->postgres($errMsg)) {
	...
	private function getFieldFromPostgre() {
		...
		for ($j = 0; $j < $i; $j++) {
			...
			/*$this->fields[$j]["name"] = pg_field_name($this->result, $j);
			$this->fields[$j]["length"] = pg_field_prtlen($this->result, $this->fields[$j]["name"]);
			$this->fields[$j]["type"] = pg_field_type($this->result, $j);*/
		}
	}
					...
					if ($this->dbTitles) {
						for ($x=0;$x<count($this->dbTitles->data);$x++) {
							if ($this->dbTitles->data[$x]->field == $this->fields[$i]["name"]){
								...
								$format = @isset($this->dbTitles->data[$x]->format) ? $this->dbTitles->data[$x]->format : "";
								if ($format == "optional") {
									$opt[0] = "<input type=\"button\" name=\"btn-bayar\" id=\"btn-bayar\" value=\"Bayar\" onclick=\"bayar('".$row['nop']."',".$row['sppt_tahun_pajak'].",'0')\">";
									$opt[1] = "<input type=\"button\" name=\"btn-bayar\" id=\"btn-bayar\" value=\"Batal\" onclick=\"bayar('".$row['nop']."',".$row['sppt_tahun_pajak'].",'1')\">";
									$opt[3] = "<input type=\"button\" name=\"btn-bayar\" id=\"btn-bayar\" value=\"Bayar\" onclick=\"bayar('".$row['nop']."',".$row['sppt_tahun_pajak'].",'0')\">";
								}
							}
						}
					}
						...
						for ($x=0;$x<count($this->dbTitles->data);$x++) {
							if ($this->dbTitles->data[$x]->field == $this->fields[$i]["name"]){
								...
								if ($format == "optional") {
									...
									$opt[1] = "<input type=\"button\" name=\"btn-bayar\" id=\"btn-bayar\" value=\"Batal\" onclick=\"bayar('".$row['nop']."',".$row['sppt_tahun_pajak'].",'1')\">";
									$opt[3] = "<input type=\"button\" name=\"btn-bayar\" id=\"btn-bayar\" value=\"Bayar\" onclick=\"bayar('".$row['nop']."',".$row['sppt_tahun_pajak'].",'0')\">";
									$opt[NULL] = "<input type=\"button\" name=\"btn-bayar\" id=\"btn-bayar\" value=\"Bayar\" onclick=\"bayar('".$row['nop']."',".$row['sppt_tahun_pajak'].",'0')\">";
								}
							}
						}
					}
