...
while ($rowData = mysqli_fetch_assoc($result['data'])) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('E' . $row, $rowData['wp_alamat']);
    $objPHPExcel->getActiveSheet()->setCellValue('F' . $row, $rowData['wp_kelurahan']);
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('H' . $row, $rowData['op_rt']);
    $objPHPExcel->getActiveSheet()->setCellValue('I' . $row, $rowData['op_rw']);
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('J' . $row, $rowData['sppt_pbb_harus_dibayar']);
...
$objPHPExcel->getActiveSheet()->setCellValue('J' . $row, ' ' . number_format($totalPokok, 0, ',', '.'));
...
$namafile = ($status=='1') ? 'SUDAH_BAYAR_VERSION_2' : 'Belum_Bayar_Version_2';
$uniq = uniqid();
