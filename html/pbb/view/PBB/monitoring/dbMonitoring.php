...
class dbMonitoring
{
	...
	public function mysql(&$msg)
	{
		...
		$msg = mysqli_connect_errno(); //mysqli_error($DBLink);
		return false;
	}
	...
	public function query_result_summary()
	{
		...
		if ($this->status == 1)
			$query = "SELECT SUM(COALESCE(A.SPPT_PBB_HARUS_DIBAYAR, 0)) AS pokok, SUM(COALESCE(A.PBB_DENDA, 0)) AS denda,  SUM(COALESCE(A.PBB_TOTAL_BAYAR, 0)) AS total " . $this->dbTable . $this->dbWhere;
		else $query = "SELECT SUM(COALESCE(A.SPPT_PBB_HARUS_DIBAYAR, 0)) AS pokok, SUM(COALESCE(B.PBB_DENDA, 0)) AS denda,  SUM(COALESCE(A.SPPT_PBB_HARUS_DIBAYAR,0) + COALESCE(B.PBB_DENDA, 0)) AS total " . $this->dbTable . $this->dbWhere;
		// print_r($this->status); print_r($query);exit;
		if ($this->dbType == 0) {
			if ($this->mysql($errMsg)) {
	...
	private function createTitleHeader()
	{
		...
		$txtHeader = "";
		for ($i = 0; $i < $c; $i++) {
			$txtHeader .= "<th>" . strtolower($this->fields[$i]["name"]) . "</th>";
		}
		if ($this->dbTitles) {
		...
		if ($this->dbType == 0) {
			while ($row = $this->getDataFromMysql()) {
				...
				for ($i = 0; $i < $c; $i++) {
					...
					if ($this->dbTitles) {
						for ($x = 0; $x < count($this->dbTitles->data); $x++) {
							if ($this->dbTitles->data[$x]->field == strtolower($this->fields[$i]["name"])) {
								$align = @isset($this->dbTitles->data[$x]->align) ? $this->dbTitles->data[$x]->align : "";
								$format = @isset($this->dbTitles->data[$x]->format) ? $this->dbTitles->data[$x]->format : "";
								if ($format == "optional") {
									$opt[0] = $this->dbTitles->data[$x]->optional[0];
	...
	public function showHTML()
	{
		...
		$html = '<div class="responsive"><table class="table table-bordered table-striped">';
	...
	public function showHTMLsearch($uid)
	{
		$this->export = false;
		$this->query_result($this->dbQuery);
		$html = "<div id=\"frame-tbl-monitoring\" class=\"tbl-monitoring\"><table class=\"table table-bordered\" border=\"0\" cellspacing=\"1\" cellpadding=\"4\">";
	...
	public function exportToXls()
	{
		...
		$this->export = true;
		$this->query_result($this->dbQuery);
		$html = "<table class=\"table table-bordered\" border=\"1\" cellspacing=\"1\" cellpadding=\"4\">";
