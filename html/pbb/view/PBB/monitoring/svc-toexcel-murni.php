...
function headerMonitoringE2 ($mod,$nama) {
	...
	$html = "<table cellspacing=\"0\" cellpadding=\"4\" border=\"1\"><tr><td colspan=\"17\"><b>{$dl}<b></td></tr>
	  <col width=\"28\" />
	  <col width=\"187\" />
	  <col width=\"47\" />
	  <col width=\"89\" />
	  ...
	  <tr>
		...
		<td colspan=\"4\" width=\"136\" align=\"center\">KETETAPAN</td>
	  ...
	  <tr>
		...
		<th align=\"center\">LUAS BUMI</td>
		<th align=\"center\">LUAS BANGUNAN</td>
...
//	$html = headerMonitoringE2 ($mod,$nama);
	...
	for ($i=0;$i<$c;$i++) {
            ...
            $tmp = array(
				...
				"luas_bumi" => number_format($dt[$i]["LT"],0,"",""),
				"luas_bangunan" => number_format($dt[$i]["LB"],0,"",""),
...
function getData($where) {
	...
	$query = "SELECT count(wp_nama) AS WP, sum(PBB_TOTAL_BAYAR) as RP, sum(OP_LUAS_BUMI) AS LT, sum(OP_LUAS_BANGUNAN) AS LB FROM pbb_sppt {$whr}"; //echo $query."<br>";
...
function getDataTargetE2($where) {
	...
	$query = "SELECT count(wp_nama) AS WP, sum(SPPT_PBB_HARUS_DIBAYAR) as RP, sum(OP_LUAS_BUMI) AS LT, sum(OP_LUAS_BANGUNAN) AS LB FROM pbb_sppt {$whr}"; //echo $query.'<br/>';
	$res = mysqli_query($myDBLink, $query);
	if ($res === false) {
		echo mysqli_error($myDBLink);
		exit();
...
$objPHPExcel->getActiveSheet()->setCellValue('D9', 'LUAS BUMI');
$objPHPExcel->getActiveSheet()->setCellValue('E9', 'LUAS BANGUNAN');
...
$objPHPExcel->getActiveSheet()->setCellValue('G9', 'WP');
$objPHPExcel->getActiveSheet()->setCellValue('H9', 'RP');
$objRichText = new PHPExcel_RichText();
$objRichText->createText('%');
...
$objPHPExcel->getActiveSheet()->setCellValue('L9', 'WP');
$objPHPExcel->getActiveSheet()->setCellValue('M9', 'RP');
$objRichText = new PHPExcel_RichText();
$objRichText->createText('%');
...
$objPHPExcel->getActiveSheet()->setCellValue('P9', 'RP');
$objRichText = new PHPExcel_RichText();
$objRichText->createText('%');
$objPHPExcel->getActiveSheet()->getCell('Q8')->setValue($objRichText);
...
$objPHPExcel->getActiveSheet()->getStyle('A8:Q9')->applyFromArray(
    array(
        'font'    => array(            
            'size' => $fontSizeHeader
        ),
...
foreach($data as $buffer){
	...
	if($buffer['name'] == 'JUMLAH'){
		...
		$objPHPExcel->getActiveSheet()->setCellValue('D'.(9+$no), $buffer['luas_bumi']);
		$objPHPExcel->getActiveSheet()->setCellValue('E'.(9+$no), $buffer['luas_bangunan']);
	    $objPHPExcel->getActiveSheet()->setCellValue('F'.(9+$no), $buffer['ketetapan_rp']);
	    $objPHPExcel->getActiveSheet()->setCellValue('G'.(9+$no), $buffer['rbl_wp']);
	    $objPHPExcel->getActiveSheet()->setCellValue('H'.(9+$no), $buffer['rbl_rp']);
	    ...
	    $objPHPExcel->getActiveSheet()->setCellValue('K'.(9+$no), $buffer['rbi_rp']);
	    $objPHPExcel->getActiveSheet()->setCellValue('L'.(9+$no), $buffer['kom_rbi_wp']);
	    $objPHPExcel->getActiveSheet()->setCellValue('M'.(9+$no), $buffer['kom_rbi_rp']);
	    $objPHPExcel->getActiveSheet()->setCellValue('N'.(9+$no), $buffer['percent2']);
	    $objPHPExcel->getActiveSheet()->setCellValue('O'.(9+$no), $buffer['sk_wp']);
	    $objPHPExcel->getActiveSheet()->setCellValue('P'.(9+$no), $buffer['sk_rp']);
	    $objPHPExcel->getActiveSheet()->setCellValue('Q'.(9+$no), $buffer['percent3']);
	}else {
	    $objPHPExcel->getActiveSheet()->setCellValue('C'.(9+$no), $buffer['ketetapan_wp'])->getStyle('C'.(9+$no))->applyFromArray($noBold);
		$objPHPExcel->getActiveSheet()->setCellValue('D'.(9+$no), $buffer['luas_bumi'])->getStyle('D'.(9+$no))->applyFromArray($noBold);
		$objPHPExcel->getActiveSheet()->setCellValue('E'.(9+$no), $buffer['luas_bangunan'])->getStyle('E'.(9+$no))->applyFromArray($noBold);
	    $objPHPExcel->getActiveSheet()->setCellValue('F'.(9+$no), $buffer['ketetapan_rp'])->getStyle('F'.(9+$no))->applyFromArray($noBold);
	    $objPHPExcel->getActiveSheet()->setCellValue('G'.(9+$no), $buffer['rbl_wp'])->getStyle('G'.(9+$no))->applyFromArray($noBold);
	    $objPHPExcel->getActiveSheet()->setCellValue('H'.(9+$no), $buffer['rbl_rp'])->getStyle('H'.(9+$no))->applyFromArray($noBold);
	    ...
	    $objPHPExcel->getActiveSheet()->setCellValue('K'.(9+$no), $buffer['rbi_rp'])->getStyle('K'.(9+$no))->applyFromArray($noBold);
	    $objPHPExcel->getActiveSheet()->setCellValue('L'.(9+$no), $buffer['kom_rbi_wp'])->getStyle('L'.(9+$no))->applyFromArray($noBold);
	    $objPHPExcel->getActiveSheet()->setCellValue('M'.(9+$no), $buffer['kom_rbi_rp'])->getStyle('M'.(9+$no))->applyFromArray($noBold);
	    $objPHPExcel->getActiveSheet()->setCellValue('N'.(9+$no), $buffer['percent2'])->getStyle('N'.(9+$no))->applyFromArray($noBold);
	    $objPHPExcel->getActiveSheet()->setCellValue('O'.(9+$no), $buffer['sk_wp'])->getStyle('O'.(9+$no))->applyFromArray($noBold);
	    $objPHPExcel->getActiveSheet()->setCellValue('P'.(9+$no), $buffer['sk_rp'])->getStyle('P'.(9+$no))->applyFromArray($noBold);
	    $objPHPExcel->getActiveSheet()->setCellValue('Q'.(9+$no), $buffer['percent3'])->getStyle('Q'.(9+$no))->applyFromArray($noBold);
	}
    $no++;
}
$objPHPExcel->getActiveSheet()->setCellValue('A'.(8+$no), '');
$objPHPExcel->getActiveSheet()->getStyle('A10:Q'.(9+count($data)))->applyFromArray(
