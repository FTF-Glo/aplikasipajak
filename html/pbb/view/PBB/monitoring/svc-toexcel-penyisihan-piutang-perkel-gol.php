...
$objRichText->createText('PENGGOLONGAN PENYISIHAN PIUTANG PAJAK BUMI DAN BANGUNAN (PBB) ' . $kota . ' ' . $namaKota);
...
$objPHPExcel->getActiveSheet()->setCellValue('F4', "KUALITAS \nPIUTANG");
$objPHPExcel->getActiveSheet()->setCellValue('G4', "PERSENTASE \nFAKTOR PENGALI");
$objPHPExcel->getActiveSheet()->setCellValue('H4', "PENYISIHAN \nPIUTANG PAJAK \nTIDAK TERTAGIH");
...
$objPHPExcel->getActiveSheet()->getStyle('A4:H4')->getFill()->getStartColor()->setRGB('E4E4E4');
...
$objPHPExcel->getActiveSheet()->getStyle('A1:H50')->applyFromArray(
	array(
		'alignment' => array(
			'vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER
		)
...
foreach ($data as $buffer) {
	...
	$objPHPExcel->getActiveSheet()->setCellValue('F' . (4 + $no), "LANCAR")->getStyle('F' . (4 + $no))->applyFromArray($noBoldCenter);
	$objPHPExcel->getActiveSheet()->setCellValue('G' . (4 + $no), "5%")->getStyle('G' . (4 + $no))->applyFromArray($noBoldCenter);
	$objPHPExcel->getActiveSheet()->setCellValue('H' . (4 + $no), (0.05 * $buffer["TAGIHAN"]))->getStyle('H' . (4 + $no))->applyFromArray($noBoldCenter);

	$summary[0] += $buffer["TAGIHAN"];
	$summary[1] += (0.05 * $buffer["TAGIHAN"]);
...
$objPHPExcel->getActiveSheet()->getStyle('A4:H' . (5 + count($data)))->applyFromArray(
...
while ($i <= 5) {
	...
	foreach ($data2 as $buffer) {
		...
		switch ($tahunsheet) {
			...
			case $tahunawal - 2:
				...
				$persentase = "10%";
				$penyisihan = (0.1 * $buffer["TAGIHAN"]);
				break;
			case $tahunawal - 3:
			case $tahunawal - 4:
				$kualitas 	= "DIRAGUKAN";
				$persentase = "50%";
				$penyisihan = (0.5 * $buffer["TAGIHAN"]);
				break;
			case $tahunawal - 5:
				$kualitas 	= "MACET";
				$persentase = "100%";
				$penyisihan = (1 * $buffer["TAGIHAN"]);
				break;
		}
		...
		$objPHPExcel->getActiveSheet()->setCellValue('D' . (4 + $no), $buffer["TAGIHAN"])->getStyle('D' . (4 + $no))->applyFromArray($noBold);
		$objPHPExcel->getActiveSheet()->setCellValue('E' . (4 + $no), $buffer["TAHUN"])->getStyle('E' . (4 + $no))->applyFromArray($noBoldCenter);
		$objPHPExcel->getActiveSheet()->setCellValue('F' . (4 + $no), $kualitas)->getStyle('F' . (4 + $no))->applyFromArray($noBoldCenter);
		$objPHPExcel->getActiveSheet()->setCellValue('G' . (4 + $no), $persentase)->getStyle('G' . (4 + $no))->applyFromArray($noBoldCenter);
		$objPHPExcel->getActiveSheet()->setCellValue('H' . (4 + $no), $penyisihan)->getStyle('H' . (4 + $no))->applyFromArray($noBold);

		$summary[0] += $buffer["TAGIHAN"];
		$summary[1] += (0.5 * $buffer["TAGIHAN"]);
	...
	$objPHPExcel->getActiveSheet()->getStyle('F' . (4 + $no))->applyFromArray(
		array(
			'alignment' => array(
				'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
				'vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER
	...
	$objPHPExcel->getActiveSheet()->setCellValue('F' . (4 + $no), 'TOTAL PENYISIHAN');
	$objPHPExcel->getActiveSheet()->mergeCells('F' . (4 + $no) . ':G' . (4 + $no));
	$objPHPExcel->getActiveSheet()->setCellValue('H' . (4 + $no), $summary[1])->getStyle('H' . (4 + $no))->applyFromArray($noBold);

	$no++;
	$objPHPExcel->getActiveSheet()->setCellValue('A' . (4 + $no), '');
	$objPHPExcel->getActiveSheet()->getStyle('A4:H' . (5 + count($data2)))->applyFromArray(
		array('borders' => array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN)))
	);
...
header('Content-Disposition: attachment;filename="PENGGOLONGAN_KUALITAS_PIUTANG.xls"');
