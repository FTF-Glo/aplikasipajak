...
function judul($row)
{
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A{$row}", " REKAP PELAYANAN PERUBAHAN DATA ");
...
function table_header($row)
{
	...
	$objPHPExcel->setActiveSheetIndex(0)
		...
		->setCellValue("K{$row}", " LUAS TANAH ")
		->setCellValue("K" . ($row + 1), " LAMA ")
		->setCellValue("L" . ($row + 1), " BARU ")
		->setCellValue("M{$row}", " LUAS BANGUNAN ")
		->setCellValue("M" . ($row + 1), " LAMA ")
		->setCellValue("N" . ($row + 1), " BARU ")
		->setCellValue("O{$row}", " PBB TERHUTANG")
		->setCellValue("P{$row}", " TANGGAL CETAK");
	...
	$objPHPExcel->getActiveSheet()->mergeCells("O" . $row . ":O" . ($row + 1));
	$objPHPExcel->getActiveSheet()->mergeCells("P" . $row . ":P" . ($row + 1));



	//style header
	$objPHPExcel->getActiveSheet()->getStyle("A{$row}:R" . ($row + 1))->applyFromArray(
		array(
			'alignment' => array(
				'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
				'vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER
...
function fetchData()
{
	...
	for ($i = 0; $i < count($data); $i++) {
		...
		$objPHPExcel->getActiveSheet()->setCellValue('F' . $row, " " . $data[$i]['NOP']);
		...
		$objPHPExcel->getActiveSheet()->setCellValue('K' . $row, $data[$i]['OLD_LT']);
		$objPHPExcel->getActiveSheet()->setCellValue('L' . $row, $data[$i]['NEW_LT']);
		$objPHPExcel->getActiveSheet()->setCellValue('M' . $row, $data[$i]['OLD_LB']);
		$objPHPExcel->getActiveSheet()->setCellValue('N' . $row, $data[$i]['NEW_LB']);
		$objPHPExcel->getActiveSheet()->setCellValue('O' . $row, $data[$i]['SPPT_PBB_HARUS_DIBAYAR']);
		$objPHPExcel->getActiveSheet()->setCellValue('P' . $row, $data[$i]['SPPT_TANGGAL_CETAK']);

		$objPHPExcel->getActiveSheet()->getStyle('E' . $row . ':Q' . $row)->getAlignment()->setWrapText(true);
	...
	$objPHPExcel->getActiveSheet()->getStyle("K{$row1}:N{$row}")->applyFromArray(
		array("alignment" => array(
	...
	//border list data
	$objPHPExcel->getActiveSheet()->getStyle("A" . ($row1 - 2) . ":P" . ($row - 1))->applyFromArray(
		array('borders' => array(
			'allborders' => array(
...
$objPHPExcel->getActiveSheet()->getPageSetup()->setPrintArea('A1:P' . ($row + 7));
...
// Rename sheet
$objPHPExcel->getActiveSheet()->setTitle('Rekap Perubahan');
...
header('Content-Disposition: attachment;filename="Rekap_Perubahan_' . date('d-m-Y') . '.xls"');
