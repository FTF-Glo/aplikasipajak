...
function showTable ($mod=0,$nama="") {
	...
	$a=1;
    $data = array();
	$summary = array('name'=>'JUMLAH', 'wp'=>0, 'pokok'=>0, 'denda'=>0);
	...
	for ($i=0;$i<$c;$i++) {
            ...
            $data[] = $tmp;
            $summary['wp'] 		+= $dt[$i]["WP"];
            $summary['pokok'] 	+= $dt[$i]["POKOK"];
            $summary['denda'] 	+= $dt[$i]["DENDA"];
	}
	$summary['wp'] 		= $summary['wp'];
    $summary['pokok'] 	= $summary['pokok'];
    $summary['denda'] 	= $summary['denda'];
	$data[] = $summary;
...
$objPHPExcel->getActiveSheet()->setCellValue('E6', 'DENDA');
...
foreach($data as $buffer){
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('A'.(6+$no), $no);
    $objPHPExcel->getActiveSheet()->setCellValue('B'.(6+$no), $buffer['name']);
	if($buffer['name'] == 'JUMLAH'){
        $objPHPExcel->getActiveSheet()->setCellValue('C'.(6+$no), $buffer['wp']);
	    $objPHPExcel->getActiveSheet()->setCellValue('D'.(6+$no), $buffer['pokok']);
	    $objPHPExcel->getActiveSheet()->setCellValue('E'.(6+$no), $buffer['denda']);
	    $objPHPExcel->getActiveSheet()->setCellValue('F'.(6+$no), ($buffer['pokok']+$buffer['denda']));
	}else {
	    $objPHPExcel->getActiveSheet()->setCellValue('C'.(6+$no), $buffer['wp'])->getStyle('C'.(6+$no))->applyFromArray($noBold);
	    $objPHPExcel->getActiveSheet()->setCellValue('D'.(6+$no), $buffer['pokok'])->getStyle('D'.(6+$no))->applyFromArray($noBold);
	    $objPHPExcel->getActiveSheet()->setCellValue('E'.(6+$no), $buffer['denda'])->getStyle('E'.(6+$no))->applyFromArray($noBold);
	    $objPHPExcel->getActiveSheet()->setCellValue('F'.(6+$no), ($buffer['pokok']+$buffer['denda']))->getStyle('F'.(6+$no))->applyFromArray($noBold);
	}
    $no++;
}
$objPHPExcel->getActiveSheet()->setCellValue('A'.(6+$no), '');
$objPHPExcel->getActiveSheet()->getStyle('A6:F'.(6+count($data)))->applyFromArray(
    array('borders' => array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN)))
); 
$objPHPExcel->getActiveSheet()->getStyle('A6:A'.(6+count($data)))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$objPHPExcel->getActiveSheet()->getStyle('C6:D'.(6+count($data)))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
