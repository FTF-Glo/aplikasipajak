...
require_once($sRootPath . "inc/PBB/dbMonitoringTL.php");
...
$nosk        = @isset($_REQUEST['nosk']) ? $_REQUEST['nosk'] : "";
...
{\"field\":\"sppt_pbb_harus_dibayar\", \"length\" : \"100px\", \"title\" : \"Pokok\", \"align\":\"right\",\"format\":\"number\"},
{\"field\":\"pbb_denda\", \"length\" : \"100px\", \"title\" : \"Denda\", \"align\":\"right\",\"format\":\"number\"},
{\"field\":\"pbb_total_bayar\", \"length\" : \"100px\", \"title\" : \"Total\", \"align\":\"right\",\"format\":\"number\"},
...
{\"field\":\"SK\", \"length\" : \"80px\", \"title\" : \"Nomor SK\"},
]}";
...
if ($status == 1) {
	if ($nosk != "") array_push($arrWhere, "B.BTL_LOG_SK LIKE '%{$nosk}%'");
}
...
if (stillInSession($DBLink, $json, $sdata)) {
    ...
    if ($status == '1') {
        $monPBB->setTable(" pbb_sppt_pembatalan A INNER JOIN pbb_pembatalan_sppt_log B ON b.BTL_LOG_NOP = A.NOP AND B.BTL_LOG_TAHUN = A.SPPT_TAHUN_PAJAK  ");
        ...
        $monPBB->query("SELECT A.nop, A.wp_nama, A.wp_alamat, A.wp_kelurahan, A.op_alamat, A.op_kecamatan, A.op_kelurahan, A.op_rt, A.op_rw,
            ...
            A.sppt_tanggal_jatuh_tempo , IFNULL(A.sppt_pbb_harus_dibayar,0) AS sppt_pbb_harus_dibayar, {$a} as pbb_denda , IFNULL(A.sppt_pbb_harus_dibayar+({$a}),0) as pbb_total_bayar, b.BTL_LOG_SK as SK, b.BTL_LOG_ALASAN as Alasan ");
    } else {
        $monPBB->setTable(" PBB_SPPT_TAHUNLAMA A LEFT JOIN PBB_DENDA B ON A.NOP=B.NOP AND A.SPPT_TAHUN_PAJAK=B.SPPT_TAHUN_PAJAK ");
        $monPBB->setWhere($where);
        $monPBB->query("SELECT A.nop, A.wp_nama, A.wp_alamat, A.wp_kelurahan, A.op_alamat, A.op_kecamatan, A.op_kelurahan, A.op_rt, A.op_rw,
            A.op_luas_bumi, A.op_luas_bangunan,A.op_njop_bumi,A.op_njop_bangunan,A.sppt_tahun_pajak, 
