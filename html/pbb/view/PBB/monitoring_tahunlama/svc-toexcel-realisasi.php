...
function showTable($mod = 0, $nama = "")
{
    ...
    for ($i = 0; $i < $c; $i++) {
        ...
        //UNDZ+- : Update Pokok dikurangi dari nilai Total - Denda
        ...
        $data[] = $tmp;
        $data = array_sort($data, 'percent2', SORT_DESC);
...
foreach ($data as $buffer) {
    ...
    if ($buffer['name'] == 'JUMLAH') {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('I' . (9 + $no), number_format($buffer['percent1'], 2, ",", "."))->getStyle('I' . (9 + $no))->applyFromArray($noBold);
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('R' . (9 + $no), number_format($buffer['percent2'], 2, ",", "."))->getStyle('R' . (9 + $no))->applyFromArray($noBold);
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('U' . (9 + $no), number_format($buffer['percent3'], 2, ",", "."))->getStyle('U' . (9 + $no))->applyFromArray($noBold);
    }
    $no++;
}
...
header('Content-Disposition: attachment;filename="realisasi_pbb.xls"');
