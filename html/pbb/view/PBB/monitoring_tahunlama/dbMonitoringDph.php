...
class dbMonitoringDph {
	...
	public function getAllQuery() {
		echo $this->dbQuery." ".$this->dbTable." ".$this->dbWhere;
	}

	public function getAllQuery1() {
		return $this->dbQuery." ".$this->dbTable." ".$this->dbWhere;
	}
...
//		echo $query;
	...
	public function insLogPembayaran($uname,$nop,$tahun,$nominal,$type){
		$query = "INSERT INTO SW_PBB_SUKABUMI.cppmod_pbb_update_log VALUES(UUID(),'$uname','$nop','$tahun','$nominal','$type',now()) ";
		if ($this->mysql($errMsg)) {
			$res = mysqli_query($this->dbConn, $query);
			if ($res) {
				$row = mysql_fetch_row($res);
...
//		return $query;
	...
	private function paging() {
		...
		if($page_amount != "0"){
			...
			$html .= $p_num." - ".$lp." dari <div class='tot-all' style='display:inline'>".$tot."</div>";
			$html .= "&nbsp;{$txtnp}</td></tr>";
		}
	...
	public function query_result ($query) {
		...
		if ($this->export)
		 $query = $query.$this->dbTable.$this->dbWhere." ORDER BY sppt_tahun_pajak DESC ";
		else
		 $query = $query.$this->dbTable.$this->dbWhere." ORDER BY sppt_tahun_pajak DESC ".$this->paging_query();
		// echo "---------- <br>".$query."<br>";
		if ($this->dbType == 1) {
			if ($this->postgres($errMsg)) {
	...
	public function query_result_cst ($query) {
		$result = array();
		 $query = $query.$this->dbTable.$this->dbWhere." ".$this->paging_query();
		// echo "---------- <br>".$query."<br>";
		if ($this->dbType == 1) {
			if ($this->postgres($errMsg)) {
				$this->result = pg_query($this->dbConn, $query);
	...
	private function createTitleHeader () {
		...
		// 		$array = array(
		// 				"Alamat Subjek Pajak"=> array("wp_alamat","blok","wp_rt","wp_rw","wp_kelurahan","wp_kecamatan"),	
		// 				"Letak Objek Pajak"=>  array("op_alamat","op_alamat","op_rt","op_rw","op_kelurahan","op_kecamatan"),	
		// 				"luas"=>array("op_luas_bumi","op_luas_bangunan"),	
		// 				"NJOP"=>array("op_njop_bumi","op_njop_bangunan"),	
		// 			);
		...
		// 				// echo "<br>";
		// 				// echo "</pre>";
		// 				if (in_array($this->fields[$i]["name"], $array['Alamat Subjek Pajak'])){
		// 					$txtHeader .= "<th $rw colspan='".count($array['Alamat Subjek Pajak'])."' >Alamat Subjek Pajak </th>";
		// 				}else{
		// 					$txtHeader .= "<th $rw >".$this->fields[$i]["name"]." </th>";
		// 				}
		// 			// }
		// 		// }
		...
		// for ($i=0;$i<$c;$i++){
		// 	$array = array("nop","npwp","nik","wp_nama");
		// 	if (!in_array($this->fields[$i]["name"], $array)) {
		...
		if ($this->dbTitles) {
			for ($i=0;$i<count($this->dbTitles->data);$i++) {
				$txtHeader = str_replace("<th>".$this->dbTitles->data[$i]->field,
				"<th width=\"".$this->dbTitles->data[$i]->length."\">".$this->dbTitles->data[$i]->title,$txtHeader);
			}
		}
		return $txtHeader;
	}
		...
		if ($this->dbType == 0) {
			while ($row = $this->getDataFromMysql()) {
				...
				for ($i=0;$i<$c;$i++){
					...
					// if jatuh tempo, simpen di var jatuh tempo
					if($this->fields[$i]["name"]=="sppt_tanggal_jatuh_tempo"){
						$jthtempo=$row[$this->fields[$i]["name"]];
					}
					// if pokok, simpan di var pokok
					if($this->fields[$i]["name"]=="PBB_TERHUTANG"){
						$pokok=$row[$this->fields[$i]["name"]];
					}
					
					// if denda, hitung_denda berdasar pokok dan jatuh tempo (hitung month interval)
					
					if($this->fields[$i]["name"]=="DENDA"){
						//$row[$this->fields[$i]["name"]]=$denda;
						$denda=$row[$this->fields[$i]["name"]];
					}
					if($this->fields[$i]["name"]=="JUMLAH"){
						$row[$this->fields[$i]["name"]]=$pokok+$denda;
						
					}
					
					if ($format == "number") {
						// isi row dengan hasil hitung_denda
			...
			$totbayar=$totpokok+$totdenda;
			$html .= "<tr><td colspan=\"4\" align=\"right\"> <b>JUMLAH PER HALAMAN</b></td><td></td><td id=\"totpokok\" align=\"right\"><b>".number_format($totpokok,0,',', '.')."</b></td><td id=\"totdenda\" align=\"right\"><b>".number_format($totdenda,0,',', '.')."</b></td><td id=\"totbayar\" align=\"right\"><b>".number_format($totbayar,0,',', '.')."</b></td></tr>";
		}

		return $html;
	}
	private function bodyConstructsudahbayar() {
		$html = "";
		$c = count($this->fields);
		//print_r ($this->getDataFromPostgres());exit;
		if ($this->dbType == 1) {
	...
	private function getdenda($jatuhtempo, $pokok){
	...
	// membuat header
	private function headerConstructMysql() { // ini cara header
		//if ($this->dbType == 1) {
	...
	public function showHTML() {
		$this->export = false;
		$this-> query_result_cst ($this->dbQuery);
		$html = "<div id=\"frame-tbl-monitoring\" class=\"tbl-monitoring\"><table id='monitoring_table'  border=\"0\" cellspacing=\"1\" cellpadding=\"4\">";
		$html .= $this->headerConstructMysql();
		$html .= '<tr><td align=""><input type="text" name="nop" id="carinop-7" style="width:100%;" /></td>
					<td align=""><input type="text" name="tahun" id="cari-tahun-7"  style="width:100%;" /></td>
					<td align=""><input type="text" name="nama" id="cari-nama-7"   style="width:100%;"/></td>
					<td align=""><input type="text" name="desa" id="cari-desa-7"  style="width:100%;" /></td>
					<td align=""><input type="text" name="kecamatan" id="cari-kecamatan-7"   style="width:100%;" /></td>
					<td align=""><input type="text" name="pbb" id="cari-pbb-7"   style="width:100%;" /></td>
					<td align=""><input type="text" name="denda" id="cari-denda-7"  style="width:100%;" /></td>
					<td align=""><input type="text" name="total" id="cari-total-7"  style="width:100%;" /></td></tr>';
	...
	public function showHTMLSudahBayar() {
		$this->export = false;
		...
		$html .= $this->headerConstructMysql();
		$html .= $this->bodyConstructsudahbayar();
							...
							if ($this->dbTitles->data[$x]->field == $this->fields[$i]["name"]){
								...
								if ($format == "optional") {
									if($row[$this->fields[$x]["name"]]==1) 
										...
										$opt[$row[$this->fields[$x]["name"]]] = "<input type=\"button\" name=\"btn-bayar\" id=\"btn-bayar\" value=\"Bayar\" onclick=\"bayar('".$row['nop']."',".$row['sppt_tahun_pajak'].",'0','".$uid."')\">";
									}
								}
							}
						}
