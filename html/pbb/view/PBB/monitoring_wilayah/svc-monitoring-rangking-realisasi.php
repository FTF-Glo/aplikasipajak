...
function headerRangkingRealisasi($mod, $nama)
{
	...
	<tr>
		<th colspan=5 class=tleft>{$dl}</td>
	</tr>
	<tr class=tright>
		<th width=9>NO</td>
		<th class=tleft>{$model}</td>
		<th>JUMLAH KETETAPAN</td>
		<th>JUMLAH REALISASI</td>
		<th>PERSENTASE (%)</td>
	</tr>
	";
	return $html;
...
function showTableAll($mod = 0, $nama = "")
{
	...
	for ($i = 0; $i < $c; $i++) {
		...
		if (!empty($data[$i]['KECAMATAN'])) {
			$html .= "<td class=tleft>" . $data[$i]['KECAMATAN'] . "</td>";
		} else {
			$html .= "<td class=tleft>" . $data[$i]['KELURAHAN'] . "</td>";
		}

		$html .= "<td>" . number_format($data[$i]['JUMLAH_KETETAPAN'], 0, ",", ".") . "</td>
				<td>" . number_format($data[$i]['JUMLAH_REALISASI'], 0, ",", ".") . "</td>
				<td>" . $data[$i]['PERC1'] . "</td>
			</tr> ";
