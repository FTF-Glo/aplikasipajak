...
function clearAll () {
	...
	de('wp-duedate').innerHTML = "";
	de('wp-address').innerHTML = "";
	de('wp-amount').innerHTML =  rupiah(0);
	de('wp-kelurahan').innerHTML = "";
	de('wp-penalty').innerHTML = rupiah(0);
	de('wp-rtRw').innerHTML = ""
	de('wp-admin').innerHTML = rupiah(0);
	de('wp-kecamatan').innerHTML = "";
	de('wp-totalamount').innerHTML =rupiah(0);
...
function inquiryPBB(areaCode,flag,nopNpwp,year,appId) {
	if ((areaCode != '') && (flag != '') && (nopNpwp != '')) {
		var params = {area_code : areaCode,flag:flag,nop_npwp:year+nopNpwp,appID:appId};
		//console.log("ID "+year+nopNpwp);
		//console.log("Flag "+flag);
		params = Base64.encode(Ext.encode(params));
		  Ext.Ajax.request({
			   url: 'svr/pbb/svc-pbb-inquiry.php',
			   timeout:80000,
...
function sendInquiry(appId) {
	...
	inquiryPBB(areaCode,flag,nop_npwp,year,appId)
}
...
function getInquirySuccess(param){
	if(param.responseText){
		...
		if(objResult.result){
			//console.log(objResult.data.subject_zip_post);
				de('wp-name').innerHTML = objResult.data.subject_name;
				de('wp-amount').innerHTML = rupiah(parseFloat(objResult.data.ori_bill));
				de('wp-address').innerHTML = objResult.data.subject_address;
				de('wp-kelurahan').innerHTML = objResult.data.subject_kelurahan;
				de('wp-penalty').innerHTML = rupiah(parseFloat(objResult.data.penalty_fee));
				de('wp-rtRw').innerHTML =objResult.data.subject_rt_rw;
				de('wp-admin').innerHTML = rupiah(parseFloat(objResult.data.admin_fee));
				de('wp-kecamatan').innerHTML = objResult.data.subject_kecamatan;
				de('wp-totalamount').innerHTML =rupiah(parseFloat(objResult.data.total_amount));
				...
				de('wp-duedate').innerHTML =objResult.data.due_date;
...
function sendPayment(appId) {
	...
	params = Base64.encode(Ext.encode(params));
	  Ext.Ajax.request({
		   url: 'svr/pbb/svc-pbb-payment.php',
		   timeout:80000,
 	...
 	if(param.responseText){
		...
		if (objResultPayment.result) {
			...
			if (printStruk()) {
				...
				alert ("Pembayaran berhasil, pencetakan struk gagal !");
				clearAll ();
			}
			//console.log("refnnum "+objResultPayment.swrefnum);
...
function getSummary(){
    ...
    Ext.Ajax.request({
       url: 'svr/pbb/svc-pbb-summary.php',
