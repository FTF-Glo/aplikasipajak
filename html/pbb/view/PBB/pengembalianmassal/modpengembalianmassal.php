...
function displayMenuPencetakan()
{
    ...
    echo "\t<ul>\n";
    echo "\t\t<li><a href=\"view/PBB/pengembalianmassal/page.php?q=" . base64_encode("{'a':'$a', 'm':'$m', 's':'70'}") . "\">SPPT</a></li>\n";
    echo "\t</ul>\n";
}
...
<script type="text/javascript">
    ...
    function hapusNOP() {
        var result = confirm("Apakah anda yakin ingin mengembalikan data-data tersebut?");
        if (result) {
            x = 0;
            $("input:checkbox[name='check-all\\[\\]']").each(function() {
                if ($(this).is(":checked")) {
            ...
            if (x == 0) alert("Pilih data yang akan dikembalikan!");
            else {
                var nop = '';
                var idx = 0;
                $("input:checkbox[name='check-all\\[\\]']").each(function() {
    ...
    function hapusDataNOP(nop) {
        ...
        $.ajax({
            type: 'POST',
            url: './function/PBB/pengembalianmassal/page.php',
            data: params,
