...
session_start();
$sRootPath = str_replace('\\', '/', str_replace(DIRECTORY_SEPARATOR . 'view' . DIRECTORY_SEPARATOR . 'PBB' . DIRECTORY_SEPARATOR . 'pengembalianmassal', '', dirname(__FILE__))) . '/';
...
{
    global $isSusulan, $kec, $kel, $jumlah, $srch, $PenilaianParam, $appConfig, $module, $m, $aKecamatanPendataan, $conf_kdkec, $aKecamatan, $aKelurahan, $aKelurahanPendataan, $a, $dbUtils, $dbGwCurrent, $tahun, $uid, $uname, $buku, $DBLink;
    ...
    if ($selected == 50) {
        ...
        if ($m == $appConfig['id_mdl_susulan']) {
            echo "<button class=\"btn btn-primary bg-maka\" type=\"button\" value=\"Proses Penetapan\" name=\"btn-penetapan\"/ onClick=\"return actionPenetapanSusulan('" . $PenilaianParam . "', '" . $appConfig['tahun_tagihan'] . "', '1','" . $uname . "');\">Proses Penetapan</button>\n";
        } else if ($m == $appConfig['id_mdl_penetapan']) {
            if (!$srch) {
                if ($kel != '') // hanya tampilkan kalau sudah memilih kelurahan
                    echo "<button class=\"btn btn-primary bg-maka\" type=\"button\" value=\"Proses Penetapan\" name=\"btn-penetapan\"/ onClick=\"return actionPenetapan('" . $PenilaianParam . "', '" . $appConfig['tahun_tagihan'] . "', '0');\">Proses Penetapan</button>\n";
                else echo "<button class=\"btn btn-primary bg-maka\" type=\"button\" value=\"Proses Penetapan Perkelurahan\" name=\"btn-penetapan\"/ onClick=\"return actionPenetapan('" . $PenilaianParam . "', '" . $appConfig['tahun_tagihan'] . "', '0','" . $uname . "');\" disabled=\"disabled\">Proses Penetapan Perkelurahan</button>\n";
            }
        }
        ...
        //   echo 'Tanggal Penetapan <input type="text" id="tgl_penetapan" readonly="true" name="tgl_penetapan" value="'.date('d-m-Y').'" maxlength="10" size="12">&nbsp;<br/><br/>';
        if ($isSusulan) {
            echo "<input type=\"button\" value=\"Proses Penilaian\" name=\"btn-penilaian-mundur\" onClick=\"penilaianMundur(1);\">\n";
            echo "<input type=\"button\" value=\"Proses Penetapan Mundur\" name=\"btn-penetapan-mundur\" id=\"btn-penetapan-mundur\" onClick=\"return actionPenetapanMundur('" . $PenilaianParam . "', '1','" . $uid . "');\"  disabled=\"disabled\" >\n";
        } else {
            echo "<input type=\"button\" value=\"Proses Penilaian\" name=\"btn-penilaian-mundur\" onClick=\"penilaianMundur(0)\">\n";
            echo "<input type=\"button\" value=\"Proses Penetapan Mundur\" name=\"btn-penetapan-mundur\" id=\"btn-penetapan-mundur\" onClick=\"return actionPenetapanMundur('" . $PenilaianParam . "', '0','" . $uid . "');\"  disabled=\"disabled\" >\n";
        }
        echo '<br><br><div id="monitoring-content-5" class="monitoring-content"></div></div>';
    } else if ($selected == 70) {
        echo "<button class=\"btn btn-primary btn-orange\" type=\"button\" value=\"Hapus NOP\" name=\"btn-print-preview\"/ onclick=\"javascript:hapusNOP()\">Pengembalian NOP</button>&nbsp;&nbsp;\n";
        echo "&nbsp;&nbsp;&nbsp;\n";
        ...
        if ($isSusulan) {
            echo "<input type=\"button\" value=\"Proses Penilaian\" id=\"btn-penilaian\" name=\"btn-penilaian\"/ onClick=\"return actionPenilaian('" . $PenilaianParam . "', '" . $appConfig['tahun_tagihan'] . "', '2', '1');\">";
        } else {
            if ($kel != '') {
                ...
                echo "<input type=\"button\" value=\"Proses Penilaian\" id=\"btn-penilaian\" name=\"btn-penilaian\"/ onClick=\"return actionPenilaian('" . $PenilaianParam . "', '" . $appConfig['tahun_tagihan'] . "', '1', '0');\">\n";
            } else echo "<input type=\"button\" value=\"Proses Penilaian\" id=\"btn-penilaian\" name=\"btn-penilaian\"/ onClick=\"return actionPenilaian('" . $PenilaianParam . "', '" . $appConfig['tahun_tagihan'] . "', '1', '0');\" disabled=\"disabled\">\n";
        }
    ...
    if ($selected != 26) {
        ...
        echo "  <div class=\"col-md-12\" style=\"margin-bottom: 20px\">
                    <div class=\"row\">
                        <div class=\"col-md-2\" style=\"margin-top: 5px;\">Kata Kunci Pencarian</div>
                        <div class=\"col-md-3\">
                            <input type=\"text\" class=\"form-control\" onKeydown=\"Javascript: if (event.keyCode==13) setTabs(" . $selected . "," . $selected . ");\" id=\"srch-" . $selected . "\" name=\"srch-" . $selected . "\" size=\"30\" placeholder=\"NOP/Nama\" value=\"" . $srch . "\"/>
                        </div>
                        <div class=\"col-md-1\">
                            <button type=\"button\" class=\"btn btn-primary bg-orange mb5\" style=\"margin-top: 4px\" onclick=\"setTabs(" . $selected . "," . $selected . ")\" id=\"btn-src\">Cari</button>";
        // echo $selected;
        echo "          <span style=\"margin-top: 5px; margin-bottom: 25px; margin-left: 10px;\">Filter</span></div>
                        <div class=\"col-md-2\">
        ...
        echo "          <div class=\"col-md-2\">
                            <div id=\"sKel" . $selected . "\" style=\"margin-left:5px; display:inline-block; width: 100%\" >
                                <select name=\"kel\" id=\"kel\" class=\"form-control\" onchange=\"filKel(" . $selected . ",this)\">";
        ...
        if ($m == $appConfig['id_mdl_penetapan']) {
            ...
            echo "<button class=\"btn btn-primary bg-maka\" type=\"button\" value=\"Proses Penetapan Terseleksi\" name=\"btn-penetapan\"/ onClick=\"return actionPenetapanTerseleksi('" . $PenilaianParam . "', '" . $appConfig['tahun_tagihan'] . "', '1','" . $uname . "');\">Proses Penetapan Terseleksi</button>\n"; #35utech 
    ...
    if ($selected == 10 || ($selected == 24) || ($selected == 80 && $isSusulan) || $selected == 81 || $selected == 70) {
...
function getData($selected)
{
    ...
    global $dbSpptTran, $dbFinalSppt, $dbGwCurrent, $dbServices, $srch, $arConfig, $appConfig, $isSusulan,
        $data, $kec, $kel, $buku, $custom, $jumlah, $totalrows, $perpage, $page, $dbUtils, $tahun, $daftarNOP;
    // echo $daftarNOP;
    ...
    if ($selected == 5) {
        ...
        if ($selected == 60) {
            $data = $dbGwCurrent->get($filter, $srch, $jumlah, $perpage, $page);
            ...
            $data = $dbGwCurrent->get70ssb($filter, $srch, $qBuku, $jumlah, $perpage, $page, $tahun, $appConfig);
            $totalrows = $dbGwCurrent->totalrows;
...
function parseData($data, $selected, $class)
{
    ...
    if ($selected != 60 && $selected != 70) {
		...
		*/
        ...
        $dBasic =
            "\t\t<td class=\"$class\"><a href='main.php?param=" . base64_encode($params) . "'>" . $data['CPM_NOP'] . "</a> </td> \n
