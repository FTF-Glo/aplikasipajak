...
$sRootPath = str_replace('\\', '/', str_replace(DIRECTORY_SEPARATOR . 'view' . DIRECTORY_SEPARATOR . 'PBB' . DIRECTORY_SEPARATOR . 'penilaian_individu' . DIRECTORY_SEPARATOR . 'print', '', dirname(__FILE__))) . '/';
...
function doPrint(&$arrValues, $tahun, $kec, $kel) {
	global $paymentDt,$host,$port,$timeOut,$DBLink;
	
	$paymentDt = strftime("%Y%m%d%H%M%S",time());
	
	$arrValues['result'] = true;
	$arrValues['message'] = 'Berhasil !';
	$arrValues['printValue'] = printRequest($id, $strHTML, $tahun, $kec, $kel);
	$arrValues['HtmlValue'] = $strHTML;
	// echo $strHTML; exit();
	return true;
}

function getValuesForPrint(&$aTemplateValues,$row,$tahun)
{
	global $appConfig;
...
function printRequest($id, &$strHTML, $tahun, $kec, $kel) {
	...
	$query = "SELECT 
                ...
                LEFT JOIN (
                    SELECT CPM_NOP, CPM_WP_NO_KTP FROM cppmod_pbb_sppt WHERE CPM_NOP='$id'
                    UNION ALL 
                    SELECT CPM_NOP, CPM_WP_NO_KTP FROM cppmod_pbb_sppt_final WHERE CPM_NOP='$id'
                    UNION ALL 
                    SELECT CPM_NOP, CPM_WP_NO_KTP FROM cppmod_pbb_sppt_susulan WHERE CPM_NOP='$id'
...
if(stillInSession($DBLink,$json,$sdata)){
	if ($params) {
		doPrint($arrValues, $tahun, $kec, $kel);                
	}
} else {
	$arrValues['result'] = false;
	$arrValues['message'] = "Gagal dengan kode error !\n Invalid access. Silahkan lakukan login";
}
