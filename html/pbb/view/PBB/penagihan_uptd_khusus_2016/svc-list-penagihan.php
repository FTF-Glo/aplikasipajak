...
class SPTPDService{
	...
	function getDocument($sts,&$dat) {
		global $DBLinkLookUp, $DBLink, $sRootPath,$json,$a,$m,$page, $sp1, $sp2, $sp3, $isAdminPenagihan, $condition;		
		if($sts==1)
		...
		$hal = (($page-1) > 0 ? ($page-1) : 0) * $this->perpage;
                ...
                switch ($sts){
                        ...
                        case 3 : $sp .= "AND (A.TGL_SP3 IS NULL OR A.TGL_SP3 = '') AND A.STATUS_PERSETUJUAN = 2 ";
								 break;
                        case 6 : $sp .= "AND (A.TGL_SP1 IS NOT NULL OR A.TGL_SP1 <> '') AND (A.TGL_SP2 IS NULL OR A.TGL_SP2 = '') AND (A.TGL_SP3 IS NULL OR A.TGL_SP3 = '') AND A.STATUS_SP <> 1 AND A.TAHUN_SP1 = '{$condition[4]}' ";
								 break;
                        case 7 : $sp .= "AND (A.TGL_SP1 IS NOT NULL OR A.TGL_SP1 <> '') AND (A.TGL_SP2 IS NOT NULL OR A.TGL_SP2 <> '') AND (A.TGL_SP3 IS NULL OR A.TGL_SP3 = '') AND A.STATUS_SP <> 1 AND A.TAHUN_SP1 = '{$condition[4]}'";
								 break;
                        case 8 : $sp .= "AND (A.TGL_SP1 IS NOT NULL OR A.TGL_SP1 <> '') AND (A.TGL_SP2 IS NOT NULL OR A.TGL_SP2 <> '') AND (A.TGL_SP3 IS NOT NULL OR A.TGL_SP3 <> '') AND A.STATUS_SP <> 1 AND A.TAHUN_SP1 = '{$condition[4]}'";
								 break;
                        case 9 : $sp .= "AND A.STATUS_SP = 1 AND A.TAHUN_SP1 = '{$condition[4]}'";
								 break;
                }
                ...
                if($sts==1){ //Tab SP1
                        $query = "SELECT
                                ...
                                AND B.NOP IS NULL AND NOW() > SPPT_TANGGAL_JATUH_TEMPO $where ORDER BY A.SPPT_PBB_HARUS_DIBAYAR DESC,A.NOP LIMIT ".$hal.",".$this->perpage;
                        ...
                        $qry = "SELECT COUNT(*) AS TOTALROWS FROM
                                ...
                                AND B.NOP IS NULL AND NOW() > SPPT_TANGGAL_JATUH_TEMPO $where ORDER BY A.SPPT_PBB_HARUS_DIBAYAR DESC,A.NOP ";
...
//			$HTML .= "\t\t<td class=\"".$class."\" align=\"center\">".$tmp['STATUS_CETAK']."</td>\n";
                        // if($isAdminPenagihan == 1){
                            if($sts != 4){
                                    ...
                                    elseif($sts == 6 || $sts == 7 || $sts == 8){
											...
											$HTML .= "\t\t<td class=\"".$class."\" align=\"center\">".(($tmp['STATUS_PERSETUJUAN'] == '1' || $tmp['STATUS_PERSETUJUAN'] == '2' || $tmp['STATUS_PERSETUJUAN'] == '3')? "Sudah disetujui" : "<button class=\"btnApprove\" value=\"".$tmp['NOP']."+".$statusPersetujuan."\">Setuju</button>")."</td> \n";
									}
                            }else{
		...
		$hal = (($page-1) > 0 ? ($page-1) : 0) * $this->perpage;
                ...
                $query = "SELECT NOP,
                        ...
                        FROM VIEW_PBB_SPPT_PENAGIHAN WHERE SPPT_TAHUN_PAJAK >= '2007' $qrangetime $where ORDER BY WP_NAMA ASC, SPPT_TAHUN_PAJAK DESC LIMIT ".$hal.",".$this->perpage;
				//echo $query;
                $qry = "SELECT COUNT(*) AS TOTALROWS FROM VIEW_PBB_SPPT_PENAGIHAN WHERE SPPT_TAHUN_PAJAK >= '2007' $qrangetime $where";
		...
		private function getKdKecUser($uid){
			global $DBLink;
			
			$query = "SELECT * FROM TBL_REG_USER_PBB WHERE ctr_u_id = '".$uid."' ";
        ...
        public function getKecamatan(){
			...
			$kdKec = $this->getKdKecUser($uname);

            $query = "SELECT * FROM cppmod_tax_kecamatan WHERE CPC_TKC_ID='$kdKec' ORDER BY CPC_TKC_KECAMATAN ASC";
	...
	public function headerContent($sts) {
		global $condition, $isAdminPenagihan, $isViewData, $lblkel, $uname, $appConfig;
		...
		for($i=0;$i<=5;$i++){
			$optTahun .= "<option value=\"".($appConfig['tahun_tagihan']-$i)."\"".($condition[4]==($appConfig['tahun_tagihan']-$i)? "selected" : "")."><b>".($appConfig['tahun_tagihan']-$i)."</b></option>";
		}
		...
		$HTML .=" <!-- <input type=\"button\" value=\"Cetak\" id=\"btn-print-{$sts}\" name=\"btn-print\" /> -->
						<div style=\"border: 1px solid #dfd9c3; padding:5px; width:180px;\">	
							&nbsp;<b>Tahun Tagihan</b> 
							<select name=\"src-tahun-{$sts}\" id=\"src-tahun-{$sts}\">
								".$optTahun."
							</select>
						</div>
						<br>
...
<script type="text/javascript">
    ...
    function printToPDF(nop, thnpajak, sp) {
        ...
        if(status == '4')
            window.open('view/PBB/penagihan_uptd_khusus_2016/svc-print-penagihan.php?nop='+nop+'&uname='+uname+'&thnpajak='+thnpajak+"&kepala="+kepala+"&nip="+nip+"&jabatan="+jabatan+"&kota="+kota+"&bank="+bank+"&denda="+denda+"&totalBulanPajak="+totalBulanPajak+"&tipeKalkulasiPajak="+tipeKalkulasiPajak+"&appId=<?php echo $a;?>", '_blank');
        else window.open('view/PBB/penagihan_uptd_khusus_2016/svc-print-penagihan.php?nop='+nop+'&uname='+uname+'&thnpajak='+thnpajak+"&kepala="+kepala+"&nip="+nip+"&jabatan="+jabatan+"&kota="+kota+"&bank="+bank+"&denda="+denda+"&totalBulanPajak="+totalBulanPajak+"&tipeKalkulasiPajak="+tipeKalkulasiPajak+"&appId=<?php echo $a;?>&SP="+status, '_blank');
    }
    function printSTPDPBBToPDF(nop, thnpajak, nourut) {
        window.open('view/PBB/penagihan_uptd_khusus_2016/svc-stpdpbb-pdf.php?nop='+nop+'&thnpajak='+thnpajak+'&dbhost='+dbhost+'&dbuser='+dbuser+'&dbpwd='+dbpwd+'&dbname='+dbname+"&kepala="+kepala+"&nip="+nip+"&jabatan="+jabatan+"&kota="+kota+"&bank="+bank+"&denda="+denda+"&totalBulanPajak="+totalBulanPajak+"&tipeKalkulasiPajak="+tipeKalkulasiPajak+"&kd_dispenda="+kd_dispenda+"&kd_bidang="+kd_bidang+"&appId="+appId, '_blank');
    }
    function printSuratPernyataanToPDF(nop, thnpajak) {
        window.open('view/PBB/penagihan_uptd_khusus_2016/svc-suratpernyataan-pdf.php?nop='+nop+'&thnpajak='+thnpajak+'&dbhost='+dbhost+'&dbuser='+dbuser+'&dbpwd='+dbpwd+'&dbname='+dbname+'&kepala='+kepala+'&nip='+nip+"&jabatan="+jabatan+'&kota='+kota+'&bank='+bank+'&denda='+denda+'&totalBulanPajak='+totalBulanPajak+'&tipeKalkulasiPajak='+tipeKalkulasiPajak+'&limitTahunPajak='+limitTahunPajak, '_blank');
    }
    function setNoUrut(){
        $.ajax({
           type: "POST",
           url: "./view/PBB/penagihan_uptd_khusus_2016/nourut.php",
    ...
    $(document).ready(function(){
		...
		$(".btnApprove").click(function(){
			...
			$.ajax({
				   type: "POST",
				   url: "./view/PBB/penagihan_uptd_khusus_2016/svc-update-persetujuan.php",
				   data: "nop="+nop+'&statusPersetujuan='+statusPersetujuan,
				   dataType : "json", 
        ...
        $("#cetakSP1").unbind('click').click(function(){
                ...
                $.ajax({
                            type: "POST",
                            url: "./view/PBB/penagihan_uptd_khusus_2016/svc-update-sp1.php",
                            data: "nop="+nop+'&listTahun='+listTahun,
                            dataType : "json", 
                            success: function(data){
                                        if(data.respon="true"){
                                                console.log(data);
        ...
        $(".linkto").click(function(){
            ...
            $.ajax({
                type: "POST",
                url: "./view/PBB/penagihan_uptd_khusus_2016/svc-update-sp1.php",
                data: "nop="+nop+"&listTahun="+v_wp[1]+"&thn="+thn,
                dataType : "json", 
                success: function(data){
                    if(data.respon="true"){
		 ...
		 $(".linkcetakup"+status).click(function(){
			...
			$.ajax({
				type: "POST",
				url: "./view/PBB/penagihan_uptd_khusus_2016/svc-update-sp23.php",
				data: "nop="+nop+'&listTahun='+listTahun+'&sts='+sts,
				dataType : "json", 
				success: function(data){
					if(data.respon="true"){
						console.log(data);
		 ...
		 $("#simpanketSP1").unbind('click').click(function(){
			...
			if(keterangan == ''){
				...
				$.ajax({
					type: "POST",
					url: "./view/PBB/penagihan_uptd_khusus_2016/svc-update-stat-sp.php",
        ...
        $("#simpantanggal").unbind('click').click(function(){
            if($("#tanggal").val() != ""){
                $.ajax({
                   type: "POST",
                   url: "./view/PBB/penagihan_uptd_khusus_2016/svc-update-datesp.php",
                   data: "nop="+$("#nop_fu").val()+"&thnpajak="+$("#thnpajak_fu").val()+"&tgl="+$("#tanggal").val()+"&sp="+$("#sp_fu").val(),
...
//        $("#simpanketerangan").live("click",function(){
            if($("#keterangan").val() != ""){
                $.ajax({
                   type: "POST",
                   url: "./view/PBB/penagihan_uptd_khusus_2016/svc-update-keterangan.php",
        ...
        $("#src-kecamatan-"+status).change(function(){
            $.ajax({
               type: "POST",
               url: "./view/PBB/penagihan_uptd_khusus_2016/svc-get-kelurahan.php",
               data: "id="+$(this).val(),
               success: function(msg){
        ...
        $("#btn-print-"+status).click(function(){
            ...
            window.open('view/PBB/penagihan_uptd_khusus_2016/svc-listpenagihan-excel.php?dbhost='+dbhost+'&dbuser='+dbuser+'&dbpwd='+dbpwd+'&dbname='+dbname+'&nm='+nm+'&tagihan='+tagihan+'&kec='+kec+'&kel='+kel+'&status='+status+"&sp1="+sp1+"&sp2="+sp2+"&sp3="+sp3+"&tahun="+tahun+"&nop="+nop+"&lblkel="+lblkel, '_blank');
        })
    });
</script>
