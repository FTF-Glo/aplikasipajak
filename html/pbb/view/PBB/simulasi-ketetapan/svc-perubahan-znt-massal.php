...
if (isset($_REQUEST['action'])) {
	...
	$_znt 		= getZNTByKode($new_znt, $kel);
	$znt 		= $_znt ? $_znt['CPM_NIR'] : 0;
	$kelasBumi 	= getKelasBumiByNir($znt);
	...
	if ($_REQUEST['action'] == 'btn-save') {
		...
		if ($c > 0) {
			...
			$bOKK = $znt ? updateSimulasi($val, $kelasBumi, $znt) : false;
			if ($bOK) {
				$bOK = updateFinal();
				if ($bOK) {
					$bOK = updateSusulan();
...
// aldes
function getZNTByKode($kode, $kel)
{
	global $DBLink, $appConfig;
	$tahunTagihan = $appConfig['tahun_tagihan'];
	$kode = trim($kode);
	$query = "SELECT * FROM cppmod_pbb_znt WHERE CPM_KODE_ZNT = '{$kode}' AND CPM_TAHUN = '{$tahunTagihan}' AND CPM_KODE_LOKASI = '{$kel}'";
...
function getKelasBumiByNir($nir)
{
	global $DBLink, $appConfig;
	$tahunTagihan = $appConfig['tahun_tagihan'];
	$nir = trim($nir);
	$query = "SELECT * FROM cppmod_pbb_kelas_bumi WHERE CPM_THN_AWAL <= '{$tahunTagihan}' AND CPM_THN_AKHIR >= '{$tahunTagihan}' AND CPM_NILAI_BAWAH <= '{$nir}' AND CPM_NILAI_ATAS >= '{$nir}' ORDER BY CPM_THN_AWAL DESC";
	$res = mysqli_query($DBLink, $query);
	if(mysqli_num_rows($res) == 0) {
		return false;
	}
...
function updateSimulasi($nop, $kelasBumi, $nir = 0)
{
	global $DBLink, $appConfig;
	$tahunTagihan = $appConfig['tahun_tagihan'];

	$NJOP_PER_M2 = (($nir) ? ($nir * 1000) : ($kelasBumi['CPM_NJOP_M2'] * 1000));

	$query = "SELECT * FROM cppmod_pbb_sppt_current_simulasi WHERE NOP ='{$nop}' AND SPPT_TAHUN_PAJAK ='{$tahunTagihan}'";
	$res = mysqli_query($DBLink, $query);
	if(mysqli_num_rows($res) > 0) {
		$data = mysqli_fetch_assoc($res);

		$luasTanah = $data['OP_LUAS_BUMI'];

		$set = array();
		$OP_NJOP_BUMI = ($luasTanah * $NJOP_PER_M2);
		$OP_NJOP = ($OP_NJOP_BUMI + $data['OP_NJOP_BANGUNAN']);
		$OP_NJOPTKP = getNJOPTKP($OP_NJOP);
		$OP_NJKP = ($OP_NJOP - $OP_NJOPTKP);
		$OP_TARIF = getTarif($OP_NJKP);
		$TAGIHAN = ($OP_NJKP * $OP_TARIF);

		$set[] = "OP_KELAS_BUMI ='" . $kelasBumi['CPM_KELAS'] . "'";
		$set[] = "OP_NJOP_BUMI ='" . $OP_NJOP_BUMI . "'";
		$set[] = "OP_NJOP ='" . $OP_NJOP . "'";
		$set[] = "OP_NJOPTKP ='" . $OP_NJOPTKP . "'";
		$set[] = "OP_NJKP ='" . $OP_NJKP . "'";
		$set[] = "OP_TARIF ='" . $OP_TARIF . "'";
		$set[] = "SPPT_PBB_HARUS_DIBAYAR ='" . $TAGIHAN . "'";

		$query = "UPDATE cppmod_pbb_sppt_current_simulasi SET " . implode(',', $set) . " WHERE NOP ='{$nop}' AND SPPT_TAHUN_PAJAK ='{$tahunTagihan}'";
...
// TOTAL NJOP
function getNJOPTKP($nilai) {
	global $DBLink, $appConfig;
	$tahunTagihan = $appConfig['tahun_tagihan'];
	$query = "SELECT * FROM cppmod_pbb_njoptkp WHERE CPM_NILAI_BAWAH <= '{$nilai}' AND CPM_NILAI_ATAS >= '{$nilai}'";
	$res = mysqli_query($DBLink, $query);
	if(mysqli_num_rows($res) > 0) {
		$data = mysqli_fetch_assoc($res);
		return $data['CPM_NJOPTKP'];
	}
...
// NJKP
function getTarif($nilai)
{
	global $DBLink, $appConfig;
	$tahunTagihan = $appConfig['tahun_tagihan'];
	$query = "SELECT * FROM cppmod_pbb_tarif WHERE CPM_TRF_NILAI_BAWAH <= '{$nilai}' AND CPM_TRF_NILAI_ATAS >= '{$nilai}'";
	$res = mysqli_query($DBLink, $query);
	if(mysqli_num_rows($res) > 0) {
		$data = mysqli_fetch_assoc($res);
		return ($data['CPM_TRF_TARIF'] / 100);
	}
