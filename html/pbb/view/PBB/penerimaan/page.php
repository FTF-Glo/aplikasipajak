...
function displayContent($selected)
{
	...
	// 					<button type=\"button\" class=\"btn btn-info\" id=\"btn-src\" onclick=\"setTabs('. $selected .', '. $selected .')\">Cari</button>
	...
	echo '<div class="form-inline">
			...
			<button type="button" class="btn btn-primary btn-green" id="btn-src" onclick="setTabs('. $selected .', '. $selected .')">Cari</button>
		</div>';
...
function getDetail($id = "", $filter = array(), $custom = "", $jumhal, $perpage, $page, &$totalrows)
{
	global $dbSpec, $appConfig, $tahun;

	if (trim($id) != '') $filter['CPM_TRAN_ID'] = mysqli_real_escape_string($dbSpec->getDBLink(), trim($id));
...
<script type="text/javascript">
	...
	function showKel(kecEl) {
		let kcid = kecEl.value;
		$('select.kel' + s + ' option[data-kcid]').addClass('hidden');
		$('select.kel' + s + ` option[data-kcid=${kcid}]`).removeClass('hidden');
	}
</script>
