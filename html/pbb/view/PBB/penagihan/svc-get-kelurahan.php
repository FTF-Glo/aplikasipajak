<?php
    ...
    if(!empty ($_REQUEST['id'])){
        $sql = "SELECT * FROM cppmod_tax_kelurahan WHERE CPC_TKL_KCID='".$_REQUEST['id']."' ORDER BY CPC_TKL_ID";
        ...
        if(mysqli_num_rows($buffer) > 0){
            ...
            for($ctr=0; $ctr<count($kelurahan); $ctr++){
                $optKel .= "<option value=\"".$kelurahan[$ctr]["id"]."\">".strtoupper($kelurahan[$ctr]["nama"])."</option>";
            }
            echo $optKel;
