		...
		$hal = (($page-1) > 0 ? ($page-1) : 0) * $this->perpage;
                ...
                switch ($sts){
                        ...
                        case 3 : $sp .= "AND (A.TGL_SP3 IS NULL OR A.TGL_SP3 = '') AND A.STATUS_PERSETUJUAN = 2 ";
								 break;
                        case 6 : $sp .= "AND (A.TGL_SP1 IS NOT NULL OR A.TGL_SP1 <> '') AND (A.TGL_SP2 IS NULL OR A.TGL_SP2 = '') AND (A.TGL_SP3 IS NULL OR A.TGL_SP3 = '') AND A.STATUS_SP <> 1";
								 break;
                        case 7 : $sp .= "AND (A.TGL_SP1 IS NOT NULL OR A.TGL_SP1 <> '') AND (A.TGL_SP2 IS NOT NULL OR A.TGL_SP2 <> '') AND (A.TGL_SP3 IS NULL OR A.TGL_SP3 = '') AND A.STATUS_SP <> 1";
								 break;
                        case 8 : $sp .= "AND (A.TGL_SP1 IS NOT NULL OR A.TGL_SP1 <> '') AND (A.TGL_SP2 IS NOT NULL OR A.TGL_SP2 <> '') AND (A.TGL_SP3 IS NOT NULL OR A.TGL_SP3 <> '') AND A.STATUS_SP <> 1";
								 break;
...
//			$HTML .= "\t\t<td class=\"".$class."\" align=\"center\">".$tmp['STATUS_CETAK']."</td>\n";
        ...
        public function getKecamatan(){
            ...
            if(mysqli_num_rows($buffer) > 0){
                while($kec = mysqli_fetch_assoc($buffer)){
                    ...
                    $tmp = array(
                        "id" => $kec["CPC_TKC_ID"],
                        "nama" => $kec["CPC_TKC_KECAMATAN"].$digit3
                    );
                    $kecamatan[] = $tmp;
                }
            }
        ...
        public function getKelurahan($idkec){
            ...
            if(mysqli_num_rows($buffer) > 0){
                while($kel = mysqli_fetch_assoc($buffer)){
                    ...
                    $tmp = array(
                        "id" => $kel['CPC_TKL_ID'],
                        "nama" => $kel['CPC_TKL_KELURAHAN'].$digit3
                    );
                    $kelurahan[] = $tmp;
                }
            }
	...
	public function headerContent($sts) {
		...
		$HTML .=" <!-- <input type=\"button\" value=\"Cetak\" id=\"btn-print-{$sts}\" name=\"btn-print\" /> -->
                        &nbsp;<!-- Tahun --> <input type=\"hidden\" onKeydown=\"Javascript: if (event.keyCode==13) setTabs (".$sts.");\" id=\"src-tahun-{$sts}\" name=\"src-tahun-{$sts}\" size=\"5\" maxlength=\"4\" value=\"{$cond4}\"/>
                        &nbsp;NOP <input type=\"text\" onKeydown=\"Javascript: if (event.keyCode==13) setTabs (".$sts.");\" id=\"src-nop-{$sts}\" name=\"src-nop-{$sts}\" size=\"20\" maxlength=\"18\" value=\"{$cond5}\"/>
...
//		$HTML .= "\t\t<td class=\"tdheader\" width=\"100\">Status Cetak</td>\n";
		...
		$HTML = "<form id=\"form-notaris\" name=\"form-notaris\" method=\"post\" action=\"\" >";
		$HTML .=" <input type=\"button\" value=\"Cetak\" id=\"btn-print-{$sts}\" name=\"btn-print\" />
                        &nbsp;Tahun <input type=\"text\" onKeydown=\"Javascript: if (event.keyCode==13) setTabs (".$sts.");\" id=\"src-tahun-{$sts}\" name=\"src-tahun-{$sts}\" size=\"5\" maxlength=\"4\" value=\"{$cond4}\"/>
                        &nbsp;NOP <input type=\"text\" onKeydown=\"Javascript: if (event.keyCode==13) setTabs (".$sts.");\" id=\"src-nop-{$sts}\" name=\"src-nop-{$sts}\" size=\"20\" maxlength=\"18\" value=\"{$cond5}\"/>
                        &nbsp;Nama WP <input type=\"text\" onKeydown=\"Javascript: if (event.keyCode==13) setTabs (".$sts.");\" id=\"src-approved-{$sts}\" name=\"src-approved-{$sts}\" size=\"30\" value=\"{$condition[0]}\"/>
                        &nbsp;Daftar Tagihan <select  id=\"src-tagihan-{$sts}\" name=\"src-tagihan-{$sts}\">
...
<script type="text/javascript">
    ...
    function printToPDF(nop, thnpajak, sp) {
        ...
        if(status == '4')
            window.open('view/PBB/penagihan/svc-print-penagihan.php?nop='+nop+'&thnpajak='+thnpajak+'&dbhost='+dbhost+'&dbuser='+dbuser+'&dbpwd='+dbpwd+'&dbname='+dbname+"&kepala="+kepala+"&nip="+nip+"&jabatan="+jabatan+"&kota="+kota+"&bank="+bank+"&denda="+denda+"&totalBulanPajak="+totalBulanPajak+"&tipeKalkulasiPajak="+tipeKalkulasiPajak+"&appId=<?php echo $a;?>", '_blank');
        else window.open('view/PBB/penagihan/svc-print-penagihan.php?nop='+nop+'&thnpajak='+thnpajak+'&dbhost='+dbhost+'&dbuser='+dbuser+'&dbpwd='+dbpwd+'&dbname='+dbname+"&kepala="+kepala+"&nip="+nip+"&jabatan="+jabatan+"&kota="+kota+"&bank="+bank+"&denda="+denda+"&totalBulanPajak="+totalBulanPajak+"&tipeKalkulasiPajak="+tipeKalkulasiPajak+"&appId=<?php echo $a;?>&SP="+status, '_blank');
    }
    function printSTPDPBBToPDF(nop, thnpajak, nourut) {
        window.open('view/PBB/penagihan/svc-stpdpbb-pdf.php?nop='+nop+'&thnpajak='+thnpajak+'&dbhost='+dbhost+'&dbuser='+dbuser+'&dbpwd='+dbpwd+'&dbname='+dbname+"&kepala="+kepala+"&nip="+nip+"&jabatan="+jabatan+"&kota="+kota+"&bank="+bank+"&denda="+denda+"&totalBulanPajak="+totalBulanPajak+"&tipeKalkulasiPajak="+tipeKalkulasiPajak+"&kd_dispenda="+kd_dispenda+"&kd_bidang="+kd_bidang+"&appId="+appId, '_blank');
    }
    function printSuratPernyataanToPDF(nop, thnpajak) {
        window.open('view/PBB/penagihan/svc-suratpernyataan-pdf.php?nop='+nop+'&thnpajak='+thnpajak+'&dbhost='+dbhost+'&dbuser='+dbuser+'&dbpwd='+dbpwd+'&dbname='+dbname+'&kepala='+kepala+'&nip='+nip+"&jabatan="+jabatan+'&kota='+kota+'&bank='+bank+'&denda='+denda+'&totalBulanPajak='+totalBulanPajak+'&tipeKalkulasiPajak='+tipeKalkulasiPajak+'&limitTahunPajak='+limitTahunPajak, '_blank');
    }
    function setNoUrut(){
        $.ajax({
           type: "POST",
