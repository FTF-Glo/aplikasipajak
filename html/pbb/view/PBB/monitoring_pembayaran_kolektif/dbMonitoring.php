...
class dbMonitoring
{
	...
	{
		if ($order) $this->dbOrder = " ORDER BY " . $order;
	}
	public function setGroup($group)
	{
		if ($group) $this->dbGroup = " GROUP BY " . $group;
	}
	public function setIDTable($id)
	{
		if ($id) $this->id_table = $id;
	}
	public function setBorderTable($border)
	{
		if ($border) $this->border_table = $border;
	}

	public function setHaving($having)
	{
		if ($having) $this->dbHaving = " HAVING " . $having;
	}
	...
	public function getArrayFromSql($query)
	{
		...
		if ($this->mysql($errMsg)) {
			...
			if ($res) {
				...
				return "Error" . mysqli_error($DBLink);
			}
		} else {
			mysqli_close($this->dbConn);
			return 0;
	...
	{
		$query = $this->dbQuery . $this->dbTable . $this->dbWhere . $this->dbGroup . $this->dbHaving . $this->dbOrder . "  " . $this->paging_query();
		return  $query;
	}
	...
	{
		$query = "INSERT INTO SW_PBB.cppmod_pbb_update_log VALUES(UUID(),'$uname','$nop','$tahun','$nominal','$type',now()) ";
		if ($this->mysql($errMsg)) {
			$res = mysqli_query($this->dbConn, $query);
			if ($res) {
				$row = mysqli_fetch_row($res);
	...
	public function getBankVA()
	{
		$query = "SELECT * FROM CDCCORE_BANK_VA";
	...
	public function query_result($query)
	{
		...
		if ($this->export)
			$query = $query . $this->dbTable . $this->dbWhere . $this->dbGroup . $this->dbHaving . $this->dbOrder . $this->paging_query();
		else
			$query = $query . $this->dbTable . $this->dbWhere . $this->dbGroup . $this->dbHaving . $this->dbOrder . $this->paging_query();
	...
	public function query_result_summary()
	{
		...
		if ($this->status == 1)
			$query = "Select COALESCE(SUM(A.SPPT_PBB_HARUS_DIBAYAR),0) as pokok, COALESCE(SUM(A.PBB_DENDA),0) as denda,  COALESCE(SUM(A.pbb_total_bayar), 0) as total, (COALESCE(SUM(A.SPPT_PBB_HARUS_DIBAYAR),0) + COALESCE(SUM(A.PBB_DENDA), 0)) - COALESCE(SUM(A.pbb_total_bayar), 0) AS SELISIH " . $this->dbTable . $this->dbWhere;
	...
	private function bodySummary()
	{
		...
		for ($i = 0; $i < $c; $i++) {
			...
			} else if ($this->fields[$i]["name"] == 'selisih') {
				$html .= "<td align=\"right\">&nbsp;<b>" . number_format($total[3], 0, ',', '.') . "</b></td>";
	...
	public function showHTML()
	{
		...
		// var_dump($this->dbQuery);
		$html = "<div id=\"frame-tbl-monitoring\" class=\"tbl-monitoring table-responsive\"><table class=\"table table-sm table-bordered\" border=\"0\" cellspacing=\"1\" cellpadding=\"4\">";
