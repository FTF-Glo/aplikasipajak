...
if(isset($_POST['action'])){
	...
	if($_POST['action'] == 'loadDataSTTS'){
		...
		FROM cppmod_pbb_stts_download a 
		...
		while($row = mysql_fetch_object($res)){
			...
			foreach($arrFile as $file){
				...
				$downloadLink .= "<a href='view/PBB/pencetakan-dhkp-sppt/download-link-stts.php?file={$file}' onclick=\"javascript:return confirm('Yakin untuk mendownload file :\\n".end($arrName)."?')\">".substr(end($arrName),0,15).".pdf</a><br>";
				$x++;
			}
		...
		$id = $_POST['id'];
		$query = sprintf("SELECT * FROM cppmod_pbb_stts_download WHERE CPM_ID = '%s'", $id);
		$res = mysqli_query($DBLink, $query);
		if($row = mysql_fetch_object($res)){
			...
			if($row->CPM_STATUS == 0){
				...
				$query = sprintf("DELETE FROM cppmod_pbb_stts_download WHERE CPM_ID = '%s'", $id);
