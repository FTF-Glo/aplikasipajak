...
// require_once($sRootPath . "inc/PBB/QRIS/QRISBL.php");
...
function getValuesForPrint(&$aTemplateValues, $row, $tahunCetak)
{
    ...
    if ($row['OP_LUAS_BUMI_BERSAMA'] != null && $row['OP_LUAS_BANGUNAN_BERSAMA'] != null) {
        ...
        $aTemplateValues["OP_LUAS_BANGUNAN_BER"] = str_pad(number_format($row['OP_LUAS_BANGUNAN_BERSAMA'], 0, '', '.'), 10, " ", STR_PAD_LEFT);
    ...
    if ($row['SPPT_PBB_PENGURANGAN'] > 0) {
        ...
        $aTemplateValues["SPPT_PBB_PENGURANGAN"] = number_format($row['SPPT_PBB_PENGURANGAN'], 0, '', '.');
    ...
    $terbilang = strtoupper(SayInIndonesian($row['SPPT_PBB_HARUS_DIBAYAR']));

    if (strlen($terbilang) > 69) {
        $txt1 = substr($terbilang, 0, 70);
        $k = explode(' ',$txt1);
        unset($k[count($k)-1]);
        $aTemplateValues["TERBILANG"] = implode(' ',$k);
        $aTemplateValues["TERBILANG_"] = trim(str_replace($aTemplateValues["TERBILANG"],'',$terbilang));
    }else{
        $aTemplateValues["TERBILANG"] = $terbilang;
        $aTemplateValues["TERBILANG_"] = ' ';
    }
    ...
    $aTemplateValues["NAMA_PEJABAT_SK2v1"] = substr($appConfig['NAMA_PEJABAT_SK2'], 0,12);
    $aTemplateValues["NAMA_PEJABAT_SK2v2"] = substr($appConfig['NAMA_PEJABAT_SK2'], 12);
    // $aTemplateValues["NAMA_PEJABAT_SK2_NIP"] = $appConfig['NAMA_PEJABAT_SK2_NIP'];
    $aTemplateValues["NAMA_PEJABAT_SK2_NIP"] = str_replace(" ", "", $appConfig['NAMA_PEJABAT_SK2_NIP']);
...
function getValuesForPrints(&$aTemplateValues, $row, $tahunCetak, $rowfive, $numfive)
{
    ...
    $aTemplateValues["OP_LUAS_BUMI"] = str_pad($OP_LUAS_TANAH_VIEW, 10, " ", STR_PAD_LEFT);
    $aTemplateValues["OP_KELAS_BUMI"] = $row['OP_KELAS_BUMI'] != null && is_numeric($row['OP_KELAS_BUMI']) ? str_pad(number_format($row['OP_KELAS_BUMI'], 0, '', '.'), 5, " ", STR_PAD_LEFT) : str_pad($row['OP_KELAS_BUMI'], 5, " ", STR_PAD_LEFT);
    $aTemplateValues["OP_NJOP_BUMI_M2"] = str_pad(number_format($row['OP_NJOP_BUMI_M2'], 0, '', '.'), 17, " ", STR_PAD_LEFT);
    $aTemplateValues["OP_NJOP_BUMI"] = str_pad(number_format($row['OP_NJOP_BUMI'], 0, '', '.'), 24, " ", STR_PAD_LEFT);
    ...
    $aTemplateValues["OP_LUAS_BANGUNAN"] = str_pad($OP_LUAS_BANGUNAN_VIEW, 10, " ", STR_PAD_LEFT);
    $aTemplateValues["OP_KELAS_BANGUNAN"] = $row['OP_KELAS_BANGUNAN'] != null && is_numeric($row['OP_KELAS_BANGUNAN']) ? str_pad(number_format($row['OP_KELAS_BANGUNAN'], 0, '', '.'), 5, " ", STR_PAD_LEFT) : str_pad($row['OP_KELAS_BANGUNAN'], 5, " ", STR_PAD_LEFT);
    $aTemplateValues["OP_NJOP_BANGUNAN_M2"] = str_pad(number_format($row['OP_NJOP_BANGUNAN_M2'], 0, '', '.'), 17, " ", STR_PAD_LEFT);
    $aTemplateValues["OP_NJOP_BANGUNAN"] = str_pad(number_format($row['OP_NJOP_BANGUNAN'], 0, '', '.'), 24, " ", STR_PAD_LEFT);
    $aTemplateValues["OP_NJOP"] = str_pad(number_format($row['OP_NJOP'], 0, '', '.'), 68, " ", STR_PAD_LEFT);
    $aTemplateValues["OP_NJOPTKP"] = str_pad(number_format($row['OP_NJOPTKP'], 0, '', '.'), 36, " ", STR_PAD_LEFT);
    ...
    if ($row['SPPT_TAHUN_PAJAK'] > 2023) {
        if ($row['OP_NJOP'] - $row['OP_NJOPTKP'] < 0) {
            ...
            $aTemplateValues["OP_TOTAL_NJOP"] = str_pad(number_format($row['OP_NJOP'] - $row['OP_NJOPTKP'], 0, '', '.'), 17, " ", STR_PAD_RIGHT);
        }
        ...
        $aTemplateValues["OP_NJKP"] = str_pad(number_format($row['OP_NJOP']-$row['OP_NJOPTKP'], 0, '', '.'), 15, " ", STR_PAD_LEFT);
    }else{
...
function printRequest($nop, &$no_ktp, $tahunCetak)
{
    ...
    if ($tahunCetak >= 2018) $sTemplateFile = $sRootPath . ("function/PBB/print/svc-print-e-sppt-new.xml");
    elseif ($tahunCetak >= 2014) $sTemplateFile = $sRootPath . ("function/PBB/print/svc-print-qrcode-e-sppt.xml");
    ...
    $query = "SELECT 
                    ...
                    /*
                    IFNULL(A.WP_NAMA, E.CPM_WP_NAMA) AS WP_NAMA, 
                    IFNULL(A.WP_ALAMAT, E.CPM_WP_ALAMAT) AS WP_ALAMAT, 
                    IFNULL(A.WP_RT, E.CPM_WP_RT) AS WP_RT, 
                    IFNULL(A.WP_RW, E.CPM_WP_RW) AS WP_RW, 
                    IFNULL(A.WP_KELURAHAN, E.CPM_WP_KELURAHAN) AS WP_KELURAHAN, 
                    IFNULL(A.WP_KECAMATAN, E.CPM_WP_KECAMATAN) AS WP_KECAMATAN, 
                    IFNULL(A.WP_KOTAKAB, E.CPM_WP_KOTAKAB) AS WP_KOTAKAB, 
                    IFNULL(A.WP_KODEPOS, E.CPM_WP_KODEPOS) AS WP_KODEPOS,
                    */
                    ...
                    A.OP_NJOP_BUMI_BERSAMA, A.OP_NJOP_BANGUNAN_BERSAMA, D.CPM_WP_NO_KTP,
                    ...
                    LEFT JOIN cppmod_pbb_wajib_pajak E ON E.CPM_WP_ID=D.CPM_WP_NO_KTP AND E.CPM_WP_ID<>''
    ...
    if (GetValuesForPrints($aTemplateValue, $row, $tahunCetak, array() /*$rowfive*/, 0 /*$numfive*/)) {
...
function saveToPDF($strHTML = "", $listNOP, $tahunCetak)
{
    ...
    foreach ($listNOP as $nop) {
        ...
        $pdf->Image("e-sppt_pesibar.jpg", -1, -1, 172, 216, 'JPG', '', 'T', false, 300, '', false);
        
        $pdf->writeHTML($HTML, true, false, false, false, '');

		$QRCodeSVG = QRCodeSVG($nop, $tahunCetak);
        if($QRCodeSVG){ //print_r($QRCodeSVG);exit;
            $pdf->ImageSVG('@'.$icoQRIS, $x=63.5, $y=135, $w=12, $h=12, $link='', $align='', $palign='', $border=0, $fitonpage=false);
            $pdf->ImageSVG('@'.$QRCodeSVG, $x=54.8, $y=143.5, $w=30, $h=30, $link='', $align='', $palign='', $border=0, $fitonpage=false);
        }
		// if (isset($QRISImages["{$nop}{$tahunCetak}"])) {
		// 	// $pdf->Image($QRISImages["{$nop}{$tahunCetak}"], 45.8, 148, 26, 26, 'PNG', '', 'T', false, 300, '', false);
		// }
