...
class DBKBJPB2
{
	...
	function getDocument(&$dat)
	{
		...
		$where = " WHERE CPM_THN_DBKB_JPB12 = '" . $tahun . "' ";
		if ($srcKelas != "") $where .= " AND CPM_TYPE_DBKB_JPB12 LIKE '%" . $srcKelas . "%' ";

		$hal = (($page - 1) > 0 ? ($page - 1) : 0) * $perpage;
		$query = "";
		$query = "SELECT * FROM cppmod_pbb_dbkb_jpb12
                  $where ORDER BY CPM_KD_PROPINSI, CPM_KD_DATI2, CPM_THN_DBKB_JPB12 LIMIT " . $hal . "," . $perpage;
		$qry   = "SELECT COUNT(*) AS TOTALROWS FROM cppmod_pbb_dbkb_jpb12 $where "; //echo $query;
		...
		if (count($data->data) > 0) {
			for ($i = 0; $i < count($data->data); $i++) {
				...
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_TYPE_DBKB_JPB12 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_NILAI_DBKB_JPB12 . "</td> \n";
				$key = $data->data[$i]->CPM_TYPE_DBKB_JPB12;
				$HTML .= "\t\t<td class=\"" . $class . "\" align=\"center\"><a href=\"main.php?param=" . base64_encode($params . "&edit=1&kelas2=" . $key . "+" . $data->data[$i]->CPM_NILAI_DBKB_JPB12) . "\">Ubah</a> | <a href=\"#\" onclick=\"prosesDel('" . $key . "','main.php?param=" . base64_encode($params . "&hapus=1&kelas2=" . $key) . "')\">Hapus</a></td>\n";
				$HTML .= "\t</tr></div>\n";
			}
			$dat = array();
			$dat[0] = $HTML;
		...
		if (!empty($modeInput) || (isset($_REQUEST['edit']) && $_REQUEST['edit'] == 1)) {
			...
			if (!empty($_REQUEST['btn-save2'])) {
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					$sqlUpdate = "UPDATE cppmod_pbb_dbkb_jpb12 SET CPM_NILAI_DBKB_JPB12='$nilai' WHERE CPM_KD_PROPINSI='$kdPropinsi' AND CPM_KD_DATI2='$kdDati2' AND CPM_THN_DBKB_JPB12='$tahun' AND CPM_TYPE_DBKB_JPB12='$kelas' ";
					// echo $sqlUpdate;exit;
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					if (!empty($kelas) and !empty($nilai)) {
						$sqlInsert = "INSERT INTO cppmod_pbb_dbkb_jpb12 (CPM_KD_PROPINSI,CPM_KD_DATI2,CPM_THN_DBKB_JPB12,CPM_TYPE_DBKB_JPB12,CPM_NILAI_DBKB_JPB12) VALUES ('$kdPropinsi','$kdDati2','$tahun','$kelas', '$nilai');";
