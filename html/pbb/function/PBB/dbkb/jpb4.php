...
class DBKBJPB2
{
	...
	function getDocument(&$dat)
	{
		...
		$where = " WHERE CPM_THN_DBKB_JPB4 = '" . $tahun . "' ";
		if ($srcKelas != "") $where .= " AND CPM_KLS_DBKB_JPB4 LIKE '%" . $srcKelas . "%' ";

		$hal = (($page - 1) > 0 ? ($page - 1) : 0) * $perpage;
		$query = "";
		$query = "SELECT * FROM cppmod_pbb_dbkb_jpb4
                  $where ORDER BY CPM_KD_PROPINSI, CPM_KD_DATI2, CPM_THN_DBKB_JPB4 LIMIT " . $hal . "," . $perpage;
		$qry   = "SELECT COUNT(*) AS TOTALROWS FROM cppmod_pbb_dbkb_jpb4 $where "; //echo $query;
		...
		if (count($data->data) > 0) {
			for ($i = 0; $i < count($data->data); $i++) {
				...
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_KLS_DBKB_JPB4 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_LANTAI_MIN_DBKB_JPB4 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_LANTAI_MAX_DBKB_JPB4 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_NILAI_DBKB_JPB4 . "</td> \n";
				$key = $data->data[$i]->CPM_KLS_DBKB_JPB4 . "+" . $data->data[$i]->CPM_LANTAI_MIN_DBKB_JPB4 . "+" . $data->data[$i]->CPM_LANTAI_MAX_DBKB_JPB4 . "+" . $data->data[$i]->CPM_NILAI_DBKB_JPB4;
				$HTML .= "\t\t<td class=\"" . $class . "\" align=\"center\"><a href=\"main.php?param=" . base64_encode($params . "&edit=1&kelas2=" . $key . "+" . $data->data[$i]->CPM_NILAI_DBKB_JPB4) . "\">Ubah</a> | <a href=\"#\" onclick=\"prosesDel('" . $key . "','main.php?param=" . base64_encode($params . "&hapus=1&kelas2=" . $key) . "')\">Hapus</a></td>\n";
				$HTML .= "\t</tr></div>\n";
			}
			$dat = array();
			$dat[0] = $HTML;
	...
	public function view()
	{
		...
		if (!empty($modeInput) || (isset($_REQUEST['edit']) && $_REQUEST['edit'] == 1)) {
			...
			$HTML .= "
				" . (!empty($modeInput) ? "<button value=\"Tambah Baris\" onclick=\"addRowsMulti()\" class=\"btn btn-primary btn-orange\">Tambah Baris</button>" : "") . "
				<div class=\"table-responsive\" style=\"margin-top: 10px;\">
					<table class=\"table table-bordered\" id=\"tableAdd\">
						...
						<tr>
							<td><input name=\"kelas[]\" class=\"form-control\" maxlength='1' type=\"text\" id=\"kelas[]\" value=\"" . (isset($val[0]) && $val[0] != "" ? $val[0] : "") . "\" " . (!empty($modeInput) ? "" : "readonly") . "></td>
							<td><input name=\"nilaiBawah[]\" class=\"form-control\" type=\"text\" id=\"nilaiBawah[0]\" value=\"" . (isset($val[1]) && $val[1] != "" ? $val[1] : "") . "\" maxlength=\"9\" onkeypress=\"return iniAngka(event)\" " . (!empty($modeInput) ? "" : "readonly") . " /></td>
							<td><input name=\"nilaiAtas[]\" class=\"form-control\" type=\"text\" id=\"nilaiAtas[0]\" value=\"" . (isset($val[2]) && $val[2] != "" ? $val[2] : "") . "\" maxlength=\"9\" onkeypress=\"return iniAngka(event)\" " . (!empty($modeInput) ? "" : "readonly") . " /></td>
							<td><input name=\"nilai[]\" class=\"form-control\" type=\"text\" id=\"nilai[0]\" value=\"" . (isset($val[3]) && $val[3] != "" ? $val[3] : "") . "\" maxlength=\"9\" onkeypress=\"return iniAngka(event)\"/></td>
						</tr>
						<tr>
							<td colspan=\"3\">&nbsp;</td>
							<td><div align=\"right\">
			...
			if (!empty($_REQUEST['btn-save2'])) {
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					$sqlUpdate = "UPDATE cppmod_pbb_dbkb_jpb4 SET CPM_NILAI_DBKB_JPB4='$nilai' WHERE CPM_KD_PROPINSI='$kdPropinsi' AND CPM_KD_DATI2='$kdDati2' AND CPM_THN_DBKB_JPB4='$tahun' AND CPM_KLS_DBKB_JPB4='$kelas' AND CPM_LANTAI_MIN_DBKB_JPB4='$nilaiBawah' AND CPM_LANTAI_MAX_DBKB_JPB4='$nilaiAtas' ";
					// echo $sqlUpdate;exit;
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					if (!empty($nilaiBawah) and !empty($nilaiAtas)) {
						$sqlInsert = "INSERT INTO cppmod_pbb_dbkb_jpb4 (CPM_KD_PROPINSI,CPM_KD_DATI2,CPM_THN_DBKB_JPB4,CPM_KLS_DBKB_JPB4,CPM_LANTAI_MIN_DBKB_JPB4,CPM_LANTAI_MAX_DBKB_JPB4,CPM_NILAI_DBKB_JPB4) VALUES ('$kdPropinsi','$kdDati2','$tahun','$kelas', '$nilaiBawah', '$nilaiAtas', '$nilai');";
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					if (!empty($kelas)) {
						$sqlTampil = "DELETE FROM cppmod_pbb_dbkb_jpb4 WHERE CPM_KD_PROPINSI='$kdPropinsi' AND CPM_KD_DATI2='$kdDati2' AND CPM_THN_DBKB_JPB4='$tahun' AND CPM_KLS_DBKB_JPB4='$kelas[0]' AND CPM_LANTAI_MIN_DBKB_JPB4='$kelas[1]' AND CPM_LANTAI_MAX_DBKB_JPB4='$kelas[2]' ";
						// echo $sqlTampil; exit;
