...
class DBKBJPB2
{
	...
	function getDocument(&$dat)
	{
		...
		$where = " WHERE CPM_THN_DBKB_DAYA_DUKUNG = '" . $tahun . "' ";
		if ($srcKelas != "") $where .= " AND CPM_TYPE_KONSTRUKSI LIKE '%" . $srcKelas . "%' ";

		$hal = (($page - 1) > 0 ? ($page - 1) : 0) * $perpage;
		$query = "";
		$query = "SELECT * FROM cppmod_pbb_dbkb_daya_dukung
                  $where ORDER BY CPM_KD_PROPINSI, CPM_KD_DATI2, CPM_THN_DBKB_DAYA_DUKUNG LIMIT " . $hal . "," . $perpage;
		$qry   = "SELECT COUNT(*) AS TOTALROWS FROM cppmod_pbb_dbkb_daya_dukung $where "; //echo $query;
		...
		if (count($data->data) > 0) {
			for ($i = 0; $i < count($data->data); $i++) {
				...
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_TYPE_KONSTRUKSI . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_NILAI_DBKB_DAYA_DUKUNG . "</td> \n";
				$key = $data->data[$i]->CPM_TYPE_KONSTRUKSI;
				$HTML .= "\t\t<td class=\"" . $class . "\" align=\"center\"><a href=\"main.php?param=" . base64_encode($params . "&edit=1&kelas2=" . $key . "+" . $data->data[$i]->CPM_NILAI_DBKB_DAYA_DUKUNG) . "\">Ubah</a> | <a href=\"#\" onclick=\"prosesDel('" . $key . "','main.php?param=" . base64_encode($params . "&hapus=1&kelas2=" . $key) . "')\">Hapus</a></td>\n";
				$HTML .= "\t</tr></div>\n";
			}
			$dat = array();
			$dat[0] = $HTML;
	...
	public function view()
	{
		...
		if (!empty($modeInput) || (isset($_REQUEST['edit']) && $_REQUEST['edit'] == 1)) {
			...
			<table width=\"auto\" border=\"0\" cellpadding=\"3\" cellspacing=\"0\" id=\"tableAdd\" class=\"table table-bordered\">
					...
					<tr>
						<td width=\"144\">Tipe : </td>
						<td width=\"144\">Nilai : </td>
					</tr>
					<tr>
						<td><input name=\"kelas[]\" maxlength=\"1\" type=\"text\" id=\"kelas[]\" value=\"" . ($val[0] != "" ? $val[0] : "") . "\" " . (!empty($modeInput) ? "" : "readonly") . "></td>
						<td><input name=\"nilai[]\" type=\"text\" id=\"nilai[0]\" value=\"" . ($val[1] != "" ? $val[1] : "") . "\" maxlength=\"9\" onkeypress=\"return iniAngka(event)\"/></td>
					</tr>
					<tr>
			...
			if (!empty($_REQUEST['btn-save2'])) {
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					$sqlUpdate = "UPDATE cppmod_pbb_dbkb_daya_dukung SET CPM_NILAI_DBKB_DAYA_DUKUNG='$nilai' WHERE CPM_KD_PROPINSI='$kdPropinsi' AND CPM_KD_DATI2='$kdDati2' AND CPM_THN_DBKB_DAYA_DUKUNG='$tahun' AND CPM_TYPE_KONSTRUKSI='$kelas' ";
					// echo $sqlUpdate;exit;
				...
				for ($i = 0; $i < $jArray; $i++) {
					$kelas 		= $_REQUEST['kelas'][$i];
					$nilai		= $_REQUEST['nilai'][$i];
					if (!empty($kelas) and !empty($nilai)) {

						$sqlCheck = "SELECT * FROM cppmod_pbb_dbkb_daya_dukung WHERE CPM_THN_DBKB_DAYA_DUKUNG = '$tahun' 
						and CPM_TYPE_KONSTRUKSI='$kelas' ";
						// echo $sqlCheck;
						// exit;
						$data = mysqli_query($DBLink, $sqlCheck);
						$coutData = mysqli_num_rows($data);
						if (intval($coutData) <= 0) {
							$sqlInsert = "INSERT INTO cppmod_pbb_dbkb_daya_dukung (CPM_KD_PROPINSI,CPM_KD_DATI2,CPM_THN_DBKB_DAYA_DUKUNG,CPM_TYPE_KONSTRUKSI,CPM_NILAI_DBKB_DAYA_DUKUNG) VALUES ('$kdPropinsi','$kdDati2','$tahun','$kelas', '$nilai');";
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					if (!empty($kelas)) {
						$sqlTampil = "DELETE FROM cppmod_pbb_dbkb_daya_dukung WHERE CPM_KD_PROPINSI='$kdPropinsi' AND CPM_KD_DATI2='$kdDati2' AND CPM_THN_DBKB_DAYA_DUKUNG='$tahun' AND CPM_TYPE_KONSTRUKSI='$kelas[0]' ";
						// echo $sqlTampil; exit;
			...
			if (!empty($_REQUEST['msg'])) {
				if ($_REQUEST['msg'] == "duplicate") {
					$tahun2 = $tahun + 1;
					echo "<b> Tipe yang di input telah tersedia  </b>";
				} else {
					echo "<b> Sebanyak " . $_REQUEST['msg'] . " data berhasil ditambahkan! </b>";
				}
			}
	...
	public function headerContent()
	{
		...
		$HTML .= "
			<div class=\"row\" style=\"margin-top: 15px; margin-bottom: 15px;\">
				<div class=\"col-md-2\">
					<button type=\"submit\" class=\"btn btn-primary btn-blue mb5\" value=\"Tambah\" id=\"btn-add\" name=\"btn-add\">Tambah</button>
					<!-- <input type=\"button\" value=\"Ubah\" id=\"btn-edit\" name=\"btn-edit\"/>
					<input type=\"button\" class=\"btn btn-primary bg-maka\" value=\"Hapus\" id=\"btn-delete\" name=\"btn-delete\" onclick=\"return delAll()\"/> -->
				</div>
				...
				<div class=\"col-md-3\" style=\"margin-top: 5px;\">Pencarian </div>
						...
						<div class=\"col-md-3\">
							<button class=\"btn btn-primary btn-orange mb5\" value=\"Cari\" id=\"btn-src\" name=\"btn-src\" onclick=\"setTabs()\">Cari</button>
						</div>
					</div>
				</div>
			</div>
		...
		$HTML .= "\t\t<td class=\"tdheader\"> Tipe </td> \n";
...
<script type="text/javascript">
	...
	function addRows() {
		...
		row.insertCell(0).innerHTML = "<input name='kelas[]'  type='text' id='kelas[]' value='' maxlength='1'/>";
		...
		row.insertCell(1).innerHTML = "<input name='nilai[]' type='text' id='nilai[" + id + "]' maxlength='9' onkeypress='return iniAngka(event)' />";
		n++;
		id++;
	}
