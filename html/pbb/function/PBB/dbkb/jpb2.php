...
class DBKBJPB2
{
	...
	function getDocument(&$dat)
	{
		...
		$where = " WHERE CPM_THN_DBKB_JPB2 = '" . $tahun . "' ";
		if ($srcKelas != "") $where .= " AND CPM_KLS_DBKB_JPB2 LIKE '%" . $srcKelas . "%' ";

		$hal = (($page - 1) > 0 ? ($page - 1) : 0) * $perpage;
		$query = "";
		$query = "SELECT * FROM cppmod_pbb_dbkb_jpb2
                  $where ORDER BY CPM_KD_PROPINSI, CPM_KD_DATI2, CPM_THN_DBKB_JPB2, CPM_KLS_DBKB_JPB2, CPM_LANTAI_MIN_JPB2 LIMIT " . $hal . "," . $perpage;
		$qry   = "SELECT COUNT(*) AS TOTALROWS FROM cppmod_pbb_dbkb_jpb2 $where "; //echo $query;
		// echo $query;
		$res = mysqli_query($DBLink, $query);
		if ($res === false) {
			return false;
		...
		if (count($data->data) > 0) {
			for ($i = 0; $i < count($data->data); $i++) {
				...
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_KLS_DBKB_JPB2 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_LANTAI_MIN_JPB2 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_LANTAI_MAX_JPB2 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_NILAI_DBKB_JPB2 . "</td> \n";
				$key = $data->data[$i]->CPM_KLS_DBKB_JPB2 . "+" . $data->data[$i]->CPM_LANTAI_MIN_JPB2 . "+" . $data->data[$i]->CPM_LANTAI_MAX_JPB2 . "+" . $data->data[$i]->CPM_THN_DBKB_JPB2;
				$HTML .= "\t\t<td class=\"" . $class . "\" align=\"center\"><a href=\"main.php?param=" . base64_encode($params . "&edit=1&kelas2=" . $key . "+" . $data->data[$i]->CPM_NILAI_DBKB_JPB2) . "\">Ubah</a> | <a href=\"#\" onclick=\"prosesDel('" . $key . "','main.php?param=" . base64_encode($params . "&hapus=1&kelas2=" . $key) . "')\">Hapus</a></td>\n";
				$HTML .= "\t</tr></div>\n";
			}
			$dat = array();
			$dat[0] = $HTML;
	...
	public function view()
	{
		...
		if (!empty($modeInput) || (isset($_REQUEST['edit']) && $_REQUEST['edit'] == 1)) {
			...
			$HTML .= "
				" . (!empty($modeInput) ? "<button class=\"btn btn-primary btn-orange\" value=\"Tambah Baris\" onclick=\"addRowsMulti()\">Tambah Baris</button>" : "") . "
				<div class=\"table-responsive\" style=\"margin-top: 15px\">
					<table class=\"table table-bordered\" id=\"tableAdd\">
						...
						<tr>
							<td><input class=\"form-control\" name=\"kelas[]\" maxlength='1' type=\"text\" id=\"kelas[]\" value=\"" . (isset($val[0]) && $val[0] != "" ? $val[0] : "") . "\" " . (!empty($modeInput) ? "" : "readonly") . "></td>
							<td><input class=\"form-control\" name=\"nilaiBawah[]\" type=\"text\" id=\"nilaiBawah[0]\" value=\"" . (isset($val[1]) && $val[1] != "" ? $val[1] : "") . "\" maxlength=\"9\" onkeypress=\"return iniAngka(event)\" " . (!empty($modeInput) ? "" : "readonly") . " /></td>
							<td><input class=\"form-control\" name=\"nilaiAtas[]\" type=\"text\" id=\"nilaiAtas[0]\" value=\"" . (isset($val[2]) && $val[2] != "" ? $val[2] : "") . "\" maxlength=\"9\" onkeypress=\"return iniAngka(event)\" " . (!empty($modeInput) ? "" : "readonly") . " /></td>
							<td><input class=\"form-control\" name=\"nilai[]\" type=\"text\" id=\"nilai[0]\" value=\"" . (isset($val[4]) && $val[4] != "" ? $val[4] : "") . "\" maxlength=\"9\" onkeypress=\"return iniAngka(event)\"/></td>
						</tr>
						<tr>
							<td colspan=\"3\">&nbsp;</td>
							<td>
								<div align=\"right\">
								" . (!empty($modeInput) ? "<button name=\"btn-save\" class=\"btn btn-primary btn-orange\" type=\"submit\" id=\"btn-save\" value=\"Simpan\">Simpan</button>" : "<button name=\"btn-save2\" class=\"btn btn-primary btn-blue\" type=\"submit\" id=\"btn-save2\" value=\"Simpan Perubahan\">Simpan Perubahan</button>") . "
								</div>
							</td>
						</tr>
					</table>
			...
			if (!empty($_REQUEST['btn-save2'])) {
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					$sqlUpdate = "UPDATE cppmod_pbb_dbkb_jpb2 SET CPM_NILAI_DBKB_JPB2='$nilai' WHERE CPM_KD_PROPINSI='$kdPropinsi' AND CPM_KD_DATI2='$kdDati2' AND CPM_THN_DBKB_JPB2='$tahun' AND CPM_KLS_DBKB_JPB2='$kelas' AND CPM_LANTAI_MIN_JPB2='$nilaiBawah' AND CPM_LANTAI_MAX_JPB2='$nilaiAtas' ";
					// echo $sqlUpdate;exit;
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					if (!empty($nilaiBawah) and !empty($nilaiAtas)) {
						$sqlInsert = "INSERT INTO cppmod_pbb_dbkb_jpb2 (CPM_KD_PROPINSI,CPM_KD_DATI2,CPM_THN_DBKB_JPB2,CPM_KLS_DBKB_JPB2,CPM_LANTAI_MIN_JPB2,CPM_LANTAI_MAX_JPB2,CPM_NILAI_DBKB_JPB2) VALUES ('$kdPropinsi','$kdDati2','$tahun','$kelas', '$nilaiBawah', '$nilaiAtas', '$nilai');";
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					if (!empty($kelas)) {
						$sqlTampil = "DELETE FROM cppmod_pbb_dbkb_jpb2 WHERE CPM_KD_PROPINSI='$kdPropinsi' AND CPM_KD_DATI2='$kdDati2' AND CPM_THN_DBKB_JPB2='$tahun' AND CPM_KLS_DBKB_JPB2='$kelas[0]' AND CPM_LANTAI_MIN_JPB2='$kelas[1]' AND CPM_LANTAI_MAX_JPB2='$kelas[2]' ";
						// echo $sqlTampil; exit;
	...
	public function headerContent()
	{
                  ...
                  <button type=\"submit\" class=\"btn btn-primary btn-orange mb5\" value=\"Tambah\" id=\"btn-add\" name=\"btn-add\">Tambah</button>
					...
					<div class=\"col-md-5\" style='float:right; text-align: right;'>
               		...
               		<div class=\"col-md-3\" style=\"margin-top: 5px;\">Pencarian </div>
							<div class=\"col-md-6\">  
								<input type=\"text\" onKeydown=\"Javascript: if (event.keyCode==13) setTabs();\" id=\"srcKelas\" name=\"srcKelas\" size=\"30\" value=\"" . $srcKelas . "\" class=\"form-control\" placeholder=\"Kelas\"/>
							</div>
							<div class=\"col-md-3\">
		...
		$HTML .= "\t\t<td class=\"tdheader\"> Min </td> \n";
		$HTML .= "\t\t<td class=\"tdheader\"> Max </td> \n";
	...
	public function displayDataDBKB()
	{
		...
		// print_r($_REQUEST);
		echo "<div class=\"col-md-12\">"; //622px
		echo $this->view();
		echo "\t</div>\n";
...
<script type="text/javascript">
	...
	function addRows() {
		...
		row.insertCell(0).innerHTML = "<input name='kelas[]' class='form-control' maxlength='1' type='text' id='kelas[]' value=''/>";
		row.insertCell(1).innerHTML = "<input name='nilaiBawah[]' class='form-control' type='text' id='nilaiBawah[" + id + "]' maxlength='9' onkeypress='return iniAngka(event)'/>";
		row.insertCell(2).innerHTML = "<input name='nilaiAtas[]' class='form-control' type='text' id='nilaiAtas[" + id + "]' maxlength='9' onkeypress='return iniAngka(event)'/>";
		row.insertCell(3).innerHTML = "<input name='nilai[]' class='form-control' type='text' id='nilai[" + id + "]' maxlength='9' onkeypress='return iniAngka(event)' />";
		n++;
		id++;
	}
