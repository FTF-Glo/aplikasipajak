...
class DBKBJPB2
{
	...
	function getDocument(&$dat)
	{
		...
		$where = " WHERE CPM_THN_DBKB_JPB5 = '" . $tahun . "' ";
		if ($srcKelas != "") $where .= " AND CPM_KLS_DBKB_JPB5 LIKE '%" . $srcKelas . "%' ";

		$hal = (($page - 1) > 0 ? ($page - 1) : 0) * $perpage;
		$query = "";
		$query = "SELECT * FROM cppmod_pbb_dbkb_jpb5
                  $where ORDER BY CPM_KD_PROPINSI, CPM_KD_DATI2, CPM_THN_DBKB_JPB5 LIMIT " . $hal . "," . $perpage;
		$qry   = "SELECT COUNT(*) AS TOTALROWS FROM cppmod_pbb_dbkb_jpb5 $where "; //echo $query;
		...
		if (count($data->data) > 0) {
			for ($i = 0; $i < count($data->data); $i++) {
				...
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_KLS_DBKB_JPB5 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_LANTAI_MIN_JPB5 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_LANTAI_MAX_JPB5 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_NILAI_DBKB_JPB5 . "</td> \n";
				$key = $data->data[$i]->CPM_KLS_DBKB_JPB5 . "+" . $data->data[$i]->CPM_LANTAI_MIN_JPB5 . "+" . $data->data[$i]->CPM_LANTAI_MAX_JPB5 . "+" . $data->data[$i]->CPM_NILAI_DBKB_JPB5;
				$HTML .= "\t\t<td class=\"" . $class . "\" align=\"center\"><a href=\"main.php?param=" . base64_encode($params . "&edit=1&kelas2=" . $key . "+" . $data->data[$i]->CPM_NILAI_DBKB_JPB5) . "\">Ubah</a> | <a href=\"#\" onclick=\"prosesDel('" . $key . "','main.php?param=" . base64_encode($params . "&hapus=1&kelas2=" . $key) . "')\">Hapus</a></td>\n";
				$HTML .= "\t</tr></div>\n";
			}
			$dat = array();
			$dat[0] = $HTML;
		...
		if (!empty($modeInput) || (isset($_REQUEST['edit']) && $_REQUEST['edit'] == 1)) {
			...
			<div class=\"table-responsive\" style=\"margin-top:10px\">
				<table width=\"auto\" border=\"0\" cellpadding=\"3\" cellspacing=\"0\" id=\"tableAdd\" class=\"table table-bordered\">
					...
					<tr>
						<td colspan=\"3\">&nbsp;</td>
						<td><div align=\"right\">
							" . (!empty($modeInput) ? "<input name=\"btn-save\" type=\"submit\" id=\"btn-save\" value=\"Simpan\" class=\"btn btn-primary bg-maka\" />" : "<input name=\"btn-save2\" type=\"submit\" id=\"btn-save2\" value=\"Simpan Perubahan\" />") . "
			...
			if (!empty($_REQUEST['btn-save2'])) {
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					$sqlUpdate = "UPDATE cppmod_pbb_dbkb_jpb5 SET CPM_NILAI_DBKB_JPB5='$nilai' WHERE CPM_KD_PROPINSI='$kdPropinsi' AND CPM_KD_DATI2='$kdDati2' AND CPM_THN_DBKB_JPB5='$tahun' AND CPM_KLS_DBKB_JPB5='$kelas' AND CPM_LANTAI_MIN_JPB5='$nilaiBawah' AND CPM_LANTAI_MAX_JPB5='$nilaiAtas' ";
					// echo $sqlUpdate;exit;
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					if (!empty($nilaiBawah) and !empty($nilaiAtas)) {
						$sqlInsert = "INSERT INTO cppmod_pbb_dbkb_jpb5 (CPM_KD_PROPINSI,CPM_KD_DATI2,CPM_THN_DBKB_JPB5,CPM_KLS_DBKB_JPB5,CPM_LANTAI_MIN_JPB5,CPM_LANTAI_MAX_JPB5,CPM_NILAI_DBKB_JPB5) VALUES ('$kdPropinsi','$kdDati2','$tahun','$kelas', '$nilaiBawah', '$nilaiAtas', '$nilai');";
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					if (!empty($kelas)) {
						$sqlTampil = "DELETE FROM cppmod_pbb_dbkb_jpb5 WHERE CPM_KD_PROPINSI='$kdPropinsi' AND CPM_KD_DATI2='$kdDati2' AND CPM_THN_DBKB_JPB5='$tahun' AND CPM_KLS_DBKB_JPB5='$kelas[0]' AND CPM_LANTAI_MIN_JPB5='$kelas[1]' AND CPM_LANTAI_MAX_JPB5='$kelas[2]' ";
						// echo $sqlTampil; exit;
