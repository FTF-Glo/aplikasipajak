...
class DBKBJPB2
{
	...
	function getDocument(&$dat)
	{
		...
		$where = " WHERE CPM_THN_DBKB_STANDARD = '" . $tahun . "' ";
		if ($srcKelas != "") $where .= " AND CPM_KD_JPB LIKE '%" . $srcKelas . "%' ";

		$hal = (($page - 1) > 0 ? ($page - 1) : 0) * $perpage;
		$query = "";
		$query = "SELECT * FROM cppmod_pbb_dbkb_standard
                  $where ORDER BY CPM_KD_PROPINSI, CPM_KD_DATI2, CPM_THN_DBKB_STANDARD LIMIT " . $hal . "," . $perpage;
		$qry   = "SELECT COUNT(*) AS TOTALROWS FROM cppmod_pbb_dbkb_standard $where "; //echo $query;
		...
		if (count($data->data) > 0) {
			for ($i = 0; $i < count($data->data); $i++) {
				...
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_KD_JPB . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_TIPE_BNG . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_KD_BNG_LANTAI . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_NILAI_DBKB_STANDARD . "</td> \n";
				$key = $data->data[$i]->CPM_KD_JPB . "+" . $data->data[$i]->CPM_TIPE_BNG . "+" . $data->data[$i]->CPM_KD_BNG_LANTAI;
				$HTML .= "\t\t<td class=\"" . $class . "\" align=\"center\"><a href=\"main.php?param=" . base64_encode($params . "&edit=1&kelas2=" . $key . "+" . $data->data[$i]->CPM_NILAI_DBKB_STANDARD) . "\">Ubah</a> | <a href=\"#\" onclick=\"prosesDel('" . $key . "','main.php?param=" . base64_encode($params . "&hapus=1&kelas2=" . $key) . "')\">Hapus</a></td>\n";
				$HTML .= "\t</tr></div>\n";
			}
			$dat = array();
			$dat[0] = $HTML;
	...
	public function view()
	{
		...
		if (!empty($modeInput) || (isset($_REQUEST['edit']) && $_REQUEST['edit'] == 1)) {
			...
			<table width=\"auto\" border=\"0\" cellpadding=\"3\" cellspacing=\"0\" id=\"tableAdd\" class=\"table table-bordered\">
					...
					<tr>
						<td width=\"144\">Jenis Tangki : </td>
						<td width=\"144\">Kapasitas Maks : </td>
						<td width=\"144\">Kapasitas Min : </td>
						<td width=\"144\">Nilai : </td>
					</tr>
					<tr>
						<td><input name=\"kelas[]\" type=\"text\" id=\"kelas[]\" value=\"" . ($val[0] != "" ? $val[0] : "") . "\" " . (!empty($modeInput) ? "" : "readonly") . "></td>
			...
			if (!empty($_REQUEST['btn-save2'])) {
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					$sqlUpdate = "UPDATE cppmod_pbb_dbkb_standard SET CPM_NILAI_DBKB_STANDARD='$nilai' WHERE CPM_KD_PROPINSI='$kdPropinsi' AND CPM_KD_DATI2='$kdDati2' AND CPM_THN_DBKB_STANDARD='$tahun' AND CPM_KD_JPB='$kelas' AND CPM_TIPE_BNG='$nilaiBawah' AND CPM_KD_BNG_LANTAI='$nilaiAtas' ";
					// echo $sqlUpdate;exit;
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					if (!empty($nilaiBawah) and !empty($nilaiAtas)) {
						$sqlInsert = "INSERT INTO cppmod_pbb_dbkb_standard (CPM_KD_PROPINSI,CPM_KD_DATI2,CPM_THN_DBKB_STANDARD,CPM_KD_JPB,CPM_TIPE_BNG,CPM_KD_BNG_LANTAI,CPM_NILAI_DBKB_STANDARD) VALUES ('$kdPropinsi','$kdDati2','$tahun','$kelas', '$nilaiBawah', '$nilaiAtas', '$nilai');";
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					if (!empty($kelas)) {
						$sqlTampil = "DELETE FROM cppmod_pbb_dbkb_standard WHERE CPM_KD_PROPINSI='$kdPropinsi' AND CPM_KD_DATI2='$kdDati2' AND CPM_THN_DBKB_STANDARD='$tahun' AND CPM_KD_JPB='$kelas[0]' AND CPM_TIPE_BNG='$kelas[1]' AND CPM_KD_BNG_LANTAI='$kelas[2]' ";
						// echo $sqlTampil; exit;
	...
	public function headerContent()
	{
		...
		$HTML .= "\t\t<td class=\"tdheader\"> Kode JPB </td> \n";
		$HTML .= "\t\t<td class=\"tdheader\"> Tipe Bangunan </td> \n";
		$HTML .= "\t\t<td class=\"tdheader\"> Kode Bangunan Lantai </td> \n";
