...
class DBKBJPB2
{
	...
	function getDocument(&$dat)
	{
		...
		$where = " WHERE CPM_THN_DBKB_JPB8 = '" . $tahun . "' ";
		if ($srcKelas != "") $where .= " AND CPM_NILAI_DBKB_JPB8 LIKE '%" . $srcKelas . "%' ";

		$hal = (($page - 1) > 0 ? ($page - 1) : 0) * $perpage;
		$query = "";
		$query = "SELECT * FROM cppmod_pbb_dbkb_jpb8
                  $where ORDER BY CPM_KD_PROPINSI, CPM_KD_DATI2, CPM_THN_DBKB_JPB8 LIMIT " . $hal . "," . $perpage;
		$qry   = "SELECT COUNT(*) AS TOTALROWS FROM cppmod_pbb_dbkb_jpb8 $where "; //echo $query;
		...
		if (count($data->data) > 0) {
			for ($i = 0; $i < count($data->data); $i++) {
				...
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_LBR_BENT_MIN_DBKB_JPB8 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_LBR_BENT_MAX_DBKB_JPB8 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_TINGGI_KOLOM_MIN_DBKB_JPB8 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_TINGGI_KOLOM_MAX_DBKB_JPB8 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_NILAI_DBKB_JPB8 . "</td> \n";
				$key = $data->data[$i]->CPM_LBR_BENT_MIN_DBKB_JPB8 . "+" . $data->data[$i]->CPM_LBR_BENT_MAX_DBKB_JPB8 . "+" . $data->data[$i]->CPM_TINGGI_KOLOM_MIN_DBKB_JPB8 . "+" . $data->data[$i]->CPM_TINGGI_KOLOM_MAX_DBKB_JPB8 . "+" . $data->data[$i]->CPM_NILAI_DBKB_JPB8;
				$HTML .= "\t\t<td class=\"" . $class . "\" align=\"center\"><a href=\"main.php?param=" . base64_encode($params . "&edit=1&kelas2=" . $key . "+" . $data->data[$i]->CPM_NILAI_DBKB_JPB8) . "\">Ubah</a> | <a href=\"#\" onclick=\"prosesDel('" . $key . "','main.php?param=" . base64_encode($params . "&hapus=1&kelas2=" . $key) . "')\">Hapus</a></td>\n";
				$HTML .= "\t</tr></div>\n";
			}
			$dat = array();
			$dat[0] = $HTML;
	...
	public function view()
	{
		...
		if (!empty($modeInput) || (isset($_REQUEST['edit']) && $_REQUEST['edit'] == 1)) {
			...
			<table width=\"auto\" border=\"0\" cellpadding=\"3\" cellspacing=\"0\" id=\"tableAdd\" class=\"table table-bordered\">
					...
					<tr>
						<td><input name=\"lBentMin[]\" type=\"text\" id=\"lBentMin[]\" value=\"" . ($val[0] != "" ? $val[0] : "") . "\" " . (!empty($modeInput) ? "" : "readonly") . "></td>
						<td><input name=\"lBentMax[]\" type=\"text\" id=\"lBentMax[0]\" value=\"" . ($val[1] != "" ? $val[1] : "") . "\" maxlength=\"9\" onkeypress=\"return iniAngka(event)\" " . (!empty($modeInput) ? "" : "readonly") . " /></td>
						<td><input name=\"tKolomMin[]\" type=\"text\" id=\"tKolomMin[0]\" value=\"" . ($val[2] != "" ? $val[2] : "") . "\" maxlength=\"9\" onkeypress=\"return iniAngka(event)\" " . (!empty($modeInput) ? "" : "readonly") . " /></td>
						<td><input name=\"tKolomMax[]\" type=\"text\" id=\"tKolomMax[0]\" value=\"" . ($val[3] != "" ? $val[3] : "") . "\" maxlength=\"9\" onkeypress=\"return iniAngka(event)\"/></td>
			...
			if (!empty($_REQUEST['btn-save2'])) {
				$jArray = count($_REQUEST['lBentMin']);
				for ($i = 0; $i < $jArray; $i++) {
					$lBentMin 	= $_REQUEST['lBentMin'][$i];
					$lBentMax 	= $_REQUEST['lBentMax'][$i];
					$tKolomMin 	= $_REQUEST['tKolomMin'][$i];
					$tKolomMax 	= $_REQUEST['tKolomMax'][$i];
					$nilai		= $_REQUEST['nilai'][$i];

					$sqlUpdate = "UPDATE cppmod_pbb_dbkb_jpb8
									SET CPM_NILAI_DBKB_JPB8 = '$nilai'
									WHERE
										CPM_KD_PROPINSI = '$kdPropinsi'
									AND CPM_KD_DATI2 = '$kdDati2'
									AND CPM_THN_DBKB_JPB8 = '$tahun'
									AND CPM_LBR_BENT_MIN_DBKB_JPB8 = '$lBentMin'
									AND CPM_LBR_BENT_MAX_DBKB_JPB8 = '$lBentMax'
									AND CPM_TINGGI_KOLOM_MIN_DBKB_JPB8 = '$tKolomMin'
									AND CPM_TINGGI_KOLOM_MAX_DBKB_JPB8 = '$tKolomMax' ";
					// echo $sqlUpdate;exit;
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					$tKolomMax 	= $_REQUEST['tKolomMax'][$i];
					$nilai		= $_REQUEST['nilai'][$i];
					if (!empty($lBentMin) and !empty($lBentMax)) {
						$sqlInsert = "INSERT INTO cppmod_pbb_dbkb_jpb8 (
									CPM_KD_PROPINSI,
									CPM_KD_DATI2,
									CPM_THN_DBKB_JPB8,
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					if (!empty($kelas)) {
						$sqlTampil = "DELETE
									...
									AND CPM_KD_DATI2 = '$kdDati2'
									AND CPM_THN_DBKB_JPB8 = '$tahun'
									AND CPM_LBR_BENT_MIN_DBKB_JPB8 = '$kelas[0]'
									AND CPM_LBR_BENT_MAX_DBKB_JPB8 = '$kelas[1]'
									AND CPM_TINGGI_KOLOM_MIN_DBKB_JPB8 = '$kelas[2]'
									AND CPM_TINGGI_KOLOM_MAX_DBKB_JPB8 = '$kelas[3]'";
						// echo $sqlTampil; exit;
	...
	public function displayDataDBKB()
	{
		...
		// print_r($_REQUEST);
		// echo "<div style=\"width : auto;\">"; //722px
...
<script type="text/javascript">
	...
	function addRows() {
		...
		// row.insertCell(0).innerHTML="<input name='kelas[]' type='text' id='kelas[]' value=''/>";
		row.insertCell(0).innerHTML = "<input name='lBentMin[]' type='text' id='lBentMin[" + id + "]' maxlength='9' onkeypress='return iniAngka(event)'/>";
		row.insertCell(1).innerHTML = "<input name='lBentMax[]' type='text' id='lBentMax[" + id + "]' maxlength='9' onkeypress='return iniAngka(event)'/>";
		row.insertCell(2).innerHTML = "<input name='tKolomMin[]' type='text' id='tKolomMin[" + id + "]' maxlength='9' onkeypress='return iniAngka(event)'/>";
		row.insertCell(3).innerHTML = "<input name='tKolomMax[]' type='text' id='tKolomMax[" + id + "]' maxlength='9' onkeypress='return iniAngka(event)'/>";
