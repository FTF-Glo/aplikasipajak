...
class DBKBJPB2
{
	...
	function getDocument(&$dat)
	{
		...
		$where = " WHERE CPM_THN_DBKB_JPB14 = '" . $tahun . "' ";
		if ($srcKelas != "") $where .= " AND CPM_NILAI_DBKB_JPB14 LIKE '%" . $srcKelas . "%' ";

		$hal = (($page - 1) > 0 ? ($page - 1) : 0) * $perpage;
		$query = "";
		$query = "SELECT * FROM cppmod_pbb_dbkb_jpb14
                  $where ORDER BY CPM_KD_PROPINSI, CPM_KD_DATI2, CPM_THN_DBKB_JPB14 LIMIT " . $hal . "," . $perpage;
		$qry   = "SELECT COUNT(*) AS TOTALROWS FROM cppmod_pbb_dbkb_jpb14 $where "; //echo $query;
		...
		if (count($data->data) > 0) {
			for ($i = 0; $i < count($data->data); $i++) {
				...
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_NILAI_DBKB_JPB14 . "</td> \n";
				$key = $data->data[$i]->CPM_NILAI_DBKB_JPB14;
				$HTML .= "\t\t<td class=\"" . $class . "\" align=\"center\"><a href=\"main.php?param=" . base64_encode($params . "&edit=1&kelas2=" . $key . "+" . $data->data[$i]->CPM_NILAI_DBKB_JPB14) . "\">Ubah</a> | <a href=\"#\" onclick=\"prosesDel('" . $key . "','main.php?param=" . base64_encode($params . "&hapus=1&kelas2=" . $key) . "')\">Hapus</a></td>\n";
				$HTML .= "\t</tr></div>\n";
			}
			$dat = array();
			$dat[0] = $HTML;
	...
	public function view()
	{
		...
		if (!empty($modeInput) || (isset($_REQUEST['edit']) && $_REQUEST['edit'] == 1)) {
			...
			<table width=\"auto\" border=\"0\" cellpadding=\"3\" cellspacing=\"0\" id=\"tableAdd\" class=\"table table-bordered\">
					...
					<tr>
						<td><input name=\"nilai[]\" type=\"text\" id=\"nilai[0]\" value=\"" . ($val[0] != "" ? $val[0] : "") . "\" maxlength=\"9\" onkeypress=\"return iniAngka(event)\"/></td>
					</tr>
					<tr>
			...
			if (!empty($_REQUEST['btn-save2'])) {
				$jArray = count($_REQUEST['nilai']);
				for ($i = 0; $i < $jArray; $i++) {
					$nilai		= $_REQUEST['nilai'][$i];

					$sqlUpdate = "UPDATE cppmod_pbb_dbkb_jpb14 SET CPM_NILAI_DBKB_JPB14='$nilai' WHERE CPM_KD_PROPINSI='$kdPropinsi' AND CPM_KD_DATI2='$kdDati2' AND CPM_THN_DBKB_JPB14='$tahun'  ";
					// echo $sqlUpdate;exit;
				...
				for ($i = 0; $i < $jArray; $i++) {
					$nilai		= $_REQUEST['nilai'][$i];
					if (!empty($nilai)) {
						$sqlCheck = "SELECT * FROM cppmod_pbb_dbkb_jpb14 WHERE CPM_THN_DBKB_JPB14 = '$tahun' ";
						$data = mysqli_query($DBLink, $sqlCheck);
						$coutData = mysqli_num_rows($data);
						// if ()
						if (intval($coutData) <= 0) {

							$sqlInsert = "INSERT INTO cppmod_pbb_dbkb_jpb14 (CPM_KD_PROPINSI,CPM_KD_DATI2,CPM_THN_DBKB_JPB14,CPM_NILAI_DBKB_JPB14) VALUES ('$kdPropinsi','$kdDati2','$tahun','$nilai');";
				...
				for ($i = 0; $i < $jArray; $i++) {
					$kelas = $_REQUEST['nilai'][$i];
					if (empty($kelas)) {
						$kelas = explode("+", $_REQUEST['kelas2']);
						// print_r($kelas);
					}
					if (!empty($kelas)) {
						$sqlTampil = "DELETE FROM cppmod_pbb_dbkb_jpb12 WHERE CPM_KD_PROPINSI='$kdPropinsi' AND CPM_KD_DATI2='$kdDati2' AND CPM_THN_DBKB_JPB12='$tahun' AND CPM_TYPE_DBKB_JPB12='$kelas[0]' ";
						// echo $sqlTampil; exit;
			...
			if (!empty($_REQUEST['msg'])) {
				if ($_REQUEST['msg'] == "duplicate") {
					$tahun2 = $tahun + 1;
					echo "<b> Nilai pada Tahun $tahun telah ada, anda bisa menambahkan Nilai kembali pada tahun $tahun2  </b>";
				} else {
			...
			if ($dt) {
				...
				$HTML .= "<tr><td colspan=\"2\" align=\"center\">Data Kosong !</td></tr> ";
			}
...
<script type="text/javascript">
	...
	function addRows() {
		...
		row.insertCell(0).innerHTML = "<input name='nilai[]' type='text' id='nilai[" + id + "]' maxlength='9' onkeypress='return iniAngka(event)' />";
		n++;
		id++;
	}
