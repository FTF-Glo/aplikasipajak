...
class DBKBJPB2
{
	...
	function getDocument(&$dat)
	{
		...
		$where = " WHERE CPM_THN_DBKB_JPB7 = '" . $tahun . "' ";
		if ($srcKelas != "") $where .= " AND CPM_JNS_DBKB_JPB7 LIKE '%" . $srcKelas . "%' ";

		$hal = (($page - 1) > 0 ? ($page - 1) : 0) * $perpage;
		$query = "";
		$query = "SELECT * FROM cppmod_pbb_dbkb_jpb7
                  $where ORDER BY CPM_KD_PROPINSI, CPM_KD_DATI2, CPM_THN_DBKB_JPB7 LIMIT " . $hal . "," . $perpage;
		$qry   = "SELECT COUNT(*) AS TOTALROWS FROM cppmod_pbb_dbkb_jpb7 $where "; //echo $query;
		// echo $query;
		$res = mysqli_query($DBLink, $query);
		if ($res === false) {
			return false;
		...
		if (count($data->data) > 0) {
			for ($i = 0; $i < count($data->data); $i++) {
				...
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_JNS_DBKB_JPB7 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_BINTANG_DBKB_JPB7 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_LANTAI_MIN_JPB7 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_LANTAI_MAX_JPB7 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_NILAI_DBKB_JPB7 . "</td> \n";
				$key = $data->data[$i]->CPM_JNS_DBKB_JPB7 . "+" . $data->data[$i]->CPM_BINTANG_DBKB_JPB7 . "+" . $data->data[$i]->CPM_LANTAI_MIN_JPB7 . "+" . $data->data[$i]->CPM_LANTAI_MAX_JPB7 . "+" . $data->data[$i]->CPM_NILAI_DBKB_JPB7;
				$HTML .= "\t\t<td class=\"" . $class . "\" align=\"center\"><a href=\"main.php?param=" . base64_encode($params . "&edit=1&kelas2=" . $key . "+" . $data->data[$i]->CPM_NILAI_DBKB_JPB7) . "\">Ubah</a> | <a href=\"#\" onclick=\"prosesDel('" . $key . "','main.php?param=" . base64_encode($params . "&hapus=1&kelas2=" . $key) . "')\">Hapus</a></td>\n";
				$HTML .= "\t</tr></div>\n";
			}
			$dat = array();
			$dat[0] = $HTML;
	...
	public function view()
	{
		...
		if (!empty($modeInput) || (isset($_REQUEST['edit']) && $_REQUEST['edit'] == 1)) {
			...
			<table width=\"auto\" border=\"0\" cellpadding=\"3\" cellspacing=\"0\" id=\"tableAdd\" class=\"table table-bordered\">
					...
					<tr>
						<td width=\"144\">Jenis : </td>
						<td width=\"144\">Bintang : </td>
						<td width=\"144\">Nilai Bawah : </td>
						<td width=\"144\">Nilai Atas : </td>
						<td width=\"144\">Nilai : </td>
					</tr>
					<tr>
						<td><input name=\"jenis[]\" maxlength='1' type=\"text\" id=\"jenis[]\" value=\"" . ($val[0] != "" ? $val[0] : "") . "\" " . (!empty($modeInput) ? "" : "readonly") . "></td>
						<td><input name=\"bintang[]\" type=\"text\" maxlength='1' id=\"bintang[]\" value=\"" . ($val[1] != "" ? $val[1] : "") . "\" " . (!empty($modeInput) ? "" : "readonly") . "></td>
						<td><input name=\"nilaiBawah[]\" type=\"text\" id=\"nilaiBawah[0]\" value=\"" . ($val[2] != "" ? $val[2] : "") . "\" maxlength=\"2\" onkeypress=\"return iniAngka(event)\" " . (!empty($modeInput) ? "" : "readonly") . " /></td>
						<td><input name=\"nilaiAtas[]\" type=\"text\" id=\"nilaiAtas[0]\" value=\"" . ($val[3] != "" ? $val[3] : "") . "\" maxlength=\"2\" onkeypress=\"return iniAngka(event)\" " . (!empty($modeInput) ? "" : "readonly") . " /></td>
			...
			if (!empty($_REQUEST['btn-save2'])) {
				$jArray = count($_REQUEST['jenis']);
				for ($i = 0; $i < $jArray; $i++) {
					$jenis 		= $_REQUEST['jenis'][$i];
					$bintang 	= $_REQUEST['bintang'][$i];
					$nilaiBawah = $_REQUEST['nilaiBawah'][$i];
					$nilaiAtas 	= $_REQUEST['nilaiAtas'][$i];
					$nilai		= $_REQUEST['nilai'][$i];

					$sqlUpdate = "UPDATE cppmod_pbb_dbkb_jpb7 SET CPM_NILAI_DBKB_JPB7='$nilai' WHERE CPM_KD_PROPINSI='$kdPropinsi' AND CPM_KD_DATI2='$kdDati2' AND CPM_THN_DBKB_JPB7='$tahun' AND CPM_JNS_DBKB_JPB7='$jenis' AND CPM_BINTANG_DBKB_JPB7 = '$bintang' AND CPM_LANTAI_MIN_JPB7='$nilaiBawah' AND CPM_LANTAI_MAX_JPB7='$nilaiAtas' ";
					// echo $sqlUpdate;exit;
				...
				// exit;
				for ($i = 0; $i < $jArray; $i++) {
					$kelas 		= $_REQUEST['jenis'][$i];
					$nilaiBawah = $_REQUEST['nilaiBawah'][$i];
					$nilaiAtas 	= $_REQUEST['nilaiAtas'][$i];
					$nilai		= $_REQUEST['nilai'][$i];
					$bintang 	= $_REQUEST['bintang'][$i];
					if (!empty($nilaiBawah) and !empty($nilaiAtas)) {
						$sqlInsert = "INSERT INTO cppmod_pbb_dbkb_jpb7 (CPM_KD_PROPINSI,CPM_KD_DATI2,CPM_THN_DBKB_JPB7,CPM_JNS_DBKB_JPB7,CPM_BINTANG_DBKB_JPB7,CPM_LANTAI_MIN_JPB7,CPM_LANTAI_MAX_JPB7,CPM_NILAI_DBKB_JPB7) VALUES ('$kdPropinsi','$kdDati2','$tahun','$kelas','$bintang', '$nilaiBawah', '$nilaiAtas', '$nilai');";
						// echo ;
						// echo $sqlInsert;
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					if (!empty($kelas)) {
						$sqlTampil = "DELETE FROM cppmod_pbb_dbkb_jpb7 WHERE CPM_KD_PROPINSI='$kdPropinsi' AND CPM_KD_DATI2='$kdDati2' AND CPM_THN_DBKB_JPB7='$tahun' AND CPM_JNS_DBKB_JPB7='$kelas[0]' AND CPM_BINTANG_DBKB_JPB7='$kelas[1]' AND CPM_LANTAI_MIN_JPB7='$kelas[2]' AND CPM_LANTAI_MAX_JPB7='$kelas[3]' AND CPM_NILAI_DBKB_JPB7='$kelas[4]' ";
						// echo $sqlTampil; exit;
	...
	public function headerContent()
	{
		...
		$HTML .= "\t\t<td class=\"tdheader\"> Jenis </td> \n";
		$HTML .= "\t\t<td class=\"tdheader\"> Bintang </td> \n";
	...
	public function displayDataDBKB()
	{
		...
		// print_r($_REQUEST);
		// echo "<div style=\"width : auto;\">"; //677px
...
<script type="text/javascript">
	...
	function addRows() {
		...
		row.insertCell(0).innerHTML = "<input name='jenis[]' maxlength='1' type='text' id='jenis[]' value=''/>";

		row.insertCell(1).innerHTML = "<input name='bintang[]' type='text' id='bintang[" + id + "]' maxlength='1' onkeypress='return iniAngka(event)'/>";

		row.insertCell(2).innerHTML = "<input name='nilaiBawah[]' type='text' id='nilaiBawah[" + id + "]' maxlength='2' onkeypress='return iniAngka(event)'/>";

		row.insertCell(3).innerHTML = "<input name='nilaiAtas[]' type='text' id='nilaiAtas[" + id + "]' maxlength='2' onkeypress='return iniAngka(event)'/>";
