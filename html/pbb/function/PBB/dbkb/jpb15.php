...
class DBKBJPB2
{
	...
	function getDocument(&$dat)
	{
		...
		$where = " WHERE CPM_THN_DBKB_JPB15 = '" . $tahun . "' ";
		if ($srcKelas != "") $where .= " AND CPM_JNS_TANGKI_DBKB_JPB15 LIKE '%" . $srcKelas . "%' ";

		$hal = (($page - 1) > 0 ? ($page - 1) : 0) * $perpage;
		$query = "";
		$query = "SELECT * FROM cppmod_pbb_dbkb_jpb15
                  $where ORDER BY CPM_KD_PROPINSI, CPM_KD_DATI2, CPM_THN_DBKB_JPB15 LIMIT " . $hal . "," . $perpage;
		$qry   = "SELECT COUNT(*) AS TOTALROWS FROM cppmod_pbb_dbkb_jpb15 $where "; //echo $query;
		...
		if (count($data->data) > 0) {
			for ($i = 0; $i < count($data->data); $i++) {
				...
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_JNS_TANGKI_DBKB_JPB15 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_KAPASITAS_MIN_DBKB_JPB15 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_KAPASITAS_MAX_DBKB_JPB15 . "</td> \n";
				$HTML .= "\t\t<td class=\"" . $class . "\">" . $data->data[$i]->CPM_NILAI_DBKB_JPB15 . "</td> \n";
				$key = $data->data[$i]->CPM_JNS_TANGKI_DBKB_JPB15 . "+" . $data->data[$i]->CPM_KAPASITAS_MIN_DBKB_JPB15 . "+" . $data->data[$i]->CPM_KAPASITAS_MAX_DBKB_JPB15;
				$HTML .= "\t\t<td class=\"" . $class . "\" align=\"center\"><a href=\"main.php?param=" . base64_encode($params . "&edit=1&kelas2=" . $key . "+" . $data->data[$i]->CPM_NILAI_DBKB_JPB15) . "\">Ubah</a> | <a href=\"#\" onclick=\"prosesDel('" . $key . "','main.php?param=" . base64_encode($params . "&hapus=1&kelas2=" . $key) . "')\">Hapus</a></td>\n";
				$HTML .= "\t</tr></div>\n";
			}
			$dat = array();
			$dat[0] = $HTML;
		...
		if (!empty($modeInput) || (isset($_REQUEST['edit']) && $_REQUEST['edit'] == 1)) {
			...
			if (!empty($_REQUEST['btn-save2'])) {
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					$sqlUpdate = "UPDATE cppmod_pbb_dbkb_jpb15 SET CPM_NILAI_DBKB_JPB15='$nilai' WHERE CPM_KD_PROPINSI='$kdPropinsi' AND CPM_KD_DATI2='$kdDati2' AND CPM_THN_DBKB_JPB15='$tahun' AND CPM_JNS_TANGKI_DBKB_JPB15='$kelas' AND CPM_KAPASITAS_MIN_DBKB_JPB15='$nilaiBawah' AND CPM_KAPASITAS_MAX_DBKB_JPB15='$nilaiAtas' ";
					// echo $sqlUpdate;exit;
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					if (!empty($nilaiBawah) and !empty($nilaiAtas)) {
						$sqlInsert = "INSERT INTO cppmod_pbb_dbkb_jpb15 (CPM_KD_PROPINSI,CPM_KD_DATI2,CPM_THN_DBKB_JPB15,CPM_JNS_TANGKI_DBKB_JPB15,CPM_KAPASITAS_MIN_DBKB_JPB15,CPM_KAPASITAS_MAX_DBKB_JPB15,CPM_NILAI_DBKB_JPB15) VALUES ('$kdPropinsi','$kdDati2','$tahun','$kelas', '$nilaiBawah', '$nilaiAtas', '$nilai');";
				...
				for ($i = 0; $i < $jArray; $i++) {
					...
					if (!empty($kelas)) {
						$sqlTampil = "DELETE FROM cppmod_pbb_dbkb_jpb15 WHERE CPM_KD_PROPINSI='$kdPropinsi' AND CPM_KD_DATI2='$kdDati2' AND CPM_THN_DBKB_JPB15='$tahun' AND CPM_JNS_TANGKI_DBKB_JPB15='$kelas[0]' AND CPM_KAPASITAS_MIN_DBKB_JPB15='$kelas[1]' AND CPM_KAPASITAS_MAX_DBKB_JPB15='$kelas[2]' ";
						// echo $sqlTampil; exit;
	...
	public function headerContent()
	{
		...
		$HTML .= "\t\t<td class=\"tdheader\"> Jenis Tangki </td> \n";
		$HTML .= "\t\t<td class=\"tdheader\"> Kapasitas Min </td> \n";
		$HTML .= "\t\t<td class=\"tdheader\"> Kapasitas Max </td> \n";
