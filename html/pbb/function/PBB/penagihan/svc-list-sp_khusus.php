		...
		$hal = (($page-1) > 0 ? ($page-1) : 0) * $this->perpage;
                ...
                if($sts==1){ //Tab SP1
                        $query = "SELECT
                                ...
                                AND A. SPPT_TAHUN_PAJAK = '2015' AND NOW() > SPPT_TANGGAL_JATUH_TEMPO $where ORDER BY A.SPPT_PBB_HARUS_DIBAYAR DESC,A.NOP LIMIT ".$hal.",".$this->perpage;
//                        echo $query;
//                                                exit();
                        $qry = "SELECT COUNT(*) AS TOTALROWS FROM
                                ...
                                AND A. SPPT_TAHUN_PAJAK = '2015' AND NOW() > SPPT_TANGGAL_JATUH_TEMPO $where ORDER BY A.SPPT_PBB_HARUS_DIBAYAR DESC,A.NOP ";
...
//			$HTML .= "\t\t<td class=\"".$class."\" align=\"center\">".$tmp['STATUS_CETAK']."</td>\n";
		...
		$hal = (($page-1) > 0 ? ($page-1) : 0) * $this->perpage;
                ...
                $query = "SELECT NOP,
                        ...
                        FROM PBB_SPPT_PENAGIHAN_KHUSUS WHERE SPPT_TAHUN_PAJAK >= '2007' $qrangetime $where ORDER BY WP_NAMA ASC, SPPT_TAHUN_PAJAK DESC LIMIT ".$hal.",".$this->perpage;
				//echo $query;
		...
		global $condition, $isAdminPenagihan, $isViewData, $lblkel;
                ...
                if(isset($condition[3]) && $condition[3] != ""){
                    ...
                    for($ctr=0; $ctr<count($kel); $ctr++){
                        $selected = "";
                        if(isset($condition[3]) && $condition[3] == $kel[$ctr]["id"]) $selected = "selected";
...
<script type="text/javascript">
    ...
    function printToPDF(nop, thnpajak, sp) {
        ...
        if(status == '4')
            window.open('function/PBB/penagihan/svc-print-penagihan-khusus.php?nop='+nop+'&thnpajak='+thnpajak+'&dbhost='+dbhost+'&dbuser='+dbuser+'&dbpwd='+dbpwd+'&dbname='+dbname+"&kepala="+kepala+"&nip="+nip+"&jabatan="+jabatan+"&kota="+kota+"&bank="+bank+"&denda="+denda+"&totalBulanPajak="+totalBulanPajak+"&tipeKalkulasiPajak="+tipeKalkulasiPajak+"&appId=<?php echo $a;?>", '_blank');
        else window.open('function/PBB/penagihan/svc-print-penagihan-khusus.php?nop='+nop+'&thnpajak='+thnpajak+'&dbhost='+dbhost+'&dbuser='+dbuser+'&dbpwd='+dbpwd+'&dbname='+dbname+"&kepala="+kepala+"&nip="+nip+"&jabatan="+jabatan+"&kota="+kota+"&bank="+bank+"&denda="+denda+"&totalBulanPajak="+totalBulanPajak+"&tipeKalkulasiPajak="+tipeKalkulasiPajak+"&appId=<?php echo $a;?>&SP="+status, '_blank');
    }
    function printSTPDPBBToPDF(nop, thnpajak, nourut) {
        window.open('function/PBB/penagihan/svc-stpdpbb-pdf.php?nop='+nop+'&thnpajak='+thnpajak+'&dbhost='+dbhost+'&dbuser='+dbuser+'&dbpwd='+dbpwd+'&dbname='+dbname+"&kepala="+kepala+"&nip="+nip+"&jabatan="+jabatan+"&kota="+kota+"&bank="+bank+"&denda="+denda+"&totalBulanPajak="+totalBulanPajak+"&tipeKalkulasiPajak="+tipeKalkulasiPajak+"&kd_dispenda="+kd_dispenda+"&kd_bidang="+kd_bidang+"&appId="+appId, '_blank');
    }
    function printSuratPernyataanToPDF(nop, thnpajak) {
        window.open('function/PBB/penagihan/svc-suratpernyataan-pdf.php?nop='+nop+'&thnpajak='+thnpajak+'&dbhost='+dbhost+'&dbuser='+dbuser+'&dbpwd='+dbpwd+'&dbname='+dbname+'&kepala='+kepala+'&nip='+nip+"&jabatan="+jabatan+'&kota='+kota+'&bank='+bank+'&denda='+denda+'&totalBulanPajak='+totalBulanPajak+'&tipeKalkulasiPajak='+tipeKalkulasiPajak+'&limitTahunPajak='+limitTahunPajak, '_blank');
    }
    function setNoUrut(){
        $.ajax({
           type: "POST",
    ...
    $(document).ready(function(){
        ...
        $(".linkto").click(function(){
            ...
            $.ajax({
                        type: "POST",
                        url: "./function/PBB/penagihan/svc-update-sp1_khusus.php",
