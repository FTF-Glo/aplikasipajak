...
function headerMonitoringE2 () {
	...
	<tbody>
	   <tr>
		...
		<th width=\"200\" align=\"center\">NOP</td>
		<th width=\"400\" align=\"center\">NAMA</td>
		<th width=\"200\" align=\"center\">PBB TERHUTANG</td>
	  </tr>
	";
	return $html; 
}
...
function showTable() {
	...
	$html = headerMonitoringE2 ();
        for ($i=0;$i<$c;$i++) {
                ...
                $html .= " <tr>
				<td width=\"20\" align=\"center\"><input name=\"check-all[]\" class=\"check-all\" type=\"checkbox\" value=\"".$dt[$i]['NOMOR']."+".$dt[$i]['NOP']."\" /></td>
	            <td align=\"center\"><a href='main.php?param=" . base64_encode($params."&f=".$arConfig['form_input_jadwal']."&mode=edit&svcid=".$dt[$i]['NOMOR']) . "'>" . $dt[$i]['NOMOR'] . "</a> </td>
	            <td align=\"center\">".$dt[$i]['NOP']."</td>
	            <td align=\"left\">".$dt[$i]['NAMA']."</td>
	            <td align=\"right\">".number_format($dt[$i]['TAGIHAN'],0,',','.')."</td>
	          </tr>";
...
function getData() {
	...
	$where="";
	if ($srch !=""){
		$where = "WHERE SP_NOP LIKE '%{$srch}%' OR SP_NAMA_WP LIKE '%{$srch}%' OR SP_NOMOR LIKE '%{$srch}%'";
	}
	...
	$query = "SELECT * FROM pbb_sppt_pemanggilan $whr";
		// echo $query.'<br/>';
	...
	$i=0;
	while ($row = mysqli_fetch_assoc($res)) {
		$data[$i]["NOMOR"] 		= $row["SP_NOMOR"];
		$data[$i]["NOP"] 		= $row["SP_NOP"];
		$data[$i]["NAMA"] 		= $row["SP_NAMA_WP"];
		$data[$i]["TAGIHAN"] 	= $row["SP_TAGIHAN"];
		$i++;
	}
	closeMysql($myDBLink);
	return $data;
