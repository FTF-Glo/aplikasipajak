<?php
    ...
    $totalBulanPajak = 24;
    $denda = 2;
    if(!empty($nop) && !empty($thn) && !empty($tahunpajak) && !empty($_REQUEST['dbhost']) && !empty($_REQUEST['dbuser']) && !empty($_REQUEST['dbpwd']) && !empty($_REQUEST['dbname'])){
		
		$arrTagihan = getTagihan($nop, $tahunpajak);
        $res1 = true;
		if($res1) {
    ...
    function getTagihan($nop, $tahunpajak) {
		global $DBLinkLookUp;	

		$qry = "SELECT SPPT_PBB_HARUS_DIBAYAR,SPPT_TAHUN_PAJAK,SPPT_TANGGAL_JATUH_TEMPO FROM PBB_SPPT WHERE NOP = '".$nop."' AND SPPT_TAHUN_PAJAK='".$tahunpajak."' AND (PAYMENT_FLAG != '1' OR PAYMENT_FLAG IS NULL) ORDER BY SPPT_TAHUN_PAJAK";
