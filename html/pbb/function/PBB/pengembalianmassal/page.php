...
$sRootPath = str_replace('\\', '/', str_replace(DIRECTORY_SEPARATOR . 'function' . DIRECTORY_SEPARATOR . 'PBB/pengembalianmassal', '', dirname(__FILE__))) . '/';
...
function doDelete($id, &$arrValues, $tahunCetak)
{
	...
	$arrValues['message'] = 'Pengembalian Massal berhasil';
...
function deleteRequest($id, $tahunCetak)
{
	...
	$query = "SELECT A.* , IFNULL(A.sppt_pbb_harus_dibayar,0) AS 'SPPT_PBB_HARUS_DIBAYAR', IFNULL(B.pbb_denda,0) as 'PBB_DENDA' , IFNULL(A.sppt_pbb_harus_dibayar+B.pbb_denda,0) as 'PBB_TOTAL_BAYAR' from pbb_sppt_temp A LEFT JOIN pbb_denda B ON A.NOP=B.NOP AND A.SPPT_TAHUN_PAJAK=B.SPPT_TAHUN_PAJAK WHERE A.NOP in (" . $idh . ")";
	...
	$queryspptcurrs = "SELECT 
					...
					A.OP_KELAS_BUMI_BERSAMA, A.OP_KELAS_BANGUNAN_BERSAMA,
					C.CPC_NM_SEKTOR, C.CPC_KD_AKUN
					FROM cppmod_pbb_sppt_current_temp A 
					LEFT JOIN cppmod_tax_kelurahan B ON A.OP_KELURAHAN_KODE=B.CPC_TKL_ID 
					LEFT JOIN cppmod_pbb_jns_sektor C ON C.CPC_KD_SEKTOR=B.CPC_TKL_KDSEKTOR
					LEFT JOIN (
						SELECT CPM_NOP, CPM_WP_NO_KTP FROM cppmod_pbb_sppt WHERE CPM_NOP in (" . $idh . ")
	...
	if ($nresultspptfinal > 0) {
		while ($rowspptfinal = mysqli_fetch_array($resultspptfinals)) {
			...
			$querydeletespptfinal = "delete from cppmod_pbb_sppt_final_temp where CPM_NOP = '" . $rowspptfinal['CPM_NOP'] . "'";
