...
class taxRePrintClass {
	...
	private function executeCekStatusQuery($REFNUM=NULL,$NOPNPWP=NULL,$DATE=NULL) {
		...
		$PPIDInfo=$User->getPPIDInfo($data->ppid);
		$myConn = $User->GetDbConnectionFromApp($_REQUEST['a']);
		$configCode = $data->ppid.".PP.pbb.PC.kabkot";
		...
		foreach ($arResult as $res) {
			...
			$sQ = "select * from ";
			$sQ .= $resDbTable ." WHERE ".$sCond . " AND CSM_TM_FLAG=1";
			//echo "$sQ  <br><br>";
			$result = mysqli_query($lookDBLink, $sQ);
			if($result){
				...
				if ($nRes > 0)
				{
				 ...
				 // $Response['totalrow'] = $nRes;
				  ...
				  {				
					...
					$Response[$i]["TAX_REFNUM"] =$row['CSM_TM_TAX_REFNUM'];
					$Response[$i]["PAY_REFNUM"] = $row['CSM_TM_REFNUM'];
					...
					$Response[$i]["JAMPRINT"]= strftime("%d-%m-%Y");
					$Response[$i]["TGLPRINT"]= strftime("%H:%M:%S");
					
					$dttgh = explode("-",trim($row['CSM_TM_SPPT_TGLTERBIT']));
					$dt2 = mktime(0,0,0,$dttgh[1],$dttgh[2],$dttgh[0]);
					$Response[$i]["JTHTMP"] = strftime("%d-%m-%Y", $dt2);
					$Response[$i]["THNTGH"] = strftime("%Y", $dt2);
					
					$Response[$i]["SUBJECT_NAME"] =  trim($row['CSM_TM_SUBJECT_NAME']);
					$Response[$i]["NOPNPWP"] = $row['CSM_TM_NOP_NPWP'];
					...
					$tot =  $row['CSM_TM_COLLECTIVE_AMOUNT']/(pow(10,$row['CSM_TM_MINOR_48'])) + $row['CSM_TM_MISC_AMOUNT']/(pow(10,$row['CSM_TM_MINOR_48'])) +  $row['CSM_TM_ADMIN_FEE']/(pow(10,$row['CSM_TM_MINOR_48']));	
					$Response[$i]["TRAN_AMOUNT_TEXT"] = number_format($row['CSM_TM_COLLECTIVE_AMOUNT']/(pow(10,$row['CSM_TM_MINOR_48'])), 2, ',', '.');
					$Response[$i]["MISC_BILL_TEXT"] = number_format($row['CSM_TM_MISC_AMOUNT']/(pow(10,$row['CSM_TM_MINOR_48'])), 2, ',', '.');
					$Response[$i]["ADMIN_FEE_TEXT"] = number_format($row['CSM_TM_ADMIN_FEE']/(pow(10,$row['CSM_TM_MINOR_48'])), 2, ',', '.');
					$Response[$i]["TOT_TRAN_AMOUNT_TEXT"] =number_format($tot, 2, ',', '.');
					...
					$Response[$i]["SREF1"] = substr($row['CSM_TM_SWITCH_REFNUM'],0,18);
					$Response[$i]["SREF2"] = substr($row['CSM_TM_SWITCH_REFNUM'],18,18);	
					
					if ($row['CSM_TM_NTRIAL']==0) {
						$Response[$i]["CETAK"] = "CA";
						$Response[$i]["NTRIAL"] = $row['CSM_TM_NTRIAL'];
					} else {
						$Response[$i]["CETAK"] = "CU-";
						$Response[$i]["NTRIAL"] = $row['CSM_TM_NTRIAL']+1;
					}
					$i++;
				  }
	...
	public function createBody($data=NULL) {
		global $PPID_setting;
		$sTemplateFile = str_replace(DIRECTORY_SEPARATOR.'function'.DIRECTORY_SEPARATOR.'pbb', '', dirname(__FILE__)).DIRECTORY_SEPARATOR."inc".DIRECTORY_SEPARATOR."report".DIRECTORY_SEPARATOR."template".DIRECTORY_SEPARATOR."pbb".DIRECTORY_SEPARATOR."pbb-receipt.xml";
		...
		if ($data) {
			...
			$body .= "\t\t <tr>\n";
			$body .= "\t\t\t <td colspan=\"3\"  style=\"background-color:transparent;\"><font color=\"blue\"><i>Data kosong, silahkan isi filter pencarian di atas ! (NOP/NPWP dan tanggal, atau hanya nomor referensi) </i></font></td>\n";
			$body .= "\t\t </tr>\n";
		}
		$body .= "\t </table>\n";
		$body .= "</div>\n";
	...
	private function createHeader($REFNUM,$NOPNPWP,$DATE) {
		...
		$header .= "<script language='javascript' src='function/pbb/pbb-reprint.js'></script>\n";
		$header .= "<script language='javascript' src='view/pbb/date.format.js'></script>\n";
		...
		$header .= "\t\t\t <tr>\n";
		$header .= "\t\t\t\t <td width=\"\" style=\"background-color:transparent;\">NOP / NPWP </td>\n";
...
if(stillInSession($DBLink,$json,$sdata)){
	...
	if (($nopnpwp != '' && $date != '') || $refnum != '') {
		...
		$pbb = new taxRePrintClass($printername,$refnum,$nopnpwp,$date);
		
	}
	else {
		$pbb = new taxRePrintClass($printername);
		//$pbb->taxFormdisplay($printername);
	}
