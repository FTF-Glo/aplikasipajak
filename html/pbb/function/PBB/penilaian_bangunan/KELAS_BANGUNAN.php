...
<div class="col-md-12">
    <h3>SISTEM PENILAIAN BANGUNAN <br /> (TABEL KELAS BANGUNAN) </h3>
    <?php
    if (!empty($_REQUEST['btTambah'])) {
        ...
        for ($i = 0; $i < $jArray; $i++) {
            ...
            if (!empty($tahunawal) and !empty($tahunakhir) && !empty($nilaiBawah) and !empty($nilaiAtas) and !empty($njop)) {
                $sqlTampil = "INSERT INTO cppmod_pbb_kelas_bangunan (CPM_KELAS, CPM_THN_AWAL, CPM_THN_AKHIR, CPM_NILAI_BAWAH, CPM_NILAI_ATAS, CPM_NJOP_M2) VALUES ('$kelas', '$tahunawal', '$tahunakhir', '$nilaiBawah', '$nilaiAtas', '$njop');";
        ...
        for ($i = 0; $i < $jArray; $i++) {
            ...
            $nilaiAtas = $_REQUEST['nilaiAtas'][$i];
            $njop = $_REQUEST['njop'][$i];
            $sqlTampil = "UPDATE cppmod_pbb_kelas_bangunan SET CPM_THN_AKHIR='$tahunakhir', CPM_NILAI_BAWAH='$nilaiBawah', CPM_NILAI_ATAS='$nilaiAtas', CPM_NJOP_M2='$njop' WHERE CPM_KELAS ='$kelas' AND CPM_THN_AWAL='$tahunawal' ;";
            $bOK += $dbSpec->sqlQuery($sqlTampil, $result);
        }
        if ($bOK) {
        ...
        for ($i = 0; $i < $jArray; $i++) {
            ...
            if (!empty($kelas)) {
                $sqlTampil = "DELETE FROM cppmod_pbb_kelas_bangunan WHERE CPM_KELAS ='$kelas' AND CPM_THN_AWAL='$tahunawal' ;";
                $bOK += $dbSpec->sqlQuery($sqlTampil, $result);
            }
        }
        if ($bOK) {
                ...
                <div class="col-md-12">
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            <tbody>
                                <tr>
                                    <th class="tdheader"><button class="btn btn-primary btn-orange mb5" onclick="Check()" type="button" value="Pilih Semua" id="tombolCheck">Pilih Semua</button></th>
                                ...
                                <?php
                                $sqlTampil = "SELECT * FROM cppmod_pbb_kelas_bangunan order by CPM_THN_AWAL desc,CPM_KELAS asc;";
                                $bOK = $dbSpec->sqlQuery($sqlTampil, $result);
                                $n = 0;
                                $no = 0;
    ...
    <?php } else if (!empty($_REQUEST['tambahData'])) { ?>
        <form id="form2" name="form2" method="post" action="main.php?param=<?php echo base64_encode("a=$a&m=$m&f=$f"); ?>">
            <?php
            $sqlTampil = "SELECT MAX(CPM_KELAS) as tKelas FROM cppmod_pbb_kelas_bangunan;";
            $bOK = $dbSpec->sqlQuery($sqlTampil, $result);
                ...
                <div class="col-md-12">
                    <div class="table-responsive">
                        <table class="table table-bordered" id="tableAdd">
                            ...
                            ?>
                                <tr>
                                    <td><input name="kelas[]" class="form-control" type="text" id="kelas[]" value=""></td>
                                    <td><input name="tahunawal[]" class="form-control" type="text" id="tahunawal[<?php echo $i; ?>]" maxlength="9" onkeypress="return iniAngka(event)" /></td>
                                    <td><input name="tahunakhir[]" class="form-control" type="text" id="tahunakhir[<?php echo $i; ?>]" maxlength="9" onkeypress="return iniAngka(event)" /></td>
                                    <td><input name="nilaiBawah[]" class="form-control" type="text" id="nilaiBawah[<?php echo $i; ?>]" onkeyup="hNJOP(this.id,'nilaiBawah')" maxlength="9" onkeypress="return iniAngka(event)" /></td>
                                    <td><input name="nilaiAtas[]" class="form-control" type="text" id="nilaiAtas[<?php echo $i; ?>]" onkeyup="hNJOP(this.id,'nilaiAtas')" maxlength="9" onkeypress="return iniAngka(event)" /></td>
                                    <td><input name="njop[]" class="form-control" type="text" id="njop[<?php echo $i; ?>]" maxlength="9" onkeypress="return iniAngka(event)" /></td>
                                </tr>
                            <?php } ?>
                            <tr>
                                <td colspan="5">&nbsp;</td>
                                <td>
                                    <div align="right">
                                        <button class="btn btn-primary bg-maka mb5" name="btTambah" type="submit" id="btTambah" value="Simpan">Simpan</button>
                                    </div>
                                </td>
                            </tr>
                        </table>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            for ($i = 0; $i < $jArray; $i++) {
                                ...
                                $sqlTampil = "SELECT * FROM cppmod_pbb_kelas_bangunan where CPM_KELAS='$kelas' AND CPM_THN_AWAL='$tahunawal' ;";
                                $bOK = $dbSpec->sqlQuery($sqlTampil, $result);
                                $r = mysqli_fetch_assoc($result);
                            ?>
                                <tr>
                                    <td width="104"><input name="kelas[]" type="text" id="kelas[]" value="<?php echo $r['CPM_KELAS']; ?>" readonly="true" /></td>
                                    <td width="90"><input name="tahunawal[]" type="text" id="tahunawal[<?php echo $i; ?>]" value="<?php echo $r['CPM_THN_AWAL']; ?>" readonly="true" maxlength="9" onkeypress="return iniAngka(event)" /></td>
                                    <td width="90"><input name="tahunakhir[]" type="text" id="tahunakhir[<?php echo $i; ?>]" value="<?php echo $r['CPM_THN_AKHIR']; ?>" maxlength="9" onkeypress="return iniAngka(event)" /></td>
                                    <td width="3"><input name="nilaiBawah[]" type="text" id="nilaiBawah[<?php echo $i; ?>]" onkeyup="hNJOP(this.id,'nilaiBawah')" value="<?php echo $r['CPM_NILAI_BAWAH']; ?>" maxlength="9" onkeypress="return iniAngka(event)" /></td>
                                    <td width="290"><input name="nilaiAtas[]" type="text" id="nilaiAtas[<?php echo $i; ?>]" onkeyup="hNJOP(this.id,'nilaiAtas')" value="<?php echo $r['CPM_NILAI_ATAS']; ?>" maxlength="9" onkeypress="return iniAngka(event)" /></td>
                                    <td width="290"><input name="njop[]" type="text" id="njop[<?php echo $i; ?>]" value="<?php echo $r['CPM_NJOP_M2']; ?>" maxlength="9" onkeypress="return iniAngka(event)" /></td>
                                </tr>
                            <?php } ?>
                        </table>
                    </div>
