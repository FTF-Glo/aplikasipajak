...
class SvcLampiran {
    ...
    public function deleteLampiran($type, $doc_id, $op_num, $cpm_id) {
        $hasil = false;
        if($type == 'PERUBAHAN'){
            $sql = "DELETE FROM cppmod_pbb_service_change_ext WHERE CPM_SPPT_DOC_ID='".$doc_id."' AND CPM_OP_NUM='".$op_num."'";
			$bOK = $this->dbSpec->sqlQuery($sql, $res);
			$sql = "SELECT COUNT(CPM_SPPT_DOC_ID) as JML_BANGUNAN FROM cppmod_pbb_service_change_ext WHERE CPM_SPPT_DOC_ID='".$doc_id."'";
			...
			if($jml_bangunan == 0){
				...
				$sql = "SELECT CPM_NOP FROM cppmod_pbb_service_change WHERE CPM_SPPT_DOC_ID='".$doc_id."'";
				$this->dbSpec->sqlQuery($sql, $res);
			...
			$sets = implode(',',$params);
			$sql = "UPDATE cppmod_pbb_service_change SET {$sets} 
					WHERE CPM_SPPT_DOC_ID='".$doc_id."' AND CPM_SID = '".$cpm_id."'"; 
			$bOK = $this->dbSpec->sqlQuery($sql, $res);
			
        }else if($type == 'PENGGABUNGAN'){
            $sql = "DELETE FROM cppmod_pbb_service_merge_sppt_ext WHERE CPM_SPPT_DOC_ID='".$doc_id."' AND CPM_OP_NUM='".$op_num."'";
            $bOK = $this->dbSpec->sqlQuery($sql, $res);
            $sql = "SELECT COUNT(CPM_SPPT_DOC_ID) as JML_BANGUNAN FROM cppmod_pbb_service_merge_sppt_ext WHERE CPM_SPPT_DOC_ID='".$doc_id."'";
			$this->dbSpec->sqlQuery($sql, $res);
			$data = mysqli_fetch_assoc($res);
			$jml_bangunan = $data['JML_BANGUNAN'];
			...
			if($jml_bangunan == 0){
				...
				$sql = "SELECT CPM_NOP FROM cppmod_pbb_service_merge_sppt WHERE CPM_SPPT_DOC_ID='".$doc_id."'";
				$this->dbSpec->sqlQuery($sql, $res);
				if($data = mysqli_fetch_assoc($res)){
					$NOP = $data['CPM_NOP'];
					$appConfig = $this->GetAppConfig('aPBB');
			...
			$sets = implode(',',$params);
			$sql = "UPDATE cppmod_pbb_service_merge_sppt SET {$sets} 
					WHERE CPM_SPPT_DOC_ID='".$doc_id."' AND CPM_SID = '".$cpm_id."'"; 
			$bOK = $this->dbSpec->sqlQuery($sql, $res);
        }
...
$bOK = $svcLampiran->deleteLampiran($type, $doc_id, $op_num, $cpm_id);
