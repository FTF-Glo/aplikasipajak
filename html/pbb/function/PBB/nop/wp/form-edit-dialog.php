...
<div id="main-content">
    <form method="post" name="formWpDialog" id="formWpDialog">
        ...
        <span id="spacer">Nomor KTP</span><input type="text" name="WP_ID" maxlength="30" disabled value="<?php echo  isset($WP_ID) ? $WP_ID : "" ?>" size=25> <span id="div-loadwp-dialog-save-wait"></span>
        <div id="newl"></div>
        ...
        <div id="newl"></div>
        <span id="spacer">&nbsp;</span><span>*) Yang penghasilannya semata-mata berasal dari gaji atau uang pensiunan</span><br>
        <div id="newl"></div>
        ...
        <span id="spacer">Nama Wajib Pajak</span><input type="text" name="WP_NAMA" id="WP_NAMA" maxlength="50" value="<?php echo  isset($WP_NAMA) ? preg_replace('/[^a-zA-Z0-9_ %\,\/\'\[\]\.\(\)%&-]/s', '', $WP_NAMA) : "" ?>" size=35>
        <div id="newl"></div>

        <span id="spacer">Nama Jalan</span><input type="text" name="WP_ALAMAT" id="WP_ALAMAT" maxlength="100" id="WP_ALAMAT" value="<?php echo  isset($WP_ALAMAT) ? str_replace($bSlash, $ktip, $WP_ALAMAT) : "" ?>" size=70>
        <div id="newl"></div>
        <span id="spacer">RT</span>
        <select name="WP_RT" id="WP_RT" style="font-size:unset;background:transparent;border:#8f8f9d solid 1px;width:70px">
            <?php
                for ($i=0; $i <= 500; $i++) {
                  $x_num = sprintf("%03d", $i);
                  $selectit = (isset($WP_RT) && $i==(int)$WP_RT) ? 'selected':'';
                  echo "<option $selectit value=$x_num>$x_num</option>";
                }
            ?>
        </select>
        <!--input type="text" name="WP _ RT" id="WP _ RT" maxlength="3" value="<?php echo  isset($WP_RT) ? $WP_RT : "" ?>" size=6 onkeypress="return iniAngka(event, this)"-->
        <div id="newl"></div>
        <span id="spacer">RW</span>
        <select name="WP_RW" id="WP_RW" style="font-size:unset;background:transparent;border:#8f8f9d solid 1px;width:50px">
            <?php
                for ($i=0; $i <= 299; $i++) {
                  $x_num = ($i<10) ? sprintf("%02d", $i) : $i;
                  $selectit = (isset($WP_RW) && $i==(int)$WP_RW) ? 'selected':'';
                  echo "<option $selectit value=$x_num>$x_num</option>";
                }
            ?>
        </select>
        <!--input type="text" name="WP _ RW" id="WP _ RW" maxlength="2" value="<?php echo  isset($WP_RW) ? $WP_RW : "" ?>" size=4 onkeypress="return iniAngka(event, this)"-->

        <div id="newl"></div>
        <span id="spacer">Provinsi</span>
        <input type="text" name="WP_PROPINSI" size="27" maxlength="50" id="WP_PROPINSI" value="<?php echo  isset($WP_PROPINSI) ? $WP_PROPINSI : "" ?>">
        <div id="newl"></div>
        <span id="spacer">Kab/kodya</span>
        <div id="sKota">
            <input type="text" name="WP_KOTAKAB" id="WP_KOTAKAB" size="27" maxlength="50" id="WP_KOTAKAB" value="<?php echo  isset($WP_KOTAKAB) ? $WP_KOTAKAB : "" ?>">
        </div><span id="div-sKota-wait"></span>

        <div id="newl"></div>
        <span id="spacer">Kecamatan</span>
        <div id="sKec">
            <input type="text" name="WP_KECAMATAN" id="WP_KECAMATAN" size="27" maxlength="50" id="WP_KECAMATAN" value="<?php echo  isset($WP_KECAMATAN) ? $WP_KECAMATAN : "" ?>">
        </div><span id="div-sKec-wait"></span>

        <div id="newl"></div>
        <span id="spacer"><?php echo $appConfig['LABEL_KELURAHAN']; ?></span>
        <div id="sKel2">
            <input type="text" name="WP_KELURAHAN" id="WP_KELURAHAN" size="27" maxlength="50" id="WP_KELURAHAN" value="<?php echo  isset($WP_KELURAHAN) ? $WP_KELURAHAN : "" ?>">
        </div><span id="div-sKel2-wait"></span>

        <div id="newl"></div>
        <span id="spacer">Kode Pos</span><input type="text" name="WP_KODEPOS" id="WP_KODEPOS" value="<?php echo  isset($WP_KODEPOS) ? $WP_KODEPOS : "" ?>" maxlength="5" size=10 onkeypress="return iniAngka(event, this)">
        <div id="newl"></div>
        <span id="spacer">Nomor HP</span><input type="text" name="WP_NO_HP" id="WP_NO_HP" value="<?php echo  isset($WP_NO_HP) ? $WP_NO_HP : "" ?>" maxlength="14" size=17 onkeypress="return iniAngka(event, this)">
        <div id="newl"></div>
        <br>
