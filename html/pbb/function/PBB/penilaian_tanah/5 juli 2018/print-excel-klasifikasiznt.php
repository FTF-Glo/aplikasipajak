...
function getListZNT($kel)
{
	...
	$queryKelas = "SELECT CPM_KELAS, CPM_NILAI_BAWAH, CPM_NILAI_ATAS, CPM_NJOP_M2 FROM cppmod_pbb_kelas_bumi WHERE CPM_KELAS <> 'XXX' AND CPM_THN_AKHIR = 9999 ORDER BY CPM_KELAS";
...
function lampiran($row)
{
	...
	$objPHPExcel->getActiveSheet()->setCellValue("F{$row}", " LAMPIRAN I : ");
	$row++; #2
	$objPHPExcel->getActiveSheet()->mergeCells("F{$row}:H{$row}");
	$objPHPExcel->getActiveSheet()->setCellValue("F{$row}", " KEPUTUSAN BUPATI " . $namakota);
	$row++; #3
	$objPHPExcel->getActiveSheet()->mergeCells("F{$row}:H{$row}");
	$objPHPExcel->getActiveSheet()->setCellValue("F{$row}", " NOMOR 	 : 188.45/    /4.4.2.1/2018");
	$row++; #4
	$objPHPExcel->getActiveSheet()->mergeCells("F{$row}:H{$row}");
	$objPHPExcel->getActiveSheet()->setCellValue("F{$row}", " TANGGAL : " . date("d") . " " . bulan(date("m")) . " " . date("Y"));
	$row++; #5
...
function judul($row)
{
	...
	$objPHPExcel->getActiveSheet()->getCell("H{$row}")->setValue($objRichText);
...
for ($i = 0; $i < $c; $i++) {
	...
	$objPHPExcel->getActiveSheet()->setCellValue('G' . ($row), $data[$i]["CPM_NILAI_ATAS"])->getStyle('G' . ($row))->applyFromArray($noBold);
	$objPHPExcel->getActiveSheet()->setCellValue('H' . ($row), $data[$i]["CPM_NJOP_M2"])->getStyle('H' . ($row))->applyFromArray($noBold);
	$row++;
}

$objPHPExcel->getActiveSheet()->getStyle('A12:H' . (8 + count($data)))->applyFromArray(
	array('borders' => array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN)))
);

#setup print
$objPHPExcel->getActiveSheet()->getPageSetup()->setRowsToRepeatAtTopByStartAndEnd(6, 17);
$objPHPExcel->getActiveSheet()->getPageSetup()->setPrintArea('A1:H' . (8 + count($data)));
