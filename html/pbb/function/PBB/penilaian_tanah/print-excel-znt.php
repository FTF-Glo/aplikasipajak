...
function getListZNT($kel, $thn2)
{
    ...
    $tahun = "";
    if ($thn2 != "") {
        $tahun = "AND CPM_TAHUN='$thn2'";
    }

    $queryZNT = "SELECT CPM_ID, CPM_KODE_ZNT, IFNULL(CPM_NIR2,CPM_NIR) AS CPM_NIR, CPM_TAHUN FROM (
                    ...
                    WHERE A.CPM_KODE_LOKASI = '" . $kel . "' $tahun  
...
$thn2            = @isset($_REQUEST['tahun']) ? $_REQUEST['tahun'] : "";
...
$objPHPExcel->getActiveSheet()->getStyle('A1:D' . (7 + count($data)))->applyFromArray(
    array(
        'alignment' => array(
            'vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER
        )
...
for ($i = 0; $i < $c; $i++) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('D' . (7 + $no), $data[$i]["CPM_TAHUN"])->getStyle('D' . (7 + $no))->applyFromArray($noBold);
    $no++;
}

$objPHPExcel->getActiveSheet()->getStyle('A7:D' . (7 + count($data)))->applyFromArray(
