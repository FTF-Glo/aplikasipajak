...
$objPHPExcel->getDefaultStyle()->getFont()->setName('Courier');
$objPHPExcel->getDefaultStyle()->getFont()->setSize($fontSizeDefault)->setBold(true);
$objPHPExcel->getActiveSheet()->getStyle('A12:J12')->getAlignment()->setWrapText(true);
...
function judul($row)
{
	...
	$objRichText = new PHPExcel_RichText();
	$objRichText->createText(": 19 - KEP. BANGKA BELITUNG");
	...
	$objPHPExcel->getActiveSheet()->getCell("A{$row}")->setValue($objRichText);
	$objRichText = new PHPExcel_RichText();
	$objRichText->createText(": 03 - $namakota");
	$objPHPExcel->getActiveSheet()->getCell("C{$row}")->setValue($objRichText);
	$objPHPExcel->getActiveSheet()->mergeCells("C{$row}:D{$row}");
	...
	$objPHPExcel->getActiveSheet()->getCell("E9")->setValue($objRichText);
	...
	$objPHPExcel->getActiveSheet()->getCell("F9")->setValue($objRichText);
...
$objPHPExcel->getActiveSheet()->getRowDimension('12')->setRowHeight(70);
...
$objPHPExcel->getActiveSheet()->getStyle('A12:J' . (12 + count($data)))->applyFromArray(
...
$objPHPExcel->getActiveSheet()->getPageSetup()->setPrintArea('A1:J' . (8 + count($data)));
