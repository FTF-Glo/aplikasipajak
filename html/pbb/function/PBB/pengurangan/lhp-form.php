...
function formPenerimaan($initData, $initDataRed, $dataLHP, $dataLHPP)
{
	...
	if ($modus == 'edit') {
		...
		foreach ($hasilEx as $val) {
			...
			$formHasil .= "<div><textarea  class='result' rows='2' cols='90' name='result[]' placeholder='Hasil " . $i . "' id='res" . $i . "'>" . $val . "</textarea></div><br>";
			$i++;
		}
		...
		if ($jmlPetugas != 0) {
			...
			while ($data = mysqli_fetch_array($dataLHPP)) {
				$formPeneliti .= "<tr><td><input  type ='text' name='nama[]' id='nama' size='40' onblur='getPeneliti2(" . $x . ")' value='" . $data['CPM_LHP_PE_NAMA'] . "'></td>
								  <td><input  type ='text' name='jabatan[]' id='jabatan' size='30' value='" . $data['CPM_LHP_PE_JABATAN'] . "'></td>
								  <td><input  type ='text' name='nip[]' id='nip' size='20' value='" . $data['CPM_LHP_PE_NIP'] . "'></td></tr>";
				$x++;
			}
			$idrow = $x;
		} else {
			$formPeneliti .= "<tr>
					<td><input type='text' name='nama[]' id='nama1' size='40' onblur='getPeneliti(1)'></td>
					<td><input type ='text' name='jabatan[]' id='jabatan1' size='30'></td>
					<td><input type ='text' name='nip[]' id='nip1' size='20'></td>
					</tr>";
		}
	} else if ($modus == 'input') {
		//echo $modus;
		...
		$formHasilIn = "<div><textarea class='result' rows='2' cols='90' name='result[]' placeholder='Hasil 1' id='res1'></textarea></div><br>";
	}
	$formPersetujuan = '';
	if ($arConfig['usertype'] == 'persetujuan') {
		$statusVerifikasi = '';
    ...
    <script language=\"javascript\">
		...
		function tambah(){ 
			...
			td1.innerHTML='<input type =\"text\" name=\"nama[]\" id=\"nama'+idrow+'\" size=\"40\" onblur=\"getPeneliti2('+idrow+')\">'; 
			td2.innerHTML='<input type =\"text\" name=\"jabatan[]\" id=\"jabatan'+idrow+'\" size=\"30\">'; 
			td3.innerHTML='<input type =\"text\" name=\"nip[]\" id=\"nip'+idrow+'\" size=\"20\">'; 
			$().ready(function() {
    	...
    	<div class=\"col-md-11\" style=\"max-width:900px;background:#FFF;padding:25px;box-shadow:0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);border:solid 1px #54936f\">
                      <table width=\"auto\" border=\"0\">  
							  <table width=\"600\" border=\"0\" cellpadding=\"10\">
								<tr>
								  <td colspan=\"3\" align=\"center\"><strong><font size=\"+2\">Laporan Hasil Pemeriksaan (LHP)</font></strong><br><br><hr /></td>
								</tr>
								<tr>
								   <td width=\"120\">Tanggal Penelitian</td>
								   <td width=\"2\">:</td>
                                   <td>
									 <input  type=\"text\" name=\"tglPenelitian\" id=\"tglPenelitian\" value=\"" . (($dataLHP['CPM_LHP_DATE'] != '') ? $dataLHP['CPM_LHP_DATE'] : $initData['CPM_DATE_RECEIVE']) . "\" size=\"12\" maxlength=\"12\" placeholder=\"Tgl Penelitian\"/>                                      
                                   </td>
								</tr>
								<tr>
								...
								<tr>
									<td colspan=\"3\">
										...
										<input type=\"button\" id=\"add2\" value=\"Tambah\">
									</td>
								</tr>
                                                                <tr>
									<td colspan=\"3\" align=\"center\"><hr><br>
                                                                        " . $formPersetujuan . "<br>
                                                                        <input type=\"submit\" name=\"btn-simpan\" id=\"btn-simpan\" value=\"Simpan\">&nbsp;
									<input type=\"button\" name=\"btn-batal\" id=\"btn-batal\" value=\"Batal\" onclick='window.location.href=\"main.php?param=" . base64_encode("a=" . $a . "&m=" . $m . "&f=" . $arConfig['lhp']) . "\"' /><br></td>
								</tr> 
                              </table>								
                      </table>
                      </div>
...
function generateLHPNumber()
{
	...
	if (trim($appConfig['NOMOR_LHP_OTOMATIS']) == '1') {
		return $newNumber . $appConfig['FORMAT_NOMOR_LHP'];
	} else
		return NULL;
}

function updateReduce($nomor = '', $noLHP = '', $dateLHP = '')
{
	global $DBLink;

	$qry = "UPDATE cppmod_pbb_service_reduce SET CPM_RE_LHP_NUMBER = '$noLHP', CPM_RE_LHP_DATE = '$dateLHP' WHERE CPM_RE_SID='$nomor'";
	// echo $qry; exit;
	$res = mysqli_query($DBLink, $qry);
	if (!$res) {
		echo $qry . "<br>";
...
function isHaveLHPNumber($svcid)
{
	...
	$qry = "SELECT CPM_RE_LHP_NUMBER AS LHP_NUMBER FROM cppmod_pbb_service_reduce WHERE CPM_RE_SID='{$svcid}'";
...
function save()
{
	...
	if ($modus == 'edit') {
		...
		foreach ($_POST['nama'] as $key => $value) {
			if (trim($value) != '') {
				$qry1 = "INSERT INTO cppmod_pbb_service_lhp_petugas (CPM_LHP_PE_ID,CPM_LHP_PE_SID,CPM_LHP_PE_NAMA,CPM_LHP_PE_JABATAN,CPM_LHP_PE_NIP,CPM_LHP_PE_URUTAN) 
					     VALUES ('" . c_uuid() . "','{$svcid}','" . $_POST['nama'][$key] . "','" . $_POST['jabatan'][$key] . "','" . $_POST['nip'][$key] . "','" . $urut . "')";
		...
		if (isset($_REQUEST['rekomendasi']) && $_REQUEST['rekomendasi'] != '' && $appConfig['NOMOR_LHP_OTOMATIS'] == '1' && $arConfig['usertype'] == 'persetujuan') {
			//Insert to GENERATE_LHP_NUMBER
			if (!isHaveLHPNumber($svcid)) {
				$LHPNumber 			= generateLHPNumber();
				$LHPDate 			= date('Y-m-d');
				$upReduce = updateReduce($svcid, $LHPNumber, $LHPDate);
				if ($upReduce) {
					...
					$resLHP   = mysqli_query($DBLink, $qryLHP, $DBLink);
					if ($resLHP === false) {
						echo $qryLHP . "<br>";
						echo mysqli_error($DBLink);
					}
