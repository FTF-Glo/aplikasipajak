...
function getDataReport(&$DB,&$total_hal_ini,&$total_sampai_ini,&$kota_kab,&$loadKecamatan,&$loadKelurahan,&$jumlahRow) {
	global $iErrCode,$sErrMsg,$DBLink, $DBConn, $limit_1, $limit_2, $kecamatan, $kelurahan;
		...
		$query="select NOP, WP_NAMA, WP_ALAMAT, SPPT_PBB_HARUS_DIBAYAR, OP_KECAMATAN_KODE, OP_KELURAHAN_KODE, OP_KECAMATAN, OP_KELURAHAN, '' AS TGL_BAYAR from cppmod_pbb_sppt_current ";
		$whereClause = array();
                if(!empty($kecamatan)){
			$whereClause[] = " OP_KECAMATAN_KODE='$kecamatan' ";
		}
		if(!empty($kelurahan)){
			$whereClause[] = " OP_KELURAHAN_KODE='$kelurahan'";
		}
		$where = '';
                if($whereClause) $where = ' WHERE '.join(' AND ', $whereClause);
                $query.=" $where ORDER BY NOP, SPPT_TAHUN_PAJAK";
		...
		$qu=mysqli_query($LDBLink, $query) or die("#er01: ".mysqli_error($LDBLink));
		$i=0;
		$jpajakTerutang=0;
		while($r=mysqli_fetch_array($qu)){
			...
			$DB[$i]['TGL_BAYAR']=$r['TGL_BAYAR']; //ambil dari field
		...
		if($kota_kab==0 or $kota_kab==NULL or $kota_kab==""){
			...
			$qu=mysqli_query($LDBLink, $sql) or die("#er02: ".mysqli_error($LDBLink));
		...
		$quJml=mysqli_query($LDBLink, $qJml) or die("#er02: ".mysqli_error($LDBLink));	
