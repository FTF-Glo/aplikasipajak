...
function sendReprint(refnum,a,ppid,uid){
	...
	if (refnum != ''){
		...
		params = Base64.encode(params);
		  Ext.Ajax.request({
			   url: 'svr/simpatda/svc-simpatda-reprint.php',
 ...
 function sendReportSuccess(params){
	...
	if(params.responseText){
		...
		if (objResultPayment.result) {
			...
			document.getElementById('simpatda-report-result').innerHTML = strBtn+objResultPayment.printHTML;
		} else {
			document.getElementById('simpatda-report-result').innerHTML ='<label style="font-weight:bold;color:red">'+objResultPayment.message+'</label>';
		}
	}
}
...
function sendReport(a) {
	...
	if (dateTrs != ''){
		...
		params = Base64.encode(params);
		  Ext.Ajax.request({
			   url: 'svr/simpatda/svc-simpatda-daily-report.php',
...
function downloadFile(a){
		...
		if(ft.value!=""){			
			...
			url='http://'+window.location.host+window.location.pathname.replace("main.php","")+'svr/simpatda/svc-simpatda-download-detail-daily.php?q='+urlparam;
