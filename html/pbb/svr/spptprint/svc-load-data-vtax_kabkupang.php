...
SCANPayment_ConnectToDB($GWDBLink, $GWDBConn, "192.168.8.2", "gw_user", "gw_pwd", "GW_TAX_O2W_KABKUPANG");
...
function GetList($aParams,&$Response)
{
  ...
  $sQ = "select A.NOP,
        ...
        A.OP_TARIF,
        A.SPPT_DOC_ID, C.CPC_KD_AKUN, C.CPC_NM_SEKTOR,
	D.CPM_WP_STATUS
        FROM
  ...
  if ($res = mysqli_query($DBLink, $sQ))
  {
    ...
    if ($nRes > 0)
    {
      ...
      {	  
		...
		$sQTunggakan = "SELECT 
                SPPT_TAHUN_PAJAK, SPPT_PBB_HARUS_DIBAYAR, SPPT_TANGGAL_JATUH_TEMPO
                FROM PBB_SPPT
                WHERE NOP='".trim($row['NOP'])."' AND (PAYMENT_FLAG <> '1' OR PAYMENT_FLAG IS NULL)   AND SPPT_TAHUN_PAJAK <> '".$year."'
