...
function GetList($aParams,&$Response)
{
  ...
  $sQ = "select 
        ...
        A.OP_TARIF,
        A.SPPT_DOC_ID
        from cppmod_pbb_sppt_current A {$sQCond} ORDER BY A.NOP ASC LIMIT {$page}, {$limit} ";
  ...
  if ($res = mysqli_query($DBLink, $sQ))
  {
    ...
    if ($nRes > 0)
    {
      ...
      {	  
		...
		$Response["sppt"][$i]["flag"] = clean(trim($row['FLAG']));
/* Toto */ $Response["sppt"][$i]["spp"] = clean(trim($row['SPPT_PBB_PENGURANGAN']));
/* Toto */ $Response["sppt"][$i]["sppp"] = clean(trim($row['SPPT_PBB_PERSEN_PENGURANGAN']));
/* Toto */ $Response["sppt"][$i]["otrf"] = clean(rtrim($row['OP_TARIF'], '0'));
/* Toto */ $Response["sppt"][$i]["sdi"] = clean(trim($row['SPPT_DOC_ID']));
		$Response["sppt"][$i]["ak"] = '';
		$Response["sppt"][$i]["sk"] = '';
