...
$p_num = $per_file * $page;
$html_limit = " LIMIT " . $p_num . "," . $per_file . "";
$sql = "SELECT M.* FROM CPPMOD_COLLECTIVE_GROUP G INNER JOIN CPPMOD_CG_MEMBER M ON  G.CPM_CG_ID = M.CPM_CGM_ID WHERE G.CPM_CG_ID= '$id'
	$html_limit
 ";
// echo $sql;
// exit;
...
while ($baris = mysqli_fetch_array($res)) {
	...
	if ($mode == 'cetak_ulang') {
		...
		$sql = "SELECT A.NOP,
			...
			FROM PBB_SPPT A
			WHERE  A.NOP = '$baris[CPM_CGM_NOP]' AND A.SPPT_TAHUN_PAJAK = '$baris[CPM_CGM_TAX_YEAR]' LIMIT 1";
		// echo $sql;
		// echo "<br>";
		// echo "<br>";
		// echo $sql; exit;
	...
	$res22 = mysqli_query($DBLink, $sql);
	if ($row = mysqli_fetch_array($res22)) {
	...
	// echo $row['CPM_CGM_NOP'];
	$strHTML .= buildHTML($templatePrintValue);
}
