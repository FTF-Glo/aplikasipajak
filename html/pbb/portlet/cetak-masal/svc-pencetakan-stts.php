...
if(isset($_POST['action'])){
	...
	if($_POST['action'] == 'loadDataSTTS'){
		
		$query = "SELECT a.*, 
		a.CPM_GROUP_ID AS BLOK
		FROM cppmod_pbb_stts_download_collective a 
		WHERE a.CPM_GROUP_ID = '$gid'
		ORDER BY a.CPM_GROUP_ID, a.CPM_TAHUN, a.CPM_DATETIME ASC
		...
		while($row = mysql_fetch_object($res)){
			...
			foreach($arrFile as $file){
				...
				$downloadLink .= "<a href='cetak-masal/download-link-stts.php?file={$file}' onclick=\"javascript:return confirm('Yakin untuk mendownload file :\\n".end($arrName)."?')\">".substr(end($arrName),0,15).".pdf</a><br>";
				$x++;
			}
			
			$blok = explode('-',$row->BLOK);
			
			$hapus = "<a data-id='$row->CPM_ID' data-group-id='$row->CPM_GROUP_ID' class='hapus-download'>Hapus</a";
		...
		$id = $_POST['id'];
		$query = sprintf("SELECT * FROM cppmod_pbb_stts_download_collective WHERE CPM_ID = '%s'", $id);
		$res = mysqli_query($DBLink, $query);
		if($row = mysql_fetch_object($res)){
			...
			if($row->CPM_STATUS == 0){
				...
				$query = sprintf("DELETE FROM cppmod_pbb_stts_download_collective WHERE CPM_ID = '%s'", $id);
...
<div id=\"main-content\" class=\"tab3\">
		...
		<table width=\"1300\" border=\"0\" cellspacing=\"0\" cellpadding=\"2\">
			...
			<tr>
			  ...
			  <td width=\"40%\">
				...
				<input type=\"button\" name=\"btn-generate-report\" id=\"btn-generate-report\" value=\"Download STTS\" class=\"ui-button ui-widget ui-state-default ui-corner-all\">
			  </td>
			</tr>
			<tr>
			  <td width=\"2%\">&nbsp;</td>
			...
			<tr>
			  ...
			  <td width=\"\">Blok</td>
			  <td width=\"\">
				<input type=\"text\" name=\"blok\" id=\"blok\" size=\"5\" maxlength=\"3\" placeholder=\"Blok\" onkeypress=\"return iniAngka(event, this)\" required=\"true\"/> sd 
				<input type=\"text\" name=\"blok2\" id=\"blok2\" size=\"5\" maxlength=\"3\" placeholder=\"Blok\" onkeypress=\"return iniAngka(event, this)\" required=\"true\"/>
			  </td>
			  <td></td>
			  <td></td>
			</tr>
					...
					<div class=\"ui-widget consol-main-content\">
						<div class=\"ui-widget-content consol-main-content-inner\">
							<table cellpadding=\"4\" cellspacing=\"1\" border=\"0\" width=\"100%\">
								...
								<tbody id=\"table-pencetakan-stts\" class=\"table-pencetakan-stts\"></tbody>
							</table>
						</div>
						<div class=\"ui-widget-header consol-main-content-footer\"><div style=\"float:left\"></div>
							<div style=\"float:left\"><div>
								Total data : <span id=\"table-pencetakan-stts-totalRows\">0</span><br/>
							</div></div>
						</div>
					</div>
				</td>
...
$(document).ready(function(){
	...
	$('.tab3 #kecamatanOP').change(function(){
		$.ajax({
		   type: 'POST',
		   url: './function/PBB/loket/svc-search-city.php',
		   data: 'type=3&id='+$(this).val(),
		   success: function(msg){
				$('.tab3 #kelurahanOP').html(msg);
		   }
		 });
	});
	
	// $('.tab3 #btn-generate-report').click(function(){
	// 	var $btn = $(this);
	// 	var $blok = $('.tab3 #blok');
	...
	// 	$.ajax({
	// 		type: 'POST',
	// 		url: 'view/PBB/pencetakan-dhkp-sppt/svc-topdf-stts.php',
	// 		data: postData,
	// 		synch:true,
