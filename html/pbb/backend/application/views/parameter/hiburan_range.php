                            ...
                            <?php
                                ...
                                foreach($range as $row){
                                    ...
                                    $btn =  $this->html->btn_icon('Edit','fa fa-edit','btn-edit','','data-id="'.$row['id'].'"').
                                            $this->html->btn_icon('Hapus','fa fa-trash','btn-del','','data-id="'.$row['id'].'" data-range="'.$row['range_min'].'-'.$range_max.'"');
                                    echo '
                                            <tr>
                                                <td>'.$row['range_min'].' - ' .$range_max.'</td>
                                                <td>'.$row['nilai'].'</td>
                                                <td>'.$btn.'</td>
                                            </tr>
                                         ';
...
<div class="modal fade" id="modAdd" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <form action="<?php echo base_url();?>parameter/hiburan_range_act" method="post" accept-charset="utf-8">
		...
		<div class="modal-content">
		  ...
		  <div class="modal-body">
            ...
            <div class="form-group">
		        <label for="tbxRanges">Range Omzet:</label>
		        <div class="row">
...
<div class="modal fade" id="modDel" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <form action="<?php echo base_url();?>parameter/hiburan_range_del" method="post" accept-charset="utf-8">
	  <div class="modal-dialog" role="document">
		<div class="modal-content">
		  <div class="modal-header">
...
<script>
    ...
    $('.btn-edit').on('click', function(){
        ...
        $.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>api_master/getPajakHiburanOmzetID/"+id,
			success: function(data){
			    var arr = JSON.parse(data);
                modEdit.find("#tbxRange1b").val(arr[0].range_min);
		        modEdit.find("#tbxRange2b").val(arr[0].range_max);
