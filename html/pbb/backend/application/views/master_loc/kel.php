        ...
        <div class="content-body">
            <div class="row">
                ...
                <div class="col-md-4 mb-1 menu-right">
                    <button class="btn btn-primary btn-add"><i class="fa fa-plus"></i> Tambah Kelurahan</button>
                </div>
            </div>
	  ...
	  <div class="modal-dialog" role="document">
		<div class="modal-content">
		  ...
		  <div class="modal-body">
                ...
                <div class="form-group">
    		        <label for="txKec">Kecamatan:</label>
    		        <select id="cbKec" name="cbKec" class="form-control" disabled>
   ...
   $('#cbKab').change(function(){
    ...
    if(idKab != 0){
        $.ajax({
            type: "POST",
            data: {idKab:idKab},
            url: "<?php echo base_url(); ?>api_master/getKec",
            success: function(msg){
              var arr = JSON.parse(msg);
              $('#cbKec').append('<option value="0">-Pilih-</option>');
              for(var i=0; i<arr.length; i++){
                
                $('#cbKec').append('<option value="'+arr[i].id+'">'+arr[i].name+'</option>');
                $("#cbKec").prop('disabled', false);
              }
            }
        });
    }else{
        $("#cbKec").prop('disabled', true);
    }
...
$('.btn-edit').on('click', function(){
    ...
    $.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>api_master/getKelID/"+id,
			success: function(data){
                ...
                $.ajax({
                    type: "POST",
                    data: {idKab:arr[0]['id_kab']},
                    url: "<?php echo base_url(); ?>api_master/getKec",
                    success: function(msg){
                      var arr = JSON.parse(msg);
                      for(var i=0; i<arr.length; i++){
                        modAct.find('#cbKec').append('<option value="'+arr[i].id+'">'+arr[i].name+'</option>');
                      }
                    }
                });
                $.ajax({
                    type: "POST",
                    data: {idProv:arr[0]['id_prov']},
                    url: "<?php echo base_url(); ?>api_master/getKab",
                    success: function(msg){
                      var arr = JSON.parse(msg);
                      for(var i=0; i<arr.length; i++){
                        modAct.find('#cbKab').append('<option value="'+arr[i].id+'">'+arr[i].name+'</option>');
                      }
                    }
                });
                ...
                modAct.find('#cbKec option[value='+arr[0]['id_kec']+']').attr('selected','selected');
