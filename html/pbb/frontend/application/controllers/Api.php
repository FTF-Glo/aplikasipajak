...
class Api extends CI_Controller {
    ...
    {
        $tokenIDENTIFIKASI = 'LAMPUNGSELATAN9PAJAK' . date('ymd');  /// T O K E N  I D E N T I F I K A S I

        $value = $this->format->no_currency($this->input->post('iValue'));
        // print_r(getConfig('api_gateway_pajak'));exit;
    ...
    {   
        $tokenIDENTIFIKASI = 'LAMPUNGSELATAN' . date('ymd');  /// T O K E N  I D E N T I F I K A S I
        ...
        if(isset($r->data->bills[0]->total) && (float)$r->data->bills[0]->total>0){
            ...
            $nop            = $r->data->bills[0]->nop;
            $modal          = (object)[];
            if($status==0) {
                // $modal->token = password_hash($tokenIDENTIFIKASI, PASSWORD_DEFAULT);
                $modal->token = sha1('#PBB#LAMPUNG#SELATAN#'.$nop.'#'.date('Ymd').'#');
            }
            $modal->nop     = $nop;
    ...
    {
        $nop = $this->format->no_currency($this->input->post('nop'));
        $tahun = $this->format->no_currency($this->input->post('tahun'));
        $expires_on      = strtotime(date('Y-m-t H:i:s', strtotime(date('Y') . '-12-01 23:59:59')));
        $now             = strtotime(date('Y-m-d H:i:s'));
        $diff            = ($expires_on - $now);
        $diff_in_minutes = round(abs($diff) / 60);
        $city_code       = '1812';  // 1812 PESAWARAN
